...
type_registry
  ...
  type_registry.define
    fn (state: registry_state, name: string, fields: list[field_spec]) -> result[registry_state, string]
  ...
  type_registry.lookup
    fn (state: registry_state, name: string) -> optional[type_descriptor]
    + returns the descriptor for a registered type
    # lookup
  type_registry.instantiate
    fn (state: registry_state, name: string) -> result[dynamic_value, string]
  ...
  type_registry.set_field
    fn (value: dynamic_value, field: string, v: dynamic_value) -> result[dynamic_value, string]

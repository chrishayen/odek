...
macservice
  macservice.new_spec
    @ (label: string, script_path: string) -> service_spec
    + creates a minimal spec with a unique label and the script to run
    # construction
  macservice.with_log
    @ (spec: service_spec, stdout_path: string, stderr_path: string) -> service_spec
  ...
  macservice.uninstall
    @ (label: string, descriptor_dir: string, host: service_host) -> result[void, string]

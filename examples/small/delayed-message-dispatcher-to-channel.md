...
delay_queue
  ...
  delay_queue.schedule
    @ (state: delay_queue_state, payload: bytes, dispatch_at_ms: i64) -> delay_queue_state
    + inserts a message into the queue ordered by dispatch time
    # scheduling
  delay_queue.schedule_in
    @ (state: delay_queue_state, payload: bytes, delay_ms: i64) -> delay_queue_state
    + schedules a message relative to the current time
    # scheduling
    -> std.time.now_millis
  delay_queue.drain_due
    @ (state: delay_queue_state, now_ms: i64) -> tuple[list[bytes], delay_queue_state]
  ...
  delay_queue.next_due_at
    @ (state: delay_queue_state) -> optional[i64]

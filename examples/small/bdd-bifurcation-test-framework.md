...
biff
  biff.scenario
    fn (name: string) -> scenario_state
    + creates an empty scenario with the given label
    # construction
  biff.given
    fn (state: scenario_state, desc: string, setup: fn() -> context) -> scenario_state
    + adds a precondition node that runs setup to produce a shared context
    # preconditions
  biff.when
    fn (state: scenario_state, desc: string, action: fn(context) -> context) -> scenario_state
    + branches the scenario tree with an action applied to each given context
    # actions
  biff.then
    fn (state: scenario_state, desc: string, check: fn(context) -> bool) -> scenario_state
    + attaches an assertion to every leaf of the current branch
    # assertions
  biff.run
    fn (state: scenario_state) -> list[step_result]

...
db_bench
  db_bench.new_suite
    ...
    # construction
  db_bench.add_case
    fn (state: suite_state, name: string, driver_name: string, query: string, iterations: i32) -> suite_state
    + registers a benchmark case
    # registration
  db_bench.record_run
    fn (state: suite_state, case_name: string, elapsed_nanos: i64) -> suite_state
    + appends a timing sample for a case
    # recording
  db_bench.time_block
    fn (state: suite_state, case_name: string, start_nanos: i64) -> suite_state
    + records elapsed time from a start marker to now
    # recording
    -> std.time.now_nanos
  db_bench.summarize
    fn (state: suite_state) -> list[case_summary]

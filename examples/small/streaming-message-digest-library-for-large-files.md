...
std
  std.fs
    std.fs.open_read
      fn (path: string) -> result[file_reader, string]
      + opens the file for sequential reading
      - returns error when the file does not exist
      # filesystem
    std.fs.read_chunk
      fn (r: file_reader, max: u32) -> result[bytes, string]
      + returns up to max bytes from the reader, empty bytes at eof
      # filesystem

streaming_digest
  streaming_digest.new
    fn (algorithm: string) -> result[digest_state, string]

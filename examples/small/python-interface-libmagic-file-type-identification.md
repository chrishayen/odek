...
std
  std.fs
    std.fs.read_head
      @ (path: string, max_bytes: i32) -> result[bytes, string]
...
magic
  ...
  magic.load_signatures
    @ (state: magic_state, entries: list[signature_entry]) -> magic_state
  ...
  magic.identify_bytes
    @ (state: magic_state, data: bytes) -> optional[file_type]
    + returns the mime type and description when a signature matches
    - returns none when no signature matches
    # detection
  magic.identify_file
    @ (state: magic_state, path: string) -> result[optional[file_type], string]

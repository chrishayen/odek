...
sitemap
  sitemap.new
    ...
    # construction
  sitemap.add_url
    fn (doc: sitemap_doc, loc: string, lastmod: optional[string], changefreq: optional[string], priority: optional[f32]) -> result[sitemap_doc, string]
    + appends the url entry
    - returns error when loc is empty or priority is outside 0.0..1.0
    # entries
  sitemap.to_xml
    fn (doc: sitemap_doc) -> string

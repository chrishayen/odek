...
leveled_logger
  leveled_logger.new
    @ (min_level: string, sink: log_sink) -> logger
  ...
  leveled_logger.should_log
    @ (l: logger, level: string) -> bool
    + returns true when level >= min_level
    - returns false for an unknown level
    # filter
  leveled_logger.format_entry
    @ (level: string, message: string, timestamp: i64) -> string
    + returns "<iso-time> <LEVEL> message"
    # format
  leveled_logger.log
    @ (l: logger, level: string, message: string) -> result[void, string]

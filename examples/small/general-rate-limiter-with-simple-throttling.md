...
throttle
  throttle.new
    @ (max_concurrent: i32, min_interval_ms: i64) -> throttle_state
    + creates a throttle with a concurrency cap and minimum spacing between job starts
    # construction
  throttle.submit
    @ (state: throttle_state, job_id: string) -> throttle_state
    + enqueues a job by id at the tail of the queue
    # enqueue
  throttle.next_ready
    @ (state: throttle_state) -> tuple[optional[string], throttle_state]

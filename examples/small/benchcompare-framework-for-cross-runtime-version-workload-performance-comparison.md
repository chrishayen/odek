...
benchcompare
  benchcompare.new
    ...
    # construction
  benchcompare.record
    fn (state: suite_state, version_label: string, benchmark_name: string, duration_nanos: i64) -> suite_state
  ...
  benchcompare.time_call
    fn (state: suite_state, version_label: string, benchmark_name: string, elapsed_fn_result: i64) -> suite_state
    + records a sample whose duration was measured by the caller against a monotonic clock
    # data_capture
    -> std.time.now_nanos
  benchcompare.summary
    fn (state: suite_state, version_label: string, benchmark_name: string) -> optional[stat_summary]
    + returns count, mean, min, and max of recorded samples
    - returns none when no samples exist for that key
    # statistics
  benchcompare.compare
    fn (state: suite_state, benchmark_name: string, baseline: string, candidate: string) -> result[f64, string]

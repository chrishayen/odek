...
std
  ...
  std.net
    std.net.tcp_probe
      @ (host: string, port: i32, timeout_ms: i32) -> bool
...
wait_for
  wait_for.check_all
    @ (targets: list[probe_target]) -> list[bool]
    + returns one boolean per target indicating current reachability
    # probing
    -> std.net.tcp_probe
  wait_for.wait_until_ready
    @ (targets: list[probe_target], timeout_ms: i64, interval_ms: i64) -> result[void, list[string]]

...
fsm
  fsm.new
    fn (initial_state: string) -> fsm_state
    + returns a machine positioned in the initial state
    # construction
  fsm.add_transition
    fn (machine: fsm_state, from_state: string, event: string, to_state: string) -> fsm_state
    + registers a transition from a state on an event
    # definition
  fsm.dispatch
    fn (machine: fsm_state, event: string) -> result[fsm_state, string]

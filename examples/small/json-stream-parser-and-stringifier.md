...
json_stream
  json_stream.new_parser
    ...
    # construction
  json_stream.feed
    fn (state: parser_state, chunk: bytes) -> result[tuple[parser_state, list[json_event]], string]
  ...
  json_stream.finish_parser
    fn (state: parser_state) -> result[list[json_event], string]
  ...
  json_stream.write_event
    fn (state: writer_state, event: json_event) -> result[tuple[writer_state, string], string]

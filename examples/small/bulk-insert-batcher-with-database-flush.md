...
bulkwriter
  bulkwriter.new
    fn (max_rows: i32, max_bytes: i64, flush_interval_ms: i64) -> bulkwriter_state
    + creates a writer with the given thresholds
    # construction
  bulkwriter.add
    fn (state: bulkwriter_state, row: string) -> tuple[bool, bulkwriter_state]
  ...
  bulkwriter.take_pending
    fn (state: bulkwriter_state) -> tuple[list[string], bulkwriter_state]
    + returns the pending batch and a reset writer
    # flush
  bulkwriter.should_flush
    fn (state: bulkwriter_state) -> bool

...
std
  ...
  std.panic
    std.panic.try
      @ (body: fn() -> void) -> optional[string]
      + runs body and returns none on clean exit or the captured error message if it crashed
      # recovery

resilient
  resilient.policy
    @ (max_attempts: i32, initial_backoff_ms: i64, backoff_multiplier: f64) -> restart_policy
    + constructs a policy; max_attempts of 0 means retry forever
    # policy
  resilient.run
    @ (body: fn() -> void, policy: restart_policy) -> result[i32, string]
  ...
  resilient.run_forever
    @ (body: fn() -> void, initial_backoff_ms: i64, backoff_multiplier: f64) -> void

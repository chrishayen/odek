...
migrator
  migrator.register
    @ (version: i64, up: migration_fn, down: migration_fn) -> migrator_state
  ...
  migrator.applied_set
    ...
    # state
  migrator.migrate
    @ (state: migrator_state, executor: sql_executor) -> result[i32, string]
  ...
  migrator.rollback_last
    @ (state: migrator_state, executor: sql_executor) -> result[i64, string]

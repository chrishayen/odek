...
cache_cleanup
  cache_cleanup.scan
    fn (roots: list[string]) -> result[list[cache_entry], string]
  ...
  cache_cleanup.total_bytes
    fn (entries: list[cache_entry]) -> i64
    + sums the size of every entry
    # reporting
  cache_cleanup.filter_older_than
    fn (entries: list[cache_entry], max_age_seconds: i64) -> list[cache_entry]
    + keeps entries whose modified time is older than now minus the threshold
    # selection
    -> std.time.now_seconds
  cache_cleanup.purge
    fn (entries: list[cache_entry]) -> result[i64, string]

...
allot
  allot.compile
    fn (template: string) -> result[pattern_state, string]
    + parses a template with <name> placeholders and returns a matcher
    - returns error when a placeholder is unclosed
    # compilation
  allot.match
    fn (pattern: pattern_state, input: string) -> result[map[string,string], string]
  ...
  allot.placeholder_names
    fn (pattern: pattern_state) -> list[string]

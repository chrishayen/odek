...
std
  std.image
    std.image.decode
      ...
      # image
    std.image.encode
      fn (img: raw_image, format: string) -> result[bytes, string]
...
resize
  resize.bilinear
    fn (img: raw_image, width: i32, height: i32) -> result[raw_image, string]
    ...
    # resampling
  resize.fit
    fn (img: raw_image, max_width: i32, max_height: i32) -> raw_image
    + resizes preserving aspect ratio so the result fits inside the box
    + returns the input unchanged when it already fits
    # resampling
  resize.process_bytes
    fn (data: bytes, width: i32, height: i32, format: string) -> result[bytes, string]

...
shell
  shell.run
    @ (program: string, args: list[string]) -> result[command_output, string]
    + runs a command with no stdin and returns its output
    - returns error when the program cannot be launched
    # execution
    -> std.process.spawn_and_wait
  shell.run_with_input
    @ (program: string, args: list[string], input: bytes) -> result[command_output, string]
    + runs a command piping input to its stdin
    # execution
    -> std.process.spawn_and_wait
  shell.succeeded
    @ (output: command_output) -> bool

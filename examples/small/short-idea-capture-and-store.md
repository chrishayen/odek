...
ideas
  ideas.capture
    fn (store_path: string, title: string, body: string) -> result[void, string]
    ...
    -> std.fs.append
  ideas.list_all
    fn (store_path: string) -> result[list[idea_entry], string]
    + returns every stored idea in chronological order
    + returns an empty list when the store is missing
    # listing
    -> std.fs.read_all
  ideas.search
    fn (store_path: string, query: string) -> result[list[idea_entry], string]

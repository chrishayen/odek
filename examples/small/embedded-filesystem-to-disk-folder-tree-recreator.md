...
fs_unembed
  fs_unembed.list_entries
    fn (efs: embedded_fs, dir: string) -> result[list[embedded_entry], string]
  ...
  fs_unembed.materialize_entry
    fn (efs: embedded_fs, src: string, dst_root: string) -> result[void, string]
    + creates the corresponding directory or writes the file under dst_root
    # extraction
    -> std.fs.mkdirs
    -> std.fs.write_bytes
  fs_unembed.materialize_tree
    fn (efs: embedded_fs, src_root: string, dst_root: string) -> result[void, string]

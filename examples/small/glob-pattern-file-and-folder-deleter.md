...
std
  std.fs
    std.fs.glob
      @ (root: string, pattern: string) -> result[list[string], string]
...
glob_delete
  glob_delete.resolve
    @ (root: string, patterns: list[string], ignore: list[string]) -> result[list[string], string]
    + returns the set of paths matching any pattern and no ignore pattern
    - returns error on an invalid pattern
    # matching
    -> std.fs.glob
  glob_delete.delete_all
    @ (paths: list[string]) -> result[i32, string]

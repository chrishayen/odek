...
struct_map
  struct_map.describe
    fn (fields: list[tuple[string, string]]) -> struct_schema
    + returns a schema listing field names and their declared type tags
    # schema
  struct_map.map
    fn (src_schema: struct_schema, src_values: map[string, string], dst_schema: struct_schema) -> result[map[string, string], list[string]]
    ...
    # mapping
  struct_map.coerce
    fn (value: string, from_type: string, to_type: string) -> result[string, string]
  ...
  struct_map.rename_rule
    fn (from_field: string, to_field: string) -> field_rule

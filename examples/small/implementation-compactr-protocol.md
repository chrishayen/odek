...
compactr
  ...
  compactr.schema_add_field
    @ (s: compactr_schema, name: string, type_tag: string) -> result[compactr_schema, string]
    ...
    # schema
  compactr.encode
    @ (s: compactr_schema, record: map[string, bytes]) -> result[bytes, string]
    ...
    # encoding
  compactr.decode
    @ (s: compactr_schema, data: bytes) -> result[map[string, bytes], string]

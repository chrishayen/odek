...
langdetect
  langdetect.new_client
    fn (api_key: string, endpoint: string) -> langdetect_client
    + stores credentials and endpoint for subsequent calls
    - accepts an empty endpoint and uses a sensible default
    # construction
  langdetect.detect
    fn (client: langdetect_client, text: string) -> result[string, string]
  ...
  langdetect.detect_batch
    fn (client: langdetect_client, texts: list[string]) -> result[list[string], string]

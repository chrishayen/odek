...
module_ideas
  module_ideas.new_board
    ...
    # construction
  module_ideas.submit
    fn (state: board_state, title: string, description: string, author: string) -> tuple[string, board_state]
    + adds a new idea and returns its assigned id alongside updated state
    # submission
    -> std.time.now_seconds
  module_ideas.upvote
    fn (state: board_state, id: string, voter: string) -> result[board_state, string]

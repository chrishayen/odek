...
puzzle_lib
  ...
  puzzle_lib.add_puzzle
    @ (store: puzzle_store, id: string, prompt: string, expected: string) -> result[void, string]
  ...
  puzzle_lib.get_prompt
    @ (store: puzzle_store, id: string) -> optional[string]
    + returns the prompt for the puzzle, or none if unknown
    # retrieval
  puzzle_lib.submit
    @ (store: puzzle_store, id: string, answer: string) -> result[bool, string]
    ...
    # grading
  puzzle_lib.hint
    @ (store: puzzle_store, id: string, index: i32) -> optional[string]

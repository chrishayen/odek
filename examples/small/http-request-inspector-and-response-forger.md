...
http_inspect
  ...
  http_inspect.set_response
    fn (state: inspect_state, status: i32, headers: map[string,string], body: bytes) -> inspect_state
    + installs the response the next incoming request will receive
    # configuration
  http_inspect.handle
    fn (state: inspect_state, req: http_request) -> tuple[http_response, inspect_state]
    + records the request in the buffer and returns the configured response
    # dispatch
  http_inspect.captures
    fn (state: inspect_state) -> list[http_request]

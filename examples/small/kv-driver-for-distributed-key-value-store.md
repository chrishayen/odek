...
kv_driver
  kv_driver.connect
    fn (host: string, port: i32) -> result[session_state, string]
    + opens a session to the server
    - returns error when the connection fails
    # session
    -> std.net.dial_tcp
  kv_driver.put
    fn (session: session_state, bucket: string, key: string, value: bytes) -> result[void, string]
    ...
    -> std.net.read_exact
  kv_driver.get
    fn (session: session_state, bucket: string, key: string) -> result[optional[bytes], string]
    ...
    -> std.net.read_exact
  kv_driver.delete
    fn (session: session_state, bucket: string, key: string) -> result[void, string]

...
bradley_terry
  bradley_terry.new
    fn (item_ids: list[string]) -> bt_state
    + creates a model with one strength parameter per item, all initialized to 1.0
    # construction
  bradley_terry.record_match
    fn (state: bt_state, winner: string, loser: string) -> bt_state
    + increments the win count for (winner, loser)
    # data_ingest
  bradley_terry.fit
    fn (state: bt_state, max_iterations: i32, tolerance: f64) -> bt_state
  ...
  bradley_terry.win_probability
    fn (state: bt_state, a: string, b: string) -> result[f64, string]

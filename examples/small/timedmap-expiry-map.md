...
timedmap
  timedmap.new
    ...
    # construction
  timedmap.set
    fn (state: timedmap_state, key: string, value: bytes, ttl_ms: i64) -> timedmap_state
    + stores the value with an expiration ttl_ms milliseconds in the future
    + overwrites any existing entry for key
    # mutation
    -> std.time.now_millis
  timedmap.get
    fn (state: timedmap_state, key: string) -> tuple[optional[bytes], timedmap_state]
    ...
    -> std.time.now_millis
  timedmap.remove
    fn (state: timedmap_state, key: string) -> timedmap_state
    + removes the entry for key if present
    # mutation
  timedmap.sweep
    fn (state: timedmap_state) -> timedmap_state

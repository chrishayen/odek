...
prompt_chain
  ...
  prompt_chain.add_step
    fn (state: chain_state, id: string, question: string, next_fn: transition_fn) -> result[chain_state, string]
    ...
    # configuration
  prompt_chain.run
    fn (state: chain_state, start_id: string, reader: input_reader, writer: output_writer) -> result[map[string, string], string]

...
subflag
  subflag.new
    @ (name: string) -> parser_state
    + creates a parser with the given program name and no flags or subcommands
    # construction
  subflag.add_flag
    @ (parser: parser_state, sub: string, name: string, default_value: string) -> parser_state
    + registers a flag on the given subcommand, or on the root when sub is ""
    # registration
  subflag.add_subcommand
    @ (parser: parser_state, name: string) -> parser_state
    + registers a subcommand name
    # registration
  subflag.parse
    @ (parser: parser_state, args: list[string]) -> result[parse_result, string]

...
ddns_client
  ddns_client.detect_public_ip
    fn (resolver_url: string) -> result[string, string]
  ...
  ddns_client.has_changed
    fn (current: string, last_known: optional[string]) -> bool
    + returns true when last_known is none or differs from current
    # diff
  ddns_client.update_record
    fn (provider: provider_config, hostname: string, ip: string) -> result[void, string]
    ...
    -> std.http.put_json
  ddns_client.tick
    fn (state: ddns_state) -> result[ddns_state, string]

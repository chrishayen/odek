...
config
  config.parse
    ...
    # parsing
  config.get_string
    @ (cfg: map[string, string], key: string, default_value: string) -> string
    + returns the value for key, or default_value when the key is absent
    # lookup
  config.get_int
    @ (cfg: map[string, string], key: string, default_value: i64) -> result[i64, string]
    + parses the value as a signed integer
    - returns error when the value exists but is not numeric
    # lookup
  config.get_bool
    @ (cfg: map[string, string], key: string, default_value: bool) -> result[bool, string]

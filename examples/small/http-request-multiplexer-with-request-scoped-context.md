...
mux
  mux.new
    ...
    # construction
  mux.handle
    fn (m: mux_state, method: string, pattern: string, handler: fn(http_request, map[string, string]) -> http_response) -> mux_state
    + registers a handler for a (method, pattern) pair
    ? pattern supports "/prefix/:name" style parameters
    # routing
  mux.dispatch
    fn (m: mux_state, req: http_request, ctx: map[string, string]) -> http_response
    ...
    # routing
  mux.match_pattern
    fn (pattern: string, path: string) -> optional[map[string, string]]

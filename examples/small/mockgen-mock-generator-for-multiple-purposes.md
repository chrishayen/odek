...
mockgen
  mockgen.new
    ...
    # construction
  mockgen.when
    fn (state: mock_state, method: string, args: list[string], returns: string) -> mock_state
    + registers that calling `method` with the given args returns the given value
    # setup
  mockgen.call
    fn (state: mock_state, method: string, args: list[string]) -> tuple[result[string, string], mock_state]

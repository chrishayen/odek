...
request_args
  ...
  request_args.add_field
    fn (schema: schema_state, name: string, kind: string, required: bool) -> schema_state
    + registers a field with its expected type ("string", "int", "bool") and required flag
    # schema
  request_args.parse_query_args
    fn (schema: schema_state, query: string) -> result[map[string, string], list[string]]
  ...
  request_args.parse_json_args
    fn (schema: schema_state, body: string) -> result[map[string, string], list[string]]

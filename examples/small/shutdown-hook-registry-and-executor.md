...
shutdown
  shutdown.new
    ...
    # construction
  shutdown.register
    fn (state: shutdown_state, priority: i32, name: string, hook: hook_fn) -> shutdown_state
    + adds a hook to the registry; lower priority numbers run first
    # registration
  shutdown.unregister
    fn (state: shutdown_state, name: string) -> shutdown_state
    + removes a hook by name; does nothing if not present
    # registration
  shutdown.run_all
    fn (state: shutdown_state) -> list[hook_result]
  ...
  shutdown.wait_and_run
    fn (state: shutdown_state, signals: list[string]) -> list[hook_result]

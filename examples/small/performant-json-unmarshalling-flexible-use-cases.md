...
json_unmarshal
  json_unmarshal.parse
    @ (raw: string) -> result[json_value, parse_error]
    + returns a generic value tree for well-formed JSON
    - returns parse_error with byte offset on malformed input
    # parsing
  json_unmarshal.bind
    @ (value: json_value, schema: list[field_spec]) -> result[map[string, json_value], list[string]]
  ...
  json_unmarshal.bind_lax
    @ (value: json_value, schema: list[field_spec]) -> map[string, json_value]

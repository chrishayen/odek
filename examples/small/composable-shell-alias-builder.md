...
std
  std.strings
    std.strings.replace_all
      fn (s: string, old: string, new: string) -> string
...
alias
  alias.new_store
    ...
    # construction
  alias.define
    fn (store: alias_store, name: string, template: string) -> alias_store
    + registers a new alias with a template containing {0}, {1}, ... placeholders
    + overwrites any existing alias with the same name
    # registration
  alias.expand
    fn (store: alias_store, name: string, args: list[string]) -> result[string, string]
    ...
    -> std.strings.replace_all
  alias.list_names
    fn (store: alias_store) -> list[string]

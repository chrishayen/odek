...
tempdb
  tempdb.new
    ...
    # construction
  tempdb.set
    @ (state: tempdb_state, key: string, value: string, ttl_seconds: i64) -> tempdb_state
    + stores the value with an expiry computed from now + ttl
    # write
    -> std.time.now_seconds
  tempdb.get
    @ (state: tempdb_state, key: string) -> optional[string]
    ...
    -> std.time.now_seconds
  tempdb.delete
    @ (state: tempdb_state, key: string) -> tempdb_state
    + removes the key if present
    # write
  tempdb.purge
    @ (state: tempdb_state) -> tempdb_state

...
row_scan
  row_scan.field_map
    @ (target_fields: list[string], columns: list[string]) -> result[list[i32], string]
    ...
    # mapping
  row_scan.scan_one
    @ (columns: list[string], row: list[optional[bytes]], field_map: list[i32], target_fields: list[string]) -> result[map[string, optional[bytes]], string]
    + produces a field->value map for a single row
    - returns error when the row has fewer cells than expected columns
    # scanning
  row_scan.scan_all
    @ (columns: list[string], rows: list[list[optional[bytes]]], target_fields: list[string]) -> result[list[map[string, optional[bytes]]], string]

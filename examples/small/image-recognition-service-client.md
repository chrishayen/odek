...
image_client
  image_client.new
    fn (base_url: string, api_key: string) -> image_client_state
    + creates a client that attaches the api key to each request
    # construction
  image_client.predict_url
    fn (state: image_client_state, image_url: string) -> result[list[tuple[string, f64]], string]
  ...
  image_client.predict_bytes
    fn (state: image_client_state, image: bytes) -> result[list[tuple[string, f64]], string]

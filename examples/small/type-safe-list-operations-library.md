...
list_ops
  list_ops.chunk
    @ (xs: list[i32], size: i32) -> list[list[i32]]
  ...
  list_ops.unique
    ...
    # deduplication
  list_ops.partition
    @ (xs: list[i32], predicate: i32 -> bool) -> tuple[list[i32], list[i32]]
    + returns (matching, non_matching) preserving order within each
    # partitioning
  list_ops.zip
    @ (xs: list[i32], ys: list[string]) -> list[tuple[i32, string]]
    + pairs elements by position, truncating to the shorter input
    # zipping
  list_ops.group_by
    @ (xs: list[i32], key: i32 -> string) -> map[string, list[i32]]

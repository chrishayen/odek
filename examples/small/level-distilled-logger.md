...
std
  std.io
    std.io.write_string
      @ (sink: writer, line: string) -> result[void, string]
...
level_log
  level_log.new
    @ (sink: writer, min_level: log_level) -> logger_state
    + returns a logger that writes to sink and drops records below min_level
    # construction
  level_log.set_level
    @ (state: logger_state, min_level: log_level) -> logger_state
    + returns a logger with an updated threshold
    # configuration
  level_log.log
    @ (state: logger_state, level: log_level, message: string) -> result[void, string]

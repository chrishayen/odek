...
webshot
  webshot.new_session
    @ (browser_path: string) -> result[session_handle, string]
    + launches a headless browser process and returns a session handle
    - returns error when the browser binary is missing
    # session
  webshot.capture
    @ (session: session_handle, url: string, width: i32, height: i32) -> result[bytes, string]
  ...
  webshot.capture_to_file
    @ (session: session_handle, url: string, width: i32, height: i32, path: string) -> result[void, string]
    + captures a url and writes the resulting PNG to disk
    # capture
    -> std.fs.write_all
  webshot.close_session
    @ (session: session_handle) -> void

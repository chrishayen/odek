...
std
  std.http
    std.http.get_json
      fn (url: string, auth_token: string) -> result[string, string]
...
dep_starrer
  dep_starrer.extract_repos
    fn (manifest: string) -> list[string]
  ...
  dep_starrer.is_already_starred
    fn (repo: string, auth_token: string) -> result[bool, string]
  ...
  dep_starrer.star_repo
    fn (repo: string, auth_token: string) -> result[void, string]
    + sends a star request for the repo
    - returns error when the host rejects the request
    # mutation
  dep_starrer.star_all_new
    fn (manifest: string, auth_token: string) -> result[list[string], string]

...
envconfig
  envconfig.define_field
    fn (name: string, env_key: string, kind: string, required: bool, default_value: string) -> field_spec
    + returns a field descriptor
    + kind is one of "string", "int", "bool", "float"
    # schema
  envconfig.load
    fn (schema: list[field_spec]) -> result[map[string, config_value], string]
    ...
    -> std.env.get
  envconfig.coerce
    fn (raw: string, kind: string) -> result[config_value, string]

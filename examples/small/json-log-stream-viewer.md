...
json_log_viewer
  ...
  json_log_viewer.ingest_line
    fn (state: log_view_state, line: string) -> log_view_state
    + parses and appends one log line; malformed lines are kept as raw
    # ingestion
    -> std.json.parse_object
  json_log_viewer.filter
    fn (state: log_view_state, field: string, substring: string) -> list[log_entry]
  ...
  json_log_viewer.columns
    fn (state: log_view_state) -> list[string]

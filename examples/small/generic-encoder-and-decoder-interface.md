...
codec
  codec.new_registry
    ...
    # construction
  codec.register
    @ (reg: codec_registry, name: string, encode_fn: fn(bytes) -> bytes, decode_fn: fn(bytes) -> result[bytes, string]) -> codec_registry
    + stores the codec pair under the given name, replacing any existing one
    # registration
  codec.encode
    @ (reg: codec_registry, name: string, data: bytes) -> result[bytes, string]

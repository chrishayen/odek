...
deploy_notify
  deploy_notify.format_message
    fn (app: string, version: string, actor: string) -> string
  ...
  deploy_notify.register_sink
    fn (state: dispatcher_state, name: string) -> dispatcher_state
    + adds a named notification sink to the dispatcher
    # configuration
  deploy_notify.dispatch
    fn (state: dispatcher_state, message: string) -> list[tuple[string, bool]]

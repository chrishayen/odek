...
i18n
  i18n.new_catalog
    ...
    # construction
  i18n.add_message
    fn (cat: catalog, locale: string, key: string, forms: map[string, string]) -> catalog
  ...
  i18n.plural_category
    fn (locale: string, n: i64) -> string
    + returns the CLDR plural category for n under the given locale
    ? categories include "zero" "one" "two" "few" "many" "other"
    # plurals
  i18n.translate
    fn (cat: catalog, locale: string, key: string, n: i64, vars: map[string, string]) -> result[string, string]

...
link_sim
  link_sim.wrap_reader
    fn (inner: reader, bytes_per_second: i64) -> link_reader_state
    + wraps an existing reader with the given throttle rate
    ? bytes_per_second must be positive
    # construction
  link_sim.wrap_writer
    fn (inner: writer, bytes_per_second: i64) -> link_writer_state
    + wraps an existing writer with the given throttle rate
    # construction
  link_sim.read
    fn (state: link_reader_state, max: i32) -> result[tuple[bytes, link_reader_state], string]
    ...
    -> std.time.sleep_nanos
  link_sim.write
    fn (state: link_writer_state, data: bytes) -> result[link_writer_state, string]

...
assert
  assert.equal
    @ (expected: string, actual: string) -> assertion_result
  ...
  assert.not_equal
    ...
    # equality
  assert.contains
    @ (haystack: string, needle: string) -> assertion_result
    + returns ok when haystack contains needle
    - returns a failing result with both values otherwise
    # substring
  assert.list_equal
    @ (expected: list[string], actual: list[string]) -> assertion_result
    ...
    # collection
  assert.compose
    @ (name: string, checks: list[assertion_result]) -> assertion_result

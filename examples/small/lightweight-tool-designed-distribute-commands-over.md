...
std
  std.ssh
    std.ssh.run
      @ (host: string, user: string, command: string) -> result[ssh_output, string]
...
fanout
  fanout.new_task
    @ (hosts: list[string], user: string, command: string) -> task
    + creates a task that will run command on every host as user
    # construction
  fanout.run
    @ (t: task) -> list[host_result]
    ...
    -> std.ssh.run
  fanout.summarize
    @ (results: list[host_result]) -> summary

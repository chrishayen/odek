...
filter
  filter.new
    fn (candidates: list[string]) -> filter_state
    + creates a filter state holding the candidate lines and an empty query
    # construction
  filter.set_query
    fn (state: filter_state, query: string) -> filter_state
    + returns a state with the query replaced
    # query
  filter.matches
    fn (state: filter_state) -> list[string]
    ...
    # filtering
  filter.select
    fn (state: filter_state, index: i32) -> result[string, string]

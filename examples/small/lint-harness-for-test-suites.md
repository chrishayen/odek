...
lintharness
  ...
  lintharness.register
    @ (state: harness_state, name: string, linter: fn(list[string]) -> list[finding]) -> harness_state
    + registers a linter under a unique name
    # registration
  lintharness.run
    @ (state: harness_state, root: string) -> result[run_report, string]

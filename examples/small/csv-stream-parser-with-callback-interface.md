...
csv_stream
  csv_stream.new
    fn (delimiter: string, has_header: bool) -> csv_parser_state
    + creates a parser with the given field delimiter
    # construction
  csv_stream.feed
    fn (state: csv_parser_state, chunk: string, on_row: fn(list[string]) -> void) -> csv_parser_state
    ...
    # parsing
  csv_stream.finish
    fn (state: csv_parser_state, on_row: fn(list[string]) -> void) -> result[void, string]
    + flushes any buffered final row
    - returns error when an unterminated quoted field remains
    # finalization
  csv_stream.header
    fn (state: csv_parser_state) -> optional[list[string]]

...
atomic_write
  atomic_write.open
    @ (target_path: string) -> result[stream_state, string]
    ...
    -> std.fs.open_temp
  atomic_write.write
    @ (state: stream_state, chunk: bytes) -> result[stream_state, string]
    + appends a chunk to the underlying temp file
    - returns error on write failure
    # writing
    -> std.fs.write
  atomic_write.commit
    @ (state: stream_state) -> result[void, string]

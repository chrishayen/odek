...
rerun
  rerun.new_watcher
    fn (paths: list[string]) -> watcher_state
    + creates a watcher tracking the given paths with no baseline yet
    # construction
  rerun.poll
    fn (state: watcher_state) -> tuple[watcher_state, list[string]]
    ...
    -> std.fs.stat_mtime
  rerun.run_on_change
    fn (state: watcher_state, cmd: string, args: list[string]) -> result[tuple[watcher_state, bool], string]

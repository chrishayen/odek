...
logdump
  logdump.new_logger
    ...
    # construction
  logdump.add_sink
    fn (state: logger_state, sink_name: string) -> logger_state
    + registers a named sink that will receive formatted records
    # sinks
  logdump.set_level
    fn (state: logger_state, level: string) -> result[logger_state, string]
    + changes the active threshold
    - returns error on an unknown level name
    # configuration
  logdump.log
    fn (state: logger_state, level: string, message: string) -> list[log_record]

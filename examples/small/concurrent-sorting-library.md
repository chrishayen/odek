...
std
  std.concurrency
    std.concurrency.parallel_map
      fn (chunks: list[list[i64]], f: fn(list[i64]) -> list[i64]) -> list[list[i64]]
      + applies f to each chunk in parallel and returns the results in order
      # parallelism

parallel_sort
  parallel_sort.sort
    fn (items: list[i64], workers: i32) -> list[i64]
  ...
  parallel_sort.sort_by
    fn (items: list[i64], workers: i32, less: fn(i64, i64) -> bool) -> list[i64]
    + sorts using the caller's comparator
    # sort
    -> std.concurrency.parallel_map
  parallel_sort.merge_sorted
    fn (a: list[i64], b: list[i64], less: fn(i64, i64) -> bool) -> list[i64]

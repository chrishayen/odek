...
std
  std.net
    std.net.http_request
      fn (method: string, url: string, headers: map[string, string], body: bytes) -> result[tuple[i32, map[string, string], bytes], string]
...
http_client
  http_client.new
    ...
    # construction
  http_client.header
    fn (c: client_state, name: string, value: string) -> client_state
    + adds a default header applied to every request
    # configuration
  http_client.build
    fn (c: client_state, method: string, path: string) -> request_builder
    + starts a fluent request builder for a method and path
    # building
  http_client.with_query
    fn (rb: request_builder, key: string, value: string) -> request_builder
    + appends a query parameter
    # building
  http_client.with_body
    fn (rb: request_builder, body: bytes) -> request_builder
    + sets the request body
    # building
  http_client.send
    fn (rb: request_builder) -> result[tuple[i32, map[string, string], bytes], string]

...
backoff
  backoff.new
    @ (initial_ms: i64, max_ms: i64, factor: f64) -> backoff_state
    ...
    # construction
  backoff.next
    @ (state: backoff_state) -> tuple[i64, backoff_state]
    + returns the current delay and an advanced state with delay * factor, capped at max_ms
    # scheduling
  backoff.reset
    @ (state: backoff_state) -> backoff_state
    + returns state restored to initial_ms
    # scheduling
  backoff.retry
    @ (state: backoff_state, max_attempts: i32, op: fn() -> result[bool, string]) -> result[bool, string]

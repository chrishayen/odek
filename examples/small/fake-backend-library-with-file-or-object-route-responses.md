...
fakeback
  fakeback.new
    ...
    # construction
  fakeback.add_route
    fn (state: fakeback_state, method: string, path: string, response: fake_response) -> fakeback_state
    + registers a response keyed by method and path
    # registration
  fakeback.respond_from_file
    fn (path: string, status: i32) -> result[fake_response, string]
    + loads response body from disk and pairs it with a status code
    - returns error when the file cannot be read
    # response_building
    -> std.fs.read_all
  fakeback.respond_from_literal
    fn (body: string, status: i32) -> fake_response
    + wraps a literal body and status into a response
    # response_building
  fakeback.handle
    fn (state: fakeback_state, method: string, path: string) -> result[fake_response, string]

...
env_loader
  ...
  env_loader.add_string
    fn (schema: schema_state, field: string, var_name: string, default_value: optional[string], required: bool) -> schema_state
    + registers a string-valued field bound to an environment variable
    # schema
  env_loader.add_int
    fn (schema: schema_state, field: string, var_name: string, default_value: optional[i64], required: bool) -> schema_state
    + registers an integer-valued field
    # schema
  env_loader.add_bool
    fn (schema: schema_state, field: string, var_name: string, default_value: optional[bool], required: bool) -> schema_state
    + registers a boolean-valued field
    # schema
  env_loader.load
    fn (schema: schema_state) -> result[map[string, string], string]

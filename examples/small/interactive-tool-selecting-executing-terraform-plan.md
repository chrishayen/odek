...
std
  ...
  std.process
    std.process.run
      @ (argv: list[string], cwd: string) -> result[process_output, string]
...
iac_runner
  iac_runner.discover_modules
    @ (root: string, marker_file: string) -> result[list[string], string]
    + returns directories under root that contain marker_file
    ? marker_file identifies a module (e.g. "main.tf")
    # discovery
    -> std.fs.walk
  iac_runner.plan
    @ (module_dir: string, tool_path: string) -> result[process_output, string]
  ...
  iac_runner.run_batch
    @ (module_dirs: list[string], action: string, tool_path: string) -> list[batch_result]

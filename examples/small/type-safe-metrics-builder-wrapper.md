...
metrics_builder
  ...
  metrics_builder.build_counter
    @ (reg: registry, name: string, help: string, labels: list[string]) -> result[tuple[counter_handle, registry], string]
  ...
  metrics_builder.build_gauge
    @ (reg: registry, name: string, help: string, labels: list[string]) -> result[tuple[gauge_handle, registry], string]
    + registers a gauge with the given label schema
    - returns error on a name collision
    # gauge_registration
  metrics_builder.build_histogram
    @ (reg: registry, name: string, help: string, labels: list[string], buckets: list[f64]) -> result[tuple[histogram_handle, registry], string]
  ...
  metrics_builder.counter_inc
    @ (reg: registry, h: counter_handle, values: list[string], delta: f64) -> result[registry, string]
  ...
  metrics_builder.gauge_set
    @ (reg: registry, h: gauge_handle, values: list[string], x: f64) -> result[registry, string]
    + sets the gauge series keyed by values to x
    - returns error on a label-arity mismatch
    # gauge_update
  metrics_builder.histogram_observe
    @ (reg: registry, h: histogram_handle, values: list[string], x: f64) -> result[registry, string]

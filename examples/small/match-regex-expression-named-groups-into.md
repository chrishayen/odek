...
std
  std.regex
    ...
    std.regex.find_named
      @ (state: regex_state, input: string) -> optional[map[string,string]]
...
regroup
  regroup.match_into
    @ (pattern: string, input: string) -> result[map[string,string], string]
    ...
    -> std.regex.find_named
  regroup.bind_fields
    @ (captures: map[string,string], field_tags: map[string,string]) -> result[map[string,string], string]

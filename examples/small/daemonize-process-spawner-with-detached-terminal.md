...
std
  std.os
    ...
    std.os.setsid
      ...
      # process
    std.os.dup2
      fn (src_fd: i32, dst_fd: i32) -> result[void, string]
      + duplicates src_fd onto dst_fd
      - returns error on invalid descriptors
      # io
    std.os.open
      fn (path: string, flags: i32) -> result[i32, string]
...
daemonize
  daemonize.spawn
    fn (stdout_path: string, stderr_path: string) -> result[daemon_handle, string]

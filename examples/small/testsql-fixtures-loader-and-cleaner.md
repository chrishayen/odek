...
std
  ...
  std.db
    std.db.exec
      fn (db: db_handle, sql: string) -> result[void, string]
...
testsql
  ...
  testsql.setup
    ...
    -> std.db.exec
  testsql.teardown
    fn (db: db_handle, tables: list[string]) -> result[void, string]

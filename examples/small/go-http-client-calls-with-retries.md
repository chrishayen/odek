...
http_client
  http_client.new
    @ (max_attempts: i32, base_backoff_millis: i64) -> client_state
    + creates a client configured with retry count and initial backoff
    # construction
  http_client.do_with_retry
    @ (state: client_state, transport_id: string, request: bytes) -> result[bytes, string]
  ...
  http_client.do_concurrent
    @ (state: client_state, transport_id: string, requests: list[bytes], max_parallel: i32) -> list[result[bytes, string]]

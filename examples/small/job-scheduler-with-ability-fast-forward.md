...
scheduler
  scheduler.new
    @ (start: i64) -> scheduler_state
    + creates a scheduler with virtual time set to start
    # construction
  scheduler.schedule
    @ (state: scheduler_state, fire_at: i64, action: job_action) -> scheduler_state
    + enqueues action to fire at virtual time fire_at
    # registration
  scheduler.advance_to
    @ (state: scheduler_state, target: i64) -> tuple[list[job_action], scheduler_state]

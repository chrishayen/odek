...
queue
  queue.new
    fn (max_retries: i32) -> queue_state
    + returns an empty queue that will retry each job up to max_retries times
    # construction
  queue.enqueue
    fn (state: queue_state, name: string, payload: bytes) -> queue_state
    + appends the job to the tail of the queue
    # producer
  queue.dequeue
    fn (state: queue_state) -> tuple[optional[job], queue_state]
    + returns the next job and removes it from the head
    - returns none when the queue is empty
    # consumer
  queue.mark_failed
    fn (state: queue_state, job: job) -> queue_state

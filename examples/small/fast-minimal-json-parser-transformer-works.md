...
ujson
  ujson.walk
    @ (source: string, visitor: json_visitor) -> result[string, string]
    ...
    # traversal
  ujson.read_value
    @ (source: string, path: list[string]) -> optional[string]
    + returns the raw json text of the value at a dotted path
    - returns none when the path does not exist
    # query
  ujson.replace_value
    @ (source: string, path: list[string], replacement: string) -> result[string, string]

...
code_search
  ...
  code_search.index_file
    fn (state: index_state, path: string, content: string) -> index_state
    + adds the document and inserts it into the posting list of each of its trigrams
    # indexing
  code_search.index_tree
    fn (state: index_state, root: string) -> result[index_state, string]
  ...
  code_search.candidate_docs
    fn (state: index_state, query: string) -> list[string]
  ...
  code_search.verify_matches
    fn (path: string, content: string, query: string) -> list[match]
    + returns (line_number, line_text) pairs containing the query substring
    # verification
    -> std.text.split_lines
  code_search.search
    fn (state: index_state, query: string) -> result[list[file_matches], string]

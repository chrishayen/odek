...
std
  std.http
    ...
    std.http.parse_request
      ...
      # networking
    std.http.response
      fn (status: i32, headers: map[string, string], body: bytes) -> http_response
...
web_framework
  ...
  web_framework.add_route
    fn (app: app_state, method: string, pattern: string, handler: fn(request_ctx) -> response_value) -> void
  ...
  web_framework.parse_form
    fn (body: bytes, content_type: string) -> result[map[string, string], string]
    + parses a form-urlencoded request body
    - returns error on unsupported content type
    # request
  web_framework.render_template
    fn (path: string, data: map[string, string]) -> result[string, string]
  ...
  web_framework.session_start
    fn (store: session_store, req: http_request) -> session_state
    + returns an existing session keyed by cookie or creates a new one
    # sessions
    -> std.crypto.random_bytes
  web_framework.session_save
    fn (store: session_store, sess: session_state, resp: http_response) -> http_response

...
std
  ...
  std.image_codec
    ...
    std.image_codec.encode
      fn (img: image_buffer, format: string, quality: i32) -> result[bytes, string]
...
imgsrv
  imgsrv.new_server
    fn (origin_dir: string, cache_dir: string, default_quality: i32) -> imgsrv_state
    + creates a server state rooted at origin and cache directories
    # construction
  imgsrv.store_original
    fn (srv: imgsrv_state, name: string, data: bytes) -> result[string, string]
  ...
  imgsrv.parse_transform
    fn (query: string) -> result[imgsrv_transform, string]
    ...
    # request
  imgsrv.cache_key
    fn (name: string, tx: imgsrv_transform) -> string
    + returns a stable hashed filename for the transformed variant
    # cache
    -> std.crypto.sha256_hex
  imgsrv.get_variant
    fn (srv: imgsrv_state, name: string, tx: imgsrv_transform) -> result[bytes, string]

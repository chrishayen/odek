...
tsstore
  tsstore.point
    fn (timestamp_ms: i64, value: f64, tags: map[string,string]) -> point
    + constructs a single data point
    # construction
  tsstore.series_id
    fn (metric: string, tags: map[string,string]) -> string
    + returns a canonical series identifier by sorting tag keys
    # identity
  tsstore.open
    fn (data_dir: string) -> result[tsstore_state, string]
    + opens or creates a store rooted at data_dir
    # construction
  tsstore.ingest
    fn (state: tsstore_state, metric: string, p: point) -> result[tsstore_state, string]
    ...
    -> std.encoding.varint_encode
  tsstore.range_query
    fn (state: tsstore_state, metric: string, tags: map[string,string], from_ms: i64, to_ms: i64) -> result[list[point], string]
  ...
  tsstore.aggregate_min_max
    fn (points: list[point]) -> tuple[f64,f64]
    + returns (min, max) across the points
    ? on empty input returns (0.0, 0.0)
    # aggregation
  tsstore.downsample
    fn (points: list[point], bucket_ms: i64) -> list[point]
    + groups points into fixed-width time buckets and emits the average per bucket
    # aggregation
  tsstore.publish_event
    fn (state: tsstore_state, category: string, message: string) -> result[tsstore_state, string]
    + appends an event record tagged with the current time
    # events
    -> std.time.now_millis
    -> std.fs.append
  tsstore.events_since
    fn (state: tsstore_state, category: string, since_ms: i64) -> result[list[tuple[i64,string]], string]

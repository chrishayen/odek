...
contract_abi
  contract_abi.parse_signature
    @ (signature: string) -> result[parsed_signature, string]
  ...
  contract_abi.encode_value
    @ (type_name: string, value: bytes) -> result[bytes, string]
  ...
  contract_abi.encode_call
    @ (signature: string, args: list[tuple[string, bytes]]) -> result[bytes, string]
  ...
  contract_abi.encode_call_hex
    @ (signature: string, args: list[tuple[string, bytes]]) -> result[string, string]
    + returns the 0x-prefixed hex string for the packed call
    # call_encoding
    -> std.encoding.hex_encode
  contract_abi.decode_value
    @ (type_name: string, data: bytes, offset: i32) -> result[tuple[bytes, i32], string]
  ...
  contract_abi.decode_return
    @ (types: list[string], data: bytes) -> result[list[bytes], string]
  ...
  contract_abi.decode_event_log
    @ (topic0_signature: string, topics: list[bytes], data: bytes) -> result[map[string, bytes], string]

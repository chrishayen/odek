...
std
  ...
  std.crypto
    ...
    std.crypto.hkdf_expand_label
      @ (secret: bytes, label: string, length: i32) -> bytes
  ...
  std.tls
    std.tls.handshake_client
      @ (server_name: string, initial_data: bytes) -> result[tls_handshake_state, string]
    ...
    std.tls.derive_traffic_keys
      @ (state: tls_handshake_state) -> tuple[bytes, bytes]
...
quic
  quic.connect
    @ (peer: string, server_name: string) -> result[quic_connection, string]
    ...
    -> std.tls.derive_traffic_keys
  quic.accept
    @ (local: string) -> result[quic_listener, string]
    + returns a listener bound to the local address
    # connection
    -> std.net.udp_open
  quic.listener_accept
    @ (l: quic_listener) -> result[quic_connection, string]
    + returns the next incoming connection after the handshake completes
    # connection
  quic.open_stream
    @ (c: quic_connection, bidirectional: bool) -> result[tuple[quic_stream_id, quic_connection], string]
    + returns a new stream id and updated connection state
    - returns error when the peer's stream limit is reached
    # streams
  quic.stream_write
    @ (c: quic_connection, sid: quic_stream_id, data: bytes) -> result[quic_connection, string]
  ...
  quic.update_loss_detection
    ...
    -> std.time.now_millis
  quic.close
    @ (c: quic_connection, error_code: i64, reason: string) -> result[void, string]

...
wasm
  wasm.parse_module
    @ (binary: bytes) -> result[module, string]
  ...
  wasm.parse_type_section
    @ (data: bytes) -> result[list[func_type], string]
  ...
  wasm.invoke
    ...
    # execution
  wasm.step
    @ (inst: instance, frame: call_frame) -> result[call_frame, string]
  ...
  wasm.decode_value_type
    @ (byte_val: u8) -> result[value_type, string]

...
netstack
  netstack.new_interface
    @ (mac: bytes, ipv4: u32, mtu: i32) -> iface_state
    + creates an interface with the given link and network addresses
    # construction
  netstack.inject_frame
    @ (ifc: iface_state, frame: bytes) -> result[void, string]
  ...
  netstack.drain_frame
    @ (ifc: iface_state) -> optional[bytes]
    + returns the next outbound frame if any is queued
    # egress
  netstack.poll
    @ (ifc: iface_state) -> result[void, string]

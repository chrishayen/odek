...
video_platform
  ...
  video_platform.enroll_member
    fn (state: platform_state, handle: string) -> result[tuple[member_id, platform_state], string]
  ...
  video_platform.grant_voting_weight
    fn (state: platform_state, member: member_id, weight: i64) -> result[platform_state, string]
    + sets the voting weight for a member
    - returns error when weight is negative
    # membership
  video_platform.upload_video
    fn (state: platform_state, owner: member_id, title: string, content_hash: string) -> result[tuple[video_id, platform_state], string]
  ...
  video_platform.list_videos
    fn (state: platform_state) -> list[video]
    + returns all cataloged videos in upload order
    # catalog
  video_platform.file_proposal
    fn (state: platform_state, proposer: member_id, body: string, deadline: i64) -> result[tuple[proposal_id, platform_state], string]
  ...
  video_platform.cast_vote
    fn (state: platform_state, voter: member_id, proposal: proposal_id, approve: bool) -> result[platform_state, string]
  ...
  video_platform.tally
    fn (state: platform_state, proposal: proposal_id) -> result[tally, string]
    + returns the current approve and reject totals
    - returns error when the proposal does not exist
    # governance
  video_platform.close_proposal
    fn (state: platform_state, proposal: proposal_id) -> result[tuple[proposal_outcome, platform_state], string]
  ...
  video_platform.takedown_video
    fn (state: platform_state, video: video_id, proposal: proposal_id) -> result[platform_state, string]

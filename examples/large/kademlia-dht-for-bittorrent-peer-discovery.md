...
std
  ...
  std.net
    std.net.udp_send
      fn (host: string, port: u16, data: bytes) -> result[void, string]
      + sends a UDP datagram to host:port
      - returns error on resolve or send failure
      # networking
    std.net.udp_recv
      fn (timeout_ms: i32) -> result[udp_packet, string]
...
dht
  ...
  dht.distance
    ...
    # metric
  dht.new_table
    fn (self_id: bytes, k: i32) -> table_state
    + creates an empty routing table with 160 k-buckets of capacity k
    # construction
  dht.table_insert
    fn (table: table_state, id: bytes, host: string, port: u16) -> table_state
    ...
    -> std.time.now_seconds
  dht.table_closest
    fn (table: table_state, target: bytes, count: i32) -> list[contact]
    + returns the count contacts closest to target by XOR distance
    # routing
    -> dht.distance
  dht.encode_ping
    fn (transaction_id: bytes, self_id: bytes) -> bytes
    + encodes a ping query as bencoded bytes
    # rpc_codec
  dht.encode_find_node
    fn (transaction_id: bytes, self_id: bytes, target: bytes) -> bytes
    + encodes a find_node query
    # rpc_codec
  dht.encode_get_peers
    fn (transaction_id: bytes, self_id: bytes, info_hash: bytes) -> bytes
    + encodes a get_peers query
    # rpc_codec
  dht.encode_announce_peer
    fn (transaction_id: bytes, self_id: bytes, info_hash: bytes, port: u16, token: bytes) -> bytes
    + encodes an announce_peer query
    # rpc_codec
  dht.decode_message
    fn (data: bytes) -> result[dht_message, string]
  ...
  dht.iterative_find_node
    fn (table: table_state, target: bytes, alpha: i32) -> tuple[table_state, list[contact]]

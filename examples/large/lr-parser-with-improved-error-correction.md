...
std
  std.collections
    ...
    std.collections.priority_queue_push
      @ (q: pqueue_state, item: string, priority: i32) -> pqueue_state
      + inserts an item with a priority key
      # data_structures
    std.collections.priority_queue_pop
      @ (q: pqueue_state) -> tuple[optional[string], pqueue_state]
...
lr_parser
  lr_parser.parse_grammar
    @ (source: string) -> result[grammar_state, string]
  ...
  lr_parser.recover_minimal_edits
    @ (table: parse_table, tokens: list[token], err: parse_error) -> list[edit]
  ...
  lr_parser.apply_edits
    @ (tokens: list[token], edits: list[edit]) -> list[token]
    + returns a new token stream with the edits applied in order
    # error_recovery
  lr_parser.format_error
    @ (err: parse_error, edits: list[edit]) -> string

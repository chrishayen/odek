...
std
  ...
  std.yaml
    std.yaml.parse
      fn (raw: string) -> result[yaml_node, string]
      + parses YAML text into a node tree
      - returns error on malformed input
      # serialization
  std.fs
    std.fs.write_file
      fn (path: string, content: bytes, mode: i32) -> result[void, string]
...
cloudinit
  cloudinit.fetch_metadata
    fn (metadata_url: string) -> result[instance_metadata, string]
  ...
  cloudinit.parse_user_config
    fn (raw: string) -> result[user_config, string]
  ...
  cloudinit.build_plan
    fn (meta: instance_metadata, config: user_config, distribution: string) -> list[init_action]
  ...
  cloudinit.apply_set_hostname
    fn (hostname: string) -> result[void, string]
  ...
  cloudinit.apply_add_user
    fn (username: string, ssh_keys: list[string]) -> result[void, string]
  ...
  cloudinit.apply_run_command
    ...
    -> std.process.run
  cloudinit.run_plan
    fn (plan: list[init_action]) -> list[result[void, string]]

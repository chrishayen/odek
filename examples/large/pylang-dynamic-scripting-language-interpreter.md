...
pylang
  ...
  pylang.parse
    ...
    # parsing
  pylang.compile
    @ (ast: ast_node) -> result[code_object, string]
  ...
  pylang.vm_install_builtin
    @ (vm: vm_state, name: string, fn: builtin_fn) -> vm_state
    + returns a new vm with the builtin bound to the given name
    # builtins
  pylang.vm_exec
    @ (vm: vm_state, code: code_object) -> result[tuple[value, vm_state], string]
    + runs the code object and returns its final value plus updated vm
    - returns error on unhandled exceptions
    # execution
  pylang.vm_call
    @ (vm: vm_state, fn: value, args: list[value]) -> result[tuple[value, vm_state], string]
  ...
  pylang.gc_collect
    ...
    # memory
  pylang.eval_string
    @ (vm: vm_state, source: string) -> result[tuple[value, vm_state], string]

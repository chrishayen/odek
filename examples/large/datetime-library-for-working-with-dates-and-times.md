...
datetime
  datetime.new
    @ (year: i32, month: i32, day: i32, hour: i32, minute: i32, second: i32) -> result[datetime, string]
  ...
  datetime.weekday
    ...
    # calendar
  datetime.format
    @ (dt: datetime, pattern: string) -> result[string, string]
    + formats using tokens like YYYY, MM, DD, HH, mm, ss
    - returns error on unknown tokens
    # formatting
  datetime.parse
    @ (input: string, pattern: string) -> result[datetime, string]

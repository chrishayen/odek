...
std
  ...
  std.encoding
    ...
    std.encoding.decode_float_list
      fn (raw: bytes) -> result[list[f32], string]
...
vector_db
  vector_db.new_collection
    fn (dim: i32, metric: string) -> result[collection_state, string]
    ...
    # construction
  vector_db.insert
    fn (state: collection_state, id: string, vector: list[f32]) -> result[collection_state, string]
    ...
    # insertion
  vector_db.delete
    fn (state: collection_state, id: string) -> result[collection_state, string]
    + removes the entry with the given id
    - returns error when id is not present
    # deletion
  vector_db.get
    fn (state: collection_state, id: string) -> optional[list[f32]]
    + returns stored vector when present
    - returns empty when id is unknown
    # lookup
  vector_db.search
    fn (state: collection_state, query: list[f32], k: i32) -> result[list[tuple[string, f32]], string]

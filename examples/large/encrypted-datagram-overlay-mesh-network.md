...
std
  ...
  std.net
    ...
    std.net.next_address_in
      fn (range: cidr_range, used: list[string]) -> result[string, string]
...
mesh
  mesh.create_network
    fn (name: string, range: string) -> result[network_state, string]
    + creates a new mesh network with a given CIDR range
    - returns error on invalid CIDR
    # administration
    -> std.net.parse_cidr
  mesh.add_peer
    fn (net: network_state, peer_name: string, endpoint: string) -> result[tuple[network_state, peer_record], string]
    ...
    -> std.net.next_address_in
  mesh.remove_peer
    fn (net: network_state, peer_name: string) -> result[network_state, string]
    + removes a peer from the network
    - returns error when the peer is not a member
    # membership
  mesh.generate_invite
    fn (net: network_state, peer_name: string, ttl_seconds: i64) -> result[invite_token, string]
    ...
    -> std.time.now_seconds
  mesh.redeem_invite
    fn (net: network_state, token: invite_token, public_key: bytes, endpoint: string) -> result[tuple[network_state, peer_record], string]
  ...
  mesh.render_peer_config
    fn (net: network_state, peer_name: string) -> result[string, string]
    + returns the rendered tunnel configuration for a peer, listing its siblings as routes
    - returns error when the peer is not a member
    # configuration
  mesh.list_peers
    fn (net: network_state) -> list[peer_record]
    + returns all peer records in the network
    # inspection
  mesh.peer_for_address
    fn (net: network_state, address: string) -> optional[peer_record]
    + returns the peer that owns an overlay address if any
    # routing
  mesh.rotate_peer_key
    fn (net: network_state, peer_name: string) -> result[tuple[network_state, peer_record], string]

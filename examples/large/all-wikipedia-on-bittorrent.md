...
std
  ...
  std.net
    std.net.tcp_connect
      ...
      # networking
    std.net.read_bytes
      @ (c: conn, n: i32) -> result[bytes, string]
...
peer_wiki
  peer_wiki.open_index
    @ (index_path: string) -> result[wiki_index, string]
  ...
  peer_wiki.lookup_article
    @ (idx: wiki_index, title: string) -> result[list[bytes], string]
  ...
  peer_wiki.fetch_chunk
    @ (hash: bytes, peers: list[string]) -> result[bytes, string]
  ...
  peer_wiki.read_article
    @ (idx: wiki_index, title: string, peers: list[string]) -> result[string, string]
  ...
  peer_wiki.announce_piece
    @ (hash: bytes, tracker_url: string) -> result[void, string]

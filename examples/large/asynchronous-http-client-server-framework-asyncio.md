...
std
  ...
  std.http
    ...
    std.http.encode_request
      @ (method: string, path: string, headers: map[string,string], body: bytes) -> bytes
...
httpx
  ...
  httpx.client_request
    @ (client: client_state, method: string, url: string, headers: map[string,string], body: bytes) -> result[http_response, string]
  ...
  httpx.server_new
    ...
    -> std.net.tcp_listen
  httpx.server_route
    @ (server: server_state, method: string, path: string, handler: handler_fn) -> server_state

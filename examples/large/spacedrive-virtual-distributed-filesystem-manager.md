...
spacedrive
  ...
  spacedrive.register_location
    @ (name: string, root: string) -> result[location_id, string]
  ...
  spacedrive.index_location
    @ (location: location_id) -> result[i32, string]
  ...
  spacedrive.catalog_entry
    @ (file_hash: string, name: string, size: i64) -> result[void, string]
    + stores a catalog entry keyed by content hash
    # catalog
    -> std.time.now_seconds
    -> std.kv.put
  spacedrive.find_by_hash
    @ (file_hash: string) -> result[optional[file_entry], string]
    + returns the catalog entry for a content hash
    # catalog
    -> std.kv.get
  spacedrive.list_replicas
    @ (file_hash: string) -> result[list[location_id], string]
    + returns every location known to store the hash
    # replication
    -> std.kv.get
  spacedrive.replicate
    @ (file_hash: string, target: location_id) -> result[void, string]
  ...
  spacedrive.create_virtual_dir
    @ (parent: virtual_path, name: string) -> result[virtual_path, string]
    + creates a virtual directory node under parent
    - returns error when parent does not exist
    # virtual_tree
    -> std.kv.put
  spacedrive.link_into_virtual
    @ (dir: virtual_path, name: string, file_hash: string) -> result[void, string]
  ...
  spacedrive.list_virtual
    @ (dir: virtual_path) -> result[list[virtual_entry], string]
    + returns entries inside a virtual directory
    # virtual_tree
    -> std.kv.get
  spacedrive.resolve_virtual
    @ (path: virtual_path) -> result[file_entry, string]

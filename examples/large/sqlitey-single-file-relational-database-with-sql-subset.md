...
std
  std.fs
    std.fs.open_rw
      ...
      # filesystem
    std.fs.read_at
      fn (handle: file_handle, offset: i64, n: i32) -> result[bytes, string]
      + reads n bytes at the given offset
      # filesystem
    std.fs.write_at
      fn (handle: file_handle, offset: i64, data: bytes) -> result[void, string]
...
sqlitey
  sqlitey.open
    fn (path: string) -> result[db_state, string]
  ...
  sqlitey.close
    ...
    -> std.fs.fsync
  sqlitey.read_page
    fn (state: db_state, page_no: i32) -> result[tuple[bytes, db_state], string]
    + returns the bytes of the given page, caching it on miss
    # paging
    -> std.fs.read_at
  sqlitey.write_page
    fn (state: db_state, page_no: i32, data: bytes) -> result[db_state, string]
    + writes a page and marks it dirty in the cache
    # paging
    -> std.fs.write_at
    -> std.hash.crc32
  sqlitey.tokenize_sql
    fn (source: string) -> result[list[sql_token], string]
  ...
  sqlitey.plan_statement
    fn (state: db_state, stmt: sql_statement) -> result[sql_plan, string]
  ...
  sqlitey.execute_plan
    fn (state: db_state, plan: sql_plan) -> result[tuple[sql_result, db_state], string]
    ...
    # executor
  sqlitey.exec
    fn (state: db_state, sql: string) -> result[tuple[sql_result, db_state], string]
    + tokenizes, parses, plans, and executes a SQL statement
    # convenience
  sqlitey.btree_insert
    fn (state: db_state, root_page: i32, key: bytes, value: bytes) -> result[db_state, string]
    + inserts a key-value pair, splitting pages as needed
    # storage
  sqlitey.btree_lookup
    fn (state: db_state, root_page: i32, key: bytes) -> optional[bytes]
    + returns the value for the given key
    # storage
  sqlitey.btree_delete
    fn (state: db_state, root_page: i32, key: bytes) -> result[db_state, string]

...
mlpkg
  mlpkg.new_registry
    fn (store_path: string) -> registry_state
    + creates a registry backed by the given on-disk store path
    # construction
  mlpkg.load_index
    fn (state: registry_state) -> result[registry_state, string]
    ...
    -> std.json.parse_object
  mlpkg.save_index
    fn (state: registry_state) -> result[void, string]
    + writes the in-memory index back to disk
    # persistence
    -> std.json.encode_object
    -> std.fs.write_all
  mlpkg.declare_artifact
    fn (state: registry_state, name: string, version: string, kind: string, deps: list[string]) -> registry_state
  ...
  mlpkg.compute_digest
    ...
    -> std.encoding.hex_encode
  mlpkg.install
    fn (state: registry_state, name: string, version: string, payload: bytes, expected_digest: string) -> result[registry_state, string]
    ...
    -> std.fs.write_all
  mlpkg.resolve
    fn (state: registry_state, name: string, version: string) -> result[list[string], string]
    ...
    # resolution
  mlpkg.locate
    fn (state: registry_state, name: string, version: string) -> optional[string]
    + returns the on-disk path for an installed artifact
    # lookup
  mlpkg.list_installed
    fn (state: registry_state) -> list[string]
    + returns every installed "name@version" pair
    # listing
  mlpkg.remove
    fn (state: registry_state, name: string, version: string) -> result[registry_state, string]

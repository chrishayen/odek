...
std
  ...
  std.expr
    std.expr.compile
      @ (source: string) -> result[expr_handle, string]
      + compiles a boolean predicate expression like "temp > 30 and site == 'A'"
      - returns error on syntax or unknown operator
      # parsing
    std.expr.eval_bool
      @ (handle: expr_handle, bindings: map[string, json_value]) -> result[bool, string]
...
stream
  ...
  stream.tumbling_window
    @ (s: stream_state, size_ms: i64, agg: string, over: string) -> result[stream_state, string]
    ...
    # operators
  stream.push
    @ (s: stream_state, event_json: string) -> result[list[output_event], string]
    ...
    -> std.time.now_monotonic_ms
  stream.flush
    @ (s: stream_state) -> list[output_event]

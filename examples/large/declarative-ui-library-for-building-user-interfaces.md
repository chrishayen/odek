...
std
  ...
  std.collections
    std.collections.topo_sort
      fn (nodes: list[string], edges: list[edge]) -> result[list[string], string]
...
declarative_ui
  declarative_ui.parse
    fn (source: string) -> result[component_tree, string]
  ...
  declarative_ui.new_state
    ...
    # construction
  declarative_ui.set
    fn (state: ui_state, key: string, value: dynamic_value) -> ui_state
    + updates a reactive variable and marks dependents dirty
    # state
  declarative_ui.get
    fn (state: ui_state, key: string) -> optional[dynamic_value]
    + returns the current value of a reactive variable
    # state
  declarative_ui.bind
    fn (tree: component_tree, state: ui_state) -> result[bound_tree, string]
  ...
  declarative_ui.layout
    fn (tree: bound_tree, viewport_w: i32, viewport_h: i32) -> layout_tree
  ...
  declarative_ui.dispatch_event
    fn (tree: bound_tree, event: ui_event) -> result[bound_tree, string]

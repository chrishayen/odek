...
std
  std.io
    std.io.byte_reader_new
      @ (data: bytes) -> byte_reader
      + wraps a byte slice in a seekable reader
      # io
    std.io.read_u32_be
      @ (reader: byte_reader) -> result[u32, string]
      + reads four bytes as a big-endian unsigned integer
      - returns error at end of stream
      # io
    std.io.read_bytes
      @ (reader: byte_reader, n: i32) -> result[bytes, string]
      + reads exactly n bytes or fails
      # io
    std.io.seek
      @ (reader: byte_reader, offset: i64) -> result[void, string]
  ...
  std.bits
    std.bits.bitreader_new
      ...
      # bits
    std.bits.read_bits
      @ (reader: bit_reader, n: i32) -> result[u32, string]
...
symphonia
  symphonia.probe_format
    @ (data: bytes) -> result[container_kind, string]
  ...
  symphonia.demuxer_new
    @ (kind: container_kind, data: bytes) -> result[demuxer_state, string]
    + opens a demuxer for the identified container
    # construction
    -> std.io.byte_reader_new
  symphonia.read_packet
    @ (demuxer: demuxer_state) -> result[optional[packet], string]
  ...
  symphonia.decoder_reset
    @ (decoder: decoder_state) -> decoder_state
    + clears internal state after a seek
    # lifecycle
  symphonia.format_info
    @ (frame: audio_frame) -> audio_format

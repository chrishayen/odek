...
std
  ...
  std.http
    std.http.route
      @ (method: string, path: string, handler: fn(http_request) -> http_response) -> void
      + registers a handler for method+path
      # http_server

task_queue
  task_queue.new
    @ (store: task_store) -> queue_state
    + creates a queue backed by the given storage
    # construction
  task_queue.push
    @ (state: queue_state, topic: string, payload: string, scheduled_at_ms: i64) -> result[string, string]
    ...
    -> std.time.now_millis
  task_queue.poll
    @ (state: queue_state, topic: string, lease_ms: i64) -> result[optional[task], string]
  ...
  task_queue.commit
    ...
    -> std.time.now_millis
  task_queue.release
    @ (state: queue_state, task_id: string, reason: string) -> result[void, string]

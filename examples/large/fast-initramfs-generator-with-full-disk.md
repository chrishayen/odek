...
std
  ...
  std.archive
    std.archive.cpio_newc_append
      @ (archive: bytes, name: string, mode: u32, content: bytes) -> bytes
...
initramfs
  initramfs.new
    ...
    # construction
  initramfs.add_file
    @ (spec: initramfs_spec, path: string, mode: u32) -> result[initramfs_spec, string]
  ...
  initramfs.add_directory
    @ (spec: initramfs_spec, path: string) -> result[initramfs_spec, string]
    + recursively stages a directory tree
    # staging
    -> std.fs.list_dir
    -> std.fs.stat
  initramfs.add_module
    @ (spec: initramfs_spec, module_name: string) -> result[initramfs_spec, string]
  ...
  initramfs.enable_encrypted_root
    @ (spec: initramfs_spec, device_uuid: string, key_source: i32) -> initramfs_spec
    + adds the cryptsetup binary, its libraries, and an init script that unlocks the device
    # crypto_support
  initramfs.set_init_script
    @ (spec: initramfs_spec, script: string) -> initramfs_spec
    + overrides the default /init script
    # configuration
  initramfs.build
    @ (spec: initramfs_spec) -> result[bytes, string]
    ...
    -> std.archive.cpio_newc_trailer
  initramfs.write
    @ (spec: initramfs_spec, path: string, compress: bool) -> result[void, string]

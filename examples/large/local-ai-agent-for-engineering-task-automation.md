...
std
  ...
  std.proc
    std.proc.run
      fn (cmd: string, args: list[string]) -> result[proc_result, string]
...
agent
  ...
  agent.append_assistant_message
    ...
    # session
  agent.register_tool
    fn (s: session_state, name: string, description: string) -> session_state
    + adds a tool descriptor the model may call
    - rejects a name already registered
    # tools
  agent.parse_tool_call
    fn (assistant_text: string) -> optional[tool_invocation]
    ...
    -> std.json.parse_object
  agent.dispatch_tool
    fn (s: session_state, invocation: tool_invocation) -> result[string, string]
  ...
  agent.tool_result_message
    fn (name: string, output: string) -> string
    + formats a tool result for the next turn
    -> std.json.encode_object
    # tools
  agent.step
    fn (s: session_state, model_call: model_fn) -> result[session_state, string]

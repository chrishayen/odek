...
std
  ...
  std.encoding
    ...
    std.encoding.base64url_decode
      @ (input: string) -> result[bytes, string]
      + decodes base64url
...
login_service
  login_service.new
    @ (signing_secret: string, token_ttl_seconds: i64) -> service_state
    + creates a service bound to a signing secret and token lifetime
    # construction
  login_service.register_backend
    @ (state: service_state, name: string, backend_id: string) -> service_state
    + registers a named authentication backend
    # backend_registration
  login_service.authenticate
    @ (state: service_state, backend_name: string, username: string, credential: string) -> result[map[string, string], string]
  ...
  login_service.authenticate_htpasswd
    @ (stored_hash: string, password: string) -> bool
    + verifies a password against a bcrypt hash from an htpasswd-style file
    # authentication
    -> std.crypto.bcrypt_verify
  login_service.authenticate_oauth2
    @ (token_url: string, code: string, client_id: string, client_secret: string) -> result[map[string, string], string]
  ...
  login_service.build_claims
    @ (user: map[string, string], issued_at: i64, ttl: i64) -> map[string, string]
    + returns the claims map with iat and exp fields added
    # claims
  login_service.sign_token
    @ (state: service_state, claims: map[string, string]) -> result[string, string]
  ...
  login_service.verify_token
    @ (state: service_state, token: string) -> result[map[string, string], string]
    ...
    -> std.time.now_seconds
  login_service.login
    @ (state: service_state, backend_name: string, username: string, credential: string) -> result[string, string]

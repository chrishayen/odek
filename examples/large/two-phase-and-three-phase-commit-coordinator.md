...
txn
  txn.new_coordinator
    ...
    # construction
  txn.begin
    fn (state: coordinator_state, participants: list[string]) -> tuple[string, coordinator_state]
    + allocates a transaction id and records the participant set in state INIT
    # transaction_lifecycle
    -> std.id.new_uuid
    -> std.time.now_millis
  txn.record_vote
    fn (state: coordinator_state, txn_id: string, participant: string, yes: bool) -> result[coordinator_state, string]
    ...
    # two_phase_commit
  txn.decide_2pc
    fn (state: coordinator_state, txn_id: string) -> result[tuple[string, coordinator_state], string]
  ...
  txn.decide_3pc_precommit
    fn (state: coordinator_state, txn_id: string) -> result[coordinator_state, string]
  ...
  txn.abort
    ...
    # transaction_lifecycle
  txn.status
    fn (state: coordinator_state, txn_id: string) -> result[string, string]
    + returns the current state name for the transaction
    - returns error when the transaction id is unknown
    # introspection
  txn.timeout_sweep
    fn (state: coordinator_state, stale_ms: i64) -> coordinator_state

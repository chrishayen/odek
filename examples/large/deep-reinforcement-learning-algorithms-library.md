...
std
  ...
  std.nn
    std.nn.mlp
      fn (layer_sizes: list[i32]) -> network_handle
      + builds a fully-connected network with the given layer widths
      # network
    std.nn.forward
      fn (net: network_handle, input: tensor) -> tensor
      + returns the output tensor for the given input
      # network
    std.nn.backward
      fn (net: network_handle, loss: f64) -> network_handle
...
rl
  rl.new_replay_buffer
    fn (capacity: i32) -> replay_state
    + creates a fixed-capacity experience buffer
    # buffer
  rl.store
    fn (buffer: replay_state, obs: tensor, action: i32, reward: f64, next_obs: tensor, done: bool) -> replay_state
    + appends a transition, dropping the oldest when full
    # buffer
  rl.sample_batch
    fn (buffer: replay_state, batch_size: i32) -> list[transition]
    + returns a random batch of transitions
    - returns an empty list when the buffer has fewer items than batch_size
    # buffer
    -> std.random.uniform
  rl.new_dqn
    fn (obs_dim: i32, action_dim: i32, hidden: list[i32]) -> agent_state
  ...
  rl.new_ppo
    ...
    -> std.nn.mlp
  rl.select_action
    fn (agent: agent_state, obs: tensor, epsilon: f64) -> i32
    ...
    -> std.random.uniform
  rl.update
    fn (agent: agent_state, batch: list[transition], learning_rate: f64) -> agent_state
  ...
  rl.sync_target
    ...
    # training
  rl.discount_returns
    fn (rewards: list[f64], gamma: f64) -> list[f64]
    + returns discounted cumulative returns in reverse order
    ? gamma in [0, 1]
    # advantage
  rl.advantage
    fn (rewards: list[f64], values: list[f64], gamma: f64, lambda_val: f64) -> list[f64]
    + returns GAE advantage estimates
    # advantage
  rl.save
    fn (agent: agent_state, path: string) -> result[void, string]
    + serializes the agent's parameters to disk
    - returns error on write failure
    # persistence
  rl.load
    fn (path: string) -> result[agent_state, string]

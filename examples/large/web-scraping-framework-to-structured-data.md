...
std
  ...
  std.html
    std.html.parse
      fn (source: bytes) -> result[html_doc, string]
  ...
  std.url
    std.url.resolve
      fn (base: string, reference: string) -> result[string, string]
...
scraper
  ...
  scraper.extract_record
    ...
    -> std.html.attr
  scraper.scrape_page
    fn (url: string, schema: schema_def) -> result[list[map[string, string]], string]
  ...
  scraper.follow_links
    fn (doc: html_doc, base_url: string, selector: string) -> list[string]
    ...
    -> std.url.resolve
  scraper.crawl
    fn (start_url: string, schema: schema_def, max_pages: i32) -> result[list[map[string, string]], string]

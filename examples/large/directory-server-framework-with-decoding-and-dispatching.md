...
std
  std.encoding
    std.encoding.ber_decode
      fn (data: bytes) -> result[ber_node, string]
...
directory_server
  directory_server.parse_request
    fn (data: bytes) -> result[directory_request, string]
  ...
  directory_server.operation_of
    fn (req: directory_request) -> string
  ...
  directory_server.register_handler
    fn (router: router_state, op: string, handler_id: string) -> router_state
    + registers a handler id for a given operation name
    # registration
  directory_server.dispatch
    fn (router: router_state, req: directory_request) -> optional[string]
  ...
  directory_server.build_response
    fn (message_id: i32, code: i32, payload: bytes) -> directory_response
    + packages the fields into a response structure
    # responses
  directory_server.encode_response
    fn (resp: directory_response) -> bytes
    + serializes the response to bytes for transmission
    # responses
    -> std.encoding.ber_encode
  directory_server.handle_connection
    fn (router: router_state, conn: conn_handle, invoker: handler_invoker) -> result[void, string]
  ...
  directory_server.serve
    fn (router: router_state, listener: listener_handle, invoker: handler_invoker) -> result[void, string]

...
db
  db.engine_new
    @ (wal_path: string) -> result[engine_state, string]
    + creates an engine and replays the write-ahead log
    - returns error when the log is corrupt
    # construction
    -> std.fs.read_all
  db.kv_set
    @ (state: engine_state, key: string, value: bytes) -> result[engine_state, string]
    + sets a value under the key-value model
    # kv
    -> std.fs.append_file
    -> std.fs.fsync
  db.kv_get
    @ (state: engine_state, key: string) -> optional[bytes]
    + returns the stored value or none
    # kv
  db.hash_set_field
    @ (state: engine_state, key: string, field: string, value: bytes) -> result[engine_state, string]
    + sets a field on a hash-model key
    - returns error when the key is already bound to a different model
    # hash_model
    -> std.fs.append_file
  db.hash_get_field
    @ (state: engine_state, key: string, field: string) -> optional[bytes]
  ...
  db.list_push
    ...
    -> std.fs.append_file
  db.list_range
    @ (state: engine_state, key: string, start: i32, stop: i32) -> list[bytes]
    + returns values in the inclusive range, negative indices count from the end
    # list_model
  db.delete
    @ (state: engine_state, key: string) -> engine_state
    + removes a key regardless of its model
    # deletion
    -> std.fs.append_file
  db.snapshot
    @ (state: engine_state, path: string) -> result[void, string]
    ...
    -> std.fs.fsync
  db.load_snapshot
    @ (path: string) -> result[engine_state, string]

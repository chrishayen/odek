...
bayes
  bayes.new_network
    ...
    # construction
  bayes.add_variable
    fn (state: network_state, name: string, values: list[string]) -> network_state
    + adds a discrete variable with the given possible values
    - panics when the name is already present
    # model
  bayes.add_edge
    fn (state: network_state, parent: string, child: string) -> result[network_state, string]
    + adds a directed edge between two existing variables
    - returns error when the edge would create a cycle
    # model
  bayes.set_cpt
    fn (state: network_state, variable: string, table: cpt) -> result[network_state, string]
  ...
  bayes.topological_order
    fn (state: network_state) -> list[string]
    + returns variables in a topological order
    # analysis
  bayes.joint_probability
    fn (state: network_state, assignment: map[string, string]) -> result[f64, string]
  ...
  bayes.variable_elimination
    fn (state: network_state, query: list[string], evidence: map[string, string]) -> result[map[list[string], f64], string]
    ...
    # inference
  bayes.gibbs_sample
    fn (state: network_state, rng: rng_state, evidence: map[string, string], iterations: i32) -> result[map[string, f64], string]
  ...
  bayes.learn_parameters_mle
    fn (state: network_state, dataset: list[map[string, string]]) -> result[network_state, string]
  ...
  bayes.learn_structure_hill_climb
    fn (variables: list[variable_spec], dataset: list[map[string, string]]) -> result[network_state, string]

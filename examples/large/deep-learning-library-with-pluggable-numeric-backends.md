...
std
  std.numeric
    std.numeric.zeros
      ...
      # numeric
    std.numeric.randn
      fn (shape: list[i32], stddev: f32) -> tensor
    ...
    std.numeric.relu
      ...
      # numeric
    std.numeric.softmax
      fn (x: tensor, axis: i32) -> tensor
      + applies softmax along axis
      # numeric
    std.numeric.grad
      fn (loss: tensor, params: list[tensor]) -> list[tensor]
      + computes gradients of loss with respect to each parameter
      # autodiff

dl
  dl.backend_register
    fn (name: string, impl: backend_impl) -> void
  ...
  dl.activation_layer
    ...
    -> std.numeric.softmax
  dl.sequential
    fn (layers: list[layer]) -> model_state
    + builds a sequential model from a list of layers
    # model
  dl.forward
    fn (model: model_state, input: tensor) -> tensor
    + runs input through the model layers in order
    # model
    -> std.numeric.matmul
    -> std.numeric.add
  dl.loss_cross_entropy
    fn (logits: tensor, targets: tensor) -> tensor
    + returns the categorical cross-entropy loss
    # loss
  dl.loss_mse
    fn (predicted: tensor, targets: tensor) -> tensor
    + returns mean squared error
    # loss
  dl.optimizer_sgd
    fn (learning_rate: f32) -> optimizer_state
    + creates a stochastic gradient descent optimizer
    # optimization
  dl.optimizer_adam
    fn (learning_rate: f32, beta1: f32, beta2: f32) -> optimizer_state
    + creates an Adam optimizer with moment estimates
    # optimization
  dl.train_step
    fn (model: model_state, opt: optimizer_state, input: tensor, targets: tensor, loss_fn: fn(tensor, tensor) -> tensor) -> tuple[model_state, optimizer_state, f32]
    + runs a forward pass, computes gradients, applies the optimizer, and returns the loss value
    # training
    -> std.numeric.grad
    -> dl.forward
  dl.evaluate
    fn (model: model_state, input: tensor, targets: tensor) -> f32

...
std
  std.http
    ...
    std.http.write_response
      fn (resp: http_response, status: i32, body: bytes, headers: map[string, string]) -> void
...
web
  web.new_router
    ...
    # construction
  web.route
    fn (router: router_state, method: string, pattern: string, handler: route_handler) -> router_state
    + registers a route, supporting path parameters like "/users/:id"
    # routing
  web.use
    fn (router: router_state, mw: middleware) -> router_state
    + appends middleware applied to every request in registration order
    # middleware
  web.group
    fn (router: router_state, prefix: string, mw: list[middleware]) -> router_group
    + creates a route group sharing a path prefix and middleware
    # routing
  web.group_route
    fn (g: router_group, method: string, pattern: string, handler: route_handler) -> router_group
    + registers a route inside a group
    # routing
  web.match
    fn (router: router_state, method: string, path: string) -> result[route_match, string]
  ...
  web.dispatch
    ...
    -> std.url.parse_query
  web.serve
    fn (router: router_state, addr: string) -> result[void, string]

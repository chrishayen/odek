...
std
  std.crypto
    std.crypto.sha256
      ...
      # hashing
    std.crypto.blake2b
      @ (data: bytes, personalization: bytes) -> bytes
      + computes BLAKE2b with a personalization tag
      # hashing
    std.crypto.ed25519_sign
      @ (key: bytes, message: bytes) -> bytes
  ...
  std.math
    std.math.mod_exp
      @ (base: bytes, exponent: bytes, modulus: bytes) -> bytes
...
shielded
  ...
  shielded.note_nullifier
    @ (n: note, spending: spending_key) -> bytes
    + returns the nullifier that will be revealed when the note is spent
    # notes
    -> std.crypto.blake2b
  shielded.build_shielded_tx
    @ (inputs: list[note], outputs: list[note], spending: spending_key) -> result[shielded_tx, string]
  ...
  shielded.prove_transfer
    @ (tx: shielded_tx, spending: spending_key) -> result[zk_proof, string]
  ...
  shielded.verify_transfer
    @ (tx: shielded_tx, proof: zk_proof) -> result[bool, string]
    ...
    -> std.crypto.sha256
  shielded.sign_tx
    @ (tx: shielded_tx, spending: spending_key) -> bytes
    + produces a binding signature committing to the transaction
    # signatures
    -> std.crypto.ed25519_sign
  shielded.verify_tx_signature
    @ (tx: shielded_tx, signature: bytes, public_key: bytes) -> bool
    + verifies the binding signature on a transaction
    # signatures
    -> std.crypto.ed25519_verify
  shielded.apply_to_state
    @ (state: chain_state, tx: shielded_tx, proof: zk_proof) -> result[chain_state, string]

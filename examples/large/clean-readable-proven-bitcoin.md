...
std
  ...
  std.crypto
    std.crypto.secp256k1_derive_pubkey
      @ (private_key: bytes) -> result[bytes, string]
    ...
    std.crypto.secp256k1_sign
      @ (private_key: bytes, digest: bytes) -> result[bytes, string]
    ...
    std.crypto.secp256k1_verify
      @ (public_key: bytes, digest: bytes, signature: bytes) -> bool
  ...
  std.encoding
    ...
    std.encoding.bech32_encode
      @ (hrp: string, data: list[u8]) -> result[string, string]
    ...
    std.encoding.bech32_decode
      @ (s: string) -> result[tuple[string, list[u8]], string]
...
btc
  btc.private_key_from_wif
    @ (wif: string) -> result[bytes, string]
  ...
  btc.private_key_to_wif
    @ (private_key: bytes, compressed: bool) -> string
  ...
  btc.public_key_from_private
    ...
    -> std.crypto.secp256k1_derive_pubkey
  btc.p2pkh_address
    @ (public_key: bytes, mainnet: bool) -> string
    ...
    -> std.encoding.base58_encode
  btc.p2wpkh_address
    @ (public_key: bytes, mainnet: bool) -> result[string, string]
    ...
    -> std.encoding.bech32_encode
  btc.parse_address
    @ (address: string) -> result[parsed_address, string]
  ...
  btc.sighash
    @ (tx: transaction, input_index: i32, script_code: bytes, sighash_type: u32) -> bytes
    + returns the 32-byte digest to be signed for the given input
    # signing
    -> std.hash.sha256
  btc.sign_input
    @ (tx: transaction, input_index: i32, private_key: bytes, script_code: bytes) -> result[transaction, string]

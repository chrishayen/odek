...
chat_bot
  chat_bot.new
    @ (api_base: string, token: string) -> bot_state
    + creates a bot client targeting the given API base and authorization token
    # construction
  chat_bot.get_me
    @ (bot: bot_state) -> result[bot_identity, string]
  ...
  chat_bot.poll_updates
    @ (bot: bot_state, offset: i64, timeout_sec: i32) -> result[tuple[list[update], i64], string]
  ...
  chat_bot.send_message
    @ (bot: bot_state, chat_id: i64, text: string) -> result[message_id, string]
    ...
    -> std.json.parse
  chat_bot.send_reply
    @ (bot: bot_state, chat_id: i64, reply_to: message_id, text: string) -> result[message_id, string]
    + sends a message as a reply to an existing message
    # sending
    -> std.http.post_json
    -> std.json.encode
  chat_bot.edit_message
    @ (bot: bot_state, chat_id: i64, mid: message_id, text: string) -> result[void, string]
  ...
  chat_bot.delete_message
    @ (bot: bot_state, chat_id: i64, mid: message_id) -> result[void, string]
    + deletes a previously sent message
    # sending
    -> std.http.post_json
  chat_bot.answer_callback
    @ (bot: bot_state, callback_id: string, text: string) -> result[void, string]
    + acknowledges an inline-button callback with an optional notification
    # sending
    -> std.http.post_json
    -> std.json.encode
  chat_bot.parse_update
    @ (raw: string) -> result[update, string]

...
std
  ...
  std.crypto
    std.crypto.generate_rsa_keypair
      fn (bits: i32) -> result[tuple[bytes, bytes], string]
      + returns (private_key, public_key) in PEM form
      - returns error when bits is too small
      # cryptography
    std.crypto.rsa_sign
      fn (private_key: bytes, data: bytes) -> result[bytes, string]
...
fediverse
  fediverse.new
    fn (conn: sql_conn, domain: string) -> fediverse_state
    + creates a server state bound to a database and host domain
    # construction
  fediverse.create_account
    fn (state: fediverse_state, username: string) -> result[account, string]
  ...
  fediverse.render_actor
    fn (state: fediverse_state, acct: account) -> string
    + renders the actor object as ActivityPub JSON with inbox and public key
    # federation
    -> std.json.encode_object
  fediverse.create_post
    fn (state: fediverse_state, author: account, content: string) -> result[post, string]
    ...
    -> std.time.now_rfc3339
  fediverse.follow
    fn (state: fediverse_state, follower: account, target_actor_url: string) -> result[void, string]
    + sends a signed Follow activity to the target actor
    - returns error on delivery failure
    # follow_graph
  fediverse.sign_request
    fn (acct: account, method: string, url: string, body: bytes) -> map[string, string]
    + returns HTTP Signature headers using the account's private key
    # federation
    -> std.crypto.rsa_sign
  fediverse.verify_signature
    fn (state: fediverse_state, method: string, url: string, headers: map[string, string], body: bytes) -> result[string, string]
    + returns the verified sender actor URL
    - returns error when the signature cannot be verified
    # federation
    -> std.crypto.rsa_verify
  fediverse.deliver
    fn (state: fediverse_state, inbox_url: string, activity: string, signer: account) -> result[void, string]
    + POSTs a signed activity to a remote inbox
    - returns error on transport failure
    # federation
    -> std.http.post
  fediverse.handle_inbox
    fn (state: fediverse_state, req: http_request) -> http_response
  ...
  fediverse.handle_webfinger
    fn (state: fediverse_state, resource: string) -> result[string, string]
    + returns the WebFinger JSON describing a local account
    - returns error when the resource is unknown
    # discovery
    -> std.json.encode_object
  fediverse.serve
    fn (state: fediverse_state, addr: string) -> result[void, string]

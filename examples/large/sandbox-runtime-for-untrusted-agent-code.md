...
sandbox_runtime
  sandbox_runtime.new
    fn (root: string, backend: isolation_backend) -> runtime_state
  ...
  sandbox_runtime.create_sandbox
    fn (state: runtime_state, image: string, cpu_quota: i32, memory_mb: i32, network: bool) -> result[tuple[runtime_state, sandbox_id], string]
  ...
  sandbox_runtime.destroy_sandbox
    fn (state: runtime_state, id: sandbox_id) -> result[runtime_state, string]
  ...
  sandbox_runtime.exec
    fn (state: runtime_state, id: sandbox_id, argv: list[string], stdin: bytes, timeout_ms: i32) -> result[exec_result, string]
  ...
  sandbox_runtime.write_file
    fn (state: runtime_state, id: sandbox_id, path: string, data: bytes) -> result[runtime_state, string]
    + writes a file inside the sandbox workspace
    - returns error when path escapes the workspace
    # files
    -> std.fs.write_file
  sandbox_runtime.read_file
    fn (state: runtime_state, id: sandbox_id, path: string) -> result[bytes, string]
    + reads a file from the sandbox workspace
    - returns error when the file does not exist
    # files
    -> std.fs.read_file
  sandbox_runtime.list_files
    fn (state: runtime_state, id: sandbox_id, subpath: string) -> result[list[string], string]
  ...
  sandbox_runtime.snapshot
    fn (state: runtime_state, id: sandbox_id) -> result[bytes, string]
    + returns a tar archive of the current sandbox workspace
    - returns error when the sandbox is unknown
    # snapshot
  sandbox_runtime.restore
    fn (state: runtime_state, image: string, archive: bytes) -> result[tuple[runtime_state, sandbox_id], string]
  ...
  sandbox_runtime.usage
    fn (state: runtime_state, id: sandbox_id) -> optional[sandbox_usage]

...
std
  ...
  std.collections
    ...
    std.collections.queue_push
      fn (q: queue_state, value: i64) -> queue_state
      + appends to the tail
      # collections
    std.collections.queue_pop
      fn (q: queue_state) -> optional[tuple[i64, queue_state]]
...
app_server
  app_server.new
    fn (worker_count: i32, max_pending: i32) -> server_state
  ...
  app_server.register_app
    fn (server: server_state, prefix: string, app_id: string) -> server_state
    + mounts an application at a path prefix
    # routing
  app_server.enqueue_request
    fn (server: server_state, req: http_request) -> result[tuple[i64, server_state], string]
  ...
  app_server.next_ready
    fn (server: server_state) -> optional[tuple[i64, http_request, server_state]]

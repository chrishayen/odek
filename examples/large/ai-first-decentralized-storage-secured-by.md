...
dstore
  ...
  dstore.encode_shards
    @ (state: store_state, data: bytes) -> list[bytes]
    + returns k+m shards under the configured Reed-Solomon parameters
    # erasure_coding
    -> std.math.gf256_mul
  dstore.decode_shards
    @ (state: store_state, shards: list[optional[bytes]]) -> result[bytes, string]
  ...
  dstore.assign_shards
    @ (state: store_state, shards: list[bytes]) -> map[string, bytes]
    + returns a peer-id to shard map using rendezvous hashing
    # placement
    -> std.crypto.sha256
  dstore.put
    @ (state: store_state, data: bytes) -> result[string, string]
    ...
    -> dstore.assign_shards
  dstore.get
    @ (state: store_state, content_id: string, shards: map[string, optional[bytes]]) -> result[bytes, string]
  ...
  dstore.sign_manifest
    @ (privkey: bytes, content_id: string, shard_map: map[string, bytes]) -> bytes
    + returns an Ed25519 signature over the canonical manifest bytes
    # manifests
    -> std.crypto.ed25519_sign
  dstore.verify_manifest
    @ (pubkey: bytes, content_id: string, shard_map: map[string, bytes], sig: bytes) -> bool

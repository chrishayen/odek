...
colstore
  colstore.open
    ...
    # construction
  colstore.add_column
    fn (store: store_state, name: string, dtype: string) -> result[store_state, string]
  ...
  colstore.build_index
    fn (store: store_state, column: string) -> result[store_state, string]
    ...
    -> std.collections.bitmap_set
  colstore.query_eq
    fn (store: store_state, column: string, value: string) -> result[list[u64], string]
    + returns row ids whose column equals the given value, using the bitmap index when present
    - returns error when the column is unknown
    # query
    -> std.collections.bitmap_iter_set
  colstore.query_and
    fn (store: store_state, predicates: list[tuple[string, string]]) -> result[list[u64], string]
    ...
    -> std.collections.bitmap_iter_set
  colstore.project
    fn (store: store_state, row_ids: list[u64], columns: list[string]) -> result[list[map[string, string]], string]

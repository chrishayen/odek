...
supervisor
  ...
  supervisor.add_program
    fn (state: supervisor_state, name: string, command: string, args: list[string], autorestart: bool) -> supervisor_state
    + registers a program definition under the given name
    - returns unchanged state when a program with that name already exists
    # configuration
  supervisor.start
    fn (state: supervisor_state, name: string) -> result[supervisor_state, string]
  ...
  supervisor.restart
    ...
    # lifecycle
  supervisor.status
    fn (state: supervisor_state, name: string) -> result[program_status, string]
  ...
  supervisor.list_programs
    fn (state: supervisor_state) -> list[string]

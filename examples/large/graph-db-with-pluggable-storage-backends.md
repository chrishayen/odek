...
std
  ...
  std.encoding
    std.encoding.encode_triple
      fn (subject: string, predicate: string, object: string) -> bytes
      + encodes a triple as a length-prefixed byte sequence
      # encoding
    std.encoding.decode_triple
      fn (data: bytes) -> result[tuple[string, string, string], string]
...
graph_db
  ...
  graph_db.open_file
    ...
    -> std.encoding.decode_triple
  graph_db.add_triple
    fn (state: db_state, subject: string, predicate: string, object: string) -> db_state
  ...
  graph_db.remove_triple
    ...
    # mutation
  graph_db.query_out
    fn (state: db_state, subject: string, predicate: string) -> list[string]
    + returns all objects reachable from subject via the given predicate
    # query
  graph_db.query_in
    fn (state: db_state, predicate: string, object: string) -> list[string]
    + returns all subjects that reach object via the given predicate
    # query
  graph_db.traverse
    fn (state: db_state, start: string, predicate: string, max_depth: i32) -> list[string]

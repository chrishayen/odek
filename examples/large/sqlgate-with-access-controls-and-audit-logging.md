...
std
  ...
  std.sql
    std.sql.parse_statement
      fn (raw: string) -> result[parsed_statement, string]
  ...
  std.json
    std.json.encode_rows
      fn (columns: list[string], rows: list[list[string]]) -> string
      + encodes a tabular result set as JSON
      # serialization

sqlgate
  sqlgate.new_gateway
    fn (db: database_host, audit: audit_sink) -> gateway_state
    + creates a gateway bound to a database host and an audit sink
    # construction
  sqlgate.define_role
    fn (state: gateway_state, role: string) -> gateway_state
    + registers a role with no permissions
    # acl
  sqlgate.grant
    fn (state: gateway_state, role: string, table: string, ops: list[string]) -> gateway_state
    + grants the given operations on a table to a role
    # acl
  sqlgate.assign_role
    fn (state: gateway_state, user: string, role: string) -> gateway_state
    + associates a user with a role
    # acl
  sqlgate.check_access
    fn (state: gateway_state, user: string, stmt: parsed_statement) -> result[void, string]
    ...
    # authorization
  sqlgate.execute
    fn (state: gateway_state, user: string, raw_sql: string) -> result[query_result, string]
  ...
  sqlgate.create_share
    fn (state: gateway_state, result: query_result, ttl_seconds: i64) -> share_record
    + persists the result and mints a random share token with expiry
    # sharing
    -> std.random.token
    -> std.time.now_millis
  sqlgate.resolve_share
    fn (state: gateway_state, token: string) -> result[query_result, string]
  ...
  sqlgate.serialize_result
    fn (result: query_result) -> string
    + encodes a result set as JSON for transport or storage
    # serialization
    -> std.json.encode_rows
  sqlgate.audit_entries
    fn (state: gateway_state, since_ms: i64) -> list[audit_entry]

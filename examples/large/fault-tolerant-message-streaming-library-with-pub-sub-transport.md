...
std
  ...
  std.encoding
    ...
    std.encoding.varint_decode
      @ (data: bytes, pos: i64) -> result[tuple[u64, i64], string]
...
message_stream
  message_stream.create_stream
    @ (name: string, partitions: i32, replicas: i32) -> result[stream_state, string]
  ...
  message_stream.append
    @ (state: stream_state, partition: i32, payload: bytes) -> result[i64, string]
  ...
  message_stream.read_from
    @ (state: stream_state, partition: i32, offset: i64, max_messages: i32) -> result[list[bytes], string]
  ...
  message_stream.subscribe
    @ (state: stream_state, partition: i32, start_offset: i64) -> subscription_state
    + creates a subscription cursor positioned at start_offset
    # subscription
  message_stream.poll
    @ (sub: subscription_state, max_messages: i32) -> tuple[list[bytes], subscription_state]
  ...
  message_stream.elect_leader
    @ (state: stream_state, partition: i32, candidate_id: string, term: i64) -> result[bool, string]
  ...
  message_stream.replicate
    @ (state: stream_state, partition: i32, follower_id: string, up_to_offset: i64) -> result[i64, string]

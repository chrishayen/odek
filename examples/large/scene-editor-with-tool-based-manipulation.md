...
scene_editor
  ...
  scene_editor.add_node
    fn (state: scene_state, kind: string, parent: optional[string]) -> tuple[string, scene_state]
    + returns (node_id, new_state) after inserting a node of the given kind
    # scene_graph
  scene_editor.remove_node
    fn (state: scene_state, node_id: string) -> result[scene_state, string]
    + removes a node and its descendants
    - returns error when node_id does not exist
    # scene_graph
  scene_editor.set_transform
    fn (state: scene_state, node_id: string, x: f64, y: f64, z: f64) -> result[scene_state, string]
    + sets translation on the node
    - returns error when node_id does not exist
    # scene_graph
    -> std.math.clamp_f64
  scene_editor.attach_media
    fn (state: scene_state, node_id: string, media_id: string) -> result[scene_state, string]
    + attaches a media asset reference to the node
    - returns error when node_id does not exist
    # media
  scene_editor.register_media
    fn (state: scene_state, media_id: string, kind: string, duration_ms: i64) -> scene_state
    + stores metadata for an image or video asset
    # media
  scene_editor.add_keyframe
    fn (state: scene_state, node_id: string, time_ms: i64, property: string, value: f64) -> result[scene_state, string]
    + adds a keyframe for a named property
    - returns error when node_id does not exist
    # animation
  scene_editor.sample_at
    fn (state: scene_state, node_id: string, property: string, time_ms: i64) -> optional[f64]
    + returns the interpolated property value at the given time
    # animation
  scene_editor.push_tool_edit
    fn (state: scene_state, tool: string, payload: string) -> scene_state
    + applies a tool-driven edit and records it in history
    # tools
    -> std.time.now_millis
  scene_editor.undo
    fn (state: scene_state) -> result[scene_state, string]
  ...
  scene_editor.snapshot
    fn (state: scene_state) -> scene_snapshot

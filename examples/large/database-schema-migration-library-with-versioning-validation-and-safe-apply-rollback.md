...
std
  ...
  std.db
    std.db.exec
      fn (conn: db_conn, sql: string) -> result[void, string]
      + executes a statement that returns no rows
      - returns error on SQL failure
      # database
    std.db.query_rows
      fn (conn: db_conn, sql: string) -> result[list[map[string,string]], string]
      + executes a query and returns rows as string maps
      - returns error on SQL failure
      # database
    std.db.begin_tx
      fn (conn: db_conn) -> result[db_tx, string]
      + opens a transaction
      # database
    std.db.commit_tx
      fn (tx: db_tx) -> result[void, string]
...
migrate
  ...
  migrate.verify_checksums
    ...
    -> std.db.query_rows
  migrate.apply_one
    fn (conn: db_conn, m: migration) -> result[void, string]
    ...
    -> std.time.now_millis
  migrate.apply_all
    fn (conn: db_conn, migrations: list[migration]) -> result[i32, string]
  ...
  migrate.rollback_one
    ...
    -> std.db.rollback_tx
  migrate.rollback_to
    fn (conn: db_conn, migrations: list[migration], target_version: i64) -> result[i32, string]

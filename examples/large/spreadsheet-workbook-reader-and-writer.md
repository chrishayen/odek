...
std
  ...
  std.text
    std.text.column_label_to_index
      @ (label: string) -> result[i32, string]
...
spreadsheet
  spreadsheet.parse_shared_strings
    @ (part: string) -> result[list[string], string]
  ...
  spreadsheet.parse_sheet
    @ (part: string, shared_strings: list[string]) -> result[sheet_data, string]
  ...
  spreadsheet.get_cell
    @ (book: workbook, sheet_name: string, row: i32, col: i32) -> optional[cell_value]

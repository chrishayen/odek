...
std
  ...
  std.codec
    ...
    std.codec.decode_block
      fn (data: bytes) -> result[block, string]
...
block_dag
  ...
  block_dag.validate_block
    ...
    -> std.time.now_seconds
  block_dag.add_block
    fn (state: dag_state, b: block) -> result[dag_state, string]
  ...
  block_dag.tips
    ...
    # tips
  block_dag.serialize
    fn (state: dag_state) -> bytes
    + returns a canonical encoding of the current DAG state
    # persistence
    -> std.codec.encode_block
  block_dag.load
    fn (data: bytes) -> result[dag_state, string]

...
std
  ...
  std.text
    std.text.pad_left
      fn (s: string, width: i32, pad: string) -> string
...
ach
  ach.parse_file_header
    fn (line: string) -> result[file_header, string]
  ...
  ach.compute_entry_hash
    fn (entries: list[entry_detail]) -> i64
    + returns the 10-digit entry hash (sum of routing numbers, mod 10^10)
    # checksums
  ach.validate
    fn (file: ach_file) -> result[void, list[string]]
  ...
  ach.format_file_header
    fn (header: file_header) -> string
    + returns the 94-character header line
    # formatting
    -> std.text.pad_left
    -> std.text.pad_right
  ach.format_entry_detail
    fn (entry: entry_detail) -> string
    + returns the 94-character entry detail line
    # formatting
    -> std.text.pad_left
    -> std.text.pad_right
  ach.write_file
    fn (path: string, file: ach_file) -> result[void, string]

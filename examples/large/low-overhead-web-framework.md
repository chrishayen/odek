...
std
  std.http
    std.http.listen
      fn (addr: string, handler: http_server_handler) -> result[void, string]
...
web
  web.new_app
    ...
    # construction
  web.route
    fn (app: web_app, method: string, path: string, handler: route_handler) -> web_app
    + registers a handler for method+path, supporting :param segments
    # routing
  web.use
    fn (app: web_app, middleware: middleware_fn) -> web_app
    + pushes a middleware onto the chain wrapping every handler
    # middleware
  web.match_route
    fn (app: web_app, method: string, path: string) -> optional[tuple[route_handler, map[string, string]]]
    + returns the matched handler and extracted path params
    - returns none when no route matches
    # routing
  web.dispatch
    fn (app: web_app, req: http_request) -> http_response
    ...
    # request_dispatch
  web.json_response
    fn (status: i32, body: json_value) -> http_response
  ...
  web.read_json_body
    ...
    -> std.json.parse_value
  web.serve
    fn (app: web_app, addr: string) -> result[void, string]

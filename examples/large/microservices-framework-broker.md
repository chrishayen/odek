...
broker
  broker.new
    @ (node_id: string) -> broker_state
    + creates a broker with no registered actions and the given node identifier
    # construction
  broker.register_action
    @ (state: broker_state, service: string, action: string, handler_id: string) -> broker_state
  ...
  broker.register_remote
    @ (state: broker_state, service: string, action: string, node_id: string) -> broker_state
    + records that a remote node handles service.action
    # registry
  broker.list_actions
    @ (state: broker_state, service: string) -> list[string]
    + returns every action registered for a service, local or remote
    # discovery
  broker.resolve
    @ (state: broker_state, service: string, action: string) -> result[resolved_target, string]
  ...
  broker.encode_request
    @ (caller: string, service: string, action: string, params: json_value) -> string

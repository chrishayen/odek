...
std
  ...
  std.net
    std.net.dial_tcp
      ...
      # networking
    std.net.send
      @ (conn: conn_state, payload: bytes) -> result[void, string]
      + writes the full payload to the socket
      # networking
    std.net.recv
      @ (conn: conn_state, max: u32) -> result[bytes, string]
...
channel_node
  channel_node.new
    @ (identity_key: bytes) -> channel_node_state
    + creates a node bound to a long-lived identity keypair
    # construction
  channel_node.connect_peer
    @ (state: channel_node_state, peer_pubkey: bytes, host: string, port: u16) -> result[channel_node_state, string]
  ...
  channel_node.open_channel
    @ (state: channel_node_state, peer_pubkey: bytes, capacity: u64) -> result[tuple[channel_id, channel_node_state], string]
  ...
  channel_node.send_htlc
    @ (state: channel_node_state, cid: channel_id, amount: u64, payment_hash: bytes, expiry: i64) -> result[channel_node_state, string]
  ...
  channel_node.settle_htlc
    @ (state: channel_node_state, cid: channel_id, preimage: bytes) -> result[channel_node_state, string]
  ...
  channel_node.route_payment
    @ (state: channel_node_state, destination: bytes, amount: u64) -> result[list[channel_id], string]
  ...
  channel_node.forward_htlc
    @ (state: channel_node_state, incoming: channel_id, outgoing: channel_id, amount: u64, payment_hash: bytes) -> result[channel_node_state, string]
  ...
  channel_node.close_channel
    @ (state: channel_node_state, cid: channel_id) -> result[bytes, string]
  ...
  channel_node.handle_peer_message
    @ (state: channel_node_state, from: bytes, msg: bytes) -> result[channel_node_state, string]
    + dispatches incoming protocol messages to the matching handler
    - returns error on unknown message types
    # protocol
    -> std.net.recv
  channel_node.tick
    @ (state: channel_node_state) -> channel_node_state

...
std
  ...
  std.css
    std.css.parse_stylesheet
      @ (source: string) -> result[stylesheet, string]
      + parses CSS rules into a stylesheet structure
      - returns error on unterminated blocks
      # parsing
    std.css.match_rules
      @ (sheet: stylesheet, node: dom_node) -> list[css_rule]
  ...
  std.event
    std.event.new_queue
      ...
      # events
    std.event.push
      @ (queue: event_queue, evt: event) -> void
      + appends an event to the queue
      # events
    std.event.pop
      @ (queue: event_queue) -> optional[event]
      + returns the next event or none when empty
      # events

apps
  apps.new_window
    @ (title: string, width: i32, height: i32) -> window_handle
    + creates a window with the given title and dimensions
    # window_management
  apps.mount
    @ (win: window_handle, html: string, css: string) -> result[view_state, string]

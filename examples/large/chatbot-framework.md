...
chatbot
  ...
  chatbot.add_middleware
    @ (state: bot_state, middleware: fn(ctx: message_context, next: fn() -> string) -> string) -> bot_state
    + appends a middleware that wraps every dispatched handler
    # middleware
  chatbot.set_transport
    @ (state: bot_state, transport: transport_config) -> bot_state

...
std
  ...
  std.collections
    ...
    std.collections.pqueue_push
      fn (q: pqueue_state, priority: i64, item_id: i64) -> pqueue_state
      + inserts an item with a priority key
      # collections
    std.collections.pqueue_pop
      fn (q: pqueue_state) -> tuple[optional[i64], pqueue_state]
...
inference_engine
  inference_engine.new_kv_cache
    fn (num_blocks: i32, block_size: i32) -> kv_cache_state
  ...
  inference_engine.allocate_blocks
    fn (cache: kv_cache_state, num_tokens: i32) -> result[tuple[list[i32], kv_cache_state], string]
  ...
  inference_engine.free_blocks
    fn (cache: kv_cache_state, block_ids: list[i32]) -> kv_cache_state
    + returns the listed blocks to the free pool
    # memory_management
  inference_engine.new_scheduler
    fn (max_batch_tokens: i32) -> scheduler_state
    + creates a scheduler bounded by a token budget per batch
    # scheduling
  inference_engine.add_request
    fn (sched: scheduler_state, request_id: i64, prompt_tokens: list[i32], max_new: i32) -> scheduler_state
    + queues a new generation request with its prompt and max new tokens
    # scheduling
    -> std.time.now_millis
  inference_engine.build_batch
    fn (sched: scheduler_state, cache: kv_cache_state) -> tuple[list[i64], scheduler_state, kv_cache_state]
  ...
  inference_engine.step_batch
    fn (sched: scheduler_state, request_ids: list[i64], sampled_tokens: list[i32]) -> tuple[list[i64], scheduler_state]

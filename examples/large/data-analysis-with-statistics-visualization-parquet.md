...
std
  ...
  std.image
    std.image.new_canvas
      @ (width: u32, height: u32) -> canvas_state
      + creates a blank RGBA canvas
      # image
    std.image.draw_line
      @ (canvas: canvas_state, x0: u32, y0: u32, x1: u32, y1: u32, color: u32) -> canvas_state
...
insyra
  insyra.column_from_f64
    @ (name: string, values: list[f64]) -> column
    + creates a numeric column
    # construction
  insyra.column_from_string
    @ (name: string, values: list[string]) -> column
    + creates a string column
    # construction
  insyra.new_dataframe
    @ (columns: list[column]) -> result[dataframe, string]
  ...
  insyra.select
    ...
    # projection
  insyra.filter
    @ (df: dataframe, column: string, predicate: fn(cell) -> bool) -> result[dataframe, string]
    + returns only rows where the predicate holds on the named column
    - returns error when the column does not exist
    # filtering
  insyra.describe
    @ (df: dataframe, column: string) -> result[stats_summary, string]
  ...
  insyra.group_by_mean
    @ (df: dataframe, group_column: string, value_column: string) -> result[dataframe, string]
    + returns a two-column dataframe with group keys and per-group means
    - returns error when either column does not exist
    # aggregation
  insyra.plot_line
    @ (df: dataframe, x_column: string, y_column: string, width: u32, height: u32) -> result[bytes, string]
  ...
  insyra.read_columnar_file
    @ (path: string) -> result[dataframe, string]
  ...
  insyra.write_columnar_file
    @ (df: dataframe, path: string) -> result[void, string]

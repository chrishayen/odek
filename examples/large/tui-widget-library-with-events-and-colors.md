...
std
  std.term
    ...
    std.term.write_frame
      @ (cells: list[cell]) -> result[void, string]
...
tui
  ...
  tui.set_root
    ...
    # construction
  tui.layout
    @ (state: app_state, width: i32, height: i32) -> layout_tree
    + computes rectangles for every widget in the tree
    # layout
  tui.render
    @ (tree: layout_tree) -> list[cell]
    + emits a full frame of styled cells
    # rendering
    -> std.text.grapheme_width
  tui.dispatch_key
    @ (state: app_state, key: key_event) -> app_state
  ...
  tui.focus_next
    ...
    # focus
  tui.make_text
    @ (content: string, style: style_spec) -> widget
    + creates a text widget with the given style
    # widgets
  tui.make_box
    @ (child: widget, border: border_spec) -> widget
    + wraps a child in a bordered box
    # widgets
  tui.make_list
    @ (items: list[string], style: style_spec) -> widget

...
std
  ...
  std.store
    ...
    std.store.get
      ...
      # storage
    std.store.range
      fn (db: store_handle, start: bytes, end: bytes) -> list[tuple[bytes, bytes]]
      + scans a half-open key range
      # storage
    std.store.delete
      fn (db: store_handle, key: bytes) -> result[void, string]
      + removes a key
      # storage
  std.time
    std.time.now_millis
...
toydb
  toydb.raft_new_node
    fn (node_id: string, peers: list[string]) -> raft_state
    + returns a follower raft node with the given peers
    # raft
  toydb.raft_tick
    fn (state: raft_state, now_ms: i64) -> raft_state
    + drives election timeouts and heartbeats
    # raft
    -> std.time.now_millis
  toydb.raft_append_entry
    fn (state: raft_state, entry: bytes) -> result[raft_state, string]
  ...
  toydb.raft_handle_rpc
    fn (state: raft_state, msg: raft_msg) -> tuple[raft_state, list[raft_msg]]
    + applies an incoming raft rpc and returns any outgoing messages
    # raft
  toydb.raft_serve
    fn (state: raft_state, addr: string) -> result[server_handle, string]
    + serves raft rpcs on the given address
    # raft
    -> std.net.rpc_serve
  toydb.mvcc_begin
    fn (db: store_handle, mode: txn_mode) -> result[txn_state, string]
    + starts a transaction and assigns it a monotonically increasing timestamp
    # mvcc
  toydb.mvcc_get
    fn (txn: txn_state, key: bytes) -> result[optional[bytes], string]
    + returns the latest visible version for the given key
    # mvcc
    -> std.store.range
  toydb.mvcc_put
    fn (txn: txn_state, key: bytes, value: bytes) -> result[txn_state, string]
    ...
    -> std.store.put
  toydb.mvcc_commit
    fn (txn: txn_state) -> result[void, string]

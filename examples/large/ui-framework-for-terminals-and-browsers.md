...
std
  std.collections
    std.collections.list_index_of
      @ (items: list[string], target: string) -> optional[i32]
...
ui
  ui.new_app
    @ (root_widget_id: string) -> app_state
    + creates an app with the given root widget and an empty event queue
    # construction
  ui.mount
    @ (state: app_state, parent_id: string, child_id: string, props: map[string,string]) -> app_state
    + attaches a child widget to a parent and stores its props
    - returns unchanged state when parent_id is not in the tree
    # tree_construction
  ui.unmount
    @ (state: app_state, widget_id: string) -> app_state
    + removes a widget and all its descendants
    # tree_construction
  ui.set_prop
    @ (state: app_state, widget_id: string, key: string, value: string) -> app_state
    + updates a single prop and marks the widget dirty
    # state_update
  ui.push_event
    @ (state: app_state, event_kind: string, target_id: string, payload: string) -> app_state
  ...
  ui.tick
    ...
    -> std.collections.list_index_of
  ui.layout
    @ (state: app_state, viewport_cols: i32, viewport_rows: i32) -> layout_tree
    ...
    -> std.strings.measure_width
  ui.render
    @ (state: app_state, layout: layout_tree) -> list[draw_command]
  ...
  ui.focus
    ...
    # focus_management
  ui.inject_key
    @ (state: app_state, key: string, modifiers: i32) -> app_state

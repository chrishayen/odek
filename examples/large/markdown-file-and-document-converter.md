...
std
  ...
  std.docx
    std.docx.extract_text
      fn (data: bytes) -> result[list[docx_block], string]
...
md
  md.detect_format
    fn (path: string, data: bytes) -> file_format
  ...
  md.convert
    ...
    -> std.fs.read_all_bytes
  md.convert_bytes
    fn (data: bytes, format: file_format) -> result[string, string]

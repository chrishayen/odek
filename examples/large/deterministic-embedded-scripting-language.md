...
std
  ...
  std.collections
    std.collections.map_get
      fn (m: map[string, i64], key: string) -> optional[i64]
...
script_lang
  ...
  script_lang.new_env
    ...
    # environment
  script_lang.bind
    fn (env: env_state, name: string, value: i64) -> env_state
  ...
  script_lang.call_function
    fn (env: env_state, name: string, args: list[i64]) -> result[i64, string]
    + invokes a user-defined function and returns its result
    - returns error when arity does not match
    # function_call
  script_lang.run
    fn (source: string, env: env_state) -> result[env_state, string]

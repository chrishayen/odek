...
di
  di.new_graph
    ...
    # construction
  di.add_provider
    @ (state: graph_state, name: string, produces: string, depends_on: list[string]) -> result[graph_state, string]
    ...
    # registration
  di.set_binding
    @ (state: graph_state, interface_type: string, concrete_type: string) -> graph_state
    + maps an abstract type to the concrete type that should satisfy it
    # binding
  di.resolve_order
    @ (state: graph_state, target_type: string) -> result[list[string], string]
  ...
  di.detect_cycles
    ...
    # diagnostics
  di.missing_types
    @ (state: graph_state, target_type: string) -> list[string]
    + returns types reachable from target_type that lack a provider
    # diagnostics
  di.emit_wiring_source
    @ (state: graph_state, target_type: string, function_name: string) -> result[string, string]
  ...
  di.parse_provider_file
    @ (source: string) -> result[list[provider_decl], string]
  ...
  di.load_providers_from_path
    @ (state: graph_state, path: string) -> result[graph_state, string]
    + reads a provider file from disk and adds each provider to the graph
    - returns error when the file cannot be read
    # loading
    -> std.fs.read_all
  di.write_generated_file
    @ (state: graph_state, target_type: string, function_name: string, path: string) -> result[void, string]

...
std
  ...
  std.net
    std.net.send_message
      @ (peer: string, payload: bytes) -> result[void, string]
...
game_server
  ...
  game_server.space_add
    @ (world: world_state, s: space) -> world_state
    + registers a space in the world
    # space_mgmt
  game_server.entity_spawn
    @ (world: world_state, space_name: string, e: entity) -> result[world_state, string]
    + places an entity into the named space
    - returns error when the space does not exist
    # space_mgmt
  game_server.entity_move
    @ (world: world_state, id: entity_id, dx: f32, dy: f32) -> result[world_state, string]
  ...
  game_server.entity_migrate
    @ (world: world_state, id: entity_id, target_space: string) -> result[world_state, string]
  ...
  game_server.aoi_query
    @ (world: world_state, space_name: string, x: f32, y: f32, radius: f32) -> list[entity_id]
    + returns entity ids within a radius of a point (area-of-interest query)
    # queries
  game_server.behavior_register
    @ (world: world_state, name: string, fn: behavior_fn) -> world_state
    + registers a named behavior function
    # behaviors
  game_server.behavior_swap
    @ (world: world_state, name: string, fn: behavior_fn) -> result[world_state, string]

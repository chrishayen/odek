...
std
  ...
  std.sort
    std.sort.indices_by_f64
      fn (xs: list[f64], ascending: bool) -> list[i64]
      + returns the stable sort permutation of indices
      # sorting

dataframe
  dataframe.new
    fn (column_names: list[string]) -> dataframe_state
    + builds an empty frame with the given column schema
    # construction
  dataframe.set_column_f64
    fn (df: dataframe_state, name: string, values: list[f64]) -> result[dataframe_state, string]
  ...
  dataframe.inner_join
    fn (left: dataframe_state, right: dataframe_state, key: string) -> result[dataframe_state, string]

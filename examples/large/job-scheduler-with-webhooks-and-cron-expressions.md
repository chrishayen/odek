...
scheduler
  ...
  scheduler.add_cron_job
    fn (state: scheduler_state, id: string, expr: string, action: job_action) -> result[scheduler_state, string]
    + registers a cron-triggered job under id
    - returns error when expr is invalid
    # registration
    -> std.strings.trim
  scheduler.add_oneshot_job
    fn (state: scheduler_state, id: string, fire_at: i64, action: job_action) -> scheduler_state
    + registers a single-fire job at an absolute unix second
    # registration
  scheduler.add_webhook_job
    fn (state: scheduler_state, id: string, secret: string, action: job_action) -> scheduler_state
    + registers a job that fires only when triggered via the webhook trigger function
    # registration
  scheduler.trigger_webhook
    fn (state: scheduler_state, id: string, secret: string) -> result[list[job_action], string]
    ...
    # triggers
  scheduler.due_jobs
    fn (state: scheduler_state) -> tuple[list[job_action], scheduler_state]
  ...
  scheduler.remove_job
    ...
    # registration
  scheduler.list_jobs
    fn (state: scheduler_state) -> list[job_summary]

...
infra
  ...
  infra.register_executor
    @ (name: string, run_fn: string, read_fn: string) -> executor
  ...
  infra.load_resource_file
    @ (path: string) -> result[list[resource], string]
    + reads a resource file and parses it into a list of resources
    # loading
    -> std.fs.read_all
  infra.write_report
    @ (path: string, report: apply_report) -> result[void, string]

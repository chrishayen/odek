...
ml_platform
  ml_platform.load_csv
    @ (path: string, label_column: i32) -> result[dataset, string]
  ...
  ml_platform.train_test_split
    @ (ds: dataset, test_fraction: f64, seed: i64) -> tuple[dataset, dataset]
  ...
  ml_platform.new_logistic_model
    @ (num_features: i32) -> logistic_model
  ...
  ml_platform.save_model
    @ (model: logistic_model, path: string) -> result[void, string]
    + persists weights and bias to disk
    # persistence
  ml_platform.load_model
    @ (path: string) -> result[logistic_model, string]

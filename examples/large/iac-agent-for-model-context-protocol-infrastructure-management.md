...
iac_agent
  ...
  iac_agent.read_config
    fn (state: iac_agent_state, relative_path: string) -> result[string, string]
  ...
  iac_agent.write_config
    fn (state: iac_agent_state, relative_path: string, contents: string) -> result[void, string]
    + writes a configuration file inside the workspace
    - returns error when the path escapes the workspace
    # config_write
    -> std.fs.write_all
  iac_agent.handle_request
    fn (state: iac_agent_state, request_json: string) -> string

...
mqtt
  mqtt.decode_packet
    fn (buffer: bytes) -> result[tuple[mqtt_packet, i32], string]
  ...
  mqtt.new_broker
    ...
    # construction
  mqtt.connect_client
    fn (state: broker_state, client_id: string, clean_session: bool) -> tuple[broker_state, mqtt_packet]
  ...
  mqtt.disconnect_client
    ...
    # lifecycle
  mqtt.subscribe
    fn (state: broker_state, client_id: string, topic_filter: string, qos: i32) -> tuple[broker_state, mqtt_packet]
    + records the subscription and returns a SUBACK packet
    - the SUBACK reports failure when qos is outside 0..2
    # subscription
  mqtt.unsubscribe
    fn (state: broker_state, client_id: string, topic_filter: string) -> tuple[broker_state, mqtt_packet]
  ...
  mqtt.publish
    ...
    -> mqtt.match_topic
  mqtt.retained_for
    fn (state: broker_state, topic_filter: string) -> list[tuple[string, bytes]]
    + returns all retained messages matching a topic filter
    # storage
    -> mqtt.match_topic
  mqtt.handle_pingreq
    fn (state: broker_state, client_id: string) -> tuple[broker_state, mqtt_packet]

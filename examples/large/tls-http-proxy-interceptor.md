...
std
  ...
  std.tls
    std.tls.server_handshake
      fn (conn: conn_handle, cert_pem: bytes, key_pem: bytes) -> result[conn_handle, string]
    ...
    std.tls.client_handshake
      fn (conn: conn_handle, server_name: string) -> result[conn_handle, string]
  ...
  std.crypto
    ...
    std.crypto.sign_leaf_cert
      fn (ca_cert: bytes, ca_key: bytes, host: string, leaf_key: bytes) -> result[bytes, string]
...
proxy
  proxy.new
    fn (ca_cert: bytes, ca_key: bytes) -> proxy_state
    + creates a proxy configured with the given signing CA
    # construction
  proxy.register_request_interceptor
    fn (state: proxy_state, handler_id: string) -> proxy_state
  ...
  proxy.register_response_interceptor
    ...
    # extension
  proxy.start
    fn (state: proxy_state, host: string, port: i32) -> result[proxy_state, string]
    + begins listening for client connections
    - returns error when the address cannot be bound
    # lifecycle
    -> std.net.tcp_listen
  proxy.accept_one
    fn (state: proxy_state) -> result[flow_record, string]
    + accepts one client, runs CONNECT/plain flow, and returns the recorded flow
    # connection_handling
    -> std.net.tcp_accept
    -> std.net.tcp_dial
  proxy.terminate_tls
    fn (state: proxy_state, client_conn: conn_handle, host: string) -> result[conn_handle, string]
  ...
  proxy.forward_request
    fn (state: proxy_state, req: http_request, host: string, port: i32) -> result[http_response, string]
  ...
  proxy.apply_response_interceptors
    fn (state: proxy_state, resp: http_response) -> http_response
    + runs each registered response interceptor in order
    # extension
  proxy.record_flow
    fn (state: proxy_state, req: http_request, resp: http_response) -> tuple[proxy_state, flow_record]
    + stores the flow for later inspection and returns it
    # recording
  proxy.list_flows
    fn (state: proxy_state) -> list[flow_record]
    + returns all recorded flows in arrival order
    # query
  proxy.replay_flow
    fn (state: proxy_state, flow_id: string) -> result[http_response, string]

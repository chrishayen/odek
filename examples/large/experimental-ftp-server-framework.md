...
std
  std.net
    std.net.parse_host_port
      @ (raw: string) -> result[tuple[string, i32], string]
      + parses a "host:port" string
      - returns error when port is missing or not numeric
      # networking
  std.text
    std.text.split_once
      @ (s: string, sep: string) -> tuple[string, string]
...
ftp
  ftp.new_session
    ...
    # construction
  ftp.parse_command
    @ (raw: string) -> result[ftp_command, string]
    ...
    -> std.text.trim_crlf
  ftp.handle_user
    @ (session: session_state, username: string) -> tuple[string, session_state]
    + records the username and returns "331 need password"
    # auth
  ftp.handle_pass
    @ (session: session_state, password: string, expected: string) -> tuple[string, session_state]
    ...
    # auth
  ftp.handle_cwd
    @ (session: session_state, path: string) -> tuple[string, session_state]
  ...
  ftp.handle_pwd
    ...
    # navigation
  ftp.handle_list
    @ (session: session_state, entries: list[string]) -> string
    + formats directory entries into an FTP list reply body
    ? the caller provides entries via the backend
    # listing
  ftp.handle_retr
    @ (session: session_state, path: string, data: bytes) -> string
  ...
  ftp.handle_quit
    ...
    # lifecycle
  ftp.dispatch
    @ (session: session_state, raw: string, backend: backend_state) -> tuple[string, session_state]

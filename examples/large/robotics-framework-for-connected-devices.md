...
std
  ...
  std.io
    std.io.read_bytes
      fn (handle: i32, n: i32) -> result[bytes, string]
      + reads up to n bytes from an opaque handle
      - returns error when the handle is closed
      # io
    std.io.write_bytes
      fn (handle: i32, data: bytes) -> result[i32, string]

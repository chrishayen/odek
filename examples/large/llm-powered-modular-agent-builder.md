...
std
  ...
  std.json
    ...
    std.json.encode_value
      ...
      # serialization
    std.json.get_string
      fn (v: json_value, path: list[string]) -> optional[string]
...
agent
  agent.new_tool_registry
    ...
    # construction
  agent.register_tool
    fn (registry: tool_registry, name: string, schema: string, handler: tool_handler) -> result[tool_registry, string]
    ...
    -> std.json.parse_value
  agent.new
    fn (system_prompt: string, tools: tool_registry, provider: model_provider) -> agent_state
    + creates an agent bound to a system prompt, tool set, and provider
    # construction
  agent.run
    fn (state: agent_state, user_message: string) -> result[list[agent_message], string]
  ...
  agent.build_request_body
    fn (state: agent_state, history: list[agent_message]) -> string
    + serializes the conversation and tool definitions for the provider
    # protocol
    -> std.json.encode_value
  agent.parse_response
    fn (raw: string) -> result[agent_step, string]
  ...
  agent.dispatch_tool_call
    fn (tools: tool_registry, name: string, args: string) -> result[string, string]
    ...
    # tools
  agent.call_provider
    fn (provider: model_provider, body: string) -> result[string, string]

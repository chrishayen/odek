...
std
  ...
  std.io
    std.io.read_line
      @ (reader: reader_handle) -> result[optional[string], string]
...
usql
  ...
  usql.register_driver
    @ (state: client_state, scheme: string, driver: driver) -> client_state
    + registers a driver to handle URLs with the given scheme
    # drivers
  usql.connect
    @ (state: client_state, url: string) -> result[tuple[conn_id, client_state], string]
    ...
    -> std.url.parse
  usql.disconnect
    @ (state: client_state, conn: conn_id) -> result[client_state, string]
    + closes the connection and removes it from the state
    - returns error when the id is unknown
    # connection
  usql.exec
    @ (state: client_state, conn: conn_id, sql: string) -> result[i64, string]
    ...
    # execution
  usql.query
    @ (state: client_state, conn: conn_id, sql: string) -> result[result_set, string]
    + runs a SELECT and returns columns plus rows
    - returns error on driver failure
    # execution
  usql.format_table
    @ (rs: result_set) -> string
  ...
  usql.read_multiline
    ...
    -> std.io.read_line
  usql.describe_table
    @ (state: client_state, conn: conn_id, table: string) -> result[list[column_info], string]

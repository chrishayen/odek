...
access_proxy
  ...
  access_proxy.issue_session
    fn (secret: bytes, subject: string, claims: map[string,string], ttl_seconds: i64) -> string
  ...
  access_proxy.verify_session
    fn (secret: bytes, token: string) -> result[session_claims, string]
  ...
  access_proxy.match_route
    fn (config: config_state, host: string, path: string) -> optional[route]
    + returns the first route whose host and path prefix match
    # routing
  access_proxy.evaluate_policy
    fn (route: route, claims: session_claims) -> result[void, string]
  ...
  access_proxy.handle_request
    fn (state: proxy_state, req: http_request) -> http_response

...
realtime
  ...
  realtime.decode_frame
    ...
    -> std.json.parse_object
  realtime.emit_to
    fn (hub: hub_state, sid: session_id, event: string, payload: map[string, string]) -> result[tuple[hub_state, list[outbound]], string]
  ...
  realtime.broadcast_room
    fn (hub: hub_state, room: string, event: string, payload: map[string, string]) -> tuple[hub_state, list[outbound]]
    + returns frames for every session currently in the room
    # dispatch
  realtime.join_room
    fn (hub: hub_state, sid: session_id, room: string) -> result[hub_state, string]
    + adds the session to the room
    - returns error when the session is not connected
    # rooms
  realtime.leave_room
    fn (hub: hub_state, sid: session_id, room: string) -> hub_state
    + removes the session from the room; no-op if not a member
    # rooms
  realtime.request_ack
    fn (hub: hub_state, sid: session_id, event: string, payload: map[string, string]) -> result[tuple[hub_state, outbound, i64], string]
    + emits a frame with a fresh ack id and records it as pending
    # acknowledgements
    -> std.time.now_millis
  realtime.receive_ack
    fn (hub: hub_state, sid: session_id, ack_id: i64) -> hub_state
    + clears the pending ack so it is no longer considered outstanding
    # acknowledgements
  realtime.tick_heartbeats
    fn (hub: hub_state, interval_ms: i64, now_ms: i64) -> tuple[hub_state, list[outbound], list[session_id]]

...
dataframe
  ...
  dataframe.new_registry
    ...
    # construction
  dataframe.compile
    @ (registry: registry_state, backend: string, expr: expr_node) -> result[compiled_plan, string]
    ...
    # compilation
  dataframe.execute
    @ (registry: registry_state, backend: string, plan: compiled_plan) -> result[result_set, string]
    + runs the compiled plan on the backend and returns rows
    - returns error when the backend driver reports a runtime failure
    # execution
  dataframe.schema_of
    @ (expr: expr_node) -> map[string,type_t]
    + returns the resulting schema of an expression
    # inference
  dataframe.explain
    @ (registry: registry_state, backend: string, expr: expr_node) -> result[string, string]

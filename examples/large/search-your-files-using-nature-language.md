...
std
  ...
  std.text
    std.text.chunk
      @ (text: string, max_chars: i32, overlap: i32) -> list[string]
...
file_search
  file_search.new_index
    @ (embed_dim: i32) -> index_state
    + creates an empty index with the given embedding dimension
    # construction
  file_search.add_document
    @ (state: index_state, path: string, chunks: list[string], embeddings: list[list[f32]]) -> result[index_state, string]
  ...
  file_search.index_directory
    @ (state: index_state, root: string, embed: fn(string) -> list[f32]) -> result[index_state, string]
    ...
    -> std.text.chunk
  file_search.query
    @ (state: index_state, query_embedding: list[f32], top_k: i32) -> list[search_hit]
  ...
  file_search.top_files
    @ (hits: list[search_hit]) -> list[string]

...
pvcs
  pvcs.init_repo
    @ (root: string) -> result[repo_state, string]
  ...
  pvcs.diff_text
    ...
    # diffing
  pvcs.make_patch
    @ (author: string, message: string, changes: list[file_change]) -> patch
    + assembles a patch with a stable content-addressed id
    # patch_creation
    -> std.hash.blake3
    -> std.time.now_seconds
  pvcs.apply_patch
    @ (state: repo_state, p: patch) -> result[repo_state, string]
  ...
  pvcs.patches_commute
    ...
    # commutation
  pvcs.unapply_patch
    @ (state: repo_state, patch_id: bytes) -> result[repo_state, string]
    ...
    # inversion
  pvcs.merge
    @ (local: repo_state, remote: repo_state) -> result[repo_state, list[conflict]]
    ...
    # merging
  pvcs.checkout_file
    @ (state: repo_state, path: string) -> result[string, string]
  ...
  pvcs.record_working_tree
    @ (state: repo_state, author: string, message: string) -> result[patch, string]
  ...
  pvcs.missing_dependencies
    @ (state: repo_state, p: patch) -> list[bytes]

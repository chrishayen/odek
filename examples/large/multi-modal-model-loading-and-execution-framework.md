...
models
  models.new_hub
    ...
    # construction
  models.register
    @ (state: hub_state, name: string, modality: string, config_path: string, weights_path: string) -> hub_state
    + records a model's metadata and on-disk locations
    - returns unchanged state when name is already registered
    # registry
  models.load
    @ (state: hub_state, name: string) -> result[model_handle, string]
    ...
    -> std.json.parse_object
  models.unload
    @ (state: hub_state, handle: model_handle) -> hub_state
    + releases resources held by a loaded model
    # loading
  models.tokenize_text
    @ (handle: model_handle, text: string) -> list[i32]
    + converts text into token ids using the model's tokenizer
    # text_preprocessing
  models.detokenize
    @ (handle: model_handle, ids: list[i32]) -> string
    + reconstructs text from token ids
    # text_postprocessing
  models.preprocess_image
    @ (handle: model_handle, pixels: bytes, width: i32, height: i32) -> result[tensor, string]
  ...
  models.preprocess_audio
    @ (handle: model_handle, samples: list[f32], sample_rate: i32) -> result[tensor, string]

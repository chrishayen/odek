...
websocket
  websocket.server_handshake
    @ (conn: conn_state) -> result[void, string]
  ...
  websocket.client_handshake
    @ (conn: conn_state, host: string, path: string) -> result[void, string]
  ...
  websocket.encode_frame
    @ (opcode: i32, payload: bytes, masked: bool) -> bytes
  ...
  websocket.decode_frame
    @ (raw: bytes) -> result[tuple[i32, bytes, i32], string]

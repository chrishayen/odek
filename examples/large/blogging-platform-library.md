...
blog
  blog.new
    ...
    # construction
  blog.create_post
    @ (state: blog_state, author: string, title: string, body: string) -> string
    ...
    -> std.time.now_seconds
  blog.update_post
    @ (state: blog_state, id: string, title: string, body: string) -> result[void, string]
    + updates title and body; regenerates the slug if the title changed
    - returns error when the post id is unknown
    # authoring
    -> std.text.slugify
  blog.publish
    @ (state: blog_state, id: string) -> result[void, string]
  ...
  blog.unpublish
    ...
    # workflow
  blog.get_by_slug
    @ (state: blog_state, slug: string) -> optional[post]
    + returns the published post with the given slug
    # retrieval
  blog.list_published
    @ (state: blog_state, limit: i32, offset: i32) -> list[post]
    + returns published posts in reverse chronological order
    # retrieval
  blog.add_tag
    @ (state: blog_state, post_id: string, tag: string) -> result[void, string]
    + attaches a tag to a post
    - returns error when the post id is unknown
    # tagging
  blog.list_by_tag
    @ (state: blog_state, tag: string) -> list[post]
    + returns published posts that carry the tag, newest first
    # tagging
  blog.add_comment
    @ (state: blog_state, post_id: string, author: string, body: string) -> result[string, string]
    ...
    -> std.time.now_seconds
  blog.delete_comment
    @ (state: blog_state, comment_id: string) -> bool
    + removes a comment; returns true if one was removed
    # comments
  blog.delete_post
    @ (state: blog_state, id: string) -> bool

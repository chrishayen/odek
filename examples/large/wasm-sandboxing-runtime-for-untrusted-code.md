...
std
  std.wasm
    std.wasm.parse_module
      fn (bytes: bytes) -> result[wasm_module, string]
    ...
    std.wasm.instantiate
      ...
      # wasm
    std.wasm.call
      fn (instance: wasm_instance, name: string, args: list[i64]) -> result[list[i64], string]
...
sandbox
  sandbox.new_limits
    fn (max_memory_pages: i32, max_fuel: i64, max_stack: i32) -> limits
    + creates a resource limit bundle
    # configuration
  sandbox.load
    fn (code: bytes, limits: limits) -> result[sandbox_state, string]
  ...
  sandbox.register_host_fn
    fn (state: sandbox_state, name: string, fn: host_fn) -> sandbox_state
    + registers a host function by import name before instantiation
    # host_bindings
  sandbox.invoke
    fn (state: sandbox_state, name: string, args: list[i64]) -> result[list[i64], string]
  ...
  sandbox.consume_fuel
    fn (state: sandbox_state, amount: i64) -> result[sandbox_state, string]
    + decrements remaining fuel by the given amount
    - returns error when amount exceeds remaining fuel
    # metering
  sandbox.remaining_fuel
    fn (state: sandbox_state) -> i64
    + returns current fuel budget
    # introspection
  sandbox.copy_out
    fn (state: sandbox_state, offset: i32, length: i32) -> result[bytes, string]

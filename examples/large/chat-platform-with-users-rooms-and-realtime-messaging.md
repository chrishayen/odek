...
chat
  chat.new
    ...
    # construction
  chat.register_user
    fn (state: chat_state, handle: string, password: string) -> result[tuple[user_id, chat_state], string]
    ...
    -> std.id.new_uuid
  chat.authenticate
    fn (state: chat_state, handle: string, password: string) -> result[user_id, string]
    + returns the user id when credentials match
    - returns error when the handle is unknown or the password is wrong
    # authentication
    -> std.crypto.bcrypt_verify
  chat.create_room
    fn (state: chat_state, owner: user_id, name: string) -> result[tuple[room_id, chat_state], string]
    ...
    -> std.id.new_uuid
  chat.join_room
    fn (state: chat_state, user: user_id, room: room_id) -> result[chat_state, string]
  ...
  chat.leave_room
    ...
    # membership
  chat.post_message
    fn (state: chat_state, room: room_id, author: user_id, body: string) -> result[tuple[message, chat_state], string]
  ...
  chat.recent_messages
    fn (state: chat_state, room: room_id, limit: i32) -> result[list[message], string]
    + returns up to limit most recent messages ordered oldest-first
    - returns error when the room does not exist
    # history
  chat.subscribe
    fn (state: chat_state, user: user_id, room: room_id, sink: fn(message) -> void) -> result[tuple[subscription_id, chat_state], string]
    + registers a callback invoked for each future message in the room
    - returns error when the user is not a member
    # subscriptions
    -> std.id.new_uuid
  chat.unsubscribe
    fn (state: chat_state, sub: subscription_id) -> chat_state

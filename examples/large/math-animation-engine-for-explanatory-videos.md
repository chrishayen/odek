...
std
  ...
  std.image
    std.image.encode_png
      @ (pixels: bytes, width: i32, height: i32) -> bytes
      + encodes an RGBA pixel buffer as PNG
      # image

math_anim
  math_anim.new_scene
    @ (width: i32, height: i32, fps: i32) -> scene
    + creates an empty scene with the given canvas and frame rate
    # construction
  math_anim.add_line
    @ (s: scene, id: string, x0: f32, y0: f32, x1: f32, y1: f32, color: u32) -> scene
    + adds a line segment shape with an id that later animations can reference
    # shape
  math_anim.add_circle
    @ (s: scene, id: string, cx: f32, cy: f32, radius: f32, color: u32) -> scene
    + adds a circle shape with an id
    # shape
  math_anim.add_text
    @ (s: scene, id: string, x: f32, y: f32, content: string, color: u32) -> scene
    + adds a text shape anchored at (x,y)
    # shape
  math_anim.animate
    @ (s: scene, id: string, attribute: string, from: f32, to: f32, start_s: f32, duration_s: f32) -> result[scene, string]

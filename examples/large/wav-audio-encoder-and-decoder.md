...
std
  std.bytes
    ...
    std.bytes.write_i16_le
      ...
      # byte_writing
    std.bytes.slice
      @ (data: bytes, start: i32, length: i32) -> bytes
...
wav
  wav.parse_header
    @ (data: bytes) -> result[wav_format, string]
    ...
    -> std.bytes.read_u32_le
  wav.decode
    @ (data: bytes) -> result[wav_audio, string]
  ...
  wav.samples_per_channel
    @ (audio: wav_audio, channel: i32) -> list[i16]

...
std
  ...
  std.time
    ...
    std.time.format_rfc1123
      fn (epoch_seconds: i64) -> string
      + returns an RFC-1123 date string
      # time

blog_generator
  blog_generator.parse_post
    fn (raw: string) -> result[post, string]
  ...
  blog_generator.render_post
    fn (p: post, template: string) -> string
  ...
  blog_generator.render_index_page
    fn (page: list[post], page_number: i32, total_pages: i32, template: string) -> string
    + fills a listing template with the page's posts and navigation
    # index_render
  blog_generator.render_rss_feed
    fn (site_title: string, site_url: string, posts: list[post]) -> string
    + returns a valid RSS 2.0 document with one item per post
    # rss
    -> std.time.format_rfc1123
  blog_generator.load_tree
    fn (content_root: string) -> result[tuple[list[post], list[tuple[string,string]]], string]
  ...
  blog_generator.build
    fn (content_root: string, out_root: string, templates: map[string,string], per_page: i32) -> result[i32, string]

...
std
  std.tui
    ...
    std.tui.draw_text
      ...
      # tui_primitive
    std.tui.draw_box
      fn (s: screen, row: i32, col: i32, w: i32, h: i32, title: string) -> screen
  ...
  std.event
    std.event.decode_key
      fn (raw: bytes) -> optional[key_event]
...
bluetooth_manager
  bluetooth_manager.new
    fn (adapter: bluetooth_adapter) -> manager_state
    + creates a manager bound to the given adapter handle
    # construction
  bluetooth_manager.start_scan
    fn (state: manager_state) -> result[manager_state, string]
  ...
  bluetooth_manager.ingest_device
    fn (state: manager_state, dev: device) -> manager_state
    + adds or updates a device record in the device list
    ? duplicates by address are merged
    # discovery
  bluetooth_manager.pair
    fn (state: manager_state, addr: string) -> result[manager_state, string]

...
std
  ...
  std.collections
    std.collections.map_merge
      @ (a: map[string, value], b: map[string, value]) -> map[string, value]
...
config_lang
  ...
  config_lang.build_schema
    @ (ast: ast_node) -> result[schema, string]
  ...
  config_lang.check_constraints
    @ (schema: schema, record_name: string, fields: map[string, value]) -> result[void, list[string]]
  ...
  config_lang.apply_function
    @ (env: eval_env, name: string, args: list[value]) -> result[value, string]
  ...
  config_lang.evaluate
    @ (ast: ast_node, env: eval_env) -> result[value, string]
  ...
  config_lang.build_env
    @ (bindings: map[string, value], parent: optional[eval_env]) -> eval_env
    + constructs a lexical environment with optional parent scope
    # environment
    -> std.collections.map_merge
  config_lang.render_record
    @ (value: value) -> result[string, string]

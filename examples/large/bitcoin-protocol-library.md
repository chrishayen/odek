...
std
  std.crypto
    ...
    std.crypto.secp256k1_pubkey_from_priv
      fn (priv: bytes) -> result[bytes, string]
...
bitcoin
  ...
  bitcoin.decode_address
    fn (address: string) -> result[decoded_address, string]
  ...
  bitcoin.serialize_tx
    fn (tx: transaction, include_witness: bool) -> bytes
    + returns the transaction in its wire format
    # transactions
  bitcoin.parse_tx
    fn (raw: bytes) -> result[transaction, string]
    + returns the parsed transaction
    - returns error on truncated input
    # transactions
  bitcoin.sighash
    fn (tx: transaction, input_index: i32, script_pubkey: bytes, value: i64, sighash_type: u8) -> bytes
    + returns the message hash to be signed for the given input
    # transactions
  bitcoin.sign_input
    fn (tx: transaction, input_index: i32, priv: bytes, script_pubkey: bytes, value: i64, sighash_type: u8) -> result[transaction, string]

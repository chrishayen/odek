...
std
  std.net
    std.net.tcp_listen
      @ (host: string, port: u16) -> result[listener, string]
...
microservice
  ...
  microservice.add_action
    @ (svc: service_state, pattern: map[string, string], handler: fn(map[string, string]) -> result[map[string, string], string]) -> service_state
  ...
  microservice.match_action
    @ (svc: service_state, message: map[string, string]) -> optional[action_ref]
  ...
  microservice.client_new
    @ (host: string, port: u16) -> client_state
    + creates a client bound to the given address
    # client
  microservice.client_act
    @ (client: client_state, payload: map[string, string]) -> result[map[string, string], string]

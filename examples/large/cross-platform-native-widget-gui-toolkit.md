...
gui
  gui.new_window
    fn (title: string, width: i32, height: i32) -> widget_state
    + creates a top-level window widget with the given size
    # construction
  gui.add_child
    fn (parent: widget_state, child: widget_state) -> widget_state
    + appends child to parent's child list
    # tree
  gui.label
    fn (text: string) -> widget_state
    + creates a text label widget
    # widgets
  gui.button
    fn (text: string, on_click: fn() -> void) -> widget_state
    + creates a button widget with a click handler
    # widgets
  gui.text_input
    fn (placeholder: string) -> widget_state
    + creates a single-line text input widget
    # widgets
  gui.vbox
    fn (spacing: i32) -> widget_state

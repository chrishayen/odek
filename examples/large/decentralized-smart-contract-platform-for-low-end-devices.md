...
std
  std.crypto
    ...
    std.crypto.ed25519_sign
      fn (secret_key: bytes, message: bytes) -> bytes
  ...
  std.serialize
    std.serialize.encode
      fn (value: serializable) -> bytes
      + returns a deterministic binary encoding
      # serialization
    std.serialize.decode
      fn (raw: bytes) -> result[serializable, string]
...
chain
  ...
  chain.apply_transaction
    fn (state: chain_state, tx: transaction) -> result[chain_state, string]
  ...
  chain.account_balance
    fn (state: chain_state, address: bytes) -> u64
    + returns the balance, or 0 for unknown addresses
    # state
  chain.produce_block
    fn (state: chain_state, mempool: list[transaction], parent_hash: bytes, height: u64) -> result[block, string]
  ...
  chain.validate_block
    fn (state: chain_state, blk: block) -> result[void, string]
  ...
  chain.deploy_contract
    fn (state: chain_state, sender: bytes, code: bytes) -> result[tuple[chain_state, bytes], string]
    + stores bytecode under a deterministic contract address
    - returns error when sender balance cannot cover the deploy fee
    # contracts
    -> std.crypto.sha256
  chain.call_contract
    fn (state: chain_state, caller: bytes, contract: bytes, input: bytes, gas: u64) -> result[tuple[chain_state, bytes], string]
    ...
    -> chain.vm_execute
  chain.vm_execute
    fn (code: bytes, input: bytes, gas: u64) -> result[tuple[bytes, u64], string]

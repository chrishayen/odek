...
websocket
  ...
  websocket.client_handshake
    @ (conn: tcp_conn, host: string, path: string) -> result[ws_conn, string]
  ...
  websocket.decode_frame
    @ (conn: tcp_conn) -> result[frame, string]
    ...
    -> std.io.read_exact
  websocket.send_text
    @ (ws: ws_conn, text: string) -> result[void, string]
    + sends a text frame
    # messaging
    -> websocket.encode_frame
    -> std.io.write_all
  websocket.send_binary
    @ (ws: ws_conn, data: bytes) -> result[void, string]
    + sends a binary frame
    # messaging
    -> websocket.encode_frame
    -> std.io.write_all
  websocket.recv_message
    @ (ws: ws_conn) -> result[ws_message, string]
    ...
    -> websocket.decode_frame
  websocket.close
    @ (ws: ws_conn, code: u16, reason: string) -> result[void, string]

...
mail
  mail.parse_message
    @ (raw: bytes) -> result[message, string]
  ...
  mail.smtp_new_session
    @ (hostname: string) -> smtp_session
    + creates a server session ready to greet a client
    # smtp
  mail.smtp_handle
    @ (session: smtp_session, line: string) -> tuple[list[string], smtp_session]
  ...
  mail.imap_new_session
    @ (hostname: string) -> imap_session
    + creates an IMAP server session in the not-authenticated state
    # imap
  mail.imap_handle
    @ (session: imap_session, line: string) -> tuple[list[string], imap_session]
    ...
    # imap
  mail.open_store
    @ (root: string) -> result[mail_store, string]
    + opens a maildir-style store rooted at root, creating directories if missing
    - returns error on permission failure
    # storage
    -> std.fs.make_dir
  mail.append
    @ (store: mail_store, mailbox: string, m: message) -> result[u64, string]
    ...
    -> std.crypto.sha256
  mail.list_mailboxes
    @ (store: mail_store) -> result[list[string], string]
    + returns mailbox names in the store
    # storage
    -> std.fs.list_dir
  mail.fetch
    @ (store: mail_store, mailbox: string, uid: u64) -> result[message, string]
    + returns the message at uid
    - returns error when uid is not present
    # storage
    -> std.io.read_all
  mail.delete
    @ (store: mail_store, mailbox: string, uid: u64) -> result[void, string]
    + removes the message at uid
    - returns error when uid is not present
    # storage
  mail.authenticate
    @ (username: string, password: string, hashed: bytes) -> bool

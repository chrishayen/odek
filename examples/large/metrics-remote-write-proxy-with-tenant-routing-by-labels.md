...
std
  ...
  std.encoding
    ...
    std.encoding.protobuf_decode_write_request
      @ (raw: bytes) -> result[write_request, string]
...
tenant_proxy
  tenant_proxy.new
    @ (tenant_label: string, default_tenant: string, upstream_url: string, header_name: string) -> proxy_config
  ...
  tenant_proxy.extract_tenant
    @ (series_labels: map[string,string], cfg: proxy_config) -> string
  ...
  tenant_proxy.group_by_tenant
    @ (req: write_request, cfg: proxy_config) -> map[string, write_request]
  ...
  tenant_proxy.handle_request
    @ (body: bytes, cfg: proxy_config) -> result[void, string]

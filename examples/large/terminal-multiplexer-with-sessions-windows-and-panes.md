...
std
  std.pty
    std.pty.spawn
      fn (command: string, args: list[string]) -> result[pty_handle, string]
      + starts a process attached to a new pseudoterminal
      - returns error when exec fails
      # process
    std.pty.write
      fn (handle: pty_handle, data: bytes) -> result[void, string]
      + writes to the pty input
      # io
    std.pty.read
      fn (handle: pty_handle, max: i32) -> result[bytes, string]
      + reads available output up to max bytes
      # io
    std.pty.resize
      fn (handle: pty_handle, cols: u16, rows: u16) -> result[void, string]
      + sends a window-size update to the pty
      # control
    std.pty.close
      fn (handle: pty_handle) -> void
      + closes the pty and terminates the child
      # lifecycle

multiplexer
  multiplexer.new_session
    fn (name: string) -> session_state
    + creates an empty session with one default window
    # session
  multiplexer.create_window
    fn (session: session_state, name: string) -> tuple[i32, session_state]
    + creates a window with a single pane and returns its id
    # window
  multiplexer.split_pane
    fn (session: session_state, pane_id: i32, direction: i8) -> result[tuple[i32, session_state], string]
  ...
  multiplexer.close_pane
    fn (session: session_state, pane_id: i32) -> result[session_state, string]
    + removes a pane and collapses the layout
    - returns error when pane_id is unknown
    # pane
  multiplexer.compute_layout
    fn (session: session_state, window_id: i32, total_cols: u16, total_rows: u16) -> result[list[pane_rect], string]
  ...
  multiplexer.attach_process
    fn (session: session_state, pane_id: i32, command: string, args: list[string]) -> result[session_state, string]
    + starts a process under pane_id
    - returns error when pane_id already has a process
    # process_attach
    -> std.pty.spawn
  multiplexer.send_input
    fn (session: session_state, pane_id: i32, data: bytes) -> result[void, string]
    + writes data to the active pane's pty
    - returns error when pane has no attached process
    # input
    -> std.pty.write
  multiplexer.read_output
    fn (session: session_state, pane_id: i32) -> result[bytes, string]
    + reads buffered output from the pane
    # output
    -> std.pty.read
  multiplexer.resize_window
    fn (session: session_state, window_id: i32, cols: u16, rows: u16) -> result[session_state, string]
  ...
  multiplexer.restore_layout
    fn (serialized: string) -> result[session_state, string]

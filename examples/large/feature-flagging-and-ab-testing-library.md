...
flags
  flags.new_registry
    ...
    # construction
  flags.upsert_flag
    fn (registry: flag_registry, flag: flag_definition) -> flag_registry
    + adds or replaces a flag by key
    # configuration
  flags.delete_flag
    fn (registry: flag_registry, key: string) -> flag_registry
    + removes a flag by key
    # configuration
  flags.bucket_user
    fn (flag_key: string, user_key: string) -> i32
    + returns a deterministic bucket 0..9999 for (flag, user)
    ? independent flags produce independent buckets for the same user
    # bucketing
    -> std.hash.fnv1a_32
  flags.match_segment
    fn (segment: segment_rule, attributes: map[string, string]) -> bool
    + returns true when all predicates in segment match the attributes
    # targeting
  flags.pick_variant
    fn (rollout: list[variant_weight], bucket: i32) -> string
    + returns the variant whose cumulative weight range contains bucket
    ? the sum of weights must be 10000
    # rollout
  flags.evaluate
    fn (registry: flag_registry, key: string, user_key: string, attributes: map[string, string]) -> flag_result
    ...
    -> std.time.now_seconds
  flags.evaluate_all
    fn (registry: flag_registry, user_key: string, attributes: map[string, string]) -> map[string, flag_result]
    + evaluates every flag in the registry for the user
    # evaluation
  flags.record_exposure
    fn (stats: exposure_stats, key: string, variant: string) -> exposure_stats
    + increments the exposure counter for (flag, variant)
    # telemetry
  flags.variant_rates
    fn (stats: exposure_stats, key: string) -> map[string, f64]
    + returns the fraction of exposures observed for each variant
    # telemetry
  flags.load_definitions
    fn (raw: string) -> result[list[flag_definition], string]

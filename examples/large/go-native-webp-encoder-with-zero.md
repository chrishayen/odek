...
std
  std.bitstream
    ...
    std.bitstream.write_bits
      @ (w: bit_writer, value: u32, n_bits: i32) -> bit_writer
  ...
  std.compression
    std.compression.huffman_build
      @ (frequencies: list[i32]) -> huffman_table
...
webp
  ...
  webp.build_huffman_codes
    @ (pixels: pixel_buffer) -> huffman_table
    + builds Huffman codes over the transformed pixel stream
    # entropy
    -> std.compression.huffman_build
  webp.write_vp8l_payload
    @ (pixels: pixel_buffer, table: huffman_table) -> bytes

...
framework
  framework.new
    ...
    # construction
  framework.get
    @ (state: framework_state, path_pattern: string, handler_id: string) -> framework_state
  ...
  framework.delete
    ...
    # route_registration
  framework.use
    @ (state: framework_state, middleware_id: string) -> framework_state
    + appends middleware to the ordered chain
    # middleware
  framework.group
    @ (state: framework_state, prefix: string) -> framework_state
    + creates a scoped subrouter sharing middleware
    # grouping
  framework.match
    @ (state: framework_state, method: string, path: string) -> result[route_match, string]
  ...
  framework.build_context
    @ (method: string, path: string, query: string, body: string) -> request_context
    + builds the per-request context passed to handlers
    # context
    -> std.url.parse_query
  framework.param
    @ (ctx: request_context, name: string) -> optional[string]
    + returns a path or query parameter by name
    # context
  framework.bind_json
    @ (ctx: request_context) -> result[string, string]
    + parses request body as JSON and returns the raw value
    - returns error on malformed JSON
    # binding
    -> std.json.parse
  framework.respond_json
    @ (ctx: request_context, status: i32, body: string) -> response
  ...
  framework.respond_text
    ...
    # response
  framework.dispatch
    @ (state: framework_state, ctx: request_context) -> response

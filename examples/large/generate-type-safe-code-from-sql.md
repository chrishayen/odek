...
sqlgen
  ...
  sqlgen.resolve_query
    @ (q: query_ast, s: schema) -> result[typed_query, string]
    ...
    # type_resolution
  sqlgen.map_sql_type
    @ (sql_type: string) -> result[string, string]
  ...
  sqlgen.emit_signature
    @ (name: string, tq: typed_query) -> string
    + renders a typed function signature and parameter list for the query
    # code_emission
    -> std.strings.to_pascal_case
    -> sqlgen.map_sql_type
  sqlgen.generate
    @ (schema_sql: string, queries: map[string, string]) -> result[string, string]

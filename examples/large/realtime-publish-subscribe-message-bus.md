...
msgbus
  ...
  msgbus.client_connect
    fn (url: string) -> result[client_state, string]
  ...
  msgbus.client_subscribe
    fn (client: client_state, channel: string) -> result[client_state, string]
    + sends a subscribe message and records the channel in the client state
    - returns error when the connection is closed
    # client
    -> std.ws.send
  msgbus.client_publish
    fn (client: client_state, channel: string, payload: json_value) -> result[void, string]
    + sends a publish message
    - returns error when the connection is closed
    # client
    -> std.ws.send
  msgbus.client_next
    fn (client: client_state) -> result[bus_message, string]

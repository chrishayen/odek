...
std
  std.net
    ...
    std.net.dial_tcp
      ...
      # network
    std.net.read
      fn (c: tcp_conn, max: i32) -> result[bytes, string]
      + reads up to max bytes
      - returns error on closed connection
      # network
    std.net.write
      fn (c: tcp_conn, data: bytes) -> result[i32, string]
...
proxyfw
  ...
  proxyfw.add_upstream
    fn (state: proxy_state, name: string, addrs: list[string]) -> proxy_state
    + registers a named pool of upstream addresses
    # upstream
  proxyfw.add_route
    fn (state: proxy_state, match: route_match, upstream: string) -> proxy_state
    + routes matching requests to a named upstream
    # routing
  proxyfw.add_filter
    fn (state: proxy_state, phase: string, filter: callback) -> proxy_state
    + installs a filter at a named phase (request, response)
    # filters
  proxyfw.pick_upstream
    fn (state: proxy_state, name: string) -> result[string, string]
  ...
  proxyfw.handle_connection
    fn (state: proxy_state, client: tcp_conn) -> result[void, string]
    ...
    -> std.net.dial_tcp
  proxyfw.serve
    fn (state: proxy_state, addr: string) -> result[void, string]
    + listens on addr and dispatches connections in a loop
    # server
    -> std.net.listen_tcp
    -> std.net.accept
  proxyfw.graceful_shutdown
    fn (state: proxy_state, deadline_ms: i64) -> result[void, string]

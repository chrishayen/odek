...
jwtvault
  jwtvault.new
    fn (secret: bytes, access_ttl: i64, refresh_ttl: i64) -> jwtvault_state
    + creates a vault with the given secret and token lifetimes
    # construction
  jwtvault.issue
    fn (state: jwtvault_state, subject: string, claims: map[string,string]) -> result[token_pair, string]
  ...
  jwtvault.sign_access
    fn (state: jwtvault_state, subject: string, claims: map[string,string], now: i64) -> string
  ...
  jwtvault.sign_refresh
    fn (state: jwtvault_state, subject: string, jti: string, now: i64) -> string
  ...
  jwtvault.verify_access
    fn (state: jwtvault_state, token: string) -> result[map[string,string], string]
  ...
  jwtvault.verify_refresh
    fn (state: jwtvault_state, token: string) -> result[refresh_claims, string]
    ...
    -> std.time.now_seconds
  jwtvault.refresh
    fn (state: jwtvault_state, refresh_token: string) -> result[token_pair, string]
    ...
    # rotation
  jwtvault.revoke
    fn (state: jwtvault_state, jti: string) -> result[void, string]
    + marks the given refresh jti as revoked in the store
    # revocation
  jwtvault.revoke_all_for_subject
    fn (state: jwtvault_state, subject: string) -> result[i32, string]

...
jobqueue
  jobqueue.app_new
    @ (broker_url: string) -> result[app_state, string]
    + creates an application wired to a message broker
    - returns error when the broker URL is unparseable
    # construction
  jobqueue.register_task
    @ (app: app_state, name: string, handler: task_handler) -> app_state
    + registers a named handler for later dispatch
    # registration
  jobqueue.delay
    @ (app: app_state, name: string, args: map[string,string]) -> result[string, string]
    ...
    -> std.time.now_seconds
  jobqueue.receive
    @ (app: app_state) -> result[optional[task_envelope], string]
    + blocks for up to one poll interval waiting for a task
    # receive
    -> std.json.parse_object
  jobqueue.dispatch
    @ (app: app_state, envelope: task_envelope) -> result[string, string]
    + invokes the registered handler with the decoded args
    - returns error when execution raises
    # dispatch
  jobqueue.ack
    @ (app: app_state, envelope: task_envelope) -> result[void, string]
    + acknowledges successful processing to the broker
    # ack
  jobqueue.nack
    @ (app: app_state, envelope: task_envelope, requeue: bool) -> result[void, string]

...
std
  ...
  std.consensus
    std.consensus.new_log
      fn (node_id: string) -> log_state
      + creates an empty replicated log for the given node
      # consensus
    std.consensus.append
      fn (log: log_state, entry: bytes) -> result[u64, string]
...
cluster
  cluster.new
    ...
    -> std.consensus.new_log
  cluster.join
    fn (c: cluster_state, peer_id: string) -> result[void, string]
    + registers a peer for replication
    - returns error when peer_id is already known
    # membership
  cluster.leave
    fn (c: cluster_state, peer_id: string) -> bool
    + removes a peer and returns true if it existed
    # membership
  cluster.is_leader
    fn (c: cluster_state) -> bool
    + returns true when this node currently holds leadership
    # consensus
  cluster.execute
    fn (c: cluster_state, stmt: string, params: list[string]) -> result[sql_result, string]
  ...
  cluster.apply_committed
    fn (c: cluster_state) -> result[i32, string]
    ...
    -> std.sql.execute_local
  cluster.snapshot
    fn (c: cluster_state) -> result[bytes, string]
    + returns a serialized snapshot of the local store at the current commit index
    # replication
  cluster.restore
    fn (c: cluster_state, snapshot: bytes, last_index: u64) -> result[void, string]
    ...
    # replication
  cluster.status
    fn (c: cluster_state) -> cluster_status

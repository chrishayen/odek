...
ui_test
  ui_test.open_session
    fn (remote_url: string, browser: string) -> result[session, string]
  ...
  ui_test.visit
    ...
    -> std.http.request
  ui_test.locate
    fn (sess: session, selector: string) -> result[element, string]
  ...
  ui_test.type_text
    ...
    -> std.json.encode
  ui_test.wait_for
    fn (sess: session, selector: string, timeout_ms: i32) -> result[element, string]
    ...
    -> std.time.now_millis
  ui_test.assert_text
    fn (sess: session, elem: element, expected: string) -> result[void, string]
  ...
  ui_test.register_test
    fn (suite: test_suite, name: string, body: test_body) -> test_suite
    + adds a named test to the suite
    # registration
  ui_test.run_suite
    fn (suite: test_suite, remote_url: string) -> suite_report

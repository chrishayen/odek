...
git
  git.hash_blob
    ...
    -> std.encoding.hex_encode
  git.write_blob
    @ (store: object_store, content: bytes) -> tuple[string, object_store]
    + stores a blob and returns its id plus the updated store
    # objects
    -> std.compression.deflate
  git.read_blob
    @ (store: object_store, id: string) -> result[bytes, string]

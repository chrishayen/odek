...
std
  ...
  std.git
    std.git.init_bare
      ...
      # git
    std.git.list_refs
      fn (path: string) -> result[list[git_ref], string]
      + returns all refs in the bare repository
      # git
    std.git.read_blob
      fn (path: string, sha: string) -> result[bytes, string]
      + returns the blob bytes for the given object id
      - returns error when the object does not exist
      # git
    std.git.log
      fn (path: string, ref: string, limit: i32) -> result[list[git_commit], string]
...
git_service
  git_service.new
    fn (data_root: string) -> service_state
    + creates a service rooted at data_root
    # construction
    -> std.fs.mkdir_all
  git_service.register_user
    fn (state: service_state, username: string, email: string, password: string) -> result[tuple[service_state, user_id], string]
  ...
  git_service.authenticate
    fn (state: service_state, username: string, password: string) -> optional[user_id]
    + returns the user id when credentials verify
    - returns none on mismatch
    # auth
    -> std.crypto.bcrypt_verify
  git_service.create_organization
    fn (state: service_state, owner: user_id, name: string) -> result[tuple[service_state, org_id], string]
    + creates an organization owned by the user
    - returns error when the organization name is taken
    # organizations
  git_service.add_org_member
    fn (state: service_state, org: org_id, user: user_id, role: string) -> result[service_state, string]
    + adds a user to an organization with a role
    - returns error when role is not "member" or "admin"
    # organizations
  git_service.create_repository
    fn (state: service_state, owner_user: optional[user_id], owner_org: optional[org_id], name: string, is_private: bool) -> result[tuple[service_state, repo_id], string]
  ...
  git_service.delete_repository
    fn (state: service_state, actor: user_id, repo: repo_id) -> result[service_state, string]
  ...
  git_service.grant_access
    fn (state: service_state, repo: repo_id, user: user_id, level: string) -> result[service_state, string]
  ...
  git_service.check_access
    fn (state: service_state, repo: repo_id, user: user_id, required: string) -> bool
    + returns true when the user has at least the required access
    # permissions
  git_service.list_branches
    fn (state: service_state, repo: repo_id) -> result[list[string], string]
    + returns branch names for the repository
    - returns error when the repo id is unknown
    # browsing
    -> std.git.list_refs
  git_service.commit_history
    fn (state: service_state, repo: repo_id, ref: string, limit: i32) -> result[list[git_commit], string]
    + returns up to limit commits reachable from ref
    - returns error when the ref does not exist
    # browsing
    -> std.git.log
  git_service.open_issue
    fn (state: service_state, repo: repo_id, author: user_id, title: string, body: string) -> result[tuple[service_state, issue_id], string]
    + creates a new issue on the repository
    - returns error when title is empty
    # issues
    -> std.time.now_seconds
  git_service.close_issue
    fn (state: service_state, actor: user_id, issue: issue_id) -> result[service_state, string]

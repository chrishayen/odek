...
oplog
  oplog.new
    @ (capacity: i64) -> oplog_state
    + creates an oplog with the given retention capacity
    ? capacity is the maximum number of retained ops; older ops are pruned
    # construction
  oplog.append
    @ (state: oplog_state, event_type: string, object_id: string, payload: map[string, string]) -> tuple[string, oplog_state]
    ...
    -> std.encoding.hex_encode
  oplog.since
    @ (state: oplog_state, cursor: string) -> result[list[string], string]
  ...
  oplog.encode_envelope
    @ (event_type: string, object_id: string, payload: map[string, string], id: string, ts_millis: i64) -> string
  ...
  oplog.decode_envelope
    ...
    -> std.json.parse_object
  oplog.apply_remote
    @ (state: oplog_state, envelope: map[string, string]) -> result[oplog_state, string]

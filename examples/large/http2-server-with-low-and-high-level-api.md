...
std
  ...
  std.io
    std.io.read_u24_be
      fn (data: bytes, offset: i32) -> result[i32, string]
...
http2
  http2.new_connection
    fn (is_server: bool) -> conn_state
    + creates a fresh connection state with default settings and empty stream table
    # construction
  http2.parse_frame
    fn (data: bytes, offset: i32) -> result[tuple[frame, i32], string]
  ...
  http2.encode_frame
    ...
    -> std.bytes.concat
  http2.apply_frame
    fn (state: conn_state, f: frame) -> result[tuple[conn_state, list[frame]], string]
    ...
    # low_level
  http2.open_stream
    fn (state: conn_state, headers: map[string, string]) -> tuple[i32, conn_state]
  ...
  http2.encode_headers
    fn (headers: map[string, string]) -> bytes
  ...
  http2.new_router
    ...
    # high_level
  http2.route
    fn (router: router_state, method: string, path: string, handler_name: string) -> router_state
    + registers a handler name for the given method/path pair
    # high_level
  http2.dispatch
    fn (router: router_state, method: string, path: string) -> optional[string]
    + returns the handler name for a request
    - returns none when no route matches
    # high_level
  http2.build_response
    fn (status: i32, headers: map[string, string], body: bytes) -> response

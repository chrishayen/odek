...
std
  ...
  std.process
    std.process.spawn
      fn (argv: list[string], cwd: string) -> result[i32, string]
...
toolkit
  toolkit.run_script
    fn (script_path: string, argv: list[string]) -> result[i32, string]
  ...
  toolkit.resolve_dependencies
    fn (manifest_path: string) -> result[map[string, string], string]
  ...
  toolkit.install_dependency
    fn (name: string, version: string, target_dir: string) -> result[string, string]
    ...
    -> std.hash.sha256_hex
  toolkit.run_tests
    fn (root: string) -> result[test_report, string]
    ...
    -> std.process.spawn
  toolkit.bundle
    fn (entry: string, output: string) -> result[void, string]
  ...
  toolkit.format_source
    ...
    # formatting
  toolkit.type_check
    fn (source: string) -> result[void, list[string]]

...
avatica
  avatica.open
    @ (endpoint: string) -> result[avatica_conn, string]
    ...
    -> std.uuid.new_v4
  avatica.close
    @ (conn: avatica_conn) -> result[void, string]
    + sends CloseConnectionRequest and releases local state
    # connection
    -> std.encoding.protobuf_encode
    -> std.http.post
  avatica.create_statement
    @ (conn: avatica_conn) -> result[avatica_stmt, string]
    ...
    -> std.http.post
  avatica.execute
    @ (stmt: avatica_stmt, sql: string) -> result[avatica_result, string]
    ...
    -> std.http.post
  avatica.fetch_next
    @ (result: avatica_result) -> result[optional[list[list[string]]], string]
    ...
    -> std.http.post
  avatica.columns
    @ (result: avatica_result) -> list[string]
    + returns the column names of the result in order
    # metadata
  avatica.prepare
    @ (conn: avatica_conn, sql: string) -> result[avatica_prepared, string]
  ...
  avatica.execute_prepared
    @ (prepared: avatica_prepared, params: list[string]) -> result[avatica_result, string]
  ...
  avatica.close_statement
    @ (stmt: avatica_stmt) -> result[void, string]

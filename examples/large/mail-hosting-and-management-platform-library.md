...
mail_platform
  ...
  mail_platform.add_domain
    fn (state: platform_state, domain: string) -> result[platform_state, string]
  ...
  mail_platform.create_mailbox
    fn (state: platform_state, address: string, password: string, quota_bytes: i64) -> result[platform_state, string]
  ...
  mail_platform.delete_mailbox
    fn (state: platform_state, address: string) -> result[platform_state, string]
    + removes a mailbox
    - returns error when the mailbox does not exist
    # mailboxes
  mail_platform.set_password
    fn (state: platform_state, address: string, password: string) -> result[platform_state, string]
    + replaces a mailbox password with a new bcrypt hash
    - returns error when the mailbox does not exist
    # mailboxes
    -> std.crypto.bcrypt_hash
  mail_platform.authenticate
    fn (state: platform_state, address: string, password: string) -> bool
    + returns true when the password verifies against the stored hash
    # mailboxes
    -> std.crypto.bcrypt_verify
  mail_platform.add_alias
    fn (state: platform_state, alias: string, target: string) -> result[platform_state, string]
  ...
  mail_platform.remove_alias
    fn (state: platform_state, alias: string) -> result[platform_state, string]
    + removes an alias
    - returns error when the alias does not exist
    # aliases
  mail_platform.record_usage
    fn (state: platform_state, address: string, bytes: i64) -> result[platform_state, string]
    + updates the mailbox's recorded storage usage
    - returns error when usage would exceed the quota
    # quota
    -> std.time.now_seconds
  mail_platform.list_mailboxes
    fn (state: platform_state, domain: string) -> list[string]

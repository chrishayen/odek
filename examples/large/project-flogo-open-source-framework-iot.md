...
flow_runtime
  ...
  flow_runtime.register_activity
    @ (rt: runtime_state, name: string, activity_id: string) -> result[runtime_state, string]
  ...
  flow_runtime.register_trigger
    @ (rt: runtime_state, name: string, trigger_id: string) -> result[runtime_state, string]
  ...
  flow_runtime.load_flow
    @ (rt: runtime_state, definition_json: string) -> result[tuple[string, runtime_state], string]
  ...
  flow_runtime.bind_trigger
    @ (rt: runtime_state, flow_id: string, trigger_name: string, config: map[string, string]) -> result[runtime_state, string]
    + attaches a configured trigger to a flow
    - returns error when flow_id or trigger_name is unknown
    # trigger
  flow_runtime.fire
    @ (rt: runtime_state, flow_id: string, input: map[string, string]) -> result[tuple[string, runtime_state], string]
    ...
    -> std.time.now_millis
  flow_runtime.step
    @ (rt: runtime_state, execution_id: string) -> result[tuple[execution_status, runtime_state], string]
  ...
  flow_runtime.deliver_activity_result
    @ (rt: runtime_state, execution_id: string, node_id: string, output: map[string, string]) -> result[runtime_state, string]
  ...
  flow_runtime.execution_status
    @ (rt: runtime_state, execution_id: string) -> optional[execution_status]
    + returns the current status of an execution
    # introspection
  flow_runtime.execution_output
    @ (rt: runtime_state, execution_id: string) -> optional[map[string, string]]
    + returns the final output of a completed execution
    # introspection
  flow_runtime.list_flows
    @ (rt: runtime_state) -> list[string]
    + returns the ids of every loaded flow
    # introspection
  flow_runtime.export_flow
    @ (rt: runtime_state, flow_id: string) -> result[string, string]

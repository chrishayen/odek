...
std
  ...
  std.http
    std.http.read_request
      fn (conn: tcp_conn) -> result[http_request, string]
      + reads an HTTP/1.1 request line and headers
      - returns error on malformed header
      # parsing
    std.http.write_response
      fn (conn: tcp_conn, status: i32, headers: map[string, string]) -> result[void, string]
      + writes a status line and header block
      # serialization

websocket
  websocket.client_handshake
    fn (host: string, port: i32, path: string) -> result[ws_conn, string]
  ...
  websocket.decode_frame
    fn (data: bytes) -> result[ws_frame, string]
  ...
  websocket.read_frame
    fn (conn: ws_conn) -> result[ws_frame, string]
    + reads a full frame from the connection
    - returns error on short read
    # io
    -> std.net.tcp_read
  websocket.write_frame
    fn (conn: ws_conn, frame: ws_frame) -> result[void, string]
    + writes an encoded frame to the connection
    # io
    -> std.net.tcp_write
  websocket.close
    fn (conn: ws_conn, code: u16, reason: string) -> result[void, string]

...
std
  ...
  std.zip
    std.zip.create
      ...
      # archive
    std.zip.add_entry
      fn (state: zip_state, name: string, data: bytes) -> zip_state
      + adds a named entry with uncompressed data
      # archive
    std.zip.read_entry
      fn (state: zip_state, name: string) -> result[bytes, string]
      + returns the bytes of a named entry
      - returns error when entry is missing
      # archive
    std.zip.finalize
      fn (state: zip_state) -> bytes
      + serializes the archive to a byte buffer
      # archive
    std.zip.open
      fn (raw: bytes) -> result[zip_state, string]
...
slides
  slides.new
    fn (title: string) -> deck_state
    + creates an empty deck with the given title
    # construction
  slides.add_slide
    fn (state: deck_state, layout: string) -> tuple[deck_state, i32]
    + appends a new slide with the given layout and returns (state, slide_index)
    ? layout names are "title", "title_content", "blank"
    # slides
  slides.set_title
    fn (state: deck_state, slide_index: i32, text: string) -> result[deck_state, string]
    + sets the title placeholder on a slide
    - returns error when slide_index is out of range
    # content
  slides.add_text_box
    fn (state: deck_state, slide_index: i32, text: string, x: i32, y: i32, width: i32, height: i32) -> result[deck_state, string]
    + adds a text box with absolute positioning in EMU units
    - returns error when slide_index is out of range
    # shapes
  slides.add_image
    fn (state: deck_state, slide_index: i32, image_path: string, x: i32, y: i32, width: i32, height: i32) -> result[deck_state, string]
    + embeds an image from disk on a slide
    - returns error when image_path cannot be read
    # shapes
    -> std.fs.read_all
  slides.add_bullet_list
    fn (state: deck_state, slide_index: i32, bullets: list[string]) -> result[deck_state, string]
    + adds a bulleted list to the content placeholder
    - returns error when slide_index is out of range
    # content
  slides.slide_count
    fn (state: deck_state) -> i32
    + returns the number of slides in the deck
    # introspection
  slides.save
    fn (state: deck_state, path: string) -> result[void, string]
    ...
    -> std.fs.write_all
  slides.load
    fn (path: string) -> result[deck_state, string]
    ...
    -> std.xml.parse
  slides.remove_slide
    fn (state: deck_state, slide_index: i32) -> result[deck_state, string]

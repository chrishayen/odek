...
std
  ...
  std.encoding
    std.encoding.encode_event
      @ (kind: string, payload: bytes) -> bytes
      + encodes an event kind and payload into bytes
      # serialization
    std.encoding.decode_event
      @ (raw: bytes) -> result[tuple[string, bytes], string]
...
durable
  ...
  durable.start_workflow
    @ (state: engine_state, name: string, input: bytes) -> tuple[string, engine_state]
  ...
  durable.schedule_step
    @ (state: engine_state, workflow_id: string, step_name: string, input: bytes) -> result[engine_state, string]
  ...
  durable.complete_step
    @ (state: engine_state, workflow_id: string, step_name: string, output: bytes) -> result[engine_state, string]
    ...
    -> std.time.now_millis
  durable.fail_step
    @ (state: engine_state, workflow_id: string, step_name: string, reason: string) -> result[engine_state, string]
    + records a STEP_FAILED event with the reason
    - returns error when no matching STEP_SCHEDULED event exists
    # step_completion
    -> std.encoding.encode_event
  durable.get_step_output
    @ (state: engine_state, workflow_id: string, step_name: string) -> optional[bytes]
  ...
  durable.complete_workflow
    @ (state: engine_state, workflow_id: string, result: bytes) -> result[engine_state, string]
    + records a WORKFLOW_COMPLETED event and marks the workflow terminal
    - returns error when the workflow id is unknown
    # workflow_lifecycle
    -> std.encoding.encode_event
  durable.history
    @ (state: engine_state, workflow_id: string) -> result[list[bytes], string]
    + returns the ordered event bytes for the workflow
    - returns error when the workflow id is unknown
    # persistence
  durable.replay
    @ (history: list[bytes]) -> result[engine_state, string]
    + rebuilds engine state from a recorded history
    - returns error when any event is corrupt
    # recovery
    -> std.encoding.decode_event
  durable.pending_steps
    @ (state: engine_state, workflow_id: string) -> result[list[string], string]

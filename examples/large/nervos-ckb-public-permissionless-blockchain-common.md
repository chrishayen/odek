...
ckb_core
  ckb_core.cell_new
    @ (capacity: u64, lock_hash: bytes, type_hash: optional[bytes], data: bytes) -> cell
  ...
  ckb_core.transaction_new
    @ (inputs: list[out_point], outputs: list[cell], witnesses: list[bytes]) -> transaction
  ...
  ckb_core.verify_scripts
    @ (tx: transaction, resolved_inputs: list[cell]) -> result[void, string]
  ...
  ckb_core.verify_capacity
    ...
    # validation
  ckb_core.block_new
    @ (parent_hash: bytes, number: u64, txs: list[transaction], nonce: u64) -> block
  ...
  ckb_core.mempool_submit
    @ (mempool: mempool_state, state: chain_state, tx: transaction) -> result[mempool_state, string]
  ...
  ckb_core.mempool_take
    @ (mempool: mempool_state, max: i32) -> tuple[list[transaction], mempool_state]

...
actor
  actor.spawn
    fn (initial_state: bytes, handler: fn(bytes, bytes) -> result[bytes, string]) -> actor_ref
    ...
    -> std.sync.channel
  actor.send
    fn (ref: actor_ref, message: bytes) -> result[void, string]
    + enqueues a message into the actor's mailbox
    - returns error when the actor has stopped
    # messaging
    -> std.sync.channel_send
  actor.ask
    fn (ref: actor_ref, message: bytes, timeout_ms: i64) -> result[bytes, string]
    ...
    -> std.async.sleep_ms
  actor.stop
    fn (ref: actor_ref) -> void
    + signals the actor to drain and terminate
    # lifecycle
  actor.supervise
    fn (parent: actor_ref, child: actor_ref, strategy: restart_strategy) -> void
    + links parent to child so parent is notified on child failure
    # supervision
  actor.on_failure
    fn (strategy: restart_strategy, child: actor_ref, error: string) -> supervisor_decision
    ...
    # supervision
  actor.restart
    fn (ref: actor_ref) -> result[void, string]

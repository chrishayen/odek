...
std
  ...
  std.http
    std.http.post_json
      fn (url: string, body: string) -> result[i32, string]
...
alerting
  ...
  alerting.register_channel
    fn (state: manager_state, name: string, kind: string, url: string) -> manager_state
    + registers a named notification channel with a sink kind and endpoint
    # channels
  alerting.add_rule
    fn (state: manager_state, name: string, script: string, interval_sec: i32, channels: list[string]) -> result[manager_state, string]
    ...
    # rule_registration
  alerting.tick
    fn (state: manager_state, now: i64) -> tuple[manager_state, list[string]]
    + runs any rule whose next_run <= now and returns the ids of fired alerts
    # scheduling
    -> std.script.eval
  alerting.evaluate_rule
    fn (state: manager_state, rule_name: string, inputs: map[string,string]) -> result[list[alert_event], string]
    ...
    -> std.script.eval
  alerting.dispatch
    fn (state: manager_state, alerts: list[alert_event]) -> list[dispatch_result]
    + sends each alert to every channel named on its rule and returns per-channel results
    # dispatch
    -> std.http.post_json
  alerting.list_rules
    fn (state: manager_state) -> list[string]
    + returns the names of all registered rules
    # introspection
  alerting.remove_rule
    fn (state: manager_state, name: string) -> manager_state
    + removes a rule by name; no-op if it does not exist
    # rule_management
  alerting.history
    fn (state: manager_state, rule_name: string, limit: i32) -> list[alert_event]
    + returns the most recent alerts fired by a rule, newest first
    # history
  alerting.ack
    fn (state: manager_state, alert_id: string) -> manager_state

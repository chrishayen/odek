...
auth
  auth.register
    fn (username: string, password: string) -> result[user_id, string]
    + hashes the password and creates a new user record
    - returns error when the username is already taken
    # account_management
    -> std.bcrypt.hash
  auth.login
    fn (username: string, password: string) -> result[session_token, string]
  ...
  auth.reset_password_confirm
    fn (reset_token: reset_token, new_password: string) -> result[void, string]

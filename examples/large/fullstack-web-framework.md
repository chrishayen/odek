...
webframework
  ...
  webframework.load_template
    fn (state: app_state, name: string) -> result[template, string]
  ...
  webframework.find_records
    fn (state: app_state, table: string, column: string, value: string) -> result[list[map[string, string]], string]
    + returns rows whose column equals value
    - returns error on sql failure
    # data
    -> std.sql.query
  webframework.insert_record
    fn (state: app_state, table: string, record: map[string, string]) -> result[i64, string]

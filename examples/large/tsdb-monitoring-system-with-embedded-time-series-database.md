...
tsdb
  ...
  tsdb.rate
    ...
    # query
  tsdb.scrape_target
    @ (db: tsdb_state, url: string) -> result[tsdb_state, string]
  ...
  tsdb.parse_exposition
    @ (body: string) -> list[tuple[string, map[string, string], f64]]
    + parses the text exposition format into metric samples
    + ignores comment and HELP lines
    # parsing
  tsdb.evaluate_alert
    @ (db: tsdb_state, metric: string, labels: map[string, string], threshold: f64, window_ms: i64) -> bool
    ...
    -> std.time.now_millis
  tsdb.series_list
    @ (db: tsdb_state) -> list[tuple[string, map[string, string]]]
    + returns all known series by (metric, labels)
    # introspection
  tsdb.delete_series
    @ (db: tsdb_state, metric: string, labels: map[string, string]) -> tsdb_state

...
std
  std.http
    std.http.get
      ...
      # http
    std.http.post_json
      @ (url: string, body: bytes, headers: map[string,string]) -> result[bytes, string]
  ...
  std.crypto
    ...
    std.crypto.generate_csr
      @ (key: bytes, domains: list[string]) -> result[bytes, string]
...
acme
  acme.new_client
    @ (directory_url: string) -> result[acme_client, string]
  ...
  acme.register_account
    @ (client: acme_client, contact_email: string) -> result[account_state, string]

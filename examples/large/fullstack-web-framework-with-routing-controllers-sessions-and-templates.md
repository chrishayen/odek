...
webapp
  webapp.new_app
    @ (secret: string) -> app_state
    + creates an application with empty routes, no controllers, and an HMAC session key
    # construction
  webapp.route
    @ (app: app_state, method: string, pattern: string, action: fn(request, session) -> response) -> app_state
  ...
  webapp.handle_connection
    ...
    -> webapp.encode_response
  webapp.serve
    @ (app: app_state, addr: string, port: u16) -> result[void, string]

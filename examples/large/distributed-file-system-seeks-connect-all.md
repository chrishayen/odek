...
std
  ...
  std.net
    std.net.p2p_connect
      @ (peer_addr: string) -> result[peer_conn, string]
      + dials a peer
      - returns error on failure to connect
      # networking
    std.net.p2p_send
      @ (conn: peer_conn, msg: bytes) -> result[void, string]
      + sends a framed message
      # networking
    std.net.p2p_recv
      @ (conn: peer_conn) -> result[bytes, string]
      + receives the next framed message
      # networking
    std.net.p2p_listen
      @ (addr: string, handler: peer_handler) -> result[server_handle, string]
...
dht_fs
  dht_fs.new_node
    @ (listen_addr: string, store: store_handle) -> result[node_state, string]
    + returns a node that serves and stores blocks
    # construction
  dht_fs.start
    @ (state: node_state) -> result[node_state, string]
    + begins listening for peer connections
    # lifecycle
    -> std.net.p2p_listen
  dht_fs.connect_peer
    @ (state: node_state, peer_addr: string) -> result[node_state, string]
  ...
  dht_fs.block_cid
    ...
    -> std.encoding.multibase_encode
  dht_fs.put_block
    @ (state: node_state, data: bytes) -> result[string, string]
    + stores a block locally and announces availability; returns its cid
    # blocks
    -> std.store.put
  dht_fs.get_block
    @ (state: node_state, cid: string) -> result[bytes, string]
    ...
    -> std.net.p2p_recv
  dht_fs.chunk_file
    @ (data: bytes, chunk_size: i32) -> list[bytes]
  ...
  dht_fs.add_file
    ...
    # files
  dht_fs.read_file
    @ (state: node_state, root_cid: string) -> result[bytes, string]
  ...
  dht_fs.add_directory
    @ (state: node_state, entries: map[string, string]) -> result[string, string]
    + writes a directory block mapping names to child cids; returns the directory cid
    # directories
  dht_fs.resolve_path
    @ (state: node_state, root_cid: string, path: list[string]) -> result[string, string]
    ...
    # directories
  dht_fs.publish_name
    @ (state: node_state, name: string, target_cid: string, signing_key: bytes) -> result[void, string]

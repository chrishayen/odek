...
ragkit
  ...
  ragkit.chunk_document
    fn (doc: document, chunk_size: u32, overlap: u32) -> list[chunk]
    ...
    -> std.text.split_whitespace
  ragkit.embed_chunk
    fn (chunk: chunk, model_url: string, api_key: string) -> result[chunk, string]
  ...
  ragkit.new_index
    ...
    # index
  ragkit.index_insert
    fn (index: vector_index, chunk: chunk) -> result[vector_index, string]
    + adds a chunk to the index
    - returns error when the chunk has no embedding
    # index
  ragkit.index_search
    fn (index: vector_index, query_vector: list[f32], top_k: u32) -> list[chunk]
    ...
    -> std.math.vector_norm
  ragkit.build_prompt
    fn (question: string, context: list[chunk]) -> string
    + assembles a prompt with the question and retrieved context
    # prompting
  ragkit.answer
    fn (index: vector_index, question: string, model_url: string, api_key: string) -> result[string, string]

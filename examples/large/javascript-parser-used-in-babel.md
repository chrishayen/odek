...
parser
  parser.tokenize
    @ (source: string) -> result[list[token], parse_error]
  ...
  parser.parse_program
    ...
    # parsing
  parser.walk
    @ (root: program_node, visit: fn(ast_node) -> void) -> void
    + invokes visit on every node in pre-order
    # traversal
  parser.source_range
    @ (n: ast_node) -> tuple[i32, i32]

...
webdriver
  webdriver.new_session
    @ (base_url: string, capabilities: map[string, string]) -> result[session, string]
  ...
  webdriver.find_element
    @ (sess: session, strategy: string, selector: string) -> result[element, string]
  ...
  webdriver.find_elements
    @ (sess: session, strategy: string, selector: string) -> result[list[element], string]
  ...
  webdriver.send_keys
    ...
    -> std.json.encode
  webdriver.get_text
    @ (sess: session, elem: element) -> result[string, string]
    + returns the visible text of the element
    # query
    -> std.http.request
    -> std.json.get_string
  webdriver.get_attribute
    @ (sess: session, elem: element, name: string) -> result[optional[string], string]
    + returns the attribute value, or none when the attribute is absent
    # query
    -> std.http.request
  webdriver.wait_until_visible
    @ (sess: session, strategy: string, selector: string, timeout_ms: i32) -> result[element, string]
    + polls find_element until the element is present and displayed
    - returns error when the timeout elapses
    # synchronization
  webdriver.screenshot
    @ (sess: session) -> result[bytes, string]

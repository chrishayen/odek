...
std
  std.net
    std.net.tcp_listen
      ...
      # networking
    std.net.tcp_accept
      fn (lst: listener) -> result[conn, string]
      + accepts the next connection
      # networking
    std.net.tcp_dial
      fn (addr: string) -> result[conn, string]
      + opens a tcp connection
      # networking
    std.net.read_frame
      fn (c: conn) -> result[bytes, string]
      + reads one length-prefixed frame
      # networking
    std.net.write_frame
      fn (c: conn, data: bytes) -> result[void, string]
...
rpcx
  ...
  rpcx.register_service
    fn (state: server_state, service_name: string, methods: map[string, handler_fn]) -> server_state
    ...
    # registration
  rpcx.set_codec
    fn (state: server_state, codec: codec) -> server_state
    + installs the codec used to decode requests and encode responses
    # pluggability
  rpcx.serve
    fn (state: server_state, addr: string) -> result[server_handle, string]
    ...
    -> std.net.write_frame
  rpcx.handle_frame
    fn (state: server_state, frame: bytes) -> bytes
    ...
    # dispatch
  rpcx.new_client
    fn (registry: registry_handle, codec: codec) -> client_state
    + returns a client that resolves service names through the given registry
    # client
  rpcx.call
    fn (state: client_state, service_name: string, method: string, request: bytes) -> result[bytes, string]
  ...
  rpcx.call_with_failover
    fn (state: client_state, service_name: string, method: string, request: bytes, attempts: i32) -> result[bytes, string]
  ...
  rpcx.register_endpoint
    fn (registry: registry_handle, service_name: string, endpoint: string) -> result[void, string]
  ...
  rpcx.resolve_endpoints
    fn (registry: registry_handle, service_name: string) -> result[list[string], string]

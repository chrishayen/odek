...
std
  ...
  std.net
    std.net.allreduce
      fn (local: list[f64]) -> result[list[f64], string]
...
gboost
  gboost.new_dataset
    fn (features: list[list[f32]], labels: list[f32]) -> result[dataset_handle, string]
  ...
  gboost.build_histograms
    fn (data: dataset_handle, max_bins: i32) -> dataset_handle
  ...
  gboost.logloss_gradient
    fn (pred: f64, label: f32) -> tuple[f64, f64]
    + returns (gradient, hessian) of binary log loss at the current prediction
    # objectives
    -> std.math.log
  gboost.find_best_split
    fn (data: dataset_handle, gradients: list[f64], hessians: list[f64], feature_idx: i32) -> split_info
    + scans histogram buckets to find the split that maximizes gain
    # tree_learning
  gboost.grow_tree
    fn (data: dataset_handle, gradients: list[f64], hessians: list[f64], max_depth: i32) -> tree_handle
    + grows one regression tree by recursively choosing best splits until max_depth
    # tree_learning
  gboost.predict_tree
    fn (tree: tree_handle, row: list[f32]) -> f64
    + returns the leaf value for a row by walking splits from the root
    # inference
  gboost.train
    fn (data: dataset_handle, num_rounds: i32, learning_rate: f64, max_depth: i32) -> result[model_handle, string]

...
geodb
  ...
  geodb.haversine_meters
    ...
    -> std.math.sqrt
  geodb.set_position
    fn (state: geodb_state, object_id: string, lat: f64, lon: f64) -> tuple[list[string], geodb_state]
    ...
    -> std.time.now_millis
  geodb.delete
    fn (state: geodb_state, object_id: string) -> geodb_state
    + removes the object and any membership in geofences
    ? no-op when the object is unknown
    # delete
  geodb.get_position
    fn (state: geodb_state, object_id: string) -> result[tuple[f64, f64], string]
    + returns the object's (lat, lon)
    - returns error when the object is unknown
    # read
  geodb.nearby
    fn (state: geodb_state, lat: f64, lon: f64, radius_meters: f64) -> list[string]
    + returns ids of objects within the given radius, sorted ascending by distance
    # radius_query
  geodb.add_geofence
    fn (state: geodb_state, fence_id: string, lat: f64, lon: f64, radius_meters: f64) -> geodb_state

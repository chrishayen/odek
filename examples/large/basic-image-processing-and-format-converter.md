...
std
  ...
  std.image
    std.image.decode_png
      fn (data: bytes) -> result[image_data, string]
    ...
    std.image.encode_jpeg
      fn (img: image_data, quality: i32) -> result[bytes, string]
...
imaging
  imaging.load
    fn (path: string) -> result[image_data, string]
    ...
    -> std.image.decode_jpeg
  imaging.save
    fn (img: image_data, path: string, format: string) -> result[void, string]

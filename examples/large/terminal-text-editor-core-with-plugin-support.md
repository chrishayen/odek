...
editor
  editor.new
    ...
    # construction
  editor.open_file
    @ (state: editor_state, path: string) -> result[editor_state, string]
    ...
    -> std.strings.split_lines
  editor.save_file
    @ (state: editor_state) -> result[void, string]
    ...
    -> std.fs.write_all
  editor.insert_text
    @ (state: editor_state, text: string) -> editor_state
    + inserts text at the cursor and advances the cursor
    # editing
  editor.delete_range
    @ (state: editor_state, start: cursor_pos, end: cursor_pos) -> editor_state
    + removes the text between start and end
    - leaves buffer unchanged when start >= end
    # editing
  editor.move_cursor
    @ (state: editor_state, pos: cursor_pos) -> editor_state
  ...
  editor.redo
    ...
    # history
  editor.get_line
    @ (state: editor_state, index: i32) -> result[string, string]
    + returns the line at the given zero-based index
    - returns error when index is out of range
    # inspection
  editor.line_count
    @ (state: editor_state) -> i32
    + returns the number of lines in the buffer
    # inspection
  editor.register_hook
    @ (state: editor_state, event: string, handler: fn(ctx: hook_context) -> editor_state) -> result[editor_state, string]
    + attaches a plugin callback to an editor event
    - returns error when event name is not recognized
    # plugins
  editor.fire_hook
    @ (state: editor_state, event: string, ctx: hook_context) -> editor_state

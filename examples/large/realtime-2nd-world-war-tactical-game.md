...
tactical_game
  tactical_game.new_world
    @ (map_width: i32, map_height: i32, seed: u64) -> world_state
    + creates an empty world with the given map dimensions and rng seed
    # construction
    -> std.rand.new_seeded
  tactical_game.spawn_unit
    @ (world: world_state, side: i32, kind: string, x: f64, y: f64) -> tuple[unit_id, world_state]
  ...
  tactical_game.issue_move_order
    @ (world: world_state, unit: unit_id, tx: f64, ty: f64) -> result[world_state, string]
  ...
  tactical_game.issue_fire_order
    @ (world: world_state, unit: unit_id, target: unit_id) -> result[world_state, string]
  ...
  tactical_game.line_of_sight
    @ (world: world_state, from: unit_id, to: unit_id) -> bool
    + returns true when no terrain obstacle blocks sight between the two units
    # perception
    -> std.math.distance2d
  tactical_game.tick
    @ (world: world_state, dt_millis: i64) -> world_state
  ...
  tactical_game.unit_view
    @ (world: world_state, unit: unit_id) -> optional[unit_snapshot]
  ...
  tactical_game.side_won
    @ (world: world_state) -> optional[i32]
  ...
  tactical_game.set_terrain
    @ (world: world_state, x: i32, y: i32, kind: string) -> world_state
    + sets the terrain kind at the given tile (open, forest, building, water)
    # terrain
  tactical_game.run_for
    @ (world: world_state, total_millis: i64, step_millis: i64) -> world_state

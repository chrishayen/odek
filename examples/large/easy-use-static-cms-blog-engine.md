...
blog
  ...
  blog.load_post
    ...
    -> std.time.parse_iso8601
  blog.load_all_posts
    @ (content_dir: string) -> result[list[post], string]
  ...
  blog.apply_template
    ...
    # templating
  blog.render_post
    @ (tmpl: template, post: post) -> result[string, string]
    + renders a single post using the given template
    - returns error when the template references unknown fields
    # rendering
  blog.render_index
    @ (tmpl: template, posts: list[post]) -> result[string, string]
    + renders a post index sorted by publish date descending
    # rendering
  blog.build_site
    @ (content_dir: string, template_dir: string, out_dir: string) -> result[i32, string]

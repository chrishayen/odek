...
engine
  engine.new_scene
    ...
    # construction
  engine.add_node
    fn (scene: scene, mesh: mesh_handle, shader: shader_handle, position: vec3) -> tuple[scene, node_id]
    + inserts a node and returns the updated scene and its id
    # scene_graph
  engine.set_node_position
    fn (scene: scene, id: node_id, position: vec3) -> result[scene, string]
    + moves a node to a new position
    - returns error when the id does not exist
    # scene_graph
  engine.new_camera
    fn (fov_rad: f32, aspect: f32, near: f32, far: f32) -> camera
    + creates a perspective camera
    # camera
    -> std.math.mat4_perspective
  engine.compute_mvp
    fn (cam: camera, node_pos: vec3) -> mat4
    ...
    -> std.math.mat4_multiply
  engine.render_frame
    fn (scene: scene, cam: camera, clear_color: vec3) -> void

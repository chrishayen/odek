...
bundler
  bundler.new_config
    fn (entry: string, output_dir: string) -> bundler_config
    + returns a config with the given entry and output directory
    # construction
  bundler.resolve_import
    fn (importer_path: string, spec: string) -> result[string, string]
  ...
  bundler.parse_imports
    ...
    # parsing
  bundler.load_module
    fn (path: string) -> result[bundler_module, string]
    + returns a module whose source, path, and import list are populated
    - returns error when the file cannot be read
    # loading
    -> std.fs.read_text
  bundler.build_graph
    fn (cfg: bundler_config) -> result[bundler_graph, string]
    ...
    # graph_construction
  bundler.topo_order
    fn (graph: bundler_graph) -> result[list[string], string]
    + returns module paths in dependency-first order
    - returns error when a cycle is detected
    # ordering
  bundler.emit_bundle
    fn (graph: bundler_graph, order: list[string]) -> string
  ...
  bundler.hash_asset
    ...
    -> std.path.extension
  bundler.copy_asset
    fn (src_path: string, output_dir: string) -> result[string, string]
    ...
    -> std.path.join
  bundler.build
    fn (cfg: bundler_config) -> result[bundler_output, string]

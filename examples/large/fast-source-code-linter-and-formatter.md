...
linter
  ...
  linter.parse
    ...
    # parsing
  linter.new_rule
    fn (name: string, check: fn(ast_node) -> list[diagnostic]) -> rule
    + creates a lint rule with the given name and checker function
    # rules
  linter.register_rule
    fn (state: linter_state, rule: rule) -> linter_state
    + adds a rule to the linter's active set
    # rules
  linter.disable_rule
    fn (state: linter_state, name: string) -> linter_state
    + removes the named rule from the active set; idempotent
    # rules
  linter.lint
    fn (state: linter_state, source: string) -> result[list[diagnostic], string]
  ...
  linter.format
    ...
    # formatting
  linter.fix
    fn (source: string, diagnostics: list[diagnostic]) -> result[string, string]

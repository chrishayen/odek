...
pbx_client
  pbx_client.connect
    fn (host: string, port: u16) -> result[pbx_conn, string]
    ...
    -> std.net.read_line
  pbx_client.login
    fn (c: pbx_conn, username: string, secret: string) -> result[pbx_conn, string]
  ...
  pbx_client.encode_message
    fn (fields: list[tuple[string, string]]) -> bytes
    + encodes action fields as "Key: Value" lines terminated by a blank line
    # protocol
  pbx_client.read_message
    fn (c: pbx_conn) -> result[list[tuple[string, string]], string]
  ...
  pbx_client.send_action
    fn (c: pbx_conn, action: string, params: list[tuple[string, string]]) -> result[list[tuple[string, string]], string]
  ...
  pbx_client.subscribe_events
    fn (c: pbx_conn, handler: fn(list[tuple[string, string]]) -> void) -> result[void, string]
    ...
    # event_loop
  pbx_client.logoff
    fn (c: pbx_conn) -> result[void, string]

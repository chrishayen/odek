...
std
  ...
  std.http
    std.http.download
      fn (url: string, dest: string, on_progress: fn(i64, i64) -> void) -> result[void, string]
...
llmhost
  llmhost.new_store
    fn (root_dir: string) -> result[model_store, string]
    + opens or creates the on-disk model store at the given directory
    # storage
    -> std.fs.ensure_dir
  llmhost.index_lookup
    fn (store: model_store, name: string) -> optional[model_descriptor]
    + returns the descriptor for an installed model or none if not present
    # storage
  llmhost.pull
    fn (store: model_store, name: string, source_url: string, expected_sha256: string) -> result[model_descriptor, string]
  ...
  llmhost.generate
    ...
    # generation
  llmhost.stream
    fn (session: session_state, max_new_tokens: i32, on_token: fn(string) -> bool) -> session_state
  ...
  llmhost.parse_manifest
    fn (raw: string) -> result[model_descriptor, string]

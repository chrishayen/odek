...
recipes
  recipes.parse_recipe
    @ (text: string) -> result[recipe, string]
    ...
    # recipe_parsing
  recipes.load_recipe
    @ (path: string) -> result[recipe, string]
    + reads and parses a recipe file
    # recipe_loading
    -> std.fs.read_all
    -> recipes.parse_recipe
  recipes.scale
    @ (r: recipe, factor: f64) -> recipe
  ...
  recipes.new_pantry
    ...
    # construction
  recipes.set_on_hand
    @ (p: pantry, ingredient: string, quantity: f64, unit: string) -> pantry
    + records or updates the quantity of an ingredient on hand
    # pantry
  recipes.shopping_list
    @ (selected: list[recipe], p: pantry) -> list[shopping_item]

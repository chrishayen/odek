...
std
  ...
  std.net
    std.net.rpc_call
      fn (peer: string, method: string, body: bytes) -> result[bytes, string]
...
tsdb
  tsdb.open
    fn (data_dir: string, shard_count: i32) -> result[tsdb_state, string]
    + opens or creates a database under data_dir with the given shard count
    - returns error when data_dir is not writable
    # construction
    -> std.fs.list_dir
  tsdb.shard_for
    fn (metric: string, shard_count: i32) -> i32
    + returns the shard index for a metric name via consistent hashing
    # routing
    -> std.hash.fnv64
  tsdb.write_point
    fn (state: tsdb_state, metric: string, tags: map[string,string], value: f64, ts_nanos: i64) -> result[void, string]
    ...
    -> std.fs.append_file
  tsdb.flush_segment
    fn (state: tsdb_state, shard: i32) -> result[void, string]
    + seals the active segment on a shard and starts a new one
    # storage
    -> std.fs.append_file
  tsdb.query_range
    fn (state: tsdb_state, metric: string, start_nanos: i64, end_nanos: i64) -> result[list[tuple[i64,f64]], string]

...
std
  std.io
    ...
    std.io.bit_writer_write
      @ (state: bit_writer_state, value: u64, count: i32) -> bit_writer_state
      + appends the low count bits of value
      # io
    std.io.bit_writer_finish
      @ (state: bit_writer_state) -> bytes
...
flac
  ...
  flac.decode_metadata_blocks
    @ (data: bytes) -> result[tuple[list[metadata_block], i32], string]
  ...
  flac.decode_frame_header
    @ (reader: bit_reader_state) -> result[tuple[flac_frame_header, bit_reader_state], string]
  ...
  flac.decode_subframe_fixed
    @ (reader: bit_reader_state, order: i32, block_size: i32, bps: i32) -> result[tuple[list[i32], bit_reader_state], string]
  ...
  flac.decode_subframe_lpc
    ...
    -> std.io.bit_reader_read
  flac.decode_frame
    @ (reader: bit_reader_state) -> result[tuple[audio_block, bit_reader_state], string]
  ...
  flac.stream_reader_new
    @ (data: bytes) -> result[stream_reader, string]
  ...
  flac.stream_read_next
    @ (reader: stream_reader) -> result[tuple[audio_block, stream_reader], string]
    + decodes and returns the next frame
    - returns error at end of stream
    # streaming
  flac.encode_frame
    @ (samples: list[i32], sample_rate: i32, channels: i32, bps: i32) -> bytes
    ...
    -> std.hash.crc16
  flac.encode_stream
    @ (blocks: list[audio_block], sample_rate: i32, channels: i32, bps: i32) -> bytes

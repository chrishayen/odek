...
container
  container.new_runtime
    @ (root: string) -> runtime_state
    + creates a runtime rooted at a storage directory
    # construction
    -> std.fs.make_dir
  container.add_image
    @ (state: runtime_state, reference: string, layers: list[bytes], config: string) -> result[runtime_state, string]
  ...
  container.lookup_image
    @ (state: runtime_state, reference: string) -> result[image_record, string]
    + returns an image record by reference
    - returns error when the reference is unknown
    # image_store
  container.create_container
    @ (state: runtime_state, image_ref: string, name: string, command: list[string]) -> result[tuple[string, runtime_state], string]
    ...
    -> std.fs.make_dir
  container.start
    @ (state: runtime_state, id: string) -> result[runtime_state, string]
  ...
  container.remove
    ...
    # lifecycle
  container.list
    @ (state: runtime_state) -> list[container_record]
    + returns a snapshot of all known containers
    # inventory
  container.create_network
    @ (state: runtime_state, name: string, cidr: string) -> result[runtime_state, string]
    + registers a named network with an address range
    - returns error when the CIDR is malformed
    # networking
  container.attach_network
    @ (state: runtime_state, container_id: string, network_name: string) -> result[tuple[string, runtime_state], string]
  ...
  container.persist_state
    @ (state: runtime_state) -> result[void, string]
    + writes the runtime's metadata to disk
    # persistence
    -> std.encoding.json_encode_any
    -> std.fs.write_all
  container.load_state
    @ (root: string) -> result[runtime_state, string]

...
browser_agent
  browser_agent.new_session
    @ (headless: bool) -> result[session_state, string]
  ...
  browser_agent.snapshot
    @ (state: session_state) -> page_snapshot
  ...
  browser_agent.describe_element
    @ (snap: page_snapshot, element_id: string) -> result[element_info, string]
    + returns role, text, and attributes for the element
    - returns error when the id is unknown
    # observation
  browser_agent.click
    @ (state: session_state, element_id: string) -> result[session_state, string]
  ...
  browser_agent.type_text
    @ (state: session_state, element_id: string, text: string) -> result[session_state, string]

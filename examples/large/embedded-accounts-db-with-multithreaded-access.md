...
std
  ...
  std.sync
    std.sync.new_rwlock
      ...
      # concurrency
    std.sync.read_lock
      fn (lock: rwlock_state) -> rwlock_state
...
accounts_db
  accounts_db.open
    fn (data_path: string, wal_path: string) -> result[db_state, string]
  ...
  accounts_db.create_account
    fn (db: db_state, account_id: string, initial_balance: i64) -> result[db_state, string]
  ...
  accounts_db.get_balance
    fn (db: db_state, account_id: string) -> result[i64, string]
  ...
  accounts_db.tx_transfer
    fn (tx: tx_state, from_id: string, to_id: string, amount: i64) -> result[tx_state, string]
    ...
    # transactions
  accounts_db.commit
    fn (db: db_state, tx: tx_state) -> result[db_state, string]
  ...
  accounts_db.checkpoint
    fn (db: db_state) -> result[db_state, string]

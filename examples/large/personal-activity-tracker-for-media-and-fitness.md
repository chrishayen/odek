...
activity_tracker
  ...
  activity_tracker.register_user
    fn (state: tracker_state, username: string, email: string) -> result[tuple[tracker_state, user_id], string]
    + creates a new user and returns their id
    - returns error when the username already exists
    # users
    -> std.hash.sha256_hex
  activity_tracker.lookup_user
    fn (state: tracker_state, username: string) -> optional[user_id]
    + returns the user id for the given username
    - returns none when no user matches
    # users
  activity_tracker.log_media
    fn (state: tracker_state, user: user_id, kind: string, title: string, progress: i32, total: i32) -> result[tuple[tracker_state, entry_id], string]
  ...
  activity_tracker.log_workout
    fn (state: tracker_state, user: user_id, name: string, duration_sec: i32, sets: list[workout_set]) -> result[tuple[tracker_state, entry_id], string]
    + records a workout session with an ordered list of sets
    - returns error when duration is negative
    # fitness
    -> std.time.now_seconds
  activity_tracker.log_meal
    fn (state: tracker_state, user: user_id, description: string, calories: i32) -> result[tuple[tracker_state, entry_id], string]
    + records a meal with a calorie estimate
    - returns error when calories is negative
    # nutrition
    -> std.time.now_seconds
  activity_tracker.rate_entry
    fn (state: tracker_state, user: user_id, entry: entry_id, stars: i32) -> result[tracker_state, string]
  ...
  activity_tracker.entries_for
    fn (state: tracker_state, user: user_id, kind: string) -> list[entry_summary]
  ...
  activity_tracker.daily_summary
    fn (state: tracker_state, user: user_id, day_unix: i64) -> daily_summary
    + aggregates workouts, meals, and media for a given calendar day
    # reports
  activity_tracker.streak
    fn (state: tracker_state, user: user_id, kind: string) -> i32
  ...
  activity_tracker.export_entries
    fn (state: tracker_state, user: user_id) -> list[entry_summary]

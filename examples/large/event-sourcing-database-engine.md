...
eventdb
  eventdb.open
    @ (log_path: string, snapshot_path: string) -> result[db_state, string]
    + opens or creates the log and snapshot files
    - returns error when paths are inaccessible
    -> std.fs.file_size
    # construction
  eventdb.append
    @ (state: db_state, stream: string, payload: bytes) -> result[tuple[db_state, i64], string]
    ...
    # ingestion
  eventdb.read_from
    @ (state: db_state, from_seq: i64, limit: i32) -> result[list[event_record], string]
  ...
  eventdb.snapshot_write
    @ (state: db_state, stream: string, up_to_seq: i64, data: bytes) -> result[db_state, string]
    + stores a snapshot of a stream at the given sequence number
    -> std.fs.append_bytes
    -> std.fs.fsync
    # snapshot
  eventdb.snapshot_read
    @ (state: db_state, stream: string) -> result[snapshot_entry, string]
    + returns the most recent snapshot for the stream
    - returns error when no snapshot exists
    -> std.fs.read_range
    # snapshot
  eventdb.replay
    @ (state: db_state, stream: string, from_seq: i64, fold_tag: string, initial: bytes) -> result[bytes, string]
    + folds events from the given sequence using a named folding function
    - returns error when read fails
    # projection
  eventdb.compact
    @ (state: db_state, before_seq: i64) -> result[db_state, string]
  ...
  eventdb.event_record
    @ (seq: i64, stream: string, payload: bytes, timestamp_millis: i64) -> event_record

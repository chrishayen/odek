...
std
  std.bencode
    std.bencode.parse
      fn (raw: bytes) -> result[bencode_value, string]
      + parses a bencoded byte string
      - returns error on malformed input
      # serialization
    std.bencode.encode
      fn (value: bencode_value) -> bytes
  ...
  std.net
    std.net.tcp_connect
      fn (host: string, port: i32) -> result[tcp_handle, string]
      + opens a TCP connection
      - returns error on failure
      # networking
    std.net.tcp_read
      fn (handle: tcp_handle, n: i32) -> result[bytes, string]
      + reads up to n bytes
      # networking
    std.net.tcp_write
      fn (handle: tcp_handle, data: bytes) -> result[void, string]
      + writes all bytes
      # networking
    std.net.tcp_close
      fn (handle: tcp_handle) -> result[void, string]
  ...
  std.fs
    std.fs.write_at
      ...
      # filesystem
    std.fs.read_at
      fn (path: string, offset: i64, n: i64) -> result[bytes, string]
...
torrent
  torrent.parse_metainfo
    fn (raw: bytes) -> result[metainfo, string]
    ...
    -> std.crypto.sha1
  torrent.announce
    fn (info: metainfo, peer_id: bytes, port: i32, uploaded: i64, downloaded: i64, left: i64) -> result[tracker_response, string]
    ...
    -> std.bencode.parse
  torrent.handshake
    fn (handle: tcp_handle, info_hash: bytes, peer_id: bytes) -> result[peer_handshake, string]

...
rpc
  rpc.new_client
    @ (conn: connection, codec: codec) -> rpc_client
    + builds a client bound to a connection with a pluggable codec
    # construction
  rpc.call
    @ (client: rpc_client, method: string, payload: bytes) -> result[bytes, rpc_error]
  ...
  rpc.new_server
    ...
    # construction
  rpc.register_method
    @ (server: rpc_server, method: string, handler: fn(bytes) -> result[bytes, string]) -> rpc_server
    + binds a handler function to a method name
    # registration
  rpc.serve_connection
    @ (server: rpc_server, conn: connection) -> result[void, string]
  ...
  rpc.decode_request
    ...
    -> std.encoding.varint_decode
  rpc.encode_response
    @ (payload: bytes, err: optional[string]) -> bytes
    + produces a response frame, tagging success or error
    # framing
  rpc.decode_response
    @ (frame: bytes) -> result[bytes, rpc_error]

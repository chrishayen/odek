...
std
  ...
  std.time
    std.time.parse_duration
      @ (input: string) -> result[i64, string]
...
file_query
  ...
  file_query.format_row
    @ (row: query_row) -> string
    + renders a row as a tab-separated line for display
    # rendering
  file_query.execute
    @ (query: string) -> result[list[query_row], string]

...
ydb_driver
  ...
  ydb_driver.execute
    ...
    -> std.net.read
  ydb_driver.begin_tx
    @ (state: driver_state, session: session_handle, mode: string) -> result[tx_handle, string]
  ...
  ydb_driver.commit_tx
    @ (state: driver_state, tx: tx_handle) -> result[void, string]
  ...
  ydb_driver.rollback_tx
    ...
    -> std.net.read
  ydb_driver.sql_open
    @ (endpoints: list[string], database: string) -> result[sql_conn, string]
    + returns a generic SQL adapter on top of the native driver
    # sql_adapter
  ydb_driver.sql_query
    @ (conn: sql_conn, query: string, args: list[typed_value]) -> result[sql_rows, string]

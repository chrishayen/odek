...
site_builder
  site_builder.new_document
    @ (source_path: string, raw: bytes) -> document
    + constructs a document with empty metadata and raw content
    # model
  site_builder.register_plugin
    @ (registry: plugin_registry, name: string, hook: plugin_hook) -> plugin_registry
  ...
  site_builder.run_load_hooks
    @ (registry: plugin_registry, doc: document) -> result[document, string]
  ...
  site_builder.run_render_hooks
    @ (registry: plugin_registry, doc: document) -> result[string, string]
  ...
  site_builder.run_write_hooks
    @ (registry: plugin_registry, doc: document, html: string, out_root: string) -> result[string, string]
  ...
  site_builder.build_one
    @ (registry: plugin_registry, source_path: string, out_root: string) -> result[string, string]
    + runs load, transform, render, and write hooks for one source file
    - returns error when any stage fails
    # build_one
    -> std.fs.read_all
  site_builder.build_all
    @ (registry: plugin_registry, source_root: string, out_root: string) -> result[i32, string]

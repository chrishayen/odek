...
agent
  agent.new
    fn (name: string, system_prompt: string, model_fn: model_call_fn) -> agent_state
    + creates an agent bound to an injected model call function
    # construction
  agent.tool_register
    fn (state: agent_state, name: string, description: string, schema: string, run_fn: fn(string) -> result[string, string]) -> result[agent_state, string]
  ...
  agent.long_term_recall
    fn (state: agent_state, key: string) -> optional[string]
    + returns a previously stored long-term value
    - returns none when the key is not present
    # memory
  agent.step
    fn (state: agent_state, user_input: string) -> result[step_result, string]
    ...
    -> agent.memory_append
  agent.run_until
    fn (state: agent_state, user_input: string, stop_fn: fn(step_result) -> bool, max_steps: i32) -> result[step_result, string]
    ...
    -> agent.step
  agent.reflect
    fn (state: agent_state) -> result[agent_state, string]
    ...
    -> agent.memory_snapshot
  agent.learn_skill
    fn (state: agent_state, name: string, description: string, steps: list[string]) -> agent_state

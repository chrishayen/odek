...
parser_kit
  ...
  parser_kit.map_result
    @ (p: parser, f: fn(parse_result) -> parse_result) -> parser
    + transforms the value produced by p without altering matching behavior
    # combinators
  parser_kit.run
    @ (p: parser, input: string) -> result[parse_result, parse_error]

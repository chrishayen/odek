...
std
  std.crypto
    std.crypto.random_bytes
      ...
      # cryptography
    std.crypto.scrypt
      @ (password: bytes, salt: bytes, n: i32, r: i32, p: i32, length: i32) -> result[bytes, string]
...
password_vault
  password_vault.hash
    @ (password: string, pepper: bytes) -> result[string, string]
  ...
  password_vault.verify
    @ (password: string, pepper: bytes, envelope: string) -> result[bool, string]
  ...
  password_vault.encrypt_secret
    @ (password: string, plaintext: bytes) -> result[bytes, string]
  ...
  password_vault.decrypt_secret
    @ (password: string, envelope: bytes) -> result[bytes, string]

...
cache
  cache.new
    fn (shard_count: i32, max_entries_per_shard: i32) -> cache_state
    + creates a sharded cache with the given shard count and per-shard capacity
    ? shard_count is rounded up to a power of two
    # construction
  cache.set
    fn (state: cache_state, key: string, value: bytes, ttl_millis: i64) -> cache_state
  ...
  cache.attach_peer_sink
    fn (state: cache_state, sink: fn(cache_event) -> void) -> cache_state
  ...
  cache.apply_peer_event
    fn (state: cache_state, event: cache_event) -> cache_state

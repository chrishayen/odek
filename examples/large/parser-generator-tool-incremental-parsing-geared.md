...
treeparse
  ...
  treeparse.grammar_add_rule
    @ (g: grammar, name: string, body: rule_body) -> grammar
  ...
  treeparse.parser_new
    @ (table: parse_table) -> parser
    + constructs a parser bound to a compiled table
    # parsing
  treeparse.parse
    @ (p: parser, source: string) -> result[syntax_tree, parse_error]
  ...
  treeparse.reparse_incremental
    @ (p: parser, old_tree: syntax_tree, edits: list[text_edit], new_source: string) -> result[syntax_tree, parse_error]

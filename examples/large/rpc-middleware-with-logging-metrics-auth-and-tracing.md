...
rpckit
  rpckit.new_logger
    ...
    # logging
  rpckit.log_call
    fn (logger: logger_state, method: string, status: string, duration_ms: i64) -> void
    + emits a structured log line describing a completed call
    # logging
    -> std.io.print_string
  rpckit.logger_unary
    fn (logger: logger_state, handler: unary_handler_fn) -> unary_handler_fn
    + wraps a unary handler with start and end logging
    # composition
    -> std.time.now_millis
    -> rpckit.log_call
  rpckit.logger_stream
    fn (logger: logger_state, handler: stream_handler_fn) -> stream_handler_fn
  ...
  rpckit.new_metrics
    ...
    # metrics
  rpckit.record_call
    fn (metrics: metrics_state, method: string, status: string, duration_ms: i64) -> metrics_state
    + records a single call observation
    # metrics
  rpckit.metrics_unary
    fn (metrics: metrics_state, handler: unary_handler_fn) -> unary_handler_fn
  ...
  rpckit.verify_token
    ...
    -> std.crypto.hmac_sha256
  rpckit.auth_unary
    fn (secret: bytes, handler: unary_handler_fn) -> unary_handler_fn
  ...
  rpckit.tracing_unary
    fn (tracer: tracer_state, handler: unary_handler_fn) -> unary_handler_fn

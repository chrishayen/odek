...
std
  std.tensor
    std.tensor.zeros
      fn (shape: list[i32]) -> tensor_state
      + returns a dense tensor of the given shape filled with zero
      # tensor
    std.tensor.matmul
      fn (a: tensor_state, b: tensor_state) -> result[tensor_state, string]
...
dl_framework
  ...
  dl_framework.add_dense
    fn (model: model_state, in_dim: i32, out_dim: i32) -> model_state
    + appends a dense layer with random initialization
    # architecture
  dl_framework.add_relu
    fn (model: model_state) -> model_state
    + appends a ReLU activation layer
    # architecture
    -> std.tensor.relu
  dl_framework.forward
    fn (model: model_state, input: tensor_state) -> result[tensor_state, string]
  ...
  dl_framework.train_step
    fn (model: model_state, batch: tensor_state, labels: list[i32], learning_rate: f64) -> result[f64, string]
  ...
  dl_framework.evaluate
    fn (model: model_state, batch: tensor_state, labels: list[i32]) -> result[f64, string]
  ...
  dl_framework.add_example
    fn (dataset: dataset_state, features: list[f64], label: i32) -> dataset_state
    + appends one (features, label) example
    # dataset
  dl_framework.iterate_batches
    fn (dataset: dataset_state, batch_size: i32) -> list[tuple[tensor_state, list[i32]]]

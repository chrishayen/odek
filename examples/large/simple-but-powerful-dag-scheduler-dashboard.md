...
std
  ...
  std.concurrency
    std.concurrency.run_parallel
      @ (fns: list[task_fn], max_parallel: i32) -> list[result[string, string]]
...
dag_scheduler
  ...
  dag_scheduler.add_task
    @ (dag: dag_state, id: string, fn: task_fn) -> result[dag_state, string]
    + registers a task with the given id and function
    - returns error when id already exists
    # construction
  dag_scheduler.add_dependency
    @ (dag: dag_state, from_id: string, to_id: string) -> result[dag_state, string]
  ...
  dag_scheduler.ready_tasks
    @ (dag: dag_state) -> list[string]
    + returns ids whose dependencies are all marked completed
    # scheduling
  dag_scheduler.run
    @ (dag: dag_state, max_parallel: i32) -> run_report
  ...
  dag_scheduler.task_status
    @ (report: run_report, id: string) -> optional[task_status]
    + returns the recorded status for a task id or none
    # reporting
  dag_scheduler.summary
    @ (report: run_report) -> run_summary

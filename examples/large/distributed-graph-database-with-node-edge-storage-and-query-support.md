...
std
  ...
  std.kv
    std.kv.get
      fn (store: kv_store, key: bytes) -> optional[bytes]
    ...
    std.kv.delete
      ...
      # storage
    std.kv.scan_prefix
      fn (store: kv_store, prefix: bytes) -> list[tuple[bytes,bytes]]
...
graphdb
  graphdb.node_id_new
    ...
    # identity
  graphdb.edge_key
    fn (from: node_id, label: string, to: node_id) -> bytes
    + returns the canonical storage key for an edge
    # identity
  graphdb.new
    fn (peers: list[string], self_peer: string, local_store: kv_store) -> graphdb_state
    + constructs a graph database node bound to a peer ring
    # construction
  graphdb.route_for
    fn (state: graphdb_state, id: node_id) -> string
    + returns the peer responsible for the given node id
    # routing
    -> std.hash.fnv64
  graphdb.put_node_local
    fn (state: graphdb_state, id: node_id, props: map[string,string]) -> result[void, string]
    + writes a node into the local store
    # local_write
    -> std.serde.encode_record
    -> std.kv.put
  graphdb.get_node_local
    fn (state: graphdb_state, id: node_id) -> optional[map[string,string]]
    + reads a node from the local store
    # local_read
    -> std.kv.get
    -> std.serde.decode_record
  graphdb.put_edge_local
    fn (state: graphdb_state, from: node_id, label: string, to: node_id, props: map[string,string]) -> result[void, string]
    + writes an edge keyed by (from, label, to) locally
    # local_write
    -> std.kv.put
    -> std.serde.encode_record
  graphdb.neighbors_local
    fn (state: graphdb_state, id: node_id, label: string) -> list[node_id]
  ...
  graphdb.put_node
    ...
    -> std.net.rpc_call
  graphdb.get_node
    fn (state: graphdb_state, id: node_id) -> result[optional[map[string,string]], string]
    + routes to the owning peer and reads
    - returns error when the owning peer is unreachable
    # routed_read
    -> std.net.rpc_call
  graphdb.neighbors
    fn (state: graphdb_state, id: node_id, label: string) -> result[list[node_id], string]
    + routes to the owning peer and returns its neighbors list
    - returns error when the owning peer is unreachable
    # routed_read
    -> std.net.rpc_call
  graphdb.bfs
    fn (state: graphdb_state, start: node_id, label: string, max_depth: i32) -> result[list[node_id], string]
    ...
    # traversal
  graphdb.delete_node
    fn (state: graphdb_state, id: node_id) -> result[void, string]

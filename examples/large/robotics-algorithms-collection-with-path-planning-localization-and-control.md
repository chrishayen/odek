...
robotics
  robotics.grid_new
    @ (width: i32, height: i32) -> grid_map
    + creates an empty occupancy grid with all cells free
    # map
  robotics.grid_set_obstacle
    @ (grid: grid_map, x: i32, y: i32) -> grid_map
    + marks a cell as blocked
    # map
  robotics.astar
    @ (grid: grid_map, start_xy: tuple[i32, i32], goal_xy: tuple[i32, i32]) -> result[list[tuple[i32, i32]], string]
    + returns a path from start to goal using 8-connected A* with Euclidean heuristic
    - returns error when no path exists
    # path_planning
    -> std.math.hypot
  robotics.rrt
    @ (grid: grid_map, start_xy: tuple[f64, f64], goal_xy: tuple[f64, f64], max_iters: i32) -> result[list[tuple[f64, f64]], string]

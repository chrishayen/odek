...
std
  ...
  std.sampling
    std.sampling.capture_stack
      @ (thread_id: i64) -> result[list[i64], string]
    ...
    std.sampling.resolve_symbol
      @ (address: i64) -> optional[symbol_info]
...
block_profiler
  ...
  block_profiler.aggregate
    @ (samples: list[stack_sample]) -> list[task_blocking_report]
  ...
  block_profiler.rank_by_blocked_time
    @ (reports: list[task_blocking_report]) -> list[task_blocking_report]
    + returns reports sorted from most to least blocked time
    # ranking
  block_profiler.top_blocking_stacks
    @ (samples: list[stack_sample], n: i32) -> list[stack_frequency]
  ...
  block_profiler.format_report
    @ (reports: list[task_blocking_report], hotspots: list[stack_frequency]) -> string
    + renders a human-readable profiling summary
    # rendering
  block_profiler.load_symbol_map
    @ (path: string) -> result[symbol_map, string]

...
std
  ...
  std.poll
    std.poll.new
      ...
      # poll
    std.poll.register
      fn (p: poller, fd: i32, interest: i32) -> result[void, string]
      + adds a file descriptor with a read/write interest mask
      # poll
    std.poll.deregister
      fn (p: poller, fd: i32) -> result[void, string]
      + removes a file descriptor
      # poll
    std.poll.wait
      fn (p: poller, timeout_ms: i32) -> list[tuple[i32,i32]]
...
coio
  ...
  coio.run_until_idle
    ...
    # loop
  coio.join
    fn (state: coio_state, tid: task_id) -> result[void, string]

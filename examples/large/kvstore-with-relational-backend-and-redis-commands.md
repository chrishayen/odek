...
std
  std.sql
    std.sql.open
      ...
      # database
    std.sql.exec
      fn (db: db_handle, query: string, args: list[sql_value]) -> result[i64, string]
      + executes a statement and returns rows affected
      - returns error on syntax or constraint failure
      # database
    std.sql.query_rows
      fn (db: db_handle, query: string, args: list[sql_value]) -> result[list[sql_row], string]
...
kvstore
  kvstore.open
    fn (db_path: string) -> result[store_state, string]
    ...
    -> std.sql.exec
  kvstore.set_string
    fn (s: store_state, key: string, value: string, ttl_ms: i64) -> result[void, string]
    + stores a string value for key with an optional ttl (0 for no expiry)
    # strings
    -> std.sql.exec
    -> std.time.now_millis
  kvstore.get_string
    fn (s: store_state, key: string) -> result[optional[string], string]
    + returns the string value or none when absent or expired
    # strings
    -> std.sql.query_rows
    -> std.time.now_millis
  kvstore.delete
    fn (s: store_state, key: string) -> result[i64, string]
    + removes the key and returns the number of keys deleted
    # keyspace
    -> std.sql.exec
  kvstore.exists
    fn (s: store_state, key: string) -> result[bool, string]
    + returns true when the key exists and is not expired
    # keyspace
  kvstore.list_push
    fn (s: store_state, key: string, value: string, left: bool) -> result[i64, string]
    + pushes value onto the left or right of a list and returns the new length
    # lists
    -> std.sql.exec
  kvstore.list_pop
    fn (s: store_state, key: string, left: bool) -> result[optional[string], string]
    + pops and returns an element from the list, or none when empty
    # lists
    -> std.sql.exec
  kvstore.hash_set
    fn (s: store_state, key: string, field: string, value: string) -> result[void, string]
    + sets a field in the hash
    # hashes
    -> std.sql.exec
  kvstore.hash_get
    fn (s: store_state, key: string, field: string) -> result[optional[string], string]
    + returns the field value or none when absent
    # hashes
    -> std.sql.query_rows
  kvstore.set_add
    fn (s: store_state, key: string, member: string) -> result[bool, string]
    + adds a member to the set and returns true when newly added
    # sets
    -> std.sql.exec
  kvstore.set_members
    fn (s: store_state, key: string) -> result[list[string], string]
    + returns all members of the set
    # sets
    -> std.sql.query_rows
  kvstore.expire_sweep
    fn (s: store_state) -> result[i64, string]
    + removes all expired keys and returns the count
    # ttl
    -> std.sql.exec
    -> std.time.now_millis
  kvstore.close
    fn (s: store_state) -> result[void, string]

...
std
  std.encoding
    ...
    std.encoding.varint_decode
      fn (data: bytes) -> result[tuple[i32, i32], string]
...
pubsub_broker
  ...
  pubsub_broker.decode_packet
    fn (data: bytes) -> result[packet, string]
  ...
  pubsub_broker.connect_client
    fn (state: broker_state, client_id: string, clean: bool) -> tuple[broker_state, bool]
  ...
  pubsub_broker.subscribe
    fn (state: broker_state, client_id: string, filter: string, qos: i32) -> result[broker_state, string]
  ...
  pubsub_broker.route_publish
    fn (state: broker_state, topic: string, qos: i32) -> list[delivery]
    + returns one delivery per matching subscription with the effective QoS
    # routing
  pubsub_broker.publish
    fn (state: broker_state, topic: string, payload: bytes, qos: i32, retain: bool) -> tuple[broker_state, list[delivery]]
  ...
  pubsub_broker.retained_for_filter
    fn (state: broker_state, filter: string) -> list[retained]
    + returns retained messages whose topics match the filter
    # retained
  pubsub_broker.assign_packet_id
    fn (state: broker_state, client_id: string) -> tuple[broker_state, i32]
    + returns a fresh packet id for the client and the updated state
    # sessions
  pubsub_broker.acknowledge
    fn (state: broker_state, client_id: string, packet_id: i32) -> broker_state

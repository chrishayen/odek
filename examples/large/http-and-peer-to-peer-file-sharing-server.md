...
std
  ...
  std.http
    ...
    std.http.response_bytes
      fn (status: i32, body: bytes, content_type: string) -> http_response
...
share_server
  share_server.add_file
    fn (path: string, piece_size: i32) -> result[file_manifest, string]
  ...
  share_server.info_hash
    fn (manifest: file_manifest) -> bytes
  ...
  share_server.register
    fn (reg: share_registry, manifest: file_manifest, path: string) -> void
    + records a file as available to both the HTTP and peer protocols
    # registration
  share_server.serve_http
    fn (reg: share_registry, addr: string) -> result[void, string]
  ...
  share_server.handle_peer_handshake
    fn (conn: tcp_conn, reg: share_registry) -> result[peer_session, string]
  ...
  share_server.handle_peer_request
    fn (session: peer_session, piece_index: i32) -> result[bytes, string]
  ...
  share_server.serve_peers
    ...
    -> std.net.accept_tcp
  share_server.start
    fn (reg: share_registry, http_addr: string, peer_addr: string) -> result[void, string]

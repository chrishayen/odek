...
std
  std.sql
    ...
    std.sql.exec
      ...
      # storage
    std.sql.query_rows
      @ (conn: sql_conn, query: string, args: list[string]) -> result[list[map[string,string]], string]
...
task_queue
  task_queue.open
    @ (path: string) -> result[queue_state, string]
    ...
    -> std.sql.exec
  task_queue.register
    @ (state: queue_state, kind: string, schema_keys: list[string]) -> queue_state
    ...
    # registration
  task_queue.enqueue
    @ (state: queue_state, kind: string, payload: map[string,string], run_at_ms: i64) -> result[string, string]
  ...
  task_queue.claim_next
    @ (state: queue_state, worker_id: string) -> result[optional[claimed_job], string]
  ...
  task_queue.mark_done
    @ (state: queue_state, job_id: string) -> result[void, string]
    + marks a claimed job as completed
    - returns error when the job id does not exist
    # completion
    -> std.sql.exec
  task_queue.mark_failed
    @ (state: queue_state, job_id: string, reason: string, retry_in_ms: i64) -> result[void, string]
  ...
  task_queue.run_worker_once
    @ (state: queue_state, worker_id: string, handler: fn(string, map[string,string]) -> result[void,string]) -> result[bool, string]
    + claims one ready job, invokes the handler, and records success or failure
    + returns false when no job was ready
    # worker_loop
    -> std.time.now_millis
  task_queue.stats
    @ (state: queue_state) -> result[map[string,i64], string]
    + returns counts by status (pending, running, done, dead)
    # introspection
    -> std.sql.query_rows
  task_queue.close
    @ (state: queue_state) -> void

...
std
  std.net
    ...
    std.net.tcp_dial
      ...
      # networking
    std.net.read_bytes
      fn (conn: conn_state, n: i32) -> result[bytes, string]
...
kvstore
  kvstore.new
    ...
    # construction
  kvstore.set_string
    fn (state: kv_state, key: string, value: string, ttl_millis: optional[i64]) -> kv_state
    + stores a string value with an optional time-to-live
    # strings
    -> std.time.now_millis
  kvstore.get_string
    fn (state: kv_state, key: string) -> optional[string]
    + returns the value if the key exists and has not expired
    # strings
    -> std.time.now_millis
  kvstore.list_push
    fn (state: kv_state, key: string, value: string, at_head: bool) -> kv_state
    + prepends or appends to the list at key, creating it when missing
    # lists
  kvstore.list_pop
    fn (state: kv_state, key: string, from_head: bool) -> tuple[optional[string], kv_state]
    + removes and returns the head or tail element
    # lists
  kvstore.hash_set
    fn (state: kv_state, key: string, field: string, value: string) -> kv_state
    + sets a field inside the hash at key
    # hashes
  kvstore.hash_get
    fn (state: kv_state, key: string, field: string) -> optional[string]
  ...
  kvstore.zset_range_by_score
    fn (state: kv_state, key: string, min_score: f64, max_score: f64) -> list[string]
  ...
  kvstore.replicate_write
    fn (cluster: cluster_state, cmd: kv_command) -> result[void, string]

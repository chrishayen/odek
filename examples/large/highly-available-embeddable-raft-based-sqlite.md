...
std
  ...
  std.raft
    std.raft.new_node
      ...
      # consensus
    std.raft.step
      @ (state: raft_state, msg: raft_message) -> tuple[raft_state, list[raft_message]]
      + advances the raft state machine with an incoming message and returns outgoing messages
      # consensus
    std.raft.tick
      @ (state: raft_state) -> tuple[raft_state, list[raft_message]]
...
replicated_sql
  replicated_sql.open
    @ (data_dir: string, node_id: i64, peer_ids: list[i64]) -> result[db_state, string]
  ...
  replicated_sql.execute
    @ (db: db_state, sql: string) -> result[db_state, string]
  ...
  replicated_sql.cache_put
    @ (db: db_state, key: string, value: bytes, ttl_ms: i64) -> db_state

...
api_fuzzer
  api_fuzzer.load_spec
    fn (raw: string) -> result[api_spec, string]
  ...
  api_fuzzer.check_response
    ...
    -> std.json.parse_value
  api_fuzzer.run_case
    fn (spec: api_spec, base_url: string, op: operation, seed: u64) -> case_result
  ...
  api_fuzzer.run_campaign
    fn (spec: api_spec, base_url: string, cases_per_op: i32, seed: u64) -> list[case_result]
    + runs cases_per_op random cases against every operation in the spec
    # orchestration
    -> api_fuzzer.list_operations
    -> api_fuzzer.run_case
  api_fuzzer.format_report
    fn (results: list[case_result]) -> string

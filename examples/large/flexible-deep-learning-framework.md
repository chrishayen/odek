...
std
  ...
  std.tensor
    ...
    std.tensor.from_values
      fn (shape: list[i32], values: list[f64]) -> result[tensor, string]
    ...
    std.tensor.add
      ...
      # tensor
    std.tensor.map
      fn (a: tensor, f: f64_unary) -> tensor
      + applies f elementwise
      # tensor

dl
  dl.linear_layer
    fn (in_features: i32, out_features: i32, rng: rng_state) -> tuple[layer_params, rng_state]
    + initializes weights from a scaled normal and biases to zero
    # layers
    -> std.random.gaussian
    -> std.math.sqrt
  dl.linear_forward
    fn (params: layer_params, input: tensor) -> result[tensor, string]
  ...
  dl.softmax
    ...
    -> std.math.exp
  dl.cross_entropy
    fn (logits: tensor, targets: list[i32]) -> f64
    + returns mean cross-entropy loss
    - returns +inf when a target index is out of range
    # loss
    -> std.math.log
  dl.build_graph
    fn (input_shape: list[i32], layers: list[layer_spec]) -> compute_graph
    + returns a graph that threads input_shape through each layer
    # graph
  dl.forward
    fn (graph: compute_graph, input: tensor, params: model_params) -> result[tensor, string]
    + runs the forward pass and returns the output tensor
    # evaluation
  dl.backward
    fn (graph: compute_graph, params: model_params, loss_grad: tensor) -> grad_map
    + returns gradients for every parameter via reverse-mode autodiff
    # autodiff
  dl.sgd_step
    fn (params: model_params, grads: grad_map, lr: f64) -> model_params
    + returns parameters with lr-scaled gradient subtracted
    # optimization
  dl.adam_step
    fn (state: adam_state, params: model_params, grads: grad_map, lr: f64) -> tuple[adam_state, model_params]
    + returns updated moments and parameters for one Adam step
    # optimization
    -> std.math.sqrt
  dl.fit_epoch
    fn (graph: compute_graph, params: model_params, data: batch_stream, lr: f64) -> tuple[model_params, f64]

...
std
  ...
  std.mm
    ...
    std.mm.unmap_guest_memory
      @ (vm: vm_handle, guest_addr: u64) -> result[void, string]
      + removes a previously-created guest mapping
      # memory
  std.io
    std.io.eventfd_new
      @ () -> result[event_fd, string]
...
vmm
  vmm.new
    @ (memory_bytes: u64, vcpu_count: u32) -> result[vmm_state, string]
    ...
    -> std.hv.create_vm
  vmm.load_kernel
    @ (state: vmm_state, kernel: bytes, load_addr: u64) -> result[vmm_state, string]

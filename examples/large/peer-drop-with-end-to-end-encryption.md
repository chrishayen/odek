...
std
  std.fs
    std.fs.walk
      ...
      # filesystem
    std.fs.read_chunk
      fn (path: string, offset: i64, length: i32) -> result[bytes, string]
  ...
  std.crypto
    ...
    std.crypto.pake_finish
      fn (state: pake_state) -> result[bytes, string]
...
peerdrop
  ...
  peerdrop.new_receiver
    fn (relay_addr: string, code: string) -> result[receiver_state, string]
    + connects to the relay under the channel derived from the code
    # receiver_setup
    -> std.net.relay_connect
  peerdrop.receiver_handshake
    fn (state: receiver_state) -> result[transport, string]
    + performs the PAKE exchange and returns an encrypted transport
    ...
    -> std.net.relay_recv
  peerdrop.send_path
    fn (transport: transport, path: string) -> result[i64, string]
    ...
    -> std.net.relay_send
  peerdrop.receive_to
    fn (transport: transport, target_root: string) -> result[list[string], string]
    ...
    -> std.fs.write_chunk
  peerdrop.close
    fn (transport: transport) -> result[void, string]

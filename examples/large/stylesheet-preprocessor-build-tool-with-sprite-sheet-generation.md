...
std
  ...
  std.image
    ...
    std.image.encode_png
      ...
      # image
    std.image.compose
      @ (canvas: image, sprite: image, x: i32, y: i32) -> image
...
style_build
  ...
  style_build.compile_source
    @ (compiler: string, input_path: string, output_path: string) -> result[void, string]
  ...
  style_build.compile_all
    @ (compiler: string, sources: list[string], out_dir: string) -> result[i32, string]
  ...
  style_build.load_sprite_set
    @ (dir: string) -> result[list[image], string]

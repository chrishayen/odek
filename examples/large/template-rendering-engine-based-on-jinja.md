...
template_engine
  ...
  template_engine.eval_expression
    @ (expr: string, context: map[string,string]) -> result[string, string]
  ...
  template_engine.register_filter
    @ (engine: engine_state, name: string, fn: filter_fn) -> engine_state
  ...
  template_engine.render
    @ (engine: engine_state, source: string, context: map[string,string]) -> result[string, string]
    + tokenizes, parses, and evaluates source against context
    - returns error when any stage fails
    # rendering
  template_engine.render_file
    @ (engine: engine_state, path: string, context: map[string,string]) -> result[string, string]
    + reads a template file and renders it
    - returns error when the file cannot be read
    # rendering
    -> std.fs.read_all
  template_engine.extend_template
    @ (child: string, parent: string) -> result[string, string]
  ...
  template_engine.execute_for
    @ (var_name: string, iterable: list[string], body: ast_node, context: map[string,string]) -> result[string, string]
    + iterates the body with var_name bound to each element
    # control_flow
  template_engine.execute_if
    @ (condition: string, then_branch: ast_node, else_branch: optional[ast_node], context: map[string,string]) -> result[string, string]

...
std
  ...
  std.collections
    std.collections.map_keys_sorted
      @ (m: map[string,bytes]) -> list[string]
...
chain
  chain.new
    @ (chain_id: string) -> chain_state
    + creates an empty chain with height 0 and no registered modules
    # construction
  chain.register_module
    @ (state: chain_state, module_name: string, message_types: list[string]) -> chain_state
  ...
  chain.account_set_balance
    @ (state: chain_state, address: string, denom: string, amount: i64) -> chain_state
    + writes balance for (address, denom) into state
    # state_store
  chain.account_get_balance
    @ (state: chain_state, address: string, denom: string) -> i64
    + returns the balance or 0 when absent
    # state_store
  chain.account_transfer
    @ (state: chain_state, from: string, to: string, denom: string, amount: i64) -> result[chain_state, string]
    ...
    # accounts
  chain.store_set
    @ (state: chain_state, module_name: string, key: string, value: bytes) -> chain_state
    + writes a value under a module-scoped key
    # state_store
  chain.store_get
    @ (state: chain_state, module_name: string, key: string) -> optional[bytes]
    + returns the stored value or none
    # state_store
  chain.verify_tx
    @ (state: chain_state, tx_bytes: bytes, signer_pubkey: bytes, signature: bytes) -> result[void, string]
    + returns ok when the signature is valid over tx_bytes
    - returns error "bad signature" when verification fails
    # tx_validation
    -> std.crypto.verify_signature
  chain.deliver_tx
    @ (state: chain_state, module_name: string, message_type: string, payload: bytes) -> result[chain_state, string]
    ...
    # tx_execution
  chain.begin_block
    @ (state: chain_state, proposer: string, timestamp_secs: i64) -> chain_state

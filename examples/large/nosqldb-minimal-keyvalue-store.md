...
nosqldb
  ...
  nosqldb.close
    ...
    -> std.fs.fsync
  nosqldb.get
    fn (state: db_state, key: bytes) -> result[optional[bytes], string]
    + returns the value for a key or none when missing
    # read
  nosqldb.put
    fn (state: db_state, key: bytes, value: bytes) -> result[db_state, string]
    + inserts or replaces the value for a key
    - returns error when the key exceeds max size
    # write
  nosqldb.delete
    fn (state: db_state, key: bytes) -> result[db_state, string]
    + removes a key from the tree
    # write
  nosqldb.alloc_page
    fn (state: db_state) -> tuple[db_state, page_id]
    + reserves a fresh page id from the free list or extends the file
    # pager
  nosqldb.read_page
    fn (state: db_state, id: page_id) -> result[bytes, string]
    + reads the raw bytes for a page
    - returns error when the page id is out of range
    # pager
    -> std.fs.read_at
  nosqldb.write_page
    fn (state: db_state, id: page_id, data: bytes) -> result[db_state, string]
    + writes a page and updates the cache
    # pager
    -> std.fs.write_at
  nosqldb.btree_search
    fn (state: db_state, root: page_id, key: bytes) -> result[optional[bytes], string]
    + walks the B+ tree from root looking for the key
    # index
  nosqldb.btree_insert
    fn (state: db_state, root: page_id, key: bytes, value: bytes) -> result[tuple[db_state, page_id], string]
    + inserts into the B+ tree and returns the new root after splits
    # index
  nosqldb.wal_append
    fn (state: db_state, record: bytes) -> result[db_state, string]

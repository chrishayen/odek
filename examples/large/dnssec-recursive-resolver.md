...
dns
  dns.encode_query
    fn (name: string, qtype: i32, qid: u16) -> bytes
    + encodes a DNS query in wire format
    # wire_protocol
  dns.decode_message
    fn (wire: bytes) -> result[dns_message, string]
    + decodes a DNS wire-format message
    - returns error on truncated or malformed input
    # wire_protocol
  dns.encode_response
    fn (msg: dns_message) -> bytes
    + encodes a DNS message as wire-format bytes
    # wire_protocol
  dns.new_resolver
    fn (root_hints: list[string]) -> resolver_state
    + creates a resolver seeded with root server addresses
    # construction
    -> std.net.parse_ipv4
  dns.resolve
    fn (state: resolver_state, name: string, qtype: i32) -> result[tuple[list[dns_record], resolver_state], string]
    + iteratively resolves a name from the nearest authoritative server
    - returns error on NXDOMAIN or network failure
    # recursion
    -> std.time.now_seconds
  dns.cache_get
    fn (state: resolver_state, name: string, qtype: i32) -> optional[list[dns_record]]
    + returns cached records when present and within TTL
    - returns none when absent or expired
    # caching
    -> std.time.now_seconds
  dns.cache_put
    fn (state: resolver_state, name: string, qtype: i32, records: list[dns_record]) -> resolver_state
    + inserts records with TTL into the cache
    # caching
    -> std.time.now_seconds
  dns.validate_chain
    fn (records: list[dns_record], rrsigs: list[dns_record], trust_anchor: bytes) -> result[void, string]

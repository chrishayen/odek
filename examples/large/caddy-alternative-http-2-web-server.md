...
std
  ...
  std.tls
    std.tls.wrap_server
      @ (conn: conn_state, cert: bytes, key: bytes, alpn: list[string]) -> result[tls_conn, string]
...
webserver
  webserver.parse_config
    @ (raw: string) -> result[server_config, string]
  ...
  webserver.serve_file
    @ (root: string, url_path: string) -> result[http_response, string]
  ...
  webserver.encode_http2_frames
    @ (resp: http_response, stream_id: i32) -> list[bytes]
    + serializes a response as HEADERS and DATA frames for HTTP/2
    # http2
  webserver.handle_request
    @ (config: server_config, req: http_request) -> http_response
    + routes the request via the host block rules to files or proxies
    # routing
  webserver.accept_loop
    @ (config: server_config, host: string, port: i32) -> result[void, string]

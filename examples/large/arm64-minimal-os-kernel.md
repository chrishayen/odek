...
kernel
  ...
  kernel.alloc_frame
    ...
    -> std.bits.set_bit
  kernel.free_frame
    fn (alloc: frame_allocator_state, addr: u64) -> frame_allocator_state
  ...
  kernel.new_page_table
    ...
    -> std.mem.zero_range
  kernel.map_page
    fn (pt: page_table_state, virt: u64, phys: u64, flags: u64) -> result[page_table_state, string]
    ...
    # memory_management
  kernel.unmap_page
    fn (pt: page_table_state, virt: u64) -> result[page_table_state, string]
    + removes a translation for a virtual page
    - returns error when the page is not mapped
    # memory_management
  kernel.new_task
    fn (entry: u64, stack_top: u64) -> task_state
    + returns a new task with the given entry point and kernel stack
    # scheduling
  kernel.schedule_next
    fn (runqueue: list[task_state]) -> tuple[task_state, list[task_state]]
    + returns the next task to run and the updated runqueue
    # scheduling
  kernel.handle_trap
    fn (task: task_state, cause: u64, value: u64) -> result[task_state, string]

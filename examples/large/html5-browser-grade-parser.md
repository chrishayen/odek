...
std
  std.text
    ...
    std.text.decode_numeric_entity
      fn (digits: string, hex: bool) -> result[i32, string]
...
html5
  html5.tokenizer_new
    fn (input: string) -> tokenizer_state
    + returns a tokenizer positioned at the start of the input
    # tokenization
  html5.next_token
    fn (state: tokenizer_state) -> tuple[optional[html_token], tokenizer_state]
  ...
  html5.parse_fragment
    fn (input: string, context_element: string) -> result[dom_node, string]
  ...
  html5.parse_document
    fn (input: string) -> result[dom_node, string]
  ...
  html5.insertion_mode_step
    fn (parser: parser_state, token: html_token) -> parser_state

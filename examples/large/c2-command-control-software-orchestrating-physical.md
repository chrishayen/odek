...
control
  ...
  control.register_device
    @ (hub: hub_state, device_id: string, shared_secret: bytes) -> hub_state
    + adds the device and stores its HMAC secret for auth
    # registration
  control.authenticate
    @ (hub: hub_state, device_id: string, nonce: bytes, mac: bytes) -> result[void, string]
    ...
    -> std.crypto.hmac_sha256
  control.device_seen
    @ (hub: hub_state, device_id: string) -> hub_state
    + updates the device's last_seen timestamp to now
    # presence
    -> std.time.now_millis
  control.is_online
    @ (hub: hub_state, device_id: string, stale_after_ms: i64) -> bool
    + returns true when last_seen is newer than now - stale_after_ms
    # presence
    -> std.time.now_millis
  control.issue_command
    @ (hub: hub_state, device_id: string, command: string, payload: map[string,string]) -> result[command_envelope, string]
  ...
  control.pop_commands
    @ (hub: hub_state, device_id: string) -> tuple[list[command_envelope], hub_state]
    + drains and returns queued commands for the device
    # command_dispatch
  control.report_telemetry
    @ (hub: hub_state, device_id: string, raw: string) -> result[hub_state, string]
    + parses raw as a JSON telemetry object and records it
    - returns error when JSON is invalid
    # telemetry
    -> std.json.parse_object
  control.query_telemetry
    @ (hub: hub_state, device_id: string) -> optional[map[string,string]]
    + returns the latest telemetry for the device
    # telemetry
  control.serve
    @ (hub: hub_state, host: string, port: i32) -> result[void, string]

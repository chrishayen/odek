...
std
  ...
  std.binary
    ...
    std.binary.read_u64_be
      @ (data: bytes, offset: i64) -> result[u64, string]
    ...
    std.binary.write_u32_be
      ...
      # binary
    std.binary.slice
      @ (data: bytes, start: i64, length: i64) -> result[bytes, string]
...
mp4
  mp4.parse_box_header
    @ (data: bytes, offset: i64) -> result[box_header, string]
  ...
  mp4.add_subtitle_track
    @ (file: mp4_file, language: string, cues: list[subtitle_cue]) -> result[mp4_file, string]
    + returns a new file with an additional subtitle track
    - returns error when the language code is not 3 letters
    # tracks
  mp4.set_metadata
    @ (file: mp4_file, key: string, value: string) -> mp4_file

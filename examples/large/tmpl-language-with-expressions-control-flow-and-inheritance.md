...
tmpl
  tmpl.tokenize
    fn (source: string) -> result[list[tmpl_token], string]
    + emits text, expression ({{ ... }}), and statement ({% ... %}) tokens
    - returns error on unterminated tags
    # lexing
  tmpl.parse
    fn (tokens: list[tmpl_token]) -> result[tmpl_node, string]
  ...
  tmpl.render
    ...
    -> std.text.join
  tmpl.render_string
    fn (env: env_state, source: string, context: map[string, value]) -> result[string, string]

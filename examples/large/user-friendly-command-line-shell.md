...
std
  std.fs
    std.fs.glob
      @ (pattern: string, cwd: string) -> result[list[string], string]
  ...
  std.process
    std.process.spawn_pipeline
      @ (stages: list[process_spec]) -> result[list[i32], string]
...
shell
  shell.new
    ...
    # construction
  shell.tokenize
    @ (line: string) -> result[list[token], string]
  ...
  shell.parse
    ...
    # parsing
  shell.expand
    @ (state: shell_state, cmd: command) -> result[command, string]
    ...
    -> std.fs.glob
  shell.run_builtin
    @ (state: shell_state, cmd: command) -> optional[result[shell_state, string]]

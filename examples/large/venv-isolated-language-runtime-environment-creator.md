...
venv
  ...
  venv.write_activate_script
    @ (paths: venv_paths) -> result[void, string]
    + writes a POSIX shell activation script that prepends bin to PATH and sets a prompt prefix
    # activation
    -> std.fs.write_all
  venv.write_config
    @ (paths: venv_paths, interpreter: string, version: string) -> result[void, string]
    + writes a config file recording the interpreter path and version
    # metadata
    -> std.fs.write_all
  venv.install_package
    @ (paths: venv_paths, package_name: string) -> result[void, string]

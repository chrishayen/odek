...
std
  ...
  std.net
    std.net.ws_connect
      ...
      # networking
    std.net.ws_send
      @ (conn: ws_conn, frame: string) -> result[void, string]
      + sends a text frame
      # networking
    std.net.ws_recv
...
devtools_driver
  devtools_driver.launch
    @ (browser_path: string, headless: bool) -> result[driver_state, string]
  ...
  devtools_driver.new_page
    @ (state: driver_state) -> result[page_handle, string]
  ...
  devtools_driver.wait_for_selector
    @ (page: page_handle, selector: string, timeout_ms: i64) -> result[void, string]
  ...
  devtools_driver.query_text
    @ (page: page_handle, selector: string) -> result[string, string]
  ...
  devtools_driver.query_all_text
    @ (page: page_handle, selector: string) -> result[list[string], string]
    + returns text content of all matching elements in document order
    # dom_query
  devtools_driver.click
    @ (page: page_handle, selector: string) -> result[void, string]
    + dispatches a click at the center of the first matching element
    - returns error when selector matches nothing
    # input
  devtools_driver.type_text
    @ (page: page_handle, selector: string, text: string) -> result[void, string]
  ...
  devtools_driver.evaluate
    @ (page: page_handle, expression: string) -> result[json_value, string]
  ...
  devtools_driver.send_command
    @ (state: driver_state, method: string, params: json_value) -> result[json_value, string]

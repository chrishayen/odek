...
std
  std.browser
    std.browser.launch
      @ (headless: bool) -> result[browser_handle, string]
      + launches a browser process
      - returns error when no browser engine is available
      # browser
    std.browser.close
      @ (handle: browser_handle) -> void
      + terminates the browser process
      # browser
    std.browser.new_page
      @ (handle: browser_handle) -> result[page_handle, string]
      + opens a new tab
      # browser
    std.browser.goto
      @ (page: page_handle, url: string) -> result[page_handle, string]
      + navigates the page to the given URL
      - returns error on network failure
      # browser
    std.browser.query
      @ (page: page_handle, selector: string) -> optional[element_handle]
      + returns the first element matching the selector
      # browser
    std.browser.query_all
      @ (page: page_handle, selector: string) -> list[element_handle]
...
web
  web.open_browser
    @ (headless: bool) -> result[browser_state, string]
    + launches the browser and returns a driver state
    # lifecycle
    -> std.browser.launch
  web.close_browser
    @ (state: browser_state) -> void
    + shuts down the browser and releases resources
    # lifecycle
    -> std.browser.close
  web.new_page
    @ (state: browser_state) -> result[page_state, string]
    + opens a fresh page
    # navigation
    -> std.browser.new_page
  web.visit
    @ (page: page_state, url: string) -> result[page_state, string]
    + navigates to a URL and waits for the page to settle
    - returns error when navigation fails
    # navigation
    -> std.browser.goto
  web.click
    @ (page: page_state, selector: string) -> result[page_state, string]
    ...
    -> std.browser.click
  web.fill
    @ (page: page_state, selector: string, text: string) -> result[page_state, string]
    ...
    -> std.browser.type_text
  web.text_content
    @ (page: page_state, selector: string) -> optional[string]
    + returns the rendered text of the first matching element
    # assertion
    -> std.browser.query
    -> std.browser.inner_text
  web.wait_for_selector
    @ (page: page_state, selector: string, timeout_ms: i32) -> result[page_state, string]
    + polls until a matching element appears or the timeout elapses
    - returns error when the timeout elapses
    # synchronization
    -> std.browser.query
  web.count
    @ (page: page_state, selector: string) -> i32
    + returns the number of elements matching the selector
    # assertion
    -> std.browser.query_all
  web.screenshot
    @ (page: page_state, path: string) -> result[void, string]

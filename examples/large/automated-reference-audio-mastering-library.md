...
mastering
  ...
  mastering.brickwall_limiter
    fn (buf: audio_buffer, ceiling_db: f64) -> audio_buffer
    + returns a buffer whose peaks do not exceed the ceiling
    - leaves samples unchanged when no peak exceeds the ceiling
    # limiting
  mastering.master
    fn (target: audio_buffer, reference: audio_buffer) -> result[audio_buffer, string]

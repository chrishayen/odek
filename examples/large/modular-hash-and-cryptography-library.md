...
crypto
  crypto.hash
    @ (algorithm: string, data: bytes) -> result[bytes, string]
    ...
    -> std.hash.blake2b
  crypto.hmac
    @ (algorithm: string, key: bytes, data: bytes) -> result[bytes, string]
  ...
  crypto.aes_cbc_encrypt
    @ (key: bytes, iv: bytes, plaintext: bytes) -> result[bytes, string]
    + PKCS#7-pads then encrypts in CBC mode
    - returns error when iv is not 16 bytes
    # cryptography
    -> std.crypto.aes_encrypt_block
  crypto.aes_cbc_decrypt
    @ (key: bytes, iv: bytes, ciphertext: bytes) -> result[bytes, string]
    + decrypts CBC and strips PKCS#7 padding
    - returns error on bad padding
    # cryptography
    -> std.crypto.aes_decrypt_block
  crypto.pbkdf2
    @ (algorithm: string, password: bytes, salt: bytes, iters: i32, key_len: i32) -> result[bytes, string]

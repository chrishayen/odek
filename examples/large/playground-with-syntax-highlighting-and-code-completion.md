...
std
  std.text
    ...
    std.text.prefix_match
      fn (candidates: list[string], prefix: string) -> list[string]
      + returns candidates that start with prefix
      # text
  std.lex
    std.lex.tokenize
      fn (source: string, grammar: grammar) -> list[token]
...
playground
  ...
  playground.register_executor
    fn (buf: buffer, executor: fn(string) -> result[string, string]) -> buffer
    + attaches an executor that runs snippets on demand
    # execution
  playground.run
    fn (buf: buffer) -> result[string, string]

...
rpc
  ...
  rpc.decode_request
    ...
    -> std.encoding.varint_decode
  rpc.encode_response
    @ (status: i32, payload: bytes) -> bytes
    + returns a framed response with the given status and body
    # framing
    -> std.encoding.varint_encode
  rpc.decode_response
    @ (frame: bytes) -> result[tuple[i32, bytes], string]
  ...
  rpc.register_handler
    @ (server: server_state, method: string, handler: fn(bytes) -> result[bytes, string]) -> server_state
    + associates method with handler
    + replaces any previous handler for the same method
    # registration
  rpc.serve_one
    @ (server: server_state, conn: connection) -> result[void, string]
    ...
    -> std.io.write_all
  rpc.call
    @ (conn: connection, method: string, payload: bytes) -> result[bytes, string]

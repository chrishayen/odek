...
kernel
  kernel.new
    @ (ram_size: i32, flash_size: i32) -> kernel_state
    + initializes a kernel with empty process table and sized memory pools
    # construction
    -> std.mem.zero
  kernel.register_process
    @ (state: kernel_state, name: string, entry: u32, stack_size: i32, grant_size: i32) -> result[process_id, string]
  ...
  kernel.handle_syscall
    ...
    # syscall
  kernel.grant_alloc
    @ (state: kernel_state, pid: process_id, size: i32) -> result[u32, string]
  ...
  kernel.capability_issue
    @ (state: kernel_state, pid: process_id, resource: string) -> capability

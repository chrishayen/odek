...
pathsim
  pathsim.new_system
    ...
    # construction
  pathsim.add_block
    fn (system: system_state, name: string, step: fn(list[f64]) -> list[f64], input_count: i32, output_count: i32) -> system_state
    + registers a block with a step function mapping input port values to output port values
    # modeling
  pathsim.connect
    fn (system: system_state, src_block: string, src_port: i32, dst_block: string, dst_port: i32) -> result[system_state, string]
  ...
  pathsim.topological_order
    fn (system: system_state) -> result[list[string], string]

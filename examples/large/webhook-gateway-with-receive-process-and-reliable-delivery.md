...
gateway
  ...
  gateway.register_endpoint
    fn (state: gateway_state, event_type: string, url: string) -> gateway_state
    + subscribes a URL to an event type
    # subscriptions
  gateway.receive
    fn (state: gateway_state, event_type: string, payload: json_value) -> tuple[gateway_state, string]
    + enqueues one delivery attempt per registered endpoint and returns the event id
    # ingress
    -> std.uuid.v4
    -> std.time.now_millis
  gateway.next_ready
    fn (state: gateway_state, now_ms: i64) -> optional[delivery_attempt]
    + returns the earliest attempt whose next-try time has passed
    # scheduling
  gateway.attempt
    fn (state: gateway_state, attempt: delivery_attempt) -> tuple[gateway_state, delivery_result]

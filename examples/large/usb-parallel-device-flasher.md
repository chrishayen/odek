...
std
  std.fs
    ...
    std.fs.open_write
      ...
      # filesystem
    std.fs.read_chunk
      @ (handle: file_handle, size: i64) -> result[bytes, string]
...
usb_flasher
  ...
  usb_flasher.verify_image
    @ (image_path: string, expected_sha256: optional[bytes]) -> result[i64, string]
  ...
  usb_flasher.flash_one
    @ (image_path: string, target_path: string, progress: progress_sink) -> result[i64, string]
  ...
  usb_flasher.flash_parallel
    @ (image_path: string, targets: list[string], progress: map[string, progress_sink]) -> result[map[string, i64], string]

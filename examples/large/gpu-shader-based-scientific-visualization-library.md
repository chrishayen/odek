...
viz
  viz.new_scene
    ...
    # construction
  viz.add_line_plot
    fn (s: scene, xs: list[f64], ys: list[f64], color: rgba) -> scene
    + adds a line-strip primitive over the given points
    - returns the scene unchanged when xs and ys differ in length
    # primitives
  viz.add_scatter
    fn (s: scene, xs: list[f64], ys: list[f64], color: rgba, size: f32) -> scene
    + adds a point-cloud primitive with per-vertex size
    # primitives
  viz.add_image
    fn (s: scene, pixels: bytes, width: u32, height: u32) -> scene
  ...
  viz.fit_view
    ...
    -> std.math.min_max_f64
  viz.set_colormap
    fn (s: scene, stops: list[rgba]) -> scene
    + stores an interpolation-ready colormap used by value-to-color lookups
    # color
  viz.value_to_color
    fn (s: scene, value: f64, vmin: f64, vmax: f64) -> rgba
    + maps value linearly onto the active colormap
    # color
    -> std.math.lerp
  viz.render
    fn (s: scene, backend: render_backend, width: u32, height: u32) -> result[bytes, string]

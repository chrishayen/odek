...
std
  std.net
    std.net.listen_tcp
      @ (address: string, port: i32) -> result[tcp_listener, string]
      + binds and listens on the given address
      - returns error when the port is in use
      # networking
    std.net.accept
      @ (listener: tcp_listener) -> result[tcp_connection, string]
      + blocks for the next incoming connection
      # networking
    std.net.read_bytes
      @ (conn: tcp_connection, max: i32) -> result[bytes, string]
      + reads up to max bytes
      # networking
    std.net.write_bytes
      @ (conn: tcp_connection, data: bytes) -> result[void, string]
      + writes all bytes
      # networking
  std.encoding
...
mqtt
  mqtt.parse_packet
    @ (raw: bytes) -> result[mqtt_packet, string]
  ...
  mqtt.broker_on_connect
    @ (state: broker_state, client_id: string, clean_session: bool) -> broker_state
  ...
  mqtt.broker_on_subscribe
    @ (state: broker_state, client_id: string, filter: string, qos: i32) -> broker_state
  ...
  mqtt.broker_on_publish
    @ (state: broker_state, topic: string, payload: bytes, qos: i32) -> tuple[broker_state, list[mqtt_delivery]]
  ...
  mqtt.broker_retained_for
    @ (state: broker_state, filter: string) -> list[mqtt_delivery]

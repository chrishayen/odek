...
std
  std.math
    std.math.sum_f64
      ...
      # math
    std.math.mean_f64
      @ (values: list[f64]) -> result[f64, string]
    ...
    std.math.argsort_f64
      @ (values: list[f64]) -> list[i32]
      + returns indices that would sort the list in ascending order
      # math

gbt
  gbt.new
    @ (learning_rate: f64, max_depth: i32) -> gbt_state
    + creates an empty model with hyperparameters
    # construction
  gbt.fit_stump
    @ (features: list[list[f64]], residuals: list[f64]) -> result[tree_node, string]

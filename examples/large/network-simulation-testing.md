...
netsim
  netsim.new_network
    ...
    # construction
  netsim.add_node
    @ (net: network_state, name: string) -> result[network_state, string]
    + registers a node with a unique name
    - returns error when a node with the same name already exists
    # topology
  netsim.add_link
    @ (net: network_state, from: string, to: string, latency_ms: i32, loss: f64) -> result[network_state, string]
    ...
    # topology
  netsim.send
    @ (net: network_state, from: string, to: string, payload: bytes, now_ms: i64) -> result[network_state, string]
    + schedules a packet along the shortest path with accumulated latency
    - returns error when no path exists between endpoints
    # transmission
  netsim.tick
    @ (net: network_state, now_ms: i64) -> tuple[list[delivered_packet], network_state]
    ...
    -> std.random.next_f64
  netsim.partition
    @ (net: network_state, group_a: list[string], group_b: list[string]) -> network_state

...
script_runtime
  ...
  script_runtime.create_context
    fn (state: runtime_state, name: string, memory_limit_bytes: i64, cpu_budget_ms: i32) -> result[tuple[runtime_state, context_id], string]
  ...
  script_runtime.destroy_context
    fn (state: runtime_state, id: context_id) -> runtime_state
    + frees the context and releases its quota
    # isolation
  script_runtime.load_script
    fn (state: runtime_state, id: context_id, source: string) -> result[runtime_state, string]
    + compiles and registers a script inside the context
    - returns error on syntax error
    # loading
  script_runtime.register_handler
    fn (state: runtime_state, id: context_id, route: string, entry: string) -> result[runtime_state, string]
  ...
  script_runtime.grant_capability
    fn (state: runtime_state, id: context_id, capability: string) -> runtime_state
  ...
  script_runtime.dispatch_request
    fn (state: runtime_state, id: context_id, req: http_request) -> result[http_response, string]
  ...
  script_runtime.stats
    fn (state: runtime_state, id: context_id) -> optional[context_stats]
  ...
  script_runtime.set_global
    fn (state: runtime_state, id: context_id, key: string, value: json_value) -> runtime_state

...
std
  std.audio
    std.audio.sine
      fn (freq_hz: f64, phase: f64, sample_rate: i32) -> f64
    ...
    std.audio.lowpass_step
      fn (input: f64, cutoff_hz: f64, state: f64, sample_rate: i32) -> f64
...
live_graph
  ...
  live_graph.compile
    ...
    # compilation
  live_graph.hot_swap
    fn (state: graph_state, source: string) -> result[graph_state, string]
  ...
  live_graph.render_block
    fn (state: graph_state, frames: i32) -> tuple[list[f32], graph_state]

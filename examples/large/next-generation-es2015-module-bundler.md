...
bundler
  ...
  bundler.parse_imports
    ...
    # parsing
  bundler.resolve
    @ (from: string, specifier: string) -> result[string, string]
    ...
    -> std.path.join
  bundler.load_module
    @ (state: bundler_state, path: string) -> result[bundler_state, string]
    ...
    -> std.fs.read_all
  bundler.build_graph
    @ (state: bundler_state) -> result[bundler_state, string]

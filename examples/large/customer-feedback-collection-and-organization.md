...
std
  ...
  std.kv
    ...
    std.kv.get
      ...
      # storage
    std.kv.list_prefix
      fn (prefix: string) -> result[list[string], string]
...
feedback
  feedback.register_user
    fn (name: string, email: string) -> result[user_id, string]
  ...
  feedback.create_idea
    fn (author: user_id, title: string, body: string) -> result[idea_id, string]
    ...
    -> std.kv.put
  feedback.vote
    fn (voter: user_id, idea: idea_id) -> result[i32, string]
    ...
    -> std.kv.put
  feedback.comment
    fn (author: user_id, idea: idea_id, body: string) -> result[comment_id, string]
  ...
  feedback.change_status
    fn (idea: idea_id, status: idea_status) -> result[void, string]
    ...
    -> std.kv.put
  feedback.tag_idea
    fn (idea: idea_id, tag: string) -> result[void, string]
    + attaches a tag to the idea; tags are a set
    # organization
    -> std.kv.get
    -> std.kv.put
  feedback.list_by_status
    fn (status: idea_status) -> result[list[idea], string]
    ...
    -> std.json.parse_object
  feedback.search
    fn (query: string) -> result[list[idea], string]
    + returns ideas whose title or body contains the query, case-insensitive
    # queries
    -> std.kv.list_prefix
  feedback.top_voted
    fn (limit: i32) -> result[list[idea], string]

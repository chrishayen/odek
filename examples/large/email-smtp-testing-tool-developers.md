...
smtp_capture
  smtp_capture.new_store
    ...
    # construction
  smtp_capture.start
    @ (store: capture_store, host: string, port: u16) -> result[server_state, string]
    + begins accepting SMTP connections on the given address
    # lifecycle
    -> std.net.tcp_listen
  smtp_capture.handle_connection
    @ (store: capture_store, conn: conn_state) -> result[void, string]
  ...
  smtp_capture.parse_command
    @ (line: string) -> result[smtp_command, string]

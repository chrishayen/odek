...
git
  git.init_repo
    ...
    -> std.fs.write_all
  git.hash_object
    fn (obj_type: string, content: bytes) -> string
    + returns the sha1 hex id for a git object with the given header and content
    # object_store
    -> std.crypto.sha1
  git.write_object
    fn (repo_path: string, obj_type: string, content: bytes) -> result[string, string]
    ...
    -> std.fs.write_all
  git.read_object
    fn (repo_path: string, id: string) -> result[tuple[string, bytes], string]
  ...
  git.clone
    ...
    -> std.http.post
  git.fetch
    fn (repo_path: string, remote_url: string) -> result[list[string], string]

...
std
  ...
  std.websocket
    ...
    std.websocket.encode_frame
      fn (opcode: i32, payload: bytes) -> bytes
...
web_framework
  ...
  web_framework.add_route
    fn (app: app_state, method: string, path: string, handler_id: string) -> app_state
    + registers a handler for the given method and path
    # routing
  web_framework.match_route
    fn (app: app_state, method: string, path: string) -> optional[string]

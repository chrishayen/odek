...
std
  ...
  std.crypto
    std.crypto.encrypt_for_recipients
      @ (plaintext: bytes, recipient_keys: list[bytes]) -> result[bytes, string]
    ...
    std.crypto.decrypt_with_identity
      @ (ciphertext: bytes, private_key: bytes) -> result[bytes, string]
...
pass_store
  pass_store.open
    @ (root: string, recipients: list[bytes]) -> result[store_state, string]
    ...
    # construction
  pass_store.path_for
    @ (state: store_state, entry: string) -> string
    ...
    # addressing
  pass_store.insert
    @ (state: store_state, entry: string, secret: string) -> result[void, string]
    ...
    -> std.fs.write_bytes
  pass_store.show
    @ (state: store_state, entry: string, private_key: bytes) -> result[string, string]
    ...
    -> std.crypto.decrypt_with_identity
  pass_store.list
    @ (state: store_state) -> result[list[string], string]
    + returns all entry names currently in the store
    # enumeration
    -> std.fs.list_tree
  pass_store.remove
    @ (state: store_state, entry: string) -> result[void, string]

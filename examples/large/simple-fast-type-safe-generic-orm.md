...
orm
  orm.new_schema
    @ (table: string) -> schema_state
    + returns an empty schema bound to the given table
    # construction
  orm.add_column
    @ (schema: schema_state, name: string, sql_type: string) -> schema_state
    + appends a column definition
    + preserves insertion order
    # schema
  orm.mark_primary_key
    @ (schema: schema_state, name: string) -> result[schema_state, string]
  ...
  orm.create_table_sql
    ...
    -> std.strings.join
  orm.insert
    @ (conn: db_conn, schema: schema_state, record: map[string, string]) -> result[i64, string]
    ...
    -> std.sql.execute
  orm.find_by_id
    @ (conn: db_conn, schema: schema_state, id: string) -> result[optional[map[string, string]], string]
    ...
    -> std.sql.query
  orm.find_where
    @ (conn: db_conn, schema: schema_state, column: string, value: string) -> result[list[map[string, string]], string]
    ...
    -> std.sql.query
  orm.update
    @ (conn: db_conn, schema: schema_state, id: string, changes: map[string, string]) -> result[i64, string]
    ...
    -> std.sql.execute
  orm.delete_by_id
    @ (conn: db_conn, schema: schema_state, id: string) -> result[i64, string]

...
microsvc
  ...
  microsvc.register_instance
    @ (state: registry_state, service_name: string, instance_id: string, address: string) -> registry_state
  ...
  microsvc.deregister_instance
    @ (state: registry_state, service_name: string, instance_id: string) -> registry_state
    + removes an instance
    # registration
  microsvc.heartbeat
    @ (state: registry_state, service_name: string, instance_id: string) -> registry_state
  ...
  microsvc.prune_unhealthy
    @ (state: registry_state, max_age_millis: i64) -> registry_state
    + removes instances whose last heartbeat is older than max_age_millis
    # health
    -> std.time.now_millis
  microsvc.list_instances
    @ (state: registry_state, service_name: string) -> list[instance]
  ...
  microsvc.resolve_random
    @ (state: registry_state, service_name: string) -> result[instance, string]
    + returns a uniformly random instance
    - returns error "no instances" when none are registered
    # load_balancing
    -> std.random.int_range
  microsvc.resolve_round_robin
    @ (state: registry_state, service_name: string) -> tuple[registry_state, result[instance, string]]
    + returns the next instance in round-robin order and advances the cursor
    - returns error "no instances" when none are registered
    # load_balancing
  microsvc.resolve_consistent_hash
    @ (state: registry_state, service_name: string, routing_key: string) -> result[instance, string]
    ...
    -> std.hash.fnv1a
  microsvc.watch
    @ (state: registry_state, service_name: string, watcher_id: string) -> registry_state
    + subscribes watcher_id to changes in the service's instance set
    # watches
  microsvc.drain_change_events
    @ (state: registry_state, watcher_id: string) -> tuple[registry_state, list[change_event]]

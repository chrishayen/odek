...
nlptk
  ...
  nlptk.split_sentences
    ...
    # segmentation
  nlptk.pos_tag
    fn (tokens: list[string]) -> list[tuple[string,string]]
  ...
  nlptk.remove_stopwords
    ...
    -> std.text.to_lower
  nlptk.frequency
    fn (tokens: list[string]) -> map[string, i32]
    + returns a token-to-count map
    # statistics
  nlptk.top_n
    fn (freq: map[string,i32], n: i32) -> list[tuple[string,i32]]
    + returns the n most frequent entries sorted descending
    # statistics
  nlptk.ngrams
    fn (tokens: list[string], n: i32) -> list[list[string]]

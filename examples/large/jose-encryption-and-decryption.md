...
std
  ...
  std.crypto
    std.crypto.aes_gcm_seal
      fn (key: bytes, nonce: bytes, aad: bytes, plaintext: bytes) -> tuple[bytes, bytes]
      + returns (ciphertext, 16-byte tag)
      # cryptography
    std.crypto.aes_gcm_open
      fn (key: bytes, nonce: bytes, aad: bytes, ciphertext: bytes, tag: bytes) -> result[bytes, string]
      + returns the plaintext after verifying the tag
      - returns error when the tag does not match
      # cryptography
    std.crypto.aes_key_wrap
      fn (kek: bytes, plaintext_key: bytes) -> result[bytes, string]
      + returns RFC 3394 key-wrapped output
      - returns error when kek length is not 16, 24, or 32
      # cryptography
    std.crypto.aes_key_unwrap
      fn (kek: bytes, wrapped: bytes) -> result[bytes, string]
...
jose
  jose.encrypt_compact
    fn (header: map[string, string], recipient_key: bytes, plaintext: bytes) -> result[string, string]
  ...
  jose.decrypt_compact
    fn (compact: string, recipient_key: bytes) -> result[bytes, string]
  ...
  jose.wrap_content_key
    fn (alg: string, recipient_key: bytes, cek: bytes) -> result[bytes, string]
  ...
  jose.unwrap_content_key
    fn (alg: string, recipient_key: bytes, wrapped: bytes) -> result[bytes, string]
  ...
  jose.build_protected_header
    fn (alg: string, enc: string, extras: map[string, string]) -> string

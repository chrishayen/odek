...
std
  ...
  std.db
    std.db.connect
      ...
      # database
    std.db.list_tables
      fn (db: db_handle) -> result[list[string], string]
      + returns the names of user tables in the current schema
      # database
    std.db.describe_table
      fn (db: db_handle, table: string) -> result[list[column_info], string]
...
sqlgen
  sqlgen.parse_schema_sql
    fn (sql: string) -> result[list[table_model], string]
  ...
  sqlgen.schema_from_dsn
    fn (dsn: string) -> result[list[table_model], string]
    ...
    -> std.db.describe_table
  sqlgen.load_schema
    fn (source: string) -> result[list[table_model], string]
  ...
  sqlgen.render_raw_sql_helpers
    ...
    # codegen
  sqlgen.generate
    fn (source: string, target: string, out_dir: string) -> result[void, string]

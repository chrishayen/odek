...
std
  std.fs
    ...
    std.fs.open_read
      ...
      # fs
    std.fs.read_chunk
      @ (handle: file_handle, size: i32) -> result[bytes, string]
  ...
  std.crypto
    ...
    std.crypto.blake3_update
      @ (hasher: hasher_state, data: bytes) -> hasher_state
      + feeds data into the hasher
      # cryptography
    std.crypto.blake3_finalize
      @ (hasher: hasher_state) -> bytes
...
dedup
  dedup.scan
    ...
    -> std.fs.walk
  dedup.group_by_size
    @ (paths: list[string]) -> result[map[i64, list[string]], string]
  ...
  dedup.find_duplicates
    @ (root: string) -> result[list[list[string]], string]

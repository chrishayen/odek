...
std
  ...
  std.sync
    std.sync.mutex_new
      ...
      # concurrency
    std.sync.mutex_with
      fn (m: mutex_handle, body: fn() -> void) -> void
      + runs body while holding the mutex
      # concurrency

statsd_server
  statsd_server.parse_line
    fn (line: string) -> result[metric, string]
  ...
  statsd_server.ingest
    fn (agg: aggregator_state, m: metric) -> void
  ...
  statsd_server.snapshot_and_reset
    fn (agg: aggregator_state) -> snapshot
  ...
  statsd_server.summarize_timers
    fn (samples: list[f64]) -> timer_summary
  ...
  statsd_server.render_snapshot
    fn (snap: snapshot, prefix: string, now_ms: i64) -> list[metric_point]
    + flattens counters, gauges, timer summaries, and set cardinalities into named points
    # rendering
    -> statsd_server.summarize_timers
  statsd_server.serve
    fn (addr: string, port: u16, sink: fn(list[metric_point]) -> void, flush_ms: i64) -> result[void, string]

...
std
  std.http
    ...
    std.http.client_send
      fn (host: string, port: i32, req: http_request) -> result[http_response, string]
...
gateway
  gateway.new_router
    ...
    # construction
  gateway.add_route
    fn (state: router_state, method: string, path_pattern: string, backends: list[backend_spec], middleware: list[string]) -> router_state
  ...
  gateway.extract_params
    fn (pattern: string, path: string) -> map[string,string]
    + returns the captured path parameters
    # routing
  gateway.run_middleware
    fn (chain: list[string], req: http_request, handlers: map[string, fn(http_request) -> result[http_request,http_response]]) -> result[http_request, http_response]
  ...
  gateway.fanout_backends
    fn (backends: list[backend_spec], req: http_request) -> result[list[http_response], string]
    + sends the request to every backend and collects responses
    - returns error when all backends fail
    # fanout
    -> std.http.client_send
  gateway.merge_responses
    fn (responses: list[http_response], strategy: string) -> http_response
    ...
    -> std.json.encode
  gateway.handle
    fn (state: router_state, raw: bytes, handlers: map[string, fn(http_request) -> result[http_request,http_response]]) -> bytes

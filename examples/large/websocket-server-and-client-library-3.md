...
websocket
  ...
  websocket.server_handshake
    @ (conn: conn_handle) -> result[ws_state, string]
  ...
  websocket.client_connect
    @ (host: string, port: i32, path: string) -> result[ws_state, string]
  ...
  websocket.encode_frame
    @ (opcode: i32, payload: bytes, is_client: bool) -> bytes
  ...
  websocket.decode_frame
    ...
    # framing
  websocket.send_text
    @ (ws: ws_state, message: string) -> result[ws_state, string]
    + sends a text frame over the connection
    # messaging
    -> std.net.write_bytes
  websocket.send_binary
    @ (ws: ws_state, data: bytes) -> result[ws_state, string]
    + sends a binary frame
    # messaging
    -> std.net.write_bytes
  websocket.receive
    @ (ws: ws_state) -> result[tuple[i32, bytes, ws_state], string]
    + reads one complete message, reassembling fragments
    - returns error on protocol violation
    # messaging
    -> std.net.read_bytes
  websocket.send_ping
    @ (ws: ws_state, payload: bytes) -> result[ws_state, string]
    + sends a ping control frame
    # keepalive
  websocket.close
    @ (ws: ws_state, code: i32, reason: string) -> result[void, string]
    + sends a close frame with the given code and reason
    # lifecycle
  websocket.listen
    @ (host: string, port: i32, callback_id: i64) -> result[void, string]

...
std
  ...
  std.random
    std.random.new
      ...
      # random
    std.random.uniform
      fn (rng: rng_state) -> tuple[f32, rng_state]
...
annindex
  ...
  annindex.query
    ...
    -> std.math.l2_distance
  annindex.save
    fn (index: ann_index, path: string) -> result[void, string]
    + serializes the index to a file
    # persistence
    -> std.fs.write_all
  annindex.load
    fn (path: string) -> result[ann_index, string]

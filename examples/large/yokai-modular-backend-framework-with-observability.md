...
std
  std.log
    std.log.info
      fn (message: string, fields: map[string, string]) -> void
  ...
  std.http
    std.http.new_router
      ...
      # http
    std.http.route
      fn (router: http_router, method: string, path: string, handler: http_handler) -> http_router
      + binds a handler to the given method and path
      # http
    std.http.listen
      fn (router: http_router, addr: string) -> result[http_server, string]
      + binds the router to addr and starts serving
      - returns error when addr is already in use
      # http
    std.http.shutdown
      fn (server: http_server) -> result[void, string]
...
yokai
  yokai.new_app
    ...
    # construction
  yokai.register
    fn (app: app_state, key: string, provider: component_provider) -> app_state
    + registers a component provider in the dependency graph
    # di
  yokai.resolve
    fn (app: app_state, key: string) -> result[component_handle, string]
  ...
  yokai.with_tracing
    ...
    -> std.trace.end_span
  yokai.start
    fn (app: app_state, addr: string) -> result[app_state, string]

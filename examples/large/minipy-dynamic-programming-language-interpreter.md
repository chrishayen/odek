...
minipy
  ...
  minipy.new_vm
    ...
    # vm
  minipy.run_module
    @ (vm: vm_state, module: bytecode_module) -> result[value, string]
  ...
  minipy.call_function
    @ (vm: vm_state, name: string, args: list[value]) -> result[value, string]
    + invokes a named function in the current globals
    - returns error when the name is not callable
    # execution
  minipy.define_builtin
    @ (vm: vm_state, name: string, fn: builtin_handle) -> vm_state
    + registers a host-provided function under a name
    # extension
  minipy.load_module
    @ (vm: vm_state, path: string) -> result[bytecode_module, string]

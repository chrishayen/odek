...
std
  std.json
    std.json.parse
      ...
      # serialization
    std.json.get_field
      fn (v: json_value, key: string) -> optional[json_value]
      + returns the named field of an object value
      # serialization
    std.json.as_string
      fn (v: json_value) -> optional[string]
...
spec_router
  spec_router.load_spec
    fn (source: string) -> result[api_spec, string]
  ...
  spec_router.compile_routes
    ...
    # routing
  spec_router.match
    fn (table: route_table, method: string, path: string) -> optional[route_match]
    + returns the operation id and extracted path parameters for a request
    # routing
    -> std.url.split_path
  spec_router.validate_parameters
    fn (spec: api_spec, op_id: string, path_params: map[string, string], query: string) -> result[map[string, string], string]
  ...
  spec_router.validate_body
    fn (spec: api_spec, op_id: string, body: string) -> result[json_value, string]
  ...
  spec_router.register_handler
    fn (r: router_state, op_id: string, handler_id: string) -> result[void, string]
  ...
  spec_router.dispatch
    fn (r: router_state, method: string, path: string, query: string, body: string) -> result[dispatch_result, string]
  ...
  spec_router.list_operations
    fn (spec: api_spec) -> list[operation_summary]

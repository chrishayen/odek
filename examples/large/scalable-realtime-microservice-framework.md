...
rtmf
  rtmf.new_server
    ...
    -> std.net.listen_tcp
  rtmf.register_rpc
    fn (state: server_state, name: string, handler: rpc_handler) -> server_state
    + registers a request/response handler under a public name
    # rpc
  rtmf.register_event
    fn (state: server_state, topic: string) -> server_state
    + declares a topic that clients can subscribe to
    # pub_sub
  rtmf.accept_client
    fn (state: server_state) -> result[tuple[client_handle, server_state], string]
    + accepts a client and assigns it a unique client id and presence entry
    # presence
    -> std.net.accept
    -> std.time.now_millis
  rtmf.dispatch_message
    fn (state: server_state, client: client_handle, raw: string) -> result[server_state, string]
    ...
    -> std.json.parse
  rtmf.call_rpc
    fn (state: server_state, name: string, payload: json_value) -> result[json_value, string]
    + invokes a registered rpc handler and returns its response
    - returns error when no handler is registered for name
    # rpc
  rtmf.subscribe
    fn (state: server_state, client: client_handle, topic: string) -> server_state
    + adds a client to the subscriber set of a topic
    # pub_sub
  rtmf.publish
    fn (state: server_state, topic: string, payload: json_value) -> result[server_state, string]
    + delivers a payload to every subscriber of a topic
    # pub_sub
    -> std.json.encode
  rtmf.presence_list
    fn (state: server_state, topic: string) -> list[string]
    + returns the ids of clients currently subscribed to a topic
    # presence
  rtmf.disconnect
    fn (state: server_state, client: client_handle) -> server_state
    + removes a client from all subscriptions and presence tracking
    # lifecycle
  rtmf.heartbeat_sweep
    fn (state: server_state, stale_after_ms: i64) -> server_state

...
std
  ...
  std.xml
    std.xml.parse_element
      @ (raw: string) -> result[xml_element, string]
      + parses a single XML document root
      - returns error on malformed XML
      # serialization
    std.xml.child_text
      @ (elem: xml_element, tag: string) -> optional[string]
      + returns the text of the first child with the given tag
      # serialization

object_store
  object_store.new_client
    @ (endpoint: string, access_key: string, secret_key: string, region: string) -> client_state
    + returns a client bound to the given credentials
    # construction
  object_store.build_put_request
    @ (client: client_state, bucket: string, key: string, body: bytes) -> signed_request
  ...
  object_store.build_get_request
    @ (client: client_state, bucket: string, key: string) -> signed_request
  ...
  object_store.build_list_request
    @ (client: client_state, bucket: string, prefix: string) -> signed_request

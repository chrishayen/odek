...
model_proxy
  ...
  model_proxy.register_backend
    @ (state: proxy_state, name: string, endpoint: string, weight: i32) -> proxy_state
  ...
  model_proxy.add_route
    @ (state: proxy_state, model: string, backend_names: list[string]) -> proxy_state
    + maps a logical model name to its set of candidate backends
    # routing
  model_proxy.pick_backend
    @ (state: proxy_state, model: string, key: string) -> result[string, string]
  ...
  model_proxy.set_rate_limit
    @ (state: proxy_state, tenant: string, rps: f64) -> proxy_state
    + sets a per-tenant requests-per-second budget
    # policy
  model_proxy.check_rate_limit
    @ (state: proxy_state, tenant: string) -> tuple[bool, proxy_state]
  ...
  model_proxy.mark_healthy
    @ (state: proxy_state, backend: string) -> proxy_state
  ...
  model_proxy.build_request
    @ (model: string, prompt: string, tenant: string) -> string
  ...
  model_proxy.record_trace
    @ (state: proxy_state, tenant: string, backend: string, latency_ms: i64) -> proxy_state
    + appends a trace entry with tenant, backend, and latency
    # observability
    -> std.time.now_millis
  model_proxy.trace_snapshot
    @ (state: proxy_state) -> list[trace_entry]

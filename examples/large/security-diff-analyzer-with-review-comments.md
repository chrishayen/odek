...
std
  ...
  std.regex
    std.regex.compile
      ...
      # regex
    std.regex.find_all
      @ (re: regex, input: string) -> list[string]
      + returns every match of re in input
      # regex

security_review
  security_review.parse_diff
    @ (raw: string) -> result[list[file_diff], string]
  ...
  security_review.add_pattern_rule
    @ (set: rule_set, id: string, severity: string, pattern: string, message: string) -> result[rule_set, string]
  ...
  security_review.scan_file
    @ (set: rule_set, diff: file_diff) -> list[finding]
  ...
  security_review.scan_pull_request
    @ (set: rule_set, diffs: list[file_diff]) -> list[finding]
  ...
  security_review.summarize
    @ (findings: list[finding]) -> map[string, i32]

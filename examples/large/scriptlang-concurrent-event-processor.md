...
std
  std.collections
    std.collections.list_append
      fn (items: list[string], item: string) -> list[string]
...
scriptlang
  ...
  scriptlang.new_interpreter
    fn () -> interpreter_state
    + returns a fresh interpreter with empty bindings and no handlers
    # construction
  scriptlang.define
    fn (state: interpreter_state, name: string, value: value) -> interpreter_state
    + binds the name to a value in the global scope
    + overwrites any existing binding
    # environment
  scriptlang.eval
    fn (state: interpreter_state, node: ast_node) -> result[tuple[value, interpreter_state], string]
  ...
  scriptlang.register_handler
    fn (state: interpreter_state, event_kind: string, handler: ast_node) -> interpreter_state
    + associates the handler with the event kind
    + allows multiple handlers per kind; order of registration is preserved
    # events
  scriptlang.dispatch
    fn (state: interpreter_state, event_kind: string, payload: value) -> result[tuple[list[value], interpreter_state], string]
  ...
  scriptlang.run_program
    fn (state: interpreter_state, source: string) -> result[interpreter_state, string]

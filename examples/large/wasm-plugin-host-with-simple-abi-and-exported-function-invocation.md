...
plugin_host
  ...
  plugin_host.register_host_function
    fn (state: host_state, name: string, impl: host_fn) -> host_state
    + registers a host function callable from wasm
    # registration
  plugin_host.load_from_file
    fn (state: host_state, name: string, path: string) -> result[host_state, string]
  ...
  plugin_host.call_with_bytes
    fn (state: host_state, name: string, export_name: string, input: bytes) -> result[bytes, string]
  ...
  plugin_host.call_with_string
    fn (state: host_state, name: string, export_name: string, input: string) -> result[string, string]
    + convenience wrapper over call_with_bytes for UTF-8 strings
    # invocation
    -> plugin_host.call_with_bytes
  plugin_host.unload
    fn (state: host_state, name: string) -> host_state
    + drops a plug-in instance and its memory
    # teardown
  plugin_host.list_plugins
    fn (state: host_state) -> list[string]

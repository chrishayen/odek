...
workflow
  workflow.new_dag
    @ (name: string) -> dag_state
    + returns an empty DAG with the given name
    # construction
  workflow.add_task
    @ (dag: dag_state, task_id: string, tag: string) -> result[dag_state, string]
    + adds a task node identified by task_id
    - returns error when task_id already exists
    # authoring
  workflow.add_dependency
    @ (dag: dag_state, upstream: string, downstream: string) -> result[dag_state, string]
  ...
  workflow.start_run
    ...
    -> std.time.now_seconds
  workflow.mark_task
    @ (run: run_state, task_id: string, status: string) -> result[run_state, string]

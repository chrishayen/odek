...
http_server
  http_server.route_match
    fn (routes: list[route], method: string, path: string) -> optional[request_handler]
  ...
  http_server.handle_connection
    fn (conn: tcp_conn, routes: list[route]) -> result[void, string]
    ...
    -> std.http.format_response
  http_server.serve
    fn (port: i32, routes: list[route]) -> result[void, string]

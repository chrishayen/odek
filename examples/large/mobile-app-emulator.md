...
std
  ...
  std.gfx
    std.gfx.create_surface
      fn (width: i32, height: i32) -> surface_handle
      + allocates a pixel surface for rendering
      # graphics
    std.gfx.blit
      fn (dst: surface_handle, src: bytes, x: i32, y: i32, w: i32, h: i32) -> void
      + copies src pixels into the surface at (x, y)
      # graphics
    std.gfx.present
      fn (surface: surface_handle) -> void
      + displays the surface on the host window
      # graphics
  std.audio
    std.audio.open_output
      fn (sample_rate: i32, channels: i32) -> audio_handle
      + opens a PCM audio output stream
      # audio
    std.audio.queue_pcm
      fn (handle: audio_handle, samples: bytes) -> void
      + queues PCM samples for playback
      # audio

emulator
  emulator.load_bundle
    fn (path: string) -> result[app_bundle, string]
  ...
  emulator.parse_macho
    fn (binary: bytes) -> result[macho_image, string]

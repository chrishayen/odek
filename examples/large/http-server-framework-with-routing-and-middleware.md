...
std
  ...
  std.http
    ...
    std.http.write_response
      @ (conn: conn_state, resp: http_response) -> result[void, string]
...
server
  ...
  server.handle
    ...
    # routing
  server.use
    @ (router: router_state, middleware: middleware_fn) -> router_state
    + appends middleware to the pipeline in registration order
    # middleware
  server.group
    @ (router: router_state, prefix: string) -> router_state
    + returns a router whose subsequent routes are prefixed and inherit current middleware
    # grouping
  server.match_route
    @ (router: router_state, method: string, path: string) -> result[tuple[handler_fn, map[string,string]], string]
  ...
  server.text_response
    ...
    # response_helpers
  server.redirect
    @ (status: i32, location: string) -> http_response

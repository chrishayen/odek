...
std
  ...
  std.url
    std.url.decode
      fn (encoded: string) -> result[string, string]
...
rest_framework
  ...
  rest_framework.route
    fn (router: router_state, method: string, pattern: string, handler: fn(request) -> response) -> router_state
    + registers a handler for the given method and path pattern
    ? pattern supports ":name" segments that become path parameters
    # routing
  rest_framework.use
    fn (router: router_state, mw: fn(request, fn(request) -> response) -> response) -> router_state
    + appends a middleware applied to every matching route
    # middleware
  rest_framework.match_route
    fn (router: router_state, method: string, path: string) -> result[tuple[handler, map[string, string]], string]
  ...
  rest_framework.dispatch
    fn (router: router_state, req: request) -> response
  ...
  rest_framework.error_response
    fn (status: i32, message: string) -> response
    + builds a standard error response with status and message fields
    # responses
    -> std.json.encode_object
  rest_framework.negotiate_content
    fn (accept_header: string, offered: list[string]) -> optional[string]

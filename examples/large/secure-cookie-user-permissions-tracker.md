...
std
  std.crypto
    std.crypto.bcrypt_hash
      @ (password: string, cost: i32) -> string
...
permissions
  ...
  permissions.has_permission
    @ (state: store_state, username: string, permission: string) -> bool
    + reports whether the user holds the permission
    # permission_query
  permissions.verify_password
    @ (state: store_state, username: string, password: string) -> bool
    + reports whether the password matches the stored hash
    # authentication
    -> std.crypto.bcrypt_verify
  permissions.issue_cookie
    @ (state: store_state, username: string, ttl_seconds: i64) -> string
  ...
  permissions.parse_cookie
    @ (state: store_state, cookie: string) -> result[string, string]
    ...
    -> std.time.now_seconds
  permissions.login
    @ (state: store_state, username: string, password: string, ttl_seconds: i64) -> result[string, string]
    + verifies credentials and issues a fresh cookie
    - returns error when credentials are invalid
    # login
  permissions.logout
    @ (state: store_state, cookie: string) -> store_state

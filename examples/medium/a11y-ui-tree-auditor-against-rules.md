...
a11y
  a11y.rule_register
    fn (id: string, description: string, check_fn: fn(node) -> list[violation]) -> result[void, string]
  ...
  a11y.rule_unregister
    fn (id: string) -> result[void, string]
    + removes a previously registered rule
    - returns error when no rule with the id is registered
    # registry
  a11y.audit
    fn (root: node, rule_ids: list[string]) -> result[audit_report, string]
    ...
    -> a11y.rule_register
  a11y.report_count
    fn (report: audit_report) -> i32
    + returns the total number of violations across all rules
    # report
  a11y.report_by_severity
    fn (report: audit_report, severity: string) -> list[violation]

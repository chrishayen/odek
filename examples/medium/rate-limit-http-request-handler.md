...
http_rate_limit
  http_rate_limit.new
    @ (rate_per_sec: f64, burst: i32) -> rate_limit_state
    + creates a limiter with the given refill rate and burst capacity per key
    # construction
  http_rate_limit.allow
    @ (state: rate_limit_state, key: string) -> tuple[bool, rate_limit_state]
  ...
  http_rate_limit.middleware
    @ (state: rate_limit_state, key_fn: fn(http_request) -> string, inner: fn(http_request) -> http_response) -> fn(http_request) -> http_response
  ...
  http_rate_limit.set_response
    @ (state: rate_limit_state, status: i32, body: string) -> rate_limit_state

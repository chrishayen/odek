...
decision
  decision.new_tree
    ...
    # construction
  decision.add_branch
    fn (state: tree_state, parent_path: list[i32], condition: string) -> result[tuple[list[i32], tree_state], string]
  ...
  decision.set_outcome
    fn (state: tree_state, path: list[i32], label: string) -> result[tree_state, string]
    + marks a node as a leaf with the given outcome label
    - returns error when the path does not resolve
    # tree_building
  decision.evaluate
    fn (state: tree_state, facts: map[string, string]) -> result[string, string]
    ...
    # evaluation
  decision.trace
    fn (state: tree_state, facts: map[string, string]) -> result[list[string], string]
  ...
  decision.parse_condition
    fn (source: string) -> result[condition_expr, string]

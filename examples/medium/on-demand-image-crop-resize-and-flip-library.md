...
std
  std.image
    ...
    std.image.encode_jpeg
      fn (img: image, quality: i32) -> result[bytes, string]
...
imaging
  imaging.crop
    ...
    -> std.image.geom.height
  imaging.resize
    fn (img: image, target_w: i32, target_h: i32) -> result[image, string]
    ...
    # transform_resize
  imaging.fit
    fn (img: image, max_w: i32, max_h: i32) -> image
  ...
  imaging.flip_vertical
    ...
    # transform_flip
  imaging.process
    fn (source: bytes, ops: list[imaging_op], out_format: string, quality: i32) -> result[bytes, string]

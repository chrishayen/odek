...
leak_scanner
  leak_scanner.load_sources
    @ (root: string, extension: string) -> result[list[source_file], string]
  ...
  leak_scanner.find_logging_calls
    @ (mods: list[ast_module], logger_names: list[string]) -> list[call_site]
    + returns every call whose receiver or function name appears in logger_names
    # discovery
  leak_scanner.analyze_call
    @ (call: call_site, sensitive: map[string, list[string]]) -> list[finding]
    ...
    # analysis
  leak_scanner.scan
    @ (root: string, marker: string, logger_names: list[string]) -> result[list[finding], string]

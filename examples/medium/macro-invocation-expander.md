...
macroexp
  macroexp.new_table
    ...
    # construction
  macroexp.define
    @ (table: macro_table, name: string, params: list[string], body: string) -> macro_table
    + registers a macro with named parameters and a body template
    # registration
  macroexp.find_invocation
    @ (source: string, start: i32) -> optional[invocation]
    ...
    # parsing
  macroexp.substitute
    @ (body: string, params: list[string], args: list[string]) -> string
  ...
  macroexp.expand_once
    @ (source: string, table: macro_table) -> string
  ...
  macroexp.expand_fully
    @ (source: string, table: macro_table, max_depth: i32) -> result[string, string]

...
dataframe
  dataframe.new
    fn (columns: list[column_spec]) -> dataframe
    + creates an empty dataframe with the given named, typed columns
    # construction
  dataframe.append_row
    fn (df: dataframe, row: list[cell]) -> result[dataframe, string]
  ...
  dataframe.select
    ...
    # projection
  dataframe.filter
    fn (df: dataframe, predicate: row_predicate) -> dataframe
    + returns rows for which the predicate is true
    # filtering
  dataframe.group_by
    fn (df: dataframe, key_col: string) -> result[map[string, dataframe], string]
  ...
  dataframe.aggregate_sum
    fn (df: dataframe, col: string) -> result[f64, string]

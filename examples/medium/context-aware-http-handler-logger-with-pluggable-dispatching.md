...
structlog
  structlog.new_logger
    ...
    # construction
  structlog.add_sink
    fn (logger: logger_state, sink_id: string, handler: sink_fn) -> logger_state
  ...
  structlog.with_request
    fn (logger: logger_state, method: string, path: string, request_id: string) -> logger_state
    + attaches common HTTP fields to the logger's context
    # context
  structlog.log
    fn (logger: logger_state, level: string, message: string) -> void
  ...
  structlog.set_min_level
    fn (logger: logger_state, level: string) -> logger_state

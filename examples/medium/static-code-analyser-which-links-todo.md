...
todo_linker
  todo_linker.scan_file
    @ (path: string, content: string) -> list[todo_item]
  ...
  todo_linker.extract_issue_id
    @ (text: string, patterns: list[string]) -> optional[string]
  ...
  todo_linker.scan_tree
    @ (root: string, patterns: list[string]) -> result[list[todo_item], string]
  ...
  todo_linker.resolve_statuses
    @ (items: list[todo_item], fetch: fn(string) -> result[issue_status, string]) -> list[resolved_todo]
  ...
  todo_linker.select_violations
    @ (resolved: list[resolved_todo]) -> list[resolved_todo]
  ...
  todo_linker.format_report
    @ (violations: list[resolved_todo]) -> string

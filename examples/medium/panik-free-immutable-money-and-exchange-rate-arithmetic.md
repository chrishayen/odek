...
money
  money.new
    fn (units: i64, nanos: i32, currency: string) -> result[money_amount, string]
  ...
  money.sub
    ...
    # arithmetic
  money.mul_scalar
    fn (m: money_amount, factor_num: i64, factor_den: i64) -> result[money_amount, string]
    + returns m * (num/den) with half-even rounding
    - returns error when den is zero
    # arithmetic
  money.compare
    fn (a: money_amount, b: money_amount) -> result[i32, string]
    + returns -1, 0, or 1
    - returns error when currencies differ
    # comparison
  ...
  money.rate_table_set
    fn (t: rate_table, from: string, to: string, num: i64, den: i64) -> result[rate_table, string]
    + returns a new table with the rate from->to set to num/den
    - returns error when den is zero
    # rate_table
  money.exchange
    fn (m: money_amount, target: string, t: rate_table) -> result[money_amount, string]

...
std
  std.http
    std.http.forward
      fn (url: string, method: string, headers: map[string,string], body: bytes) -> result[http_response, string]
  ...
  std.json
    std.json.parse
      fn (raw: bytes) -> result[json_value, string]
...
record_proxy
  record_proxy.new
    fn (mode: string) -> proxy_state
    + returns a proxy in "record" or "simulate" mode with no captures
    - returns an empty state for an unknown mode string
    # construction
  record_proxy.handle
    fn (state: proxy_state, req: http_request) -> result[tuple[http_response, proxy_state], string]
    ...
    -> std.http.forward
  record_proxy.save
    fn (state: proxy_state, path: string) -> result[void, string]
    + serializes captured pairs to a json file
    # persistence
    -> std.json.encode
    -> std.io.write_file
  record_proxy.load
    fn (path: string) -> result[proxy_state, string]
  ...
  record_proxy.add_request_hook
    fn (state: proxy_state, hook: request_hook) -> proxy_state
    + appends a middleware invoked on every incoming request before matching
    # middleware
  record_proxy.add_response_hook
    fn (state: proxy_state, hook: response_hook) -> proxy_state

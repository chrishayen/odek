...
std
  std.time
    std.time.now_seconds
      ...
      # time
    std.time.compose
      fn (year: i32, month: i32, day: i32, hour: i32, minute: i32, second: i32) -> result[i64, string]
...
dateparser
  ...
  dateparser.enable_language
    fn (state: dateparser_state, lang: string) -> dateparser_state
    + enables pattern tables for the given language code
    # registration
  dateparser.tokenize
    fn (state: dateparser_state, input: string) -> list[token]
    + splits input into normalized words, numbers, and punctuation
    # tokenization
  dateparser.match_relative
    fn (state: dateparser_state, tokens: list[token]) -> optional[i64]
  ...
  dateparser.match_absolute
    ...
    -> std.time.compose
  dateparser.parse
    fn (state: dateparser_state, input: string) -> result[i64, string]

...
csv_utils
  csv_utils.from_text
    @ (text: string, has_header: bool) -> result[table, string]
  ...
  csv_utils.select_columns
    @ (t: table, names: list[string]) -> result[table, string]
  ...
  csv_utils.filter_rows
    @ (t: table, pred: fn(map[string, string]) -> bool) -> table
    + returns a table with only the rows for which pred returns true
    # selection
  csv_utils.convert_column
    @ (t: table, column: string, target: string) -> result[table, string]
  ...
  csv_utils.summarize_column
    @ (t: table, column: string) -> result[column_summary, string]
    + returns count, nulls, min, max, and mean for a numeric column
    - returns error when the column is not numeric
    # analytics
  csv_utils.join
    @ (left: table, right: table, on: string) -> result[table, string]

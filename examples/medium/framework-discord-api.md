...
chatapi
  chatapi.new
    @ (token: string, base_url: string) -> client_state
  ...
  chatapi.send_message
    @ (state: client_state, channel_id: string, content: string) -> result[string, string]
  ...
  chatapi.fetch_history
    @ (state: client_state, channel_id: string, limit: i32) -> result[list[map[string,string]], string]
  ...
  chatapi.register_handler
    @ (state: client_state, event_type: string, handler_id: string) -> client_state
    + binds a handler id to an inbound event type
    # event_handlers
  chatapi.dispatch_event
    @ (state: client_state, raw_event: string) -> result[string, string]

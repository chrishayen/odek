...
std
  std.net
    std.net.dial_tcp
      ...
      # networking
    std.net.send
      @ (sock: socket, data: bytes) -> result[void, string]
      + sends data on the socket
      # networking
    std.net.recv
      @ (sock: socket, max: i32) -> result[bytes, string]
      + reads up to max bytes from the socket
      # networking
  std.random
    std.random.token_hex
      @ (bytes_count: i32) -> string
      + returns a random hex token of the given byte length
      # randomness

tunnel
  tunnel.open
    @ (relay_host: string, relay_port: i32, local_port: i32) -> result[tunnel_state, string]
    ...
    -> std.random.token_hex
  tunnel.public_url
    @ (t: tunnel_state) -> string
    + returns the public URL assigned by the relay
    # query
  tunnel.next_request
    @ (t: tunnel_state) -> result[optional[tunnel_request], string]
  ...
  tunnel.forward_response
    @ (t: tunnel_state, req_id: string, response: bytes) -> result[void, string]
    + sends a response frame back to the relay for the given request id
    # forwarding
    -> std.net.send
  tunnel.close
    @ (t: tunnel_state) -> result[void, string]

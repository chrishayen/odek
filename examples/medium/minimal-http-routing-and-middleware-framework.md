...
web
  web.new_router
    ...
    # construction
  web.use
    @ (r: router_state, middleware: fn(request, next_fn) -> response) -> router_state
  ...
  web.route
    ...
    # registration
  web.group
    @ (r: router_state, prefix: string, build: fn(router_state) -> router_state) -> router_state
    + applies build inside a sub-router whose routes are prefixed
    # grouping
  web.match
    @ (r: router_state, method: string, path: string) -> optional[matched_route]
    + returns the matching route with extracted path parameters, or none
    # matching
  web.handle
    @ (r: router_state, req: request) -> response

...
std
  ...
  std.sync
    std.sync.mutex_new
      ...
      # concurrency
    std.sync.mutex_with
      fn (m: mutex, body: fn() -> void) -> void
      + runs body while holding the mutex
      # concurrency

prep
  prep.wrap
    fn (db_handle: db_handle) -> prep_state
    + wraps a database handle with an empty statement cache
    # construction
  prep.get_or_prepare
    fn (state: prep_state, query: string) -> result[stmt_handle, string]
    ...
    -> std.sync.mutex_with
  prep.exec
    fn (state: prep_state, query: string, args: list[string]) -> result[i64, string]
    ...
    # execution
  prep.query
    fn (state: prep_state, query: string, args: list[string]) -> result[list[map[string, string]], string]
    + runs the prepared statement and returns rows as string-to-string maps
    - returns error when the query fails
    # querying
  prep.close
    fn (state: prep_state) -> result[void, string]

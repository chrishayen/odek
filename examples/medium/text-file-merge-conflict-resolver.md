...
conflicts
  conflicts.parse
    fn (text: string) -> result[list[conflict_block], string]
    ...
    # parsing
  conflicts.load
    fn (path: string) -> result[list[conflict_block], string]
    + reads the file and parses its conflict blocks
    # loading
    -> std.fs.read_all
    -> conflicts.parse
  conflicts.resolve
    fn (blocks: list[conflict_block], choices: list[conflict_choice]) -> result[string, string]
    ...
    # resolution
  conflicts.save
    fn (path: string, resolved: string) -> result[void, string]

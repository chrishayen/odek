...
std
  std.fs
    std.fs.read_all
      ...
      # io
    std.fs.open_reader
      fn (path: string) -> result[byte_reader, string]
  ...
  std.audio_codec
    ...
    std.audio_codec.decode_stream
      fn (reader: byte_reader) -> result[audio_stream, string]
...
ears
  ears.load_sound
    fn (path: string) -> result[sound_handle, string]
    ...
    -> std.audio_codec.decode_all
  ears.play_sound
    fn (device: audio_device, sound: sound_handle) -> result[void, string]
    + writes the decoded samples to the device
    # sounds
    -> std.audio_device.write
  ears.open_music
    fn (path: string) -> result[music_handle, string]
    ...
    -> std.audio_codec.decode_stream
  ears.pump_music
    fn (device: audio_device, music: music_handle, max_samples: i32) -> result[bool, string]
    + decodes and writes up to max_samples to the device; returns false when the stream is exhausted
    # music
    -> std.audio_device.write
  ears.close_music
    fn (music: music_handle) -> result[void, string]

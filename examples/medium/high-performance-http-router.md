...
router
  router.new
    ...
    # construction
  router.add
    fn (state: router_state, method: string, pattern: string, handler_id: string) -> result[router_state, string]
    ...
    -> std.strings.split
  router.lookup
    fn (state: router_state, method: string, path: string) -> result[route_match, string]
    + returns handler id and captured parameters on match
    - returns error when no route matches
    # routing
    -> std.strings.split
  router.allowed_methods
    fn (state: router_state, path: string) -> list[string]

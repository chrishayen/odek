...
std
  std.image
    std.image.new_bitmap
      @ (width: i32, height: i32) -> bitmap
      + returns a bitmap filled with white pixels
      # image
    std.image.set_pixel
      @ (bm: bitmap, x: i32, y: i32, black: bool) -> bitmap
...
code
  code.encode_qr
    @ (text: string, error_level: string) -> result[matrix, string]
    ...
    # qr_encoding
  code.encode_barcode
    @ (symbology: string, value: string) -> result[list[bool], string]
    ...
    # barcode_encoding
  code.render_qr_png
    @ (matrix: matrix, module_pixels: i32) -> bytes
  ...
  code.render_barcode_png
    @ (pattern: list[bool], height: i32, module_pixels: i32) -> bytes
    ...
    -> std.image.encode_png
  code.render_qr_svg
    @ (matrix: matrix, module_pixels: i32) -> string

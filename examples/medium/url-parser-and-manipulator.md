...
url_lib
  url_lib.parse
    ...
    # parsing
  url_lib.format
    fn (parts: url_parts) -> string
    + reassembles the parts into a canonical URL string
    # formatting
  url_lib.set_path
    fn (parts: url_parts, path: string) -> url_parts
    + returns parts with the path replaced
    # mutation
  url_lib.get_query
    fn (parts: url_parts, key: string) -> optional[string]
    + returns the first query value for the key
    - returns none when the key is absent
    # query
  url_lib.set_query
    fn (parts: url_parts, key: string, value: string) -> url_parts
    + returns parts with the query value added or replaced
    # query
  url_lib.remove_query
    fn (parts: url_parts, key: string) -> url_parts
    + returns parts with all values for the key removed
    # query
  url_lib.join
    fn (base: url_parts, reference: string) -> result[url_parts, string]

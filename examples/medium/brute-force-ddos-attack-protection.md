...
protect
  protect.new
    @ (max_attempts: i32, window_seconds: i64, block_seconds: i64) -> protect_state
    + creates a protector with a sliding window and block duration
    # construction
  protect.record_attempt
    @ (state: protect_state, key: string) -> protect_state
    + adds a timestamped attempt for the given key
    # tracking
    -> std.time.now_seconds
  protect.is_blocked
    @ (state: protect_state, key: string) -> bool
  ...
  protect.prune_expired
    @ (state: protect_state) -> protect_state

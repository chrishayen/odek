...
profile_manager
  profile_manager.load_store
    @ (path: string) -> result[profile_store, string]
  ...
  profile_manager.save_store
    @ (store: profile_store, path: string) -> result[void, string]
  ...
  profile_manager.upsert_profile
    @ (store: profile_store, name: string, config: map[string, string]) -> profile_store
    + inserts or replaces the named profile
    # editing
  profile_manager.remove_profile
    @ (store: profile_store, name: string) -> result[profile_store, string]
    + removes the named profile
    - returns error when the profile does not exist
    # editing
  profile_manager.activate
    @ (store: profile_store, name: string, target_path: string) -> result[profile_store, string]
  ...
  profile_manager.active_profile
    @ (store: profile_store) -> optional[string]
    + returns the currently active profile name, if any
    # introspection
  profile_manager.list_profiles
    @ (store: profile_store) -> list[string]

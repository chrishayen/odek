...
std
  ...
  std.bitstream
    ...
    std.bitstream.write_bits
      fn (w: bit_writer, value: u64, bit_count: i32) -> bit_writer
...
av1_encoder
  ...
  av1_encoder.transform_quantize
    fn (state: encoder_state, residual: list[i16]) -> list[i16]
    + applies DCT and quantization to a residual block
    # transform
    -> std.dsp.dct_8x8
  av1_encoder.encode_frame
    fn (state: encoder_state, frame: raw_frame) -> tuple[encoder_state, bytes]

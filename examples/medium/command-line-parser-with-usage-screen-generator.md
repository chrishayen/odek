...
argparse
  argparse.new
    @ (program: string, description: string) -> parser
    + creates an empty parser with metadata for usage rendering
    # construction
  argparse.option
    @ (p: parser, long: string, help: string, takes_value: bool) -> parser
    + registers a named option
    # option_registration
  argparse.positional
    @ (p: parser, name: string, help: string) -> parser
    + registers a required positional argument in order
    # positional_registration
  argparse.parse
    @ (p: parser, argv: list[string]) -> result[parse_result, string]

...
remote_input
  ...
  remote_input.new_session
    @ (device: device_handle, pointer_speed: f32) -> session_state
    + creates a session bound to a device handle with a pointer speed multiplier
    # construction
  remote_input.apply_intent
    @ (session: session_state, intent: input_intent) -> result[session_state, string]
  ...
  remote_input.type_text
    @ (session: session_state, text: string) -> result[session_state, string]

...
deploy
  deploy.app_spec
    fn (name: string, image: string, replicas: i32, env: map[string, string]) -> app_spec
  ...
  deploy.register_provider
    fn (name: string, apply_fn: provider_apply_fn, destroy_fn: provider_destroy_fn, status_fn: provider_status_fn) -> result[void, string]
    + registers a named provider driver
    - returns error when a provider with the same name is already registered
    # registry
  deploy.plan
    fn (current: list[app_spec], desired: list[app_spec]) -> deployment_plan
    + returns a plan containing create, update, and delete actions
    + apps are matched by name between current and desired lists
    # planning
  deploy.apply
    fn (provider_name: string, plan: deployment_plan) -> result[deployment_result, string]
    ...
    -> deploy.register_provider
  deploy.destroy
    fn (provider_name: string, app_name: string) -> result[void, string]
    + tears down a previously deployed application
    - returns error when the provider is unknown
    # execution
    -> deploy.register_provider
  deploy.status
    fn (provider_name: string, app_name: string) -> result[app_status, string]

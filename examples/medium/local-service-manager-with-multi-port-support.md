...
std
  std.net
    std.net.tcp_probe
      fn (host: string, port: u16, timeout_ms: i32) -> bool
      + returns true when a TCP connection to host:port succeeds within the timeout
      # networking
  std.process
    std.process.spawn
      fn (command: string, args: list[string], env: map[string, string]) -> result[process_handle, string]
...
service_manager
  ...
  service_manager.register
    fn (state: manager_state, name: string, port: u16, command: string, args: list[string]) -> result[manager_state, string]
  ...
  service_manager.start
    fn (state: manager_state, name: string) -> result[manager_state, string]

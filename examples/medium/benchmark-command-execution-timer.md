...
std
  ...
  std.process
    std.process.run
      fn (argv: list[string]) -> result[i32, string]
...
benchmark
  benchmark.time_once
    fn (argv: list[string]) -> result[i64, string]
  ...
  benchmark.run_samples
    fn (argv: list[string], samples: i32) -> result[list[i64], string]
    + returns one duration per sample
    - returns error when samples is less than 1
    # sampling
    -> benchmark.time_once
  benchmark.summarize
    fn (durations: list[i64]) -> benchmark_summary

...
raytracer
  raytracer.vec3
    @ (x: f64, y: f64, z: f64) -> vec3_value
    + returns a 3-component vector
    # math
  raytracer.vec3_dot
    @ (a: vec3_value, b: vec3_value) -> f64
    + returns the dot product
    # math
  raytracer.vec3_normalize
    @ (v: vec3_value) -> vec3_value
  ...
  raytracer.scene_add_sphere
    @ (s: scene_state, center: vec3_value, radius: f64, color: vec3_value) -> scene_state
    + returns a new scene with a sphere added
    # scene
  raytracer.scene_add_light
    @ (s: scene_state, position: vec3_value, intensity: f64) -> scene_state
    + returns a new scene with a point light added
    # scene
  raytracer.trace_ray
    @ (s: scene_state, origin: vec3_value, direction: vec3_value, depth: i32) -> vec3_value
    ...
    -> raytracer.vec3_normalize
  raytracer.render
    @ (s: scene_state, width: i32, height: i32) -> image_buffer

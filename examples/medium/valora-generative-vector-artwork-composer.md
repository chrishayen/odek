...
valora
  ...
  valora.fill_background
    fn (c: canvas_state, r: u8, g: u8, b: u8) -> canvas_state
    + fills every pixel with the given color
    # shading
  valora.draw_circle
    fn (c: canvas_state, cx: f64, cy: f64, radius: f64, r: u8, g: u8, b: u8) -> canvas_state
    + draws a filled circle, clipped to the canvas bounds
    # drawing
  valora.draw_polygon
    fn (c: canvas_state, points: list[tuple[f64, f64]], r: u8, g: u8, b: u8) -> canvas_state
    + draws a filled polygon defined by ordered vertices
    # drawing
  valora.rotate_points
    fn (points: list[tuple[f64, f64]], center: tuple[f64, f64], radians: f64) -> list[tuple[f64, f64]]
    + returns the points rotated around the center by the given angle
    # transform
    -> std.math.sin
    -> std.math.cos
  valora.jitter_color
    fn (rng: rng_state, r: u8, g: u8, b: u8, amount: f64) -> tuple[tuple[u8, u8, u8], rng_state]
    + perturbs each channel by a uniform offset scaled by amount
    # shading
    -> std.random.uniform
  valora.pixels
    fn (c: canvas_state) -> bytes

...
kv_client
  ...
  kv_client.read_reply
    ...
    -> std.net.read_exact
  kv_client.get
    @ (state: client_state, key: string) -> result[optional[string], string]
    + returns the value at key or none when missing
    # commands
    -> std.net.write_all
  kv_client.set
    @ (state: client_state, key: string, value: string) -> result[void, string]
    + stores value at key
    # commands
    -> std.net.write_all
  kv_client.del
    @ (state: client_state, keys: list[string]) -> result[i32, string]

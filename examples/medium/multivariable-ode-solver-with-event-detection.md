...
odesolve
  odesolve.new_system
    @ (initial_state: list[f64], t0: f64) -> ode_state
    + creates a system with the given initial variable values at time t0
    # construction
  odesolve.step_rk4
    @ (state: ode_state, dt: f64, deriv: list[f64]) -> ode_state
    ...
    # integration
  odesolve.integrate
    @ (state: ode_state, dt: f64, steps: i32, deriv_seq: list[list[f64]]) -> ode_state
  ...
  odesolve.detect_zero_crossing
    @ (prev_value: f64, curr_value: f64, prev_t: f64, curr_t: f64) -> optional[f64]
  ...
  odesolve.record_event
    @ (state: ode_state, t: f64, tag: string) -> ode_state
    + appends an event with its time and tag to the state
    # events
  odesolve.trajectory
    @ (state: ode_state) -> list[tuple[f64, list[f64]]]

...
std
  std.random
    ...
    std.random.next_in_range
      @ (state: rng_state, lo: i32, hi_exclusive: i32) -> tuple[i32, rng_state]
...
fuzzgen
  ...
  fuzzgen.define_literal
    @ (g: grammar, name: string, value: string) -> grammar
    + registers a rule that always expands to the given literal
    # rule
  fuzzgen.define_alt
    @ (g: grammar, name: string, choices: list[string]) -> grammar
    + registers a rule that picks uniformly from a list of rule references
    # rule
  fuzzgen.define_sequence
    @ (g: grammar, name: string, parts: list[string]) -> grammar
    + registers a rule that concatenates the expansions of each referenced rule
    # rule
  fuzzgen.expand
    @ (g: grammar, start: string, state: rng_state, max_depth: i32) -> result[tuple[string, rng_state], string]

...
config_cascade
  ...
  config_cascade.add_backend
    @ (state: cascade_state, name: string, values: map[string, string]) -> cascade_state
  ...
  config_cascade.resolve
    @ (state: cascade_state) -> map[string, string]
    ...
    # merge
  config_cascade.get
    @ (state: cascade_state, key: string) -> optional[string]

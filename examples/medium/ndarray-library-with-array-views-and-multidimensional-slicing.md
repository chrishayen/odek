...
ndarray
  ndarray.new
    @ (shape: list[i32], fill: f64) -> ndarray_state
    ...
    # construction
  ndarray.from_data
    @ (shape: list[i32], data: list[f64]) -> result[ndarray_state, string]
    + wraps an existing flat buffer
    - returns error when len(data) != product(shape)
    # construction
  ndarray.view
    @ (state: ndarray_state) -> ndarray_view
    + returns a full view over the array without copying
    # views
  ndarray.slice
    @ (view: ndarray_view, ranges: list[range]) -> result[ndarray_view, string]
    ...
    # slicing
  ndarray.reshape
    @ (view: ndarray_view, new_shape: list[i32]) -> result[ndarray_view, string]

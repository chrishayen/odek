...
supervisor
  supervisor.new_child_spec
    fn (id: string, start: function[void, child_state], restart: string) -> child_spec
    + builds a child spec with an id, start function, and restart policy string ("permanent", "transient", or "temporary")
    # configuration
  supervisor.new
    fn (strategy: string, children: list[child_spec]) -> result[sup_state, string]
  ...
  supervisor.start_all
    fn (sup: sup_state) -> sup_state
    + starts every child in order and stores their running state
    # lifecycle
  supervisor.handle_exit
    fn (sup: sup_state, failed_id: string, reason: string) -> result[tuple[sup_state, list[string]], string]
  ...
  supervisor.intensity_exceeded
    fn (sup: sup_state, max_restarts: i32, window_ms: i64) -> bool

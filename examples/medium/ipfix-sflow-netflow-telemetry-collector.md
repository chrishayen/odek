...
flow_collector
  flow_collector.decode_netflow_v5
    fn (wire: bytes) -> result[list[flow_record], string]
  ...
  flow_collector.decode_ipfix
    fn (wire: bytes, templates: map[i32, ipfix_template]) -> result[tuple[list[flow_record], map[i32, ipfix_template]], string]
  ...
  flow_collector.new_aggregator
    fn (flush_interval_sec: i64) -> aggregator_state
    + creates a flow aggregator with a flush cadence
    # construction
  flow_collector.ingest
    fn (state: aggregator_state, records: list[flow_record]) -> aggregator_state
    + merges records into existing flows keyed by 5-tuple
    # aggregation
    -> std.time.now_seconds
  flow_collector.flush_ready
    fn (state: aggregator_state) -> tuple[list[flow_record], aggregator_state]

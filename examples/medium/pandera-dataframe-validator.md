...
pandera
  pandera.column
    @ (name: string, dtype: string) -> column_spec
  ...
  pandera.with_nullable
    @ (spec: column_spec, nullable: bool) -> column_spec
    + sets whether the column permits null values
    # schema
  pandera.with_range
    @ (spec: column_spec, min: f64, max: f64) -> column_spec
    + attaches an inclusive numeric range constraint
    # schema
  pandera.with_regex
    @ (spec: column_spec, pattern: string) -> column_spec
    + attaches a regex constraint applied to every cell
    # schema
  pandera.with_unique
    @ (spec: column_spec) -> column_spec
    + requires the column to contain no duplicates
    # schema
  pandera.schema
    @ (columns: list[column_spec]) -> schema_state
    + assembles a set of column specs into a schema
    # schema
  pandera.validate
    @ (schema: schema_state, frame: dataframe) -> result[void, list[validation_error]]

...
mux_bus
  mux_bus.new
    fn (queue_capacity: i32) -> mux_bus_state
    + creates a bus with no producers, no consumers, and a bounded intake channel
    # construction
    -> std.sync.channel_new
  mux_bus.register_producer
    fn (state: mux_bus_state, name: string) -> tuple[producer_handle, mux_bus_state]
    + assigns an opaque producer handle and records its name
    # registration
  mux_bus.register_consumer
    fn (state: mux_bus_state, name: string) -> tuple[consumer_handle, mux_bus_state]
    + creates a dedicated outbound channel for the consumer
    # registration
    -> std.sync.channel_new
  mux_bus.publish
    fn (state: mux_bus_state, producer: producer_handle, payload: bytes) -> result[void, string]
  ...
  mux_bus.run_dispatch
    fn (state: mux_bus_state) -> result[void, string]
    ...
    -> std.sync.channel_send
  mux_bus.consume
    fn (state: mux_bus_state, consumer: consumer_handle) -> result[bytes, string]

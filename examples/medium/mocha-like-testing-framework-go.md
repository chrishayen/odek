...
tests
  tests.new_suite
    ...
    # construction
  tests.describe
    @ (parent: suite, name: string, body: fn(suite) -> void) -> suite
    + adds a nested suite and runs body with it so body can register specs
    # grouping
  tests.it
    @ (parent: suite, name: string, body: fn() -> result[void, string]) -> suite
    + registers a spec with a pass/fail body
    # specification
  tests.before_each
    @ (parent: suite, hook: fn() -> void) -> suite

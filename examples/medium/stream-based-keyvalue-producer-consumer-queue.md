...
queue
  queue.new_producer
    @ (conn: stream_conn, stream: string) -> producer_state
    + creates a producer bound to the given stream name
    # construction
  queue.enqueue
    @ (state: producer_state, payload: map[string,string]) -> result[string, string]
    + appends an entry and returns its stream id
    - returns error when the connection rejects the write
    # production
    -> std.uuid.new_v4
  queue.new_consumer
    @ (conn: stream_conn, stream: string, group: string, name: string) -> consumer_state
    + creates or reuses a named consumer within a group
    # construction
  queue.poll
    @ (state: consumer_state, max: i32, wait_millis: i32) -> result[list[queue_message], string]
    ...
    -> std.time.now_millis
  queue.ack
    @ (state: consumer_state, id: string) -> result[void, string]
    ...
    # acknowledgement
  queue.reclaim_stale
    @ (state: consumer_state, idle_millis: i64) -> result[list[queue_message], string]

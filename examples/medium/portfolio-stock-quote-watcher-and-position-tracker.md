...
portfolio
  portfolio.new
    ...
    # construction
  portfolio.add_lot
    @ (state: portfolio_state, symbol: string, shares: f64, cost_per_share: f64) -> portfolio_state
    + records a purchase lot
    # mutation
  portfolio.remove_lot
    @ (state: portfolio_state, lot_id: string) -> result[portfolio_state, string]
    + removes a lot by id
    - returns error when lot_id is unknown
    # mutation
  portfolio.fetch_quote
    @ (source_url: string, symbol: string) -> result[f64, string]
  ...
  portfolio.compute_positions
    @ (state: portfolio_state, quotes: map[string, f64]) -> list[position]

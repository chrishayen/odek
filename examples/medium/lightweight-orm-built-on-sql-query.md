...
orm
  orm.define_model
    @ (table: string, columns: list[string], primary_key: string) -> model
    + returns a model description used by subsequent operations
    # schema
  orm.build_select
    @ (m: model, where_columns: list[string]) -> string
    + returns a SELECT statement with placeholders for the where columns
    # query_building
  orm.build_insert
    @ (m: model, columns: list[string]) -> string
    + returns an INSERT statement with placeholders
    # query_building
  orm.build_update
    @ (m: model, set_columns: list[string]) -> string
    + returns an UPDATE ... WHERE <pk> = ? statement
    # query_building
  orm.find
    @ (conn: sql_conn, m: model, pk_value: string) -> result[optional[map[string, string]], string]
    + returns the row matching the primary key, or none when absent
    - returns error on query failure
    # read
    -> std.sql.query_rows
  orm.save
    @ (conn: sql_conn, m: model, row: map[string, string]) -> result[void, string]
    ...
    -> std.sql.query_rows
  orm.delete
    @ (conn: sql_conn, m: model, pk_value: string) -> result[i64, string]

...
std
  std.sql
    std.sql.connect
      ...
      # database
    std.sql.query
      fn (conn: sql_conn, sql: string) -> result[row_cursor, string]
      + executes a query returning a cursor over rows
      # database
    std.sql.next_row
      fn (cursor: row_cursor) -> optional[list[sql_value]]
    ...
    std.sql.describe_table
      fn (conn: sql_conn, table: string) -> result[list[sql_column], string]
...
sql_to_avro
  sql_to_avro.discover_schemas
    fn (conn: sql_conn) -> result[list[avro_schema], string]
    + returns an avro record schema for every table in the database
    # discovery
    -> std.sql.list_tables
    -> std.sql.describe_table
  sql_to_avro.table_to_avro
    fn (table: string, columns: list[sql_column]) -> avro_schema
  ...
  sql_to_avro.avro_schema_to_json
    fn (schema: avro_schema) -> string
    + serializes an avro schema to its canonical json form
    # schema_translation
    -> std.json.encode
  sql_to_avro.query_to_avro_bytes
    fn (conn: sql_conn, sql: string, schema: avro_schema) -> result[bytes, string]
  ...
  sql_to_avro.encode_row
    fn (row: list[sql_value], schema: avro_schema) -> result[bytes, string]

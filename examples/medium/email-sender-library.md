...
std
  std.net
    ...
    std.net.tcp_read_line
      @ (conn: tcp_conn) -> result[string, string]
  ...
  std.crypto
    std.crypto.tls_upgrade
      @ (conn: tcp_conn, host: string) -> result[tcp_conn, string]
...
email
  email.message
    @ (from: string, to: list[string], subject: string, body: string) -> email_msg
    + builds a message with the given headers and body
    # composition
  email.connect
    @ (host: string, port: i32, use_tls: bool) -> result[smtp_session, string]
    ...
    -> std.crypto.tls_upgrade
  email.authenticate
    @ (session: smtp_session, user: string, password: string) -> result[smtp_session, string]
    ...
    -> std.net.tcp_write
  email.send
    @ (session: smtp_session, msg: email_msg) -> result[void, string]

...
borders
  ...
  borders.simplify_polygon
    fn (points: list[pair_i32], epsilon: f64) -> list[pair_i32]
    + reduces points using the Douglas-Peucker algorithm
    # simplification
  borders.pixel_to_geo
    fn (points: list[pair_i32], transform: affine_transform) -> list[pair_f64]
    + maps pixel coordinates to geographic coordinates using an affine transform
    # georeferencing
  borders.to_geojson
    fn (geo_points: list[pair_f64]) -> string
    + emits a GeoJSON Feature containing a single Polygon geometry
    # geojson
    -> std.json.encode_value
  borders.run
    fn (image_data: bytes, background: pixel_rgba, transform: affine_transform) -> result[string, string]

...
sketchpad
  ...
  sketchpad.begin_stroke
    fn (state: sketch_state, color: u32, width: f32) -> sketch_state
    + starts a new stroke with the given color and pen width
    # input
  sketchpad.add_point
    fn (state: sketch_state, x: f32, y: f32, pressure: f32) -> result[sketch_state, string]
    + appends a sample to the currently open stroke
    - returns error when there is no open stroke
    # input
  sketchpad.end_stroke
    fn (state: sketch_state) -> sketch_state
  ...
  sketchpad.bounding_box
    fn (state: sketch_state) -> optional[bbox]

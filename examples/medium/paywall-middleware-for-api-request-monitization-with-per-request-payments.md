...
paywall
  paywall.new
    fn (price: i64, client: payment_client) -> paywall_state
    + creates a paywall with the given per-request price in the smallest currency unit
    # construction
  paywall.issue_invoice
    fn (state: paywall_state, request_id: string) -> result[invoice, string]
  ...
  paywall.verify_payment
    fn (state: paywall_state, invoice: invoice, preimage: string) -> result[bool, string]
    ...
    -> std.crypto.sha256_hex
  paywall.protect
    fn (state: paywall_state, handler: fn(request) -> response) -> fn(request) -> response

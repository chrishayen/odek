...
command_tree
  command_tree.new
    @ (program_name: string) -> command_state
    + creates an empty root command with the given program name
    # construction
  command_tree.register
    @ (state: command_state, path: list[string], handler_id: string, flags: list[string]) -> result[command_state, string]
    ...
    # registration
  command_tree.parse
    @ (state: command_state, argv: list[string]) -> result[parsed_invocation, string]
    ...
    -> std.strings.has_prefix
  command_tree.usage
    @ (state: command_state, path: list[string]) -> result[string, string]
  ...
  command_tree.dispatch
    @ (state: command_state, argv: list[string], handlers: map[string, command_handler]) -> result[i32, string]

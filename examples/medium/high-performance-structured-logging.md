...
std
  ...
  std.io
    std.io.write_line
      @ (w: writer_handle, line: string) -> result[void, string]
...
logger
  logger.new
    @ (min_level: log_level, writer: writer_handle) -> logger_state
    + creates a logger that drops records below min_level
    # construction
  logger.with_field
    @ (l: logger_state, key: string, value: string) -> logger_state
    + returns a child logger that always attaches the given key/value
    # context
  logger.log
    @ (l: logger_state, level: log_level, message: string, fields: map[string, string]) -> void
  ...
  logger.format_record
    @ (ts_nanos: i64, level: log_level, message: string, fields: map[string, string]) -> string

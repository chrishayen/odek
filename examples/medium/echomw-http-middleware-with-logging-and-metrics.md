...
echomw
  echomw.new_logger
    ...
    # logging
  echomw.log_request
    fn (logger: logger_state, method: string, path: string, status: i32, start_ms: i64) -> void
  ...
  echomw.new_metrics
    ...
    # metrics
  echomw.record
    fn (metrics: metrics_state, path: string, status: i32, duration_ms: i64) -> metrics_state
    + increments the request counter and updates the latency histogram for (path, status)
    # metrics
  echomw.snapshot
    fn (metrics: metrics_state) -> map[string,i64]
    + returns a flat map of metric_name to value
    # metrics
  echomw.logger_middleware
    fn (logger: logger_state, metrics: metrics_state, next: handler_fn) -> handler_fn

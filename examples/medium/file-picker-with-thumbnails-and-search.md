...
std
  std.fs
    std.fs.list_dir
      fn (dir: string) -> result[list[dir_entry], string]
...
pikeru
  pikeru.open
    fn (root: string) -> result[browser_state, string]
    + creates a browser rooted at the given directory
    # navigation
    -> std.fs.list_dir
  pikeru.navigate
    fn (state: browser_state, child: string) -> result[browser_state, string]
  ...
  pikeru.generate_thumbnail
    ...
    -> std.image.encode_png
  pikeru.select
    fn (state: browser_state, name: string) -> result[string, string]

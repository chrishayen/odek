...
std
  std.sql
    std.sql.exec
      fn (conn: sql_conn, sql: string, args: list[string]) -> result[i64, string]
      ...
      # sql
    std.sql.query
      fn (conn: sql_conn, sql: string, args: list[string]) -> result[list[map[string, string]], string]
...
sqlx
  sqlx.rewrite_named
    fn (sql: string, params: map[string, string]) -> result[tuple[string, list[string]], string]
    ...
    # named_parameters
  sqlx.exec_named
    fn (conn: sql_conn, sql: string, params: map[string, string]) -> result[i64, string]
    + rewrites named parameters and executes, returning the affected row count
    - propagates errors from rewrite or execution
    # named_parameters
    -> std.sql.exec
  sqlx.query_named
    fn (conn: sql_conn, sql: string, params: map[string, string]) -> result[list[map[string, string]], string]
    + rewrites named parameters and runs the query, returning rows as maps
    - propagates errors from rewrite or execution
    # named_parameters
    -> std.sql.query
  sqlx.scan_one
    fn (rows: list[map[string, string]], key: string) -> result[map[string, string], string]
    ...
    # struct_scanning
  sqlx.batch_exec
    fn (conn: sql_conn, sql: string, rows: list[map[string, string]]) -> result[i64, string]

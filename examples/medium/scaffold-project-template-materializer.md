...
scaffold
  scaffold.template_entry
    fn (relative_path: string, body: string) -> template_entry
    + creates a single template file record
    # construction
  scaffold.render_body
    fn (body: string, vars: map[string,string]) -> string
    + substitutes "{{name}}" placeholders with their values
    ? unknown placeholders are left as-is
    # templating
  scaffold.render_path
    fn (relative_path: string, vars: map[string,string]) -> string
    + substitutes placeholders in the destination path
    # templating
  scaffold.materialize_entry
    fn (out_root: string, entry: template_entry, vars: map[string,string]) -> result[void, string]
  ...
  scaffold.materialize
    fn (out_root: string, entries: list[template_entry], vars: map[string,string], overwrite: bool) -> result[i32, string]

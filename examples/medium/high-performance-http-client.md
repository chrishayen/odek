...
std
  std.net
    ...
    std.net.send_bytes
      ...
      # networking
    std.net.recv_until
      @ (conn: conn_state, delimiter: bytes, max_bytes: i32) -> result[bytes, string]
...
httpc
  httpc.client_new
    @ (max_conns_per_host: i32) -> client_state
    + returns a client with an empty connection pool
    # construction
  httpc.request_build
    @ (method: string, url: string) -> result[request_state, string]
  ...
  httpc.request_set_body
    ...
    # request_building
  httpc.send
    @ (client: client_state, req: request_state) -> result[tuple[response, client_state], string]

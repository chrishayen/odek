...
image
  image.decode
    @ (data: bytes) -> result[image_source, string]
    ...
    # decoding
  image.resize
    @ (src: image_source, width: i32, height: i32) -> image_source
    + returns a source that resamples tiles to the target dimensions using lanczos
    # transform
  image.crop
    @ (src: image_source, x: i32, y: i32, width: i32, height: i32) -> image_source
  ...
  image.convert_colorspace
    @ (src: image_source, target: i32) -> image_source
    + returns a source in the target colorspace (srgb, linear, grayscale)
    # transform
  image.encode
    @ (src: image_source, format: i32, quality: i32) -> result[bytes, string]
    ...
    # encoding
  image.load_file
    @ (path: string) -> result[image_source, string]
    + reads a file and decodes it
    # convenience
    -> std.fs.read_all
  image.save_file
    @ (src: image_source, path: string, format: i32, quality: i32) -> result[void, string]

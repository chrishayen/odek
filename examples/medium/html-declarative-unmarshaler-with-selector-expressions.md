...
std
  std.html
    std.html.parse
      fn (source: string) -> result[html_doc, string]
      + parses an HTML document into an opaque tree
      - returns error on severely malformed input
      # parsing
    std.html.select_all
      fn (doc: html_doc, selector: string) -> result[list[html_node], string]
...
html_unmarshal
  ...
  html_unmarshal.bind_text
    fn (schema: schema_state, field: string, selector: string) -> schema_state
    + binds a field to the text of the first element matching selector
    # schema
  html_unmarshal.bind_attr
    fn (schema: schema_state, field: string, selector: string, attr: string) -> schema_state
  ...
  html_unmarshal.unmarshal
    fn (schema: schema_state, source: string) -> result[map[string, string], string]

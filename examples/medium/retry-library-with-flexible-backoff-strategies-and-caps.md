...
retry
  retry.constant_backoff
    fn (delay_ms: i64) -> backoff_policy
    + produces a policy that always returns delay_ms
    # policy_construction
  retry.exponential_backoff
    fn (base_ms: i64, factor: f64, cap_ms: i64) -> backoff_policy
    ...
    # policy_construction
  retry.with_jitter
    fn (inner: backoff_policy, jitter_ratio: f64) -> backoff_policy
    + wraps a policy so each delay is multiplied by (1 - jitter_ratio + 2*jitter_ratio*random)
    - jitter_ratio outside [0, 1] is clamped
    # policy_construction
    -> std.random.uniform_f64
  retry.delay_for
    fn (policy: backoff_policy, attempt: i32) -> i64
    + returns the delay in milliseconds for the given attempt index
    # policy_query
  retry.run
    fn (policy: backoff_policy, max_attempts: i32, op: fn() -> result[bytes, string]) -> result[bytes, string]

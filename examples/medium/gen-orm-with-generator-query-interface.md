...
std
  std.sql
    std.sql.query
      fn (conn: sql_conn, sql: string, args: list[bytes]) -> result[row_cursor, string]
      + returns a cursor over rows matching the query
      - returns error when the database rejects the sql
      # database
    std.sql.next_row
      fn (cursor: row_cursor) -> result[optional[map[string, bytes]], string]

...
std
  std.encoding
    std.encoding.encode_request
      fn (method: string, body: bytes) -> bytes
...
microservices
  ...
  microservices.register_method
    fn (state: service_state, method: string) -> service_state
    + records that the service exposes a named method
    + replaces an existing registration
    # service_definition
  microservices.handle_request
    fn (state: service_state, frame: bytes) -> bytes
  ...
  microservices.new_client
    fn (target: string) -> client_state
    + creates a client bound to a transport target
    # construction
  microservices.call
    fn (client: client_state, method: string, body: bytes) -> result[bytes, string]

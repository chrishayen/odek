...
transcoder
  transcoder.load_schema
    fn (descriptor_bytes: bytes) -> result[schema_state, string]
  ...
  transcoder.json_to_proto
    fn (schema: schema_state, message_name: string, raw_json: string) -> result[bytes, string]
  ...
  transcoder.handle_request
    fn (schema: schema_state, upstream_base: string, method: string, path: string, headers: map[string, string], body: string) -> result[http_response, string]

...
import_fixer
  import_fixer.parse_imports
    @ (source: string) -> result[parsed_source, string]
  ...
  import_fixer.drop_unused
    @ (parsed: parsed_source, used: list[string]) -> parsed_source
    + removes imports whose alias does not appear in used
    # rewriting
  import_fixer.add_missing
    @ (parsed: parsed_source, used: list[string], index: map[string, string]) -> parsed_source
  ...
  import_fixer.fix_file
    @ (path: string, index: map[string, string]) -> result[void, string]

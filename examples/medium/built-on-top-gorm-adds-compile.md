...
typed_query
  ...
  typed_query.and
    ...
    # predicates
  typed_query.select
    @ (table: table_schema, columns: list[column_ref]) -> query_builder
    + starts a SELECT over the given table and columns
    + validates that each column belongs to the table
    # query_construction
  typed_query.where
    @ (q: query_builder, pred: predicate) -> query_builder
    + attaches a WHERE clause
    # query_construction
  typed_query.render
    @ (q: query_builder) -> tuple[string, list[value]]
    + returns parameterized SQL and the ordered bound values
    ? placeholders are rendered as $1, $2, ...
    # rendering
  typed_query.insert
    @ (table: table_schema, values: map[string,value]) -> result[tuple[string, list[value]], string]

...
elastic
  elastic.to_i64
    fn (value: dynamic) -> result[i64, string]
  ...
  elastic.to_list
    ...
    # coercion
  elastic.to_map
    fn (value: dynamic, key_target: type_desc, val_target: type_desc) -> result[map[dynamic, dynamic], string]
    + returns a map when the source is already a map, coercing keys and values
    - returns error when the source is not a map
    # coercion
  elastic.convert
    fn (value: dynamic, target: type_desc) -> result[dynamic, string]

...
std
  ...
  std.xml
    std.xml.parse
      ...
      # parsing
    std.xml.find_all
      fn (root: xml_node, tag: string) -> list[xml_node]
      + returns every descendant element matching the tag name
      # parsing
    std.xml.attr
      fn (node: xml_node, name: string) -> optional[string]
...
edgar
  ...
  edgar.parse_xbrl_facts
    ...
    -> std.xml.text
  edgar.select_fact
    fn (facts: list[xbrl_fact], concept: string) -> optional[xbrl_fact]

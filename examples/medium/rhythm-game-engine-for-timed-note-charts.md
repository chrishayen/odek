...
rhythm
  rhythm.parse_chart
    @ (raw: string) -> result[chart, string]
    ...
    -> std.text.parse_i32
  rhythm.new_session
    @ (chart: chart) -> session_state
    + returns a fresh session at time 0 with score 0
    # construction
  rhythm.register_hit
    @ (state: session_state, time_ms: i32, lane: i32) -> session_state
    ...
    # scoring
  rhythm.advance
    @ (state: session_state, time_ms: i32) -> session_state
    + marks unhit notes older than the miss window as missed
    # timing
  rhythm.score
    @ (state: session_state) -> i64
    + returns the current score
    # accessor
  rhythm.is_finished
    @ (state: session_state) -> bool

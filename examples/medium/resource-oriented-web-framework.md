...
std
  std.http
    ...
    std.http.render_response
      fn (status: i32, body: string) -> string
...
resource_framework
  resource_framework.define_resource
    fn (name: string, fields: list[string], operations: list[string]) -> resource_definition
    + returns a resource descriptor with the allowed operations
    # schema
  resource_framework.routes_for
    fn (resource: resource_definition) -> list[route_entry]
  ...
  resource_framework.dispatch
    fn (resources: list[resource_definition], handlers: resource_handlers, request: http_request) -> http_response
  ...
  resource_framework.validate_payload
    fn (resource: resource_definition, payload: map[string, string]) -> result[void, string]
    + returns ok when every declared field is present
    - returns error listing missing fields
    # validation
  resource_framework.handle
    fn (resources: list[resource_definition], handlers: resource_handlers, raw_request: string) -> string

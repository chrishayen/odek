...
cell_ring
  cell_ring.new
    fn (replicas: i32) -> ring_state
    + creates an empty ring with the given virtual-node count per real node
    # construction
  cell_ring.add_node
    fn (state: ring_state, node_id: string) -> ring_state
  ...
  cell_ring.rebalance_report
    fn (state: ring_state, cells: list[u64]) -> map[string, i32]

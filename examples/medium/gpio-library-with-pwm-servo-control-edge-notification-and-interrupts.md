...
gpio
  gpio.open_pin
    fn (pin: i32, mode: i32) -> result[pin_handle, string]
    ...
    -> std.hardware.mmio_write
  gpio.digital_write
    fn (handle: pin_handle, level: bool) -> void
    + drives the pin high or low
    # digital_output
    -> std.hardware.mmio_write
  gpio.digital_read
    fn (handle: pin_handle) -> bool
    + returns the current logic level of the pin
    # digital_input
    -> std.hardware.mmio_read
  gpio.set_pwm
    fn (handle: pin_handle, frequency_hz: i32, duty_cycle: f32) -> result[void, string]

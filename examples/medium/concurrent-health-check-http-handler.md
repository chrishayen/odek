...
std
  ...
  std.concurrent
    std.concurrent.run_parallel
      @ (tasks: list[task_handle]) -> list[string]
...
health
  ...
  health.register_liveness
    @ (r: registry_state, name: string, check: check_fn) -> registry_state
  ...
  health.register_readiness
    ...
    # registration
  health.run_all
    @ (r: registry_state, timeout_ms: i64) -> health_report
  ...
  health.handle_request
    @ (r: registry_state, path: string, timeout_ms: i64) -> tuple[i32, string]

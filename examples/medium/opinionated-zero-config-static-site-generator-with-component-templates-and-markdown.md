...
std
  ...
  std.path
    std.path.replace_extension
      fn (path: string, new_ext: string) -> string
...
ssg
  ssg.parse_component_template
    fn (source: string) -> result[component_template, string]
  ...
  ssg.parse_markdown_with_components
    fn (source: string) -> result[page_ast, string]
    ...
    # content_parsing
  ssg.render_page
    fn (page: page_ast, layout: component_template) -> result[string, string]
    ...
    # rendering
  ssg.build_site
    fn (src_dir: string, out_dir: string) -> result[i32, string]

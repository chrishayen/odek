...
std
  ...
  std.http
    std.http.serve_static
      @ (root: string, port: i32) -> result[server_handle, string]
...
wasmrun
  ...
  wasmrun.prepare_bundle
    ...
    -> std.fs.read_all
  wasmrun.run
    @ (state: runner_state, bundle_dir: string, timeout_ms: i64) -> result[test_report, string]
  ...
  wasmrun.parse_report
    @ (raw: string) -> result[test_report, string]

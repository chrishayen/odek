...
petrovich
  petrovich.load_rules
    @ (rules_yaml: string) -> result[rules_state, string]
  ...
  petrovich.inflect_first_name
    @ (state: rules_state, name: string, gender: gender_t, case: case_t) -> string
  ...
  petrovich.detect_gender
    @ (state: rules_state, middle_name: string) -> gender_t

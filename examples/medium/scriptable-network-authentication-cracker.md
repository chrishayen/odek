...
cracker
  ...
  cracker.new_attempt_iterator
    fn (users: list[string], passwords: list[string]) -> attempt_iterator
    + produces (user, password) pairs in user-major order
    # candidates
  cracker.compile_script
    fn (source: string) -> result[auth_script, string]
    + parses a scriptable authentication recipe into an executable plan
    - returns error on invalid script syntax
    # scripting
  cracker.run_attempt
    fn (script: auth_script, target: string, user: string, password: string) -> result[attempt_result, string]
    + executes the script against the target and returns success or failure
    # execution
    -> std.net.http_request
  cracker.run_campaign
    fn (script: auth_script, target: string, iter: attempt_iterator, delay_ms: i64) -> list[attempt_result]

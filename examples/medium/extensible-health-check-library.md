...
health
  health.new
    ...
    # construction
  health.register
    fn (registry: health_registry, name: string, check: fn() -> result[void, string]) -> result[health_registry, string]
    + stores a named check in the registry
    - returns error when a check with the same name is already registered
    # registration
  health.run_all
    fn (registry: health_registry) -> health_report
  ...
  health.mark_critical
    fn (registry: health_registry, name: string) -> result[health_registry, string]

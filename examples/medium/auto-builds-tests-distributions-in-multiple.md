...
std
  ...
  std.process
    std.process.run
      @ (program: string, args: list[string], cwd: string, env: map[string,string]) -> result[process_result, string]
      + runs a subprocess and returns stdout, stderr, and exit code
      - returns error when the program cannot be launched
      # process
  std.text
    std.text.parse_ini
      @ (text: string) -> result[map[string, map[string,string]], string]
...
tox
  tox.config_load
    @ (path: string) -> result[tox_config, string]
    + reads and parses the environment definitions
    # config
    -> std.fs.read_all
    -> std.text.parse_ini
  tox.list_envs
    @ (config: tox_config) -> list[string]
    + returns the declared environment names
    # config
  tox.env_create
    @ (config: tox_config, name: string, workdir: string) -> result[env_handle, string]
    + provisions an isolated directory for the environment
    - returns error when the environment is not declared
    # provisioning
    -> std.fs.make_dir_all
  tox.env_install
    @ (env: env_handle, deps: list[string]) -> result[void, string]
    + runs the dependency installer inside the environment
    # provisioning
    -> std.process.run
  tox.env_run_command
    @ (env: env_handle, program: string, args: list[string]) -> result[process_result, string]
    + executes a command within the environment
    - returns error on nonzero exit
    # execution
    -> std.process.run
  tox.env_destroy
    @ (env: env_handle) -> result[void, string]
    + removes the provisioned directory
    # cleanup
    -> std.fs.remove_dir_all
  tox.run_all
    @ (config: tox_config, workdir: string) -> result[list[env_result], string]

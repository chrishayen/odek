...
prodkit
  ...
  prodkit.register_check
    fn (state: health_state, name: string, check_tag: string) -> health_state
    + registers a named health check referenced by tag
    # health
  prodkit.run_checks
    fn (state: health_state) -> list[check_result]
  ...
  prodkit.shutdown_state
    ...
    # construction
  prodkit.on_shutdown
    fn (state: shutdown_state, hook_tag: string) -> shutdown_state
    + registers a hook to run during graceful shutdown
    # shutdown
  prodkit.begin_shutdown
    fn (state: shutdown_state, deadline_millis: i64) -> shutdown_state
    + marks the system as draining with an absolute deadline
    -> std.time.now_millis
    # shutdown
  prodkit.is_draining
    fn (state: shutdown_state) -> bool

...
cache
  cache.register_driver
    fn (name: string, driver: cache_driver) -> void
    + associates a driver implementation with a name
    # registration
  cache.new
    fn (driver_name: string) -> result[cache_state, string]
    + constructs a cache backed by the named driver
    - returns error when the driver name is not registered
    # construction
  cache.set
    fn (state: cache_state, key: string, value: bytes, ttl_seconds: i64) -> result[void, string]
    ...
    -> std.serialization.encode
  cache.get
    fn (state: cache_state, key: string) -> result[optional[bytes], string]
    ...
    -> std.serialization.decode
  cache.delete
    fn (state: cache_state, key: string) -> result[void, string]

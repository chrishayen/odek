...
cmd_hook
  cmd_hook.new
    ...
    # construction
  cmd_hook.register
    @ (reg: hook_registry, path: string, command: string, arg_templates: list[string]) -> hook_registry
    + adds a hook binding the path to a command whose argument templates may reference request fields
    # registration
  cmd_hook.render_args
    @ (templates: list[string], request: http_request) -> result[list[string], string]
    ...
    -> std.json.parse_object
  cmd_hook.dispatch
    @ (reg: hook_registry, request: http_request) -> http_response
    ...
    -> std.process.run
  cmd_hook.serve
    @ (reg: hook_registry, port: u16) -> result[void, string]

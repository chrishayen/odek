...
std
  ...
  std.regex
    std.regex.compile
      ...
      # pattern_matching
    std.regex.is_match
      fn (re: compiled_regex, input: string) -> bool
      + returns true when the input contains a match
      # pattern_matching

grep
  grep.search_file
    fn (path: string, re: compiled_regex) -> result[list[search_hit], string]
    ...
    -> std.regex.is_match
  grep.search_tree
    fn (root: string, pattern: string) -> result[list[search_hit], string]

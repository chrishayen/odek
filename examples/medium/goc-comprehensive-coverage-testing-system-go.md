...
coverage
  ...
  coverage.register_file
    @ (profile: coverage_profile, path: string, line_count: i32) -> coverage_profile
    + adds a file to the profile with an all-zero hit count vector
    # registration
  coverage.record_hit
    @ (profile: coverage_profile, path: string, line: i32) -> coverage_profile
    + increments the hit counter for a line
    - is a no-op when the file is not registered
    # recording
  coverage.file_percent
    @ (profile: coverage_profile, path: string) -> f64

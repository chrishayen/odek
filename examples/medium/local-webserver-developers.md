...
std
  ...
  std.path
    std.path.join
      ...
      # paths
    std.path.is_within
      @ (root: string, candidate: string) -> bool
...
devserver
  devserver.new
    ...
    # construction
  devserver.add_route
    @ (state: server_state, path: string, body: bytes, content_type: string) -> server_state
    + registers a fixed response for the given request path
    # routing
  devserver.handle
    @ (state: server_state, method: string, request_path: string) -> http_response

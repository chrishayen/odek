...
std
  std.broker
    std.broker.connect
      fn (brokers: list[string]) -> result[broker_client, string]
    ...
    std.broker.list_topics
      fn (c: broker_client) -> result[list[string], string]
      + returns every topic name the broker knows about
      # broker
    std.broker.describe_topic
      fn (c: broker_client, topic: string) -> result[list[partition_info], string]
      + returns partition id, leader, and high-water offset for each partition
      - returns error when the topic does not exist
      # broker
    std.broker.fetch
      fn (c: broker_client, topic: string, partition: i32, offset: i64, max_messages: i32) -> result[list[broker_message], string]
...
topic_inspector
  ...
  topic_inspector.tail
    fn (c: broker_client, topic: string, partition: i32, count: i32) -> result[list[broker_message], string]

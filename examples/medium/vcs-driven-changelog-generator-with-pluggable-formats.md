...
changelog
  changelog.fetch_commits
    @ (repo_path: string, from_rev: string, to_rev: string) -> result[list[commit], string]
    + returns commits reachable from to_rev but not from_rev, oldest first
    - returns error when either revision is unknown
    # vcs
    -> std.process.run
  changelog.classify
    @ (commits: list[commit], matchers: list[commit_matcher]) -> map[string, list[commit]]
  ...
  changelog.new_matcher
    @ (label: string, pattern: string) -> result[commit_matcher, string]
  ...
  changelog.render_markdown
    @ (groups: map[string, list[commit]], version: string) -> string

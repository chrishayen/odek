...
browser_driver
  browser_driver.new
    @ (endpoint: string) -> driver_state
    + stores the automation server base URL
    # construction
  browser_driver.new_session
    @ (state: driver_state, capabilities: map[string, string]) -> result[tuple[driver_state, session_id], string]
  ...
  browser_driver.close_session
    @ (state: driver_state, id: session_id) -> result[driver_state, string]
    + closes the browser session
    # session
    -> std.http.delete
  browser_driver.navigate
    @ (state: driver_state, id: session_id, url: string) -> result[void, string]
    + navigates the session to the URL
    - returns error on navigation failure
    # navigation
    -> std.http.post_json
  browser_driver.find_element
    @ (state: driver_state, id: session_id, strategy: string, selector: string) -> result[element_handle, string]
  ...
  browser_driver.click
    @ (state: driver_state, id: session_id, element: element_handle) -> result[void, string]
    + dispatches a click at the element center
    - returns error when the element is stale
    # input
    -> std.http.post_json
  browser_driver.send_keys
    @ (state: driver_state, id: session_id, element: element_handle, text: string) -> result[void, string]
    + types text into the element
    # input
    -> std.http.post_json
  browser_driver.get_text
    @ (state: driver_state, id: session_id, element: element_handle) -> result[string, string]

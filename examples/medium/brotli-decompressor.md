...
std
  std.bits
    std.bits.new_reader
      ...
      # bit_reading
    std.bits.read_bits
      fn (r: bit_reader, n: i32) -> result[tuple[u32, bit_reader], string]
...
brotli
  brotli.decompress
    ...
    -> std.bits.new_reader
  brotli.read_header
    fn (r: bit_reader) -> result[tuple[brotli_header, bit_reader], string]
  ...
  brotli.decode_meta_block
    fn (r: bit_reader, out: bytes) -> result[tuple[bytes, bit_reader, bool], string]
  ...
  brotli.build_huffman_tree
    fn (code_lengths: list[i32]) -> result[huffman_tree, string]

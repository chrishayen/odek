...
std
  std.xml
    std.xml.parse
      @ (source: bytes) -> result[xml_node, string]
...
xmap
  xmap.to_map
    ...
    # conversion
  xmap.from_map
    @ (value: json_value) -> result[xml_node, string]
    ...
    # conversion
  xmap.parse
    @ (source: bytes) -> result[json_value, string]
    + parses XML bytes directly into the map form
    - returns error on malformed markup
    # pipeline
    -> std.xml.parse
  xmap.serialize
    @ (value: json_value) -> result[bytes, string]

...
test_framework
  ...
  test_framework.register
    @ (state: suite_state, case_name: string, body: string) -> suite_state
    ...
    # registration
  test_framework.run
    @ (state: suite_state) -> suite_report
  ...
  test_framework.record_pass
    @ (report: suite_report, case_name: string, duration_millis: i64) -> suite_report
    + marks a case as passed with its measured duration
    # reporting
  test_framework.record_fail
    @ (report: suite_report, case_name: string, message: string, duration_millis: i64) -> suite_report
    + marks a case as failed with a message and duration
    # reporting
  test_framework.summary
    @ (report: suite_report) -> map[string, i32]
    + returns counts keyed by "total", "passed", "failed"
    # reporting
  test_framework.failed_cases
    @ (report: suite_report) -> list[string]

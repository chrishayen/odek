...
iron
  iron.new_chain
    ...
    # construction
  iron.link
    @ (chain: chain_state, middleware: middleware_fn) -> chain_state
    + appends a middleware to the chain
    ? each middleware may return a response directly, forward to next, or return an error
    # composition
  iron.handle
    @ (chain: chain_state, req: http_request) -> result[http_response, string]
    ...
    # dispatch
  iron.serve
    @ (chain: chain_state, port: i32) -> result[void, string]
    ...
    -> iron.handle
  iron.before_after
    @ (before: middleware_fn, inner: middleware_fn, after: middleware_fn) -> middleware_fn

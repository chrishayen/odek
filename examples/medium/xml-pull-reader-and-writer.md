...
xml
  xml.new_reader
    fn (source: string) -> reader_state
    + creates a reader positioned at the start of source
    # construction
  xml.next_event
    fn (r: reader_state) -> result[optional[xml_event], string]
    ...
    # parsing
  xml.skip_subtree
    fn (r: reader_state) -> result[void, string]
  ...
  xml.new_writer
    ...
    # construction
  xml.write_start
    fn (w: writer_state, name: string, attrs: map[string, string]) -> writer_state
    + appends "<name key=\"value\">" with attribute values escaped
    # writing
  xml.write_text
    fn (w: writer_state, text: string) -> writer_state
    + appends text with XML special characters escaped
    # writing
  xml.write_end
    fn (w: writer_state, name: string) -> writer_state
    + appends "</name>"
    # writing
  xml.finish
    fn (w: writer_state) -> string

...
std
  ...
  std.kv
    std.kv.get
      @ (store: kv_store, key: string) -> result[optional[string], string]
      + returns the value for key, or none when missing
      # storage
    std.kv.put
      @ (store: kv_store, key: string, value: string) -> result[void, string]
      + stores value under key
      # storage
    std.kv.list_prefix
      @ (store: kv_store, prefix: string) -> result[list[string], string]
      + returns all keys starting with prefix
      # storage

push_service
  push_service.new
    @ (store: kv_store) -> service_state
    + wraps a kv store and an empty gateway registry
    # construction
  push_service.register_gateway
    @ (svc: service_state, kind: string, endpoint: string, auth: string) -> service_state
    + registers a vendor gateway under the given kind (e.g. "android", "ios")
    # configuration
  push_service.subscribe_device
    @ (svc: service_state, service_key: string, kind: string, device_token: string) -> result[void, string]
  ...
  push_service.unsubscribe_device
    @ (svc: service_state, service_key: string, device_token: string) -> result[void, string]
    + removes the device from the service key
    # registration
  push_service.list_devices
    @ (svc: service_state, service_key: string) -> result[list[device_record], string]
    + returns all devices under the service key
    # query
    -> std.kv.list_prefix
    -> std.kv.get
  push_service.send_to_service
    @ (svc: service_state, service_key: string, payload: map[string, string]) -> result[send_report, string]

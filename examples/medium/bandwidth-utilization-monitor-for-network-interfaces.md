...
std
  std.net
    ...
    std.net.list_connections
      @ () -> result[list[connection_info], string]
...
bandwidth_monitor
  bandwidth_monitor.snapshot
    @ () -> result[monitor_snapshot, string]
  ...
  bandwidth_monitor.throughput_between
    @ (prev: monitor_snapshot, curr: monitor_snapshot) -> throughput_report
  ...
  bandwidth_monitor.top_talkers
    @ (report: throughput_report, n: i32) -> list[connection_throughput]
    + returns the n connections with the highest combined rx+tx rate
    # analysis
  bandwidth_monitor.group_by_process
    @ (report: throughput_report) -> map[i32, process_throughput]

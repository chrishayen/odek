...
hypermedia
  hypermedia.new
    ...
    # construction
  hypermedia.route
    fn (state: server_state, method: string, path: string, handler: callback) -> server_state
    + registers a handler that returns an html fragment for a method and path
    # routing
  hypermedia.render_page
    fn (title: string, head: string, body: string) -> string
  ...
  hypermedia.render_fragment
    ...
    -> std.strings.concat
  hypermedia.handle
    fn (state: server_state, method: string, path: string, headers: map[string, string]) -> result[hypermedia_response, string]
    ...
    # dispatch
  hypermedia.trigger
    fn (name: string, detail: map[string, string]) -> string

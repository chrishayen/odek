...
std
  ...
  std.net
    std.net.http_put
      @ (url: string, headers: map[string, string], body: bytes) -> result[u16, string]
...
onedump
  onedump.register_source
    @ (driver: string, handler: fn(map[string, string]) -> result[bytes, string]) -> void
    + registers a source driver that maps a config map to a byte stream
    # driver_registry
  onedump.register_destination
    @ (name: string, handler: fn(bytes, map[string, string]) -> result[void, string]) -> void
    + registers a destination handler keyed by name
    # driver_registry
  onedump.dump_local_file
    @ (data: bytes, config: map[string, string]) -> result[void, string]
  ...
  onedump.dump_http
    ...
    -> std.net.http_put
  onedump.run_job
    @ (source_driver: string, source_config: map[string, string], destination: string, destination_config: map[string, string]) -> result[backup_receipt, string]
    ...
    -> std.time.now_unix
  onedump.run_config
    @ (config: backup_config) -> result[list[backup_receipt], list[string]]

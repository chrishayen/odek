...
csv_stream
  ...
  csv_stream.parse_all
    @ (delimiter: u8, quote: u8, input: bytes) -> result[list[list[string]], string]
    + returns every record in a single input buffer
    - returns error on malformed quoting
    # convenience
  csv_stream.records_to_maps
    @ (records: list[list[string]]) -> result[list[map[string,string]], string]

...
cliprompt
  cliprompt.new_session
    ...
    # construction
  cliprompt.add_text
    @ (state: session_state, key: string, prompt: string, default_value: string) -> session_state
    + appends a free-text question with an optional default
    # question_definition
  cliprompt.add_choice
    @ (state: session_state, key: string, prompt: string, choices: list[string]) -> session_state
    + appends a multiple-choice question
    - returns unchanged state when choices is empty
    # question_definition
  cliprompt.add_confirm
    @ (state: session_state, key: string, prompt: string, default_yes: bool) -> session_state
    + appends a yes/no question
    # question_definition
  cliprompt.next_prompt
    @ (state: session_state) -> optional[string]
  ...
  cliprompt.submit_answer
    @ (state: session_state, raw: string) -> result[session_state, string]
    ...
    -> std.strings.parse_i32
  cliprompt.answers
    @ (state: session_state) -> map[string,string]

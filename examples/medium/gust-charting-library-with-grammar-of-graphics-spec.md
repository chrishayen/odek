...
gust
  ...
  gust.bar_marks
    ...
    -> gust.apply_scale
  gust.line_marks
    @ (data: dataset, x_field: string, y_field: string, x_scale: scale, y_scale: scale) -> result[list[point], string]
    + returns scaled points in row order suitable for a connected line
    - returns error when a field is missing from the dataset
    # marks
    -> gust.apply_scale
  gust.axis_ticks
    @ (s: scale, count: i32) -> list[tick]

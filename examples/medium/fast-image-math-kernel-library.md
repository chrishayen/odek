...
imath
  imath.new_image
    ...
    # allocation
  imath.blend
    fn (dst: image, src: image, op: blend_op) -> image
    + blends src over dst using the named porter-duff operator
    + assumes premultiplied alpha
    # compositing
  imath.convolve
    fn (img: image, kernel: list[f32], kernel_w: i32) -> image
    ...
    -> std.math.clamp_f64
  imath.gamma
    fn (img: image, gamma: f64) -> image
    + applies a per-channel gamma curve to RGB, preserving alpha
    # color
    -> std.math.pow
  imath.resize
    fn (img: image, new_w: i32, new_h: i32) -> image

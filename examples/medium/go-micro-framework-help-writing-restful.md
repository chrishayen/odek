...
std
  ...
  std.json
    std.json.get_path
      @ (raw: string, path: string) -> result[string, string]
...
restit
  restit.call
    @ (method: string, url: string, body: bytes) -> result[test_response, string]
    + issues a request and wraps the response for assertions
    # request
    -> std.http.request
  restit.expect_status
    @ (resp: test_response, code: i32) -> result[test_response, string]
    + passes through when the status matches
    - returns an error describing the mismatch otherwise
    # assertion
  restit.expect_header
    @ (resp: test_response, name: string, value: string) -> result[test_response, string]
  ...
  restit.expect_json_path
    @ (resp: test_response, path: string, value: string) -> result[test_response, string]

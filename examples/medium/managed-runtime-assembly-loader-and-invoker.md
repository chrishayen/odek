...
std
  ...
  std.dynlib
    std.dynlib.load
      fn (path: string) -> result[dynlib_handle, string]
      + loads a native runtime host library
      - returns error when the file is missing or not a valid library
      # ffi
    std.dynlib.call
      fn (handle: dynlib_handle, symbol: string, args: list[bytes]) -> result[bytes, string]
...
managed_bridge
  managed_bridge.init_runtime
    fn () -> result[runtime_state, string]
  ...
  managed_bridge.load_assembly
    fn (state: runtime_state, path: string) -> result[assembly_handle, string]
  ...
  managed_bridge.resolve_type
    fn (state: runtime_state, assembly: assembly_handle, full_name: string) -> result[type_handle, string]
  ...
  managed_bridge.resolve_method
    fn (state: runtime_state, type: type_handle, method_name: string, arg_type_names: list[string]) -> result[method_handle, string]
    + looks up a method by name and argument type signature
    - returns error when no matching overload exists
    # reflection
    -> std.dynlib.call
  managed_bridge.invoke
    fn (state: runtime_state, method: method_handle, target: optional[object_handle], args: list[bytes]) -> result[bytes, string]
  ...
  managed_bridge.new_instance
    fn (state: runtime_state, type: type_handle, ctor_args: list[bytes]) -> result[object_handle, string]
    + constructs a new instance of a type
    - returns error when no matching constructor exists
    # invocation
    -> std.dynlib.call
  managed_bridge.shutdown
    fn (state: runtime_state) -> void

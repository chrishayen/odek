...
sandbox
  sandbox.parse_spec
    fn (source: string) -> result[container_spec, string]
    + parses a declarative spec of base image, mounts, env vars, and command
    - returns error on malformed input
    # parsing
  sandbox.resolve_spec
    fn (spec: container_spec, cache_dir: string) -> result[resolved_spec, string]
    ...
    -> std.hash.sha256_hex
  sandbox.run
    fn (resolved: resolved_spec, command: list[string], isolation: isolation_handle) -> result[i32, string]
    ...
    # execution
  sandbox.cleanup
    fn (resolved: resolved_spec, keep_cache: bool) -> result[void, string]
    + removes per-run state; optionally keeps the cached root directory
    # cleanup
  sandbox.list_cached
    fn (cache_dir: string) -> result[list[string], string]

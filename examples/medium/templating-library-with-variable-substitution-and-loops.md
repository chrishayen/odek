...
templating
  templating.tokenize
    fn (source: string) -> result[list[template_token], string]
    ...
    # tokenization
  templating.parse
    fn (tokens: list[template_token]) -> result[template_node, string]
    + builds a tree with nested loop nodes
    - returns error when %endfor does not match the innermost %for
    # parsing
  templating.render
    fn (node: template_node, context: map[string, template_value]) -> result[string, string]
    ...
    # rendering
  templating.compile
    fn (source: string) -> result[template_node, string]

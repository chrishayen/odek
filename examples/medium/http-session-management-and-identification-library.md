...
session
  session.new_store
    @ (ttl_seconds: i64) -> session_store
    + creates an empty store where each session expires after ttl_seconds of inactivity
    # construction
  session.create
    @ (store: session_store, user_id: string) -> tuple[string, session_store]
    ...
    -> std.time.now_seconds
  session.lookup
    @ (store: session_store, session_id: string) -> optional[string]
    ...
    -> std.time.now_seconds
  session.touch
    @ (store: session_store, session_id: string) -> session_store
  ...
  session.format_cookie
    @ (session_id: string, secure: bool) -> string

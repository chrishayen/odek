...
column_db
  column_db.connect
    @ (host: string, port: i32, database: string, user: string, password: string) -> result[column_session, string]
    ...
    -> std.net.tcp_recv
  column_db.close
    @ (session: column_session) -> void
    + releases the underlying connection
    # session
  column_db.query
    @ (session: column_session, sql: string) -> result[list[map[string, string]], string]
  ...
  column_db.insert_block
    @ (session: column_session, table: string, columns: list[string], rows: list[list[string]]) -> result[i64, string]
    ...
    -> std.net.tcp_recv
  column_db.execute
    @ (session: column_session, sql: string) -> result[void, string]

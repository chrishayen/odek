...
htpl
  ...
  htpl.register_helper
    @ (env: env_state, name: string, helper: helper_fn) -> env_state
    ...
    # helpers
  htpl.render
    @ (env: env_state, ast: template_ast, context: map[string, string]) -> result[string, string]
    + resolves variables via context, walks blocks, and invokes helpers
    - returns error when a helper name is not registered
    # rendering
  htpl.render_string
    @ (env: env_state, source: string, context: map[string, string]) -> result[string, string]

...
std
  ...
  std.smtp
    std.smtp.send
      fn (host: string, port: i32, from: string, to: list[string], subject: string, body: string) -> result[void, string]
...
email_templates
  ...
  email_templates.register
    fn (store: template_store, name: string, subject: string, body: string) -> result[void, string]
  ...
  email_templates.render
    fn (store: template_store, name: string, vars: map[string, string]) -> result[rendered_email, string]
    + returns the rendered subject and body
    - returns error when the template name is unknown
    # rendering
    -> std.template.render
  email_templates.preview
    fn (store: template_store, name: string, vars: map[string, string]) -> result[string, string]
    + returns a human-readable preview combining subject and body
    - returns error when rendering fails
    # preview
  email_templates.send
    fn (store: template_store, name: string, to: list[string], vars: map[string, string], transport: smtp_config) -> result[void, string]

...
std
  std.sql
    std.sql.exec
      ...
      # sql
    std.sql.query
      @ (conn: sql_conn, query: string, args: list[sql_value]) -> result[row_iter, string]
      + executes a query and returns an iterator of rows
      - returns error on driver or syntax failure
      # sql
    std.sql.next_row
      @ (iter: row_iter) -> result[optional[map[string, sql_value]], string]
...
sql_ext
  sql_ext.rewrite_named
    @ (query: string, params: map[string, sql_value]) -> result[tuple[string, list[sql_value]], string]
  ...
  sql_ext.rows_to_records
    @ (rows: list[map[string, sql_value]], field_map: map[string, string]) -> list[map[string, sql_value]]
    + renames row columns according to the field_map, producing caller-facing records
    # mapping
  sql_ext.bulk_insert
    @ (conn: sql_conn, table: string, columns: list[string], rows: list[list[sql_value]]) -> result[i64, string]

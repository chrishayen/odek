...
std
  std.reflect
    std.reflect.describe
      @ (value: dynamic_value) -> value_shape
      + returns the kind (function, record, map) and field or parameter info
      # reflection
    std.reflect.call
      @ (value: dynamic_value, args: list[dynamic_value]) -> result[dynamic_value, string]
...
auto_cli
  auto_cli.build
    @ (root: dynamic_value) -> command_tree
    ...
    -> std.reflect.describe
  auto_cli.parse_args
    @ (tree: command_tree, argv: list[string]) -> result[parsed_command, string]

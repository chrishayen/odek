...
ci_client
  ci_client.new
    @ (base_url: string, token: string) -> ci_client_state
    + creates a client that authenticates requests with the given token
    # construction
  ci_client.list_projects
    @ (state: ci_client_state) -> result[list[map[string, string]], string]
    ...
    -> std.json.parse_array_of_objects
  ci_client.get_build
    @ (state: ci_client_state, project: string, build_num: i64) -> result[map[string, string], string]
  ...
  ci_client.trigger_build
    @ (state: ci_client_state, project: string, branch: string) -> result[i64, string]
  ...
  ci_client.list_artifacts
    @ (state: ci_client_state, project: string, build_num: i64) -> result[list[map[string, string]], string]

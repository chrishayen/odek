...
secret_tree
  secret_tree.list_tree
    fn (client: secret_client, root: string) -> result[list[string], string]
  ...
  secret_tree.read_tree
    fn (client: secret_client, root: string) -> result[map[string, map[string, string]], string]
    + returns a map from leaf path to its key/value pairs
    - returns error when any leaf cannot be read
    # reading
    -> secret_tree.list_tree
  secret_tree.copy_tree
    fn (client: secret_client, src: string, dst: string) -> result[i32, string]
  ...
  secret_tree.delete_tree
    fn (client: secret_client, root: string) -> result[i32, string]
    + deletes every leaf under root and returns how many were removed
    # deletion
    -> secret_tree.list_tree
  secret_tree.search_tree
    fn (client: secret_client, root: string, needle: string) -> result[list[match], string]

...
std
  ...
  std.json
    std.json.encode_rows
      fn (rows: list[map[string, string]]) -> string
      + encodes rows as a JSON array of objects
      # serialization
  std.csv
    std.csv.parse
      fn (raw: string) -> result[list[list[string]], string]
...
explorer
  explorer.open_csv
    fn (path: string) -> result[table_handle, string]
    ...
    -> std.csv.parse
  explorer.schema
    fn (t: table_handle) -> list[string]
    + returns the column names in order
    # introspection
  explorer.row_count
    fn (t: table_handle) -> i64
    + returns the number of data rows (excluding the header)
    # introspection
  explorer.select_columns
    fn (t: table_handle, columns: list[string]) -> result[table_handle, string]
  ...
  explorer.filter_equals
    fn (t: table_handle, column: string, value: string) -> result[table_handle, string]

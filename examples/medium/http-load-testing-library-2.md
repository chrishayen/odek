...
std
  std.http
    std.http.request
      @ (method: string, url: string, body: bytes) -> result[http_response, string]
...
load_test
  load_test.new_plan
    @ (target_url: string, method: string, rate_per_sec: f64, duration_sec: i32) -> attack_plan
    + creates a plan describing the load profile
    ? request body is empty; callers extend the plan if needed
    # construction
  load_test.run
    @ (plan: attack_plan) -> attack_report
    ...
    -> std.time.now_micros
  load_test.summarize
    @ (report: attack_report) -> load_summary

...
std
  std.bytes
    std.bytes.index_of
      fn (haystack: bytes, needle: bytes, from: i32) -> i32
...
multipart
  ...
  multipart.split_parts
    fn (body: bytes, boundary: string) -> result[list[bytes], string]
  ...
  multipart.parse_part
    fn (raw_part: bytes) -> result[part, string]
  ...
  multipart.parse_body
    fn (body: bytes, content_type: string) -> result[parsed_upload, string]

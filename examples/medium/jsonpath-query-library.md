...
jsonpath
  jsonpath.compile
    fn (expression: string) -> result[jsonpath_program, string]
    ...
    # compilation
  jsonpath.evaluate
    fn (program: jsonpath_program, root: json_value) -> list[json_value]
    + returns all matching json values in document order
    + returns an empty list when nothing matches
    # evaluation
  jsonpath.query
    fn (expression: string, raw_json: string) -> result[list[json_value], string]

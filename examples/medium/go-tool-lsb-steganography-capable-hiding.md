...
std
  std.image
    std.image.decode_rgb
      @ (raw: bytes) -> result[list[u8], string]
    ...
    std.image.encode_rgb
      @ (pixels: list[u8], width: i32, height: i32) -> bytes
...
stego
  stego.capacity_bytes
    ...
    # capacity
  stego.embed
    @ (pixels: list[u8], payload: bytes) -> result[list[u8], string]
    ...
    # embedding
  stego.extract
    @ (pixels: list[u8]) -> result[bytes, string]

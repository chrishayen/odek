...
std
  std.random
    std.random.new
      ...
      # randomness
    std.random.next_int
      @ (state: rng_state, min: i64, max: i64) -> tuple[i64, rng_state]
...
fake
  fake.new
    ...
    -> std.random.new
  fake.first_name
    @ (state: fake_state) -> tuple[string, fake_state]
  ...
  fake.address
    ...
    # geography
  fake.int_between
    @ (state: fake_state, low: i64, high: i64) -> tuple[i64, fake_state]
    + returns an integer in [low, high] inclusive
    - returns an error-valued tuple when low > high
    # numeric
    -> std.random.next_int
  fake.date_between
    @ (state: fake_state, start_unix: i64, end_unix: i64) -> tuple[i64, fake_state]

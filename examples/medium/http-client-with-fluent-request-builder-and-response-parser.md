...
std
  std.http
    std.http.send
      fn (method: string, url: string, headers: map[string,string], body: bytes, timeout_ms: i32) -> result[http_response, string]
...
http_client
  http_client.new
    fn (base_url: string) -> client_state
    + returns a client with a base URL applied to relative paths
    # construction
  http_client.with_header
    fn (state: client_state, name: string, value: string) -> client_state
    + adds a default header to every request
    # configuration
  http_client.with_timeout
    fn (state: client_state, ms: i32) -> client_state
    + sets the default request timeout in milliseconds
    # configuration
  http_client.build_request
    fn (state: client_state, method: string, path: string, query: map[string,string], body: bytes) -> prepared_request
    + resolves the URL and merges default headers
    # builder
    -> std.encoding.url_encode_query
  http_client.send
    fn (state: client_state, req: prepared_request) -> result[http_response, string]
    + sends the prepared request
    - returns error on network or timeout
    # send
    -> std.http.send
  http_client.get_json
    fn (state: client_state, path: string, query: map[string,string]) -> result[json_value, string]
  ...
  http_client.post_json
    fn (state: client_state, path: string, body: json_value) -> result[json_value, string]

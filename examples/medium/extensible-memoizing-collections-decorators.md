...
cache
  ...
  cache.new_lfu
    ...
    # construction
  cache.new_ttl
    @ (max_entries: i32, ttl_millis: i64) -> cache_state
    + creates an empty cache where entries expire after the given time-to-live
    # construction
    -> std.time.now_millis
  cache.get
    @ (state: cache_state, key: string) -> tuple[optional[string], cache_state]
  ...
  cache.put
    ...
    -> std.time.now_millis
  cache.memoize
    @ (state: cache_state, key: string, compute: fn(string) -> string) -> tuple[string, cache_state]

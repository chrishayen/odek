...
transport
  transport.register_driver
    fn (name: string, open_fn: driver_open_fn, send_fn: driver_send_fn, close_fn: driver_close_fn) -> result[void, string]
    + registers a named transport driver in the global registry
    - returns error when a driver with the same name already exists
    # registry
  transport.open
    fn (driver_name: string, endpoint: string) -> result[connection, string]
    ...
    -> transport.register_driver
  transport.send
    fn (conn: connection, payload: bytes) -> result[void, string]
    + sends a payload through the underlying driver
    - returns error when the connection is already closed
    # messaging
  transport.on_message
    fn (conn: connection, handler: fn(bytes) -> void) -> connection

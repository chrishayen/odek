...
socket_router
  socket_router.new
    ...
    # construction
  socket_router.route
    fn (state: router_state, channel: string, handler: string) -> router_state
    + registers a handler identifier for the given channel name
    + overwrites any existing handler on that channel
    # routing
  socket_router.use
    fn (state: router_state, middleware: string) -> router_state
  ...
  socket_router.dispatch
    fn (state: router_state, channel: string, frame: bytes) -> result[bytes, string]
    + runs the middleware chain then the handler for the channel
    - returns error when no handler is registered for the channel
    # dispatch
  socket_router.serve
    fn (state: router_state, host: string, port: u16) -> result[void, string]

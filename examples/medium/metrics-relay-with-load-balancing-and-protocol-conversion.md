...
relay
  relay.new
    @ (backends: list[string]) -> relay_state
    + creates a relay with an ordered list of backend identifiers
    # construction
  relay.pick_backend
    @ (state: relay_state, event_key: string) -> string
    + returns the backend assigned to the given event key using consistent hashing
    # load_balancing
    -> std.hash.fnv1a_64
  relay.forward
    @ (state: relay_state, event: metric_event, sink: event_sink) -> result[void, string]
  ...
  relay.mark_backend_down
    @ (state: relay_state, backend: string) -> relay_state

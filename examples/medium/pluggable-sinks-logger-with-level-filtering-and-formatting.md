...
logger
  logger.new
    ...
    # construction
  logger.add_sink
    fn (state: logger_state, name: string, accepts: log_level) -> logger_state
    ...
    # registration
  logger.set_format
    fn (state: logger_state, pattern: string) -> logger_state
    + sets a format pattern with placeholders for time, level, and message
    ? supported placeholders are {time}, {level}, and {msg}
    # configuration
  logger.log
    fn (state: logger_state, level: log_level, message: string) -> list[tuple[string, string]]
  ...
  logger.format_record
    fn (pattern: string, level: log_level, message: string, timestamp_ms: i64) -> string
    + substitutes the pattern placeholders with the record fields
    # formatting
  logger.level_at_least
    fn (level: log_level, min: log_level) -> bool

...
std
  ...
  std.crypto
    std.crypto.ssh_handshake
      fn (conn: conn_handle, host_key: bytes) -> result[ssh_session, string]
    ...
    std.crypto.load_host_key
      fn (pem: string) -> result[bytes, string]
...
ssh_server
  ssh_server.new
    fn (host_key_pem: string) -> result[server_state, string]
    + creates a server configured with a host key
    - returns error when the host key is invalid
    # construction
    -> std.crypto.load_host_key
  ssh_server.handle
    fn (srv: server_state, command: string, callback_id: i64) -> server_state
    + registers a handler for an exact-match command
    # routing
  ssh_server.default_handler
    fn (srv: server_state, callback_id: i64) -> server_state
  ...
  ssh_server.listen_and_serve
    ...
    -> std.crypto.ssh_handshake
  ssh_server.dispatch
    fn (srv: server_state, sess: ssh_session, command: string) -> i64

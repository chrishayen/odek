...
probds
  ...
  probds.cms_new
    ...
    # count_min
  probds.cms_add
    @ (cms: cms_state, item: bytes, count: i64) -> cms_state
    + increments the counters at each row's hashed column by count
    # count_min
    -> std.hash.murmur3_64
  probds.cms_estimate
    @ (cms: cms_state, item: bytes) -> i64
  ...
  probds.hll_new
    ...
    # hyperloglog
  probds.hll_add
    @ (hll: hll_state, item: bytes) -> hll_state

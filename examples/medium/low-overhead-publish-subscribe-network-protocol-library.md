...
std
  std.net
    std.net.send
      fn (socket: socket_handle, data: bytes) -> result[i32, string]
      + sends bytes on a connected socket and returns the number written
      - returns error on transport failure
      # networking
    std.net.recv
      fn (socket: socket_handle, max: i32) -> result[bytes, string]
...
zproto
  ...
  zproto.add_subscription
    fn (router: router_state, subscription_id: i64, pattern: string) -> router_state
    + registers a topic pattern under the given subscription id
    # routing
  zproto.remove_subscription
    fn (router: router_state, subscription_id: i64) -> router_state
    + removes the subscription with the given id
    # routing
  zproto.match_subscribers
    fn (router: router_state, topic: string) -> list[i64]
    + returns the subscription ids whose patterns match the topic
    # routing
  zproto.send_frame
    fn (socket: socket_handle, frame_bytes: bytes) -> result[void, string]
    + writes a frame to a socket in full
    - returns error on transport failure
    # transport
    -> std.net.send
  zproto.read_frame
    fn (socket: socket_handle) -> result[frame, string]

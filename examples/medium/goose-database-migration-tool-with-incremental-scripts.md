...
goose
  goose.register_script
    @ (version: i64, up: migration_fn, down: migration_fn) -> void
  ...
  goose.load_sql_scripts
    @ (sources: map[string, string]) -> result[void, string]
  ...
  goose.down_to
    ...
    # rollback
  goose.status
    @ (executor: sql_executor) -> result[list[script_status], string]

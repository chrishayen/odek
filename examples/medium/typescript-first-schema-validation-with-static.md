...
schema
  schema.string_schema
    @ (min_len: i32, max_len: i32) -> schema_node
    + builds a string constraint node
    # builders
  schema.number_schema
    @ (min: f64, max: f64) -> schema_node
  ...
  schema.object_schema
    @ (fields: map[string, schema_node], required: list[string]) -> schema_node
    + builds an object node with required keys
    # builders
  schema.array_schema
    @ (item: schema_node, min_items: i32, max_items: i32) -> schema_node
    + builds an array node with per-item constraints
    # builders
  schema.optional
    @ (inner: schema_node) -> schema_node
    + wraps a node so null is accepted
    # builders
  schema.validate
    @ (root: schema_node, value: json_value) -> result[void, list[string]]
    + returns ok when the value satisfies the schema
    - returns a list of path-scoped error messages when it does not
    # validation
  schema.validate_raw
    @ (root: schema_node, raw_json: string) -> result[void, list[string]]

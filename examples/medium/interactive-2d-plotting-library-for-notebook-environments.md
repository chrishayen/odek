...
plot
  plot.figure
    fn (width_px: i32, height_px: i32) -> figure_state
    + creates an empty figure with the given pixel dimensions
    # construction
  plot.add_line
    fn (fig: figure_state, xs: list[f64], ys: list[f64], label: string) -> figure_state
  ...
  plot.add_scatter
    ...
    # series
  plot.set_axes
    fn (fig: figure_state, x_label: string, y_label: string) -> figure_state
    + sets axis labels
    # axes
  plot.on_point_click
    fn (fig: figure_state, handler_id: string) -> figure_state
    + registers a named handler to be invoked on point-click events from the host
    # interaction
  plot.dispatch_event
    fn (fig: figure_state, event: plot_event) -> figure_state
    + routes an event from the host to the registered handlers
    - returns the figure unchanged when no handler matches
    # interaction
  plot.to_spec
    fn (fig: figure_state) -> string

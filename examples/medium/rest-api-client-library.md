...
rest_client
  rest_client.new
    @ (base_url: string) -> rest_client_state
    + returns a client rooted at the base URL
    # construction
  rest_client.resource
    @ (client: rest_client_state, segment: string) -> rest_client_state
    + returns a client with the segment appended to the path
    # path
  rest_client.with_query
    @ (client: rest_client_state, key: string, value: string) -> rest_client_state
    + returns a client with an additional query parameter
    # query
  rest_client.get
    @ (client: rest_client_state) -> result[map[string, string], string]
    ...
    -> std.json.parse_object
  rest_client.post
    @ (client: rest_client_state, body: string) -> result[map[string, string], string]

...
std
  std.http
    std.http.get
      @ (url: string, timeout_ms: i32) -> result[http_response, string]
...
health
  health.register
    @ (dash: dashboard_state, name: string, url: string, interval_ms: i32) -> dashboard_state
    + adds a new endpoint to the dashboard with its check interval
    # registration
  health.check_once
    @ (endpoint: endpoint_config) -> check_result

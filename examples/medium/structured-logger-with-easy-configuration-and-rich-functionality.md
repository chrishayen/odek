...
std
  ...
  std.io
    std.io.write_line
      @ (fd: i32, line: string) -> result[void, string]
      + writes line followed by a newline to file descriptor fd
      # io

zl
  zl.new
    @ (name: string, min_level: log_level) -> logger_state
  ...
  zl.with_field
    ...
    # context
  zl.with_fields
    @ (logger: logger_state, fields: map[string, string]) -> logger_state
    + returns a child logger with multiple fields attached
    # context
  zl.format_entry
    @ (logger: logger_state, level: log_level, message: string, fields: map[string, string]) -> string
    + renders a log entry as a JSON line including timestamp, level, name, message, and fields
    # formatting
    -> std.time.now_iso8601
  zl.log
    @ (logger: logger_state, level: log_level, message: string, fields: map[string, string]) -> void
    ...
    -> std.io.write_line
  zl.set_sink
    @ (logger: logger_state, sink: fn(line: string) -> void) -> logger_state

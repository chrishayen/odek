...
std
  ...
  std.json
    std.json.parse
      ...
      # serialization
    std.json.get_field
      fn (value: json_value, field: string) -> optional[json_value]
...
chainjack
  chainjack.parse_dependency_path
    fn (path: string) -> result[repo_ref, string]
  ...
  chainjack.list_direct_dependencies
    fn (manifest: string) -> result[list[repo_ref], string]
  ...
  chainjack.check_owner_exists
    fn (host: string, owner: string) -> result[bool, string]
  ...
  chainjack.check_repo_exists
    fn (ref: repo_ref) -> result[bool, string]
    + returns true when the repository resolves on the host
    # probing
    -> std.http.get
    -> std.json.parse
  chainjack.classify
    fn (ref: repo_ref, owner_exists: bool, repo_exists: bool) -> chainjack_status
    ...
    # classification
  chainjack.audit
    fn (manifest: string) -> result[list[chainjack_finding], string]
  ...
  chainjack.format_report
    fn (findings: list[chainjack_finding]) -> string

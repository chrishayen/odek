...
docgen
  docgen.load_source
    @ (root: string) -> result[list[source_page], string]
    ...
    -> std.fs.read_all
  docgen.render_page
    @ (page: source_page, template: string, nav_html: string) -> string
    + renders a single source page by substituting its HTML body and nav into template
    # rendering
    -> std.markdown.render_html
  docgen.build_nav
    @ (pages: list[source_page]) -> string
    + returns an HTML list of links grouped by directory in deterministic order
    # navigation
  docgen.build
    @ (source_root: string, output_root: string, template: string) -> result[i32, string]

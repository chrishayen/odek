...
serial
  serial.list_ports
    ...
    # discovery
  serial.open
    @ (device: string, baud: i32, data_bits: i32, stop_bits: i32, parity: string) -> result[port, string]
    ...
    # lifecycle
  serial.read
    @ (p: port, max_bytes: i32, timeout_ms: i32) -> result[bytes, string]
    ...
    # io
  serial.write
    @ (p: port, data: bytes) -> result[i32, string]
    + returns the number of bytes actually written
    - returns error when the port is closed
    # io
  serial.flush
    @ (p: port) -> result[void, string]

...
std
  std.http
    std.http.put_bytes
      @ (url: string, data: bytes) -> result[string, string]
...
object_transfer
  object_transfer.new
    @ (endpoint: string, bucket: string, part_size: i32, concurrency: i32) -> transfer_state
    + returns a transfer configuration
    # construction
  object_transfer.split_parts
    @ (total_size: i64, part_size: i32) -> list[part_range]
    + returns the ordered byte ranges for each part
    ? the final part absorbs any remainder
    # planning
  object_transfer.upload_part
    @ (state: transfer_state, key: string, part: part_range, data: bytes) -> result[part_etag, string]
  ...
  object_transfer.complete_upload
    @ (state: transfer_state, key: string, parts: list[part_etag]) -> result[string, string]
  ...
  object_transfer.download
    @ (state: transfer_state, key: string, total_size: i64) -> result[bytes, string]
  ...
  object_transfer.verify_checksum
    @ (data: bytes, expected: bytes) -> bool

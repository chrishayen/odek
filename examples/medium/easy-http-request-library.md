...
easy_http
  easy_http.get
    @ (url: string, params: map[string, string]) -> result[http_response, string]
    + appends params as a query string and issues a GET request
    # http
    -> std.http.request
    -> std.url.encode_query
  easy_http.post_json
    @ (url: string, body: map[string, string]) -> result[http_response, string]
    + sends a JSON body with the correct content type
    # http
    -> std.http.request
    -> std.json.encode_object
  easy_http.post_form
    @ (url: string, fields: map[string, string]) -> result[http_response, string]
    + sends a form-encoded body with the correct content type
    # http
    -> std.http.request
    -> std.url.encode_query
  easy_http.response_json
    @ (response: http_response) -> result[map[string, string], string]

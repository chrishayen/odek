...
std
  std.net
    std.net.tcp_dial
      ...
      # transport
    std.net.tcp_send
      fn (conn: tcp_conn, data: bytes) -> result[u32, string]
      + writes data and returns the number of bytes written
      # transport
    std.net.tcp_recv
      fn (conn: tcp_conn, max: u32) -> result[bytes, string]
      + reads up to max bytes from the connection
      - returns error when the peer has closed
      # transport
    std.net.udp_bind
      fn (port: u16) -> result[udp_socket, string]
      + binds a UDP socket on the given port
      # transport
    std.net.udp_send_to
      fn (sock: udp_socket, host: string, port: u16, data: bytes) -> result[u32, string]
      + sends a datagram to the given address
      # transport
    std.net.udp_recv_from
      fn (sock: udp_socket) -> result[tuple[string, u16, bytes], string]
      + blocks for a datagram, returns (host, port, payload)
      # transport

protocol
  protocol.field_u8
    fn (name: string) -> field_spec
  ...
  protocol.define_message
    ...
    # schema
  protocol.encode
    fn (schema: message_schema, values: map[string, bytes]) -> result[bytes, string]
    ...
    # encoding
  protocol.decode
    fn (schema: message_schema, data: bytes) -> result[map[string, bytes], string]
    + parses bytes into a field map according to the schema
    - returns error when data is shorter than the schema requires
    # decoding
  protocol.send_tcp
    fn (conn: tcp_conn, schema: message_schema, values: map[string, bytes]) -> result[void, string]
    + encodes and writes a message over a TCP connection
    # transport
    -> std.net.tcp_send
  protocol.recv_tcp
    fn (conn: tcp_conn, schema: message_schema) -> result[map[string, bytes], string]

...
router
  router.new
    ...
    # construction
  router.add
    @ (r: router_state, method: string, pattern: string, handler_id: string) -> result[void, string]
    ...
    -> std.strings.split
  router.lookup
    @ (r: router_state, method: string, path: string) -> optional[lookup_result]
  ...
  router.allowed_methods
    @ (r: router_state, path: string) -> list[string]

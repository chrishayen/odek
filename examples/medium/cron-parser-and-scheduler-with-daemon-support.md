...
std
  std.time
    ...
    std.time.to_calendar
      fn (unix_seconds: i64) -> calendar_time
...
cron
  cron.parse
    ...
    # parsing
  cron.matches
    fn (spec: cron_spec, at: calendar_time) -> bool
  ...
  cron.new_runner
    ...
    # construction
  cron.schedule
    fn (state: runner_state, name: string, expr: string, task: task_handler) -> result[runner_state, string]
    ...
    -> cron.parse
  cron.tick
    fn (state: runner_state, now: i64) -> runner_state
    + fires every task whose next-fire time is <= now and reschedules it
    # execution
    -> cron.next_after
  cron.run_forever
    fn (state: runner_state) -> result[void, string]

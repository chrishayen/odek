...
bundler
  bundler.new_project
    fn (root: string) -> project_state
    + returns a project rooted at the given directory
    # construction
  bundler.resolve_imports
    fn (project: project_state, entry: string) -> result[list[string], string]
    ...
    -> std.fs.read_all
  bundler.bundle
    fn (project: project_state, entry: string) -> result[bytes, string]
  ...
  bundler.compute_update
    fn (project: project_state, changed_path: string) -> result[bytes, string]
  ...
  bundler.watch_for_updates
    fn (project: project_state) -> result[string, string]

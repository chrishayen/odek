...
device_manager
  device_manager.new
    fn (heartbeat_timeout_ms: i64) -> manager_state
    + creates a manager with a liveness timeout
    # construction
  device_manager.connect
    fn (state: manager_state, device_id: string, metadata: map[string, string]) -> tuple[string, manager_state]
  ...
  device_manager.heartbeat
    fn (state: manager_state, session_id: string) -> result[manager_state, string]
  ...
  device_manager.sweep
    fn (state: manager_state) -> tuple[list[string], manager_state]
  ...
  device_manager.list_connected
    fn (state: manager_state) -> list[device]
    + returns every currently connected device
    # introspection
  device_manager.drain_events
    fn (state: manager_state) -> tuple[list[event], manager_state]

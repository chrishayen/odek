...
std
  ...
  std.regex
    std.regex.compile
      ...
      # regex
    std.regex.matches
      @ (re: regex_handle, input: string) -> bool
      + reports whether the input matches
      # regex
  std.process
    std.process.run
      @ (cmd: string, args: list[string], workdir: string) -> result[i32, string]
...
generate_runner
  generate_runner.resolve_root
    @ (path_or_env: string) -> result[string, string]
  ...
  generate_runner.collect_targets
    @ (root: string, include_pattern: string) -> result[list[string], string]
  ...
  generate_runner.run_all
    @ (targets: list[string], command: string, command_args: list[string]) -> result[i32, string]

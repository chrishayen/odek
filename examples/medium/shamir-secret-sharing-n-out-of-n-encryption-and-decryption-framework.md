...
secret_split
  secret_split.split_key
    fn (key: bytes, threshold: i32, shares: i32) -> result[list[tuple[u8, bytes]], string]
  ...
  secret_split.combine_key
    fn (parts: list[tuple[u8, bytes]]) -> result[bytes, string]
  ...
  secret_split.encrypt
    fn (plaintext: bytes, threshold: i32, shares: i32) -> result[tuple[bytes, list[tuple[u8, bytes]]], string]
  ...
  secret_split.decrypt
    fn (ciphertext: bytes, parts: list[tuple[u8, bytes]]) -> result[bytes, string]

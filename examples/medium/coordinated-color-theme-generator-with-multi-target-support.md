...
std
  ...
  std.text
    std.text.render_template
      fn (tmpl: string, vars: map[string,string]) -> string
      + substitutes {{name}} placeholders with the map values
      # templating

themer
  themer.normalize_palette
    fn (colors: map[string,string]) -> result[palette, string]
  ...
  themer.palette_to_vars
    fn (pal: palette) -> map[string,string]
  ...
  themer.render_target
    fn (target: string, pal: palette) -> result[list[file_entry], string]
    + returns one or more (relative_path, content) pairs for the target
    - returns error when the target is not supported
    # rendering
    -> std.text.render_template
  themer.write_target
    fn (out_dir: string, files: list[file_entry]) -> result[void, string]
    + writes each rendered file under out_dir
    # io
    -> std.fs.write_all
  themer.generate_all
    fn (pal: palette, targets: list[string], out_dir: string) -> result[void, string]

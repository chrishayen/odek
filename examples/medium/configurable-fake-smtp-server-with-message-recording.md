...
fake_smtp
  ...
  fake_smtp.new_session
    fn (s: server_state) -> session_state
    + starts a session in the initial state expecting HELO/EHLO
    # session
  fake_smtp.feed
    fn (sess: session_state, line: string) -> feed_response
    ...
    # session
  fake_smtp.captured
    fn (s: server_state) -> list[captured_message]
    + returns all envelopes captured so far (from, to, data)
    # inspection
  fake_smtp.reset
    fn (s: server_state) -> void

...
migrator
  migrator.embed_source
    fn (files: map[string, string]) -> migration_source
    + wraps a name-to-sql map as a source of migrations
    # source
  migrator.list_migrations
    fn (source: migration_source) -> list[migration]
  ...
  migrator.applied_versions
    ...
    # state
  migrator.apply
    fn (executor: sql_executor, source: migration_source) -> result[i32, string]
    ...
    # migration
  migrator.rollback
    fn (executor: sql_executor, source: migration_source, steps: i32) -> result[i32, string]

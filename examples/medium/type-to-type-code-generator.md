...
convergen
  convergen.parse_type_spec
    fn (source: string) -> result[type_spec, string]
  ...
  convergen.match_fields
    fn (src: type_spec, dst: type_spec) -> list[field_mapping]
  ...
  convergen.convertible
    fn (src_type: string, dst_type: string) -> bool
    + returns true when source type can be assigned or trivially cast to destination
    # mapping
  convergen.emit_function
    fn (name: string, src: type_spec, dst: type_spec, mappings: list[field_mapping]) -> string
    ...
    # generation
  convergen.generate
    fn (input_path: string, output_path: string, function_name: string) -> result[void, string]

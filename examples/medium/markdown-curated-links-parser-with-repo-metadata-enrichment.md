...
awesome
  ...
  awesome.extract_entries
    fn (doc: links_doc) -> list[link_entry]
    + returns every link entry flattened in document order with its section path attached
    # extraction
  awesome.fetch_repo_info
    fn (url: string, fetch_fn: fn(string) -> result[string, string]) -> result[repo_info, string]
  ...
  awesome.enrich_entries
    fn (entries: list[link_entry], fetch_fn: fn(string) -> result[string, string]) -> list[enriched_entry]
    + returns an enriched entry for each input, with failures recorded on the entry itself
    # enrichment
  awesome.render_doc
    fn (doc: links_doc, enriched: list[enriched_entry]) -> string
  ...
  awesome.process_file
    fn (input_path: string, output_path: string, fetch_fn: fn(string) -> result[string, string]) -> result[void, string]

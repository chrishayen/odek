...
std
  std.bytes
    ...
    std.bytes.write_u16_be
      @ (data: bytes, offset: i32, value: u16) -> bytes
...
arp
  arp.encode_request
    @ (sender_hw: bytes, sender_ip: bytes, target_ip: bytes) -> bytes
    + encodes an ARP request frame
    # framing
    -> std.bytes.write_u16_be
  arp.encode_reply
    @ (sender_hw: bytes, sender_ip: bytes, target_hw: bytes, target_ip: bytes) -> bytes
    + encodes an ARP reply frame
    # framing
    -> std.bytes.write_u16_be
  arp.decode
    @ (frame: bytes) -> result[arp_frame, string]
    ...
    -> std.bytes.read_u16_be
  arp.new_cache
    @ (ttl_seconds: i64) -> arp_cache_state
    + returns an empty cache with the given entry lifetime
    # caching
  arp.remember
    @ (cache: arp_cache_state, ip: bytes, hw: bytes) -> arp_cache_state
    + stores or refreshes an ip-to-hardware mapping with the current timestamp
    # caching
    -> std.time.now_seconds
  arp.lookup
    @ (cache: arp_cache_state, ip: bytes) -> optional[bytes]

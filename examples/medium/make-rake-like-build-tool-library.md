...
build_tool
  build_tool.new
    ...
    # construction
  build_tool.task
    fn (state: build_state, name: string, deps: list[string], action: string) -> build_state
    + registers a task with its dependencies and an action identifier
    # registration
  build_tool.target
    fn (state: build_state, name: string, inputs: list[string], outputs: list[string]) -> build_state
    + marks a task as producing files from inputs for staleness checks
    # registration
  build_tool.plan
    fn (state: build_state, goal: string) -> result[list[string], string]
    ...
    # planning
  build_tool.is_stale
    fn (state: build_state, task_name: string) -> bool
    + returns true when any input is newer than any output or an output is missing
    # caching
    -> std.fs.mtime
    -> std.fs.exists
  build_tool.run
    fn (state: build_state, goal: string) -> result[void, string]

...
agent
  agent.new
    fn (model_endpoint: string, api_key: string) -> agent_state
    + creates an agent configured with an LLM endpoint and credentials
    # construction
  agent.define_schema
    fn (state: agent_state, name: string, fields: map[string, string]) -> agent_state
    ...
    # schema_registry
  agent.register_tool
    fn (state: agent_state, name: string, description: string, input_fields: map[string, string]) -> agent_state
    + adds a tool the agent can call during a run
    # tool_registry
  agent.run
    fn (state: agent_state, prompt: string, schema_name: string) -> result[map[string, string], string]
  ...
  agent.validate_against_schema
    fn (state: agent_state, schema_name: string, obj: map[string, string]) -> result[void, string]

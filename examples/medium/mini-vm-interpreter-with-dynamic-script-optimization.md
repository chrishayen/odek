...
mini_vm
  mini_vm.tokenize
    @ (source: string) -> result[list[tuple[string, string]], string]
  ...
  mini_vm.new_vm
    ...
    # construction
  mini_vm.execute
    @ (vm: vm_state, bytecode: list[i64]) -> result[tuple[optional[i64], vm_state], string]
  ...
  mini_vm.inline_cache_lookup
    @ (vm: vm_state, object_shape: i64, property: string) -> optional[i32]
    + returns the cached slot offset for a (shape, property) pair
    - returns none on cache miss
    # optimization
  mini_vm.inline_cache_store
    @ (vm: vm_state, object_shape: i64, property: string, slot: i32) -> vm_state
    + records a slot offset for fast future lookups
    # optimization
  mini_vm.mark_hot
    @ (vm: vm_state, function_id: i64) -> tuple[bool, vm_state]

...
release_pipeline
  release_pipeline.prepare_workspace
    fn (source_dir: string) -> result[string, string]
  ...
  release_pipeline.run_tests
    fn (workspace: string, test_cmd: string) -> result[void, string]
    + runs the test command in the workspace
    - returns error when the test command exits non-zero
    # test
    -> std.process.run
  release_pipeline.build_binary
    fn (workspace: string, build_cmd: string, output_path: string) -> result[string, string]
  ...
  release_pipeline.sign_artifact
    fn (artifact_path: string, private_key: bytes) -> result[string, string]
    + writes a detached signature file alongside the artifact and returns its path
    # signing
    -> std.crypto.sign_detached
  release_pipeline.publish_artifact
    fn (artifact_path: string, signature_path: string, destination_uri: string) -> result[void, string]
  ...
  release_pipeline.run_all
    fn (source_dir: string, test_cmd: string, build_cmd: string, private_key: bytes, destination_uri: string) -> result[void, string]

...
push_cluster
  push_cluster.new_node
    fn (node_id: string) -> node_state
    + creates an empty node with no subscribers and no peers
    # construction
  push_cluster.register_peer
    fn (state: node_state, peer_id: string, address: string) -> node_state
  ...
  push_cluster.encode_broadcast
    fn (topic: string, payload: bytes, origin_node: string) -> bytes
    + encodes a broadcast envelope for transmission to peer nodes
    # cluster_replication
  push_cluster.apply_broadcast
    fn (state: node_state, envelope: bytes) -> result[list[string], string]

...
std
  ...
  std.diff
    std.diff.line_diff
      @ (old: string, new_text: string) -> string
      + returns a unified line diff between two multiline strings
      # diff

watch
  watch.new_watcher
    @ (cmd: string, args: list[string], interval_ms: i64) -> watcher_state

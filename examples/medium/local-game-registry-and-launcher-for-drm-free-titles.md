...
game_registry
  game_registry.new
    ...
    # construction
  game_registry.add
    fn (state: registry_state, name: string, executable: string) -> result[registry_state, string]
  ...
  game_registry.remove
    fn (state: registry_state, name: string) -> result[registry_state, string]
    + returns a new state without the entry
    - returns error when no such game is registered
    # registration
  game_registry.list
    fn (state: registry_state) -> list[tuple[string, string]]
    + returns every (name, executable) pair sorted by name
    # listing
  game_registry.save
    fn (state: registry_state, path: string) -> result[void, string]
    ...
    -> std.fs.write_all
  game_registry.load
    fn (path: string) -> result[registry_state, string]
  ...
  game_registry.launch
    fn (state: registry_state, name: string, runner: process_runner) -> result[void, string]

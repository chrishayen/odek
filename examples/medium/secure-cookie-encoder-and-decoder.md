...
securecookie
  securecookie.new
    fn (key: bytes, max_age_seconds: i64) -> cookie_codec
    + creates a codec bound to the encryption key and max age
    - returns error when key is not 32 bytes
    # construction
  securecookie.encode
    fn (codec: cookie_codec, name: string, value: string) -> result[string, string]
    ...
    -> std.encoding.base64url_encode
  securecookie.decode
    fn (codec: cookie_codec, name: string, encoded: string) -> result[string, string]

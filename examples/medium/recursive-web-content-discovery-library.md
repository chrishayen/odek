...
std
  std.http
    std.http.get_status
      fn (url: string) -> result[i32, string]
...
discovery
  discovery.new
    fn (base_url: string, wordlist: list[string], max_depth: i32) -> discovery_state
    + returns a scanner rooted at base_url with the given wordlist and recursion limit
    # construction
  discovery.candidate_urls
    fn (state: discovery_state, current: string) -> list[string]
  ...
  discovery.is_directory_status
    ...
    # classification
  discovery.probe_url
    fn (url: string) -> result[probe_result, string]
    + returns a probe_result containing url and status
    - returns error when the fetch fails
    # probe
    -> std.http.get_status
  discovery.scan
    fn (state: discovery_state) -> list[probe_result]

...
audio
  audio.buffer_new
    @ (sample_rate: i32, channels: i32, samples: list[f32]) -> result[audio_buffer, string]
    ...
    # construction
  audio.gain
    @ (buf: audio_buffer, db: f64) -> audio_buffer
    + scales every sample by 10^(db/20)
    ? clipping to [-1, 1] is the caller's responsibility
    # effects
    -> std.math.log10
  audio.mix
    @ (tracks: list[audio_buffer]) -> result[audio_buffer, string]
  ...
  audio.resample_linear
    ...
    # resampling
  audio.generate_sine
    @ (sample_rate: i32, freq_hz: f64, duration_seconds: f64) -> audio_buffer
    + returns a mono buffer containing a sine wave of the given frequency and length
    # synthesis
    -> std.math.sin
  audio.fade
    @ (buf: audio_buffer, fade_in_seconds: f64, fade_out_seconds: f64) -> audio_buffer

...
std
  std.json
    std.json.parse
      fn (raw: string) -> result[generic_tree, string]
      - returns error on invalid JSON
      # parsing
    std.json.render
      fn (tree: generic_tree) -> string
      # serialization
  std.yaml
    std.yaml.parse
      fn (raw: string) -> result[generic_tree, string]
...
translator
  translator.parse
    fn (format: string, raw: string) -> result[generic_tree, string]
    ...
    -> std.xml.parse
  translator.render
    fn (format: string, tree: generic_tree) -> result[string, string]
  ...
  translator.compile_template
    fn (template: string) -> result[compiled_template, string]
  ...
  translator.apply_template
    fn (compiled: compiled_template, tree: generic_tree) -> string
    + walks the tree and renders the template
    ? missing paths render as the empty string
    # templates
  translator.translate
    fn (from_format: string, raw: string, template: string) -> result[string, string]

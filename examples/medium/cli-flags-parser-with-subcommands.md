...
flags
  flags.new_command
    fn (name: string) -> command_state
    + creates a command with the given name, no flags, and no subcommands
    # construction
  flags.add_string
    fn (cmd: command_state, name: string, default_value: string) -> command_state
    + registers a string-valued flag
    - returns unchanged command when name is empty
    # flag_registration
  flags.add_int
    fn (cmd: command_state, name: string, default_value: i64) -> command_state
    + registers an integer-valued flag
    # flag_registration
  flags.add_bool
    fn (cmd: command_state, name: string) -> command_state
    + registers a boolean switch that defaults to false
    # flag_registration
  flags.add_subcommand
    fn (cmd: command_state, child: command_state) -> command_state
    + attaches a subcommand under the parent
    # subcommand
  flags.parse
    fn (cmd: command_state, argv: list[string]) -> result[parsed_args, string]

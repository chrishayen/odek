...
std
  ...
  std.http
    std.http.parse_query
      @ (raw: string) -> result[map[string, list[string]], string]
...
http_decode
  ...
  http_decode.from_query
    @ (schema: schema_state, field: string, key: string, required: bool) -> schema_state
  ...
  http_decode.from_header
    @ (schema: schema_state, field: string, header: string, required: bool) -> schema_state
    + binds a field to a request header
    # schema
  http_decode.decode
    @ (schema: schema_state, query: string, body: string, headers: map[string, string]) -> result[map[string, string], string]

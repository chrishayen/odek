...
doc_db_driver
  doc_db_driver.connect
    fn (host: string, port: i32, auth_key: string) -> result[session_state, string]
    ...
    -> std.net.read_exact
  doc_db_driver.table
    fn (db_name: string, table_name: string) -> query_term
    + builds a table-reference term
    # query_builder
  doc_db_driver.filter
    fn (source: query_term, field: string, value: string) -> query_term
    + wraps source in a filter term matching field == value
    # query_builder
  doc_db_driver.limit
    fn (source: query_term, n: i32) -> query_term
    + wraps source in a limit term
    # query_builder
  doc_db_driver.run
    fn (session: session_state, term: query_term) -> result[list[string], string]

...
supervisor
  supervisor.new
    fn (watch_root: string, build_cmd: list[string], run_cmd: list[string]) -> supervisor_state
    + creates a supervisor with empty file snapshot and no running process
    # construction
  supervisor.snapshot
    fn (root: string) -> map[string, i64]
    + builds a path-to-mtime map for all files under root
    # file_tracking
    -> std.fs.walk
    -> std.fs.mtime
  supervisor.changed_files
    fn (before: map[string,i64], after: map[string,i64]) -> list[string]

...
graphql_engine
  ...
  graphql_engine.add_field
    fn (schema: schema_state, type_name: string, field_name: string, return_type: string, resolve: closure[string]) -> schema_state
  ...
  graphql_engine.validate
    fn (schema: schema_state, ast: query_ast) -> result[void, list[string]]
    + checks every selected field exists on its type
    - returns the list of validation errors
    # validation
  graphql_engine.execute
    fn (schema: schema_state, ast: query_ast, variables: map[string, string]) -> result[string, string]

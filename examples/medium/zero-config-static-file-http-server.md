...
static_server
  static_server.resolve_path
    fn (root: string, url_path: string) -> result[string, string]
  ...
  static_server.handle
    fn (root: string, req: http_request) -> http_response
    ...
    -> std.fs.stat
  static_server.serve
    fn (root: string, host: string, port: i32) -> result[void, string]

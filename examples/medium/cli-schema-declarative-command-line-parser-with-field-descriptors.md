...
cli_schema
  cli_schema.new_command
    ...
    # construction
  cli_schema.add_flag
    fn (cmd: command_spec, long: string, short: string, kind: string, required: bool, default_value: string) -> command_spec
  ...
  cli_schema.add_subcommand
    fn (parent: command_spec, child: command_spec) -> command_spec
    + attaches a child command under the parent
    # schema
  cli_schema.parse
    fn (root: command_spec, argv: list[string]) -> result[parsed_invocation, string]
  ...
  cli_schema.render_help
    fn (cmd: command_spec) -> string
    + produces a help text listing usage, flags, and subcommands
    # help
  cli_schema.get_string
    fn (invocation: parsed_invocation, name: string) -> optional[string]
    + retrieves a string flag value by its long name
    # accessor
  cli_schema.get_i64
    fn (invocation: parsed_invocation, name: string) -> optional[i64]
    + retrieves an integer flag value by its long name
    # accessor
  cli_schema.get_bool
    fn (invocation: parsed_invocation, name: string) -> bool

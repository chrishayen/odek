...
envite
  envite.new_environment
    fn (name: string) -> environment_spec
    + creates an empty environment specification
    # construction
  envite.add_service
    fn (env: environment_spec, svc: service_spec) -> environment_spec
    + registers a service with its start command, ports, and dependencies
    # registration
  envite.resolve_order
    fn (env: environment_spec) -> result[list[string], string]
  ...
  envite.start_service
    fn (svc: service_spec) -> result[service_handle, string]
    + launches a single service and returns a handle
    - returns error when the service fails to start
    # lifecycle
    -> std.process.spawn
  envite.wait_ready
    fn (svc: service_spec, timeout_ms: i64) -> result[void, string]
    ...
    -> std.time.sleep_millis
  envite.start_all
    fn (env: environment_spec) -> result[environment_state, string]
    ...
    # orchestration
  envite.stop_all
    fn (state: environment_state) -> result[void, string]

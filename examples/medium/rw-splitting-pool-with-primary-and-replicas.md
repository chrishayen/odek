...
rwdb
  rwdb.new
    fn (primary_dsn: string, replica_dsns: list[string]) -> rwdb_pool
    ...
    # construction
  rwdb.pick_writer
    fn (pool: rwdb_pool) -> string
    + returns the primary DSN
    # routing
  rwdb.pick_reader
    fn (pool: rwdb_pool) -> tuple[string, rwdb_pool]
    ...
    # routing
  rwdb.mark_unhealthy
    fn (pool: rwdb_pool, dsn: string) -> rwdb_pool
    + records a replica as unhealthy and excludes it from selection
    # health
    -> std.time.now_millis
  rwdb.recover
    fn (pool: rwdb_pool, cooldown_ms: i64) -> rwdb_pool

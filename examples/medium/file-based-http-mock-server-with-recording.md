...
smoke
  smoke.load_mocks
    fn (dir: string) -> result[list[mock], string]
    ...
    -> std.fs.read_all
  smoke.match_mock
    fn (mocks: list[mock], method: string, path: string) -> optional[mock]
    + returns the first mock whose method and path match
    + supports wildcard segments like "/users/{id}"
    # matching
  smoke.handle
    fn (mocks: list[mock], request: bytes) -> result[bytes, string]
    ...
    -> std.http.serialize_response
  smoke.record
    fn (dir: string, method: string, path: string, response: http_response) -> result[void, string]

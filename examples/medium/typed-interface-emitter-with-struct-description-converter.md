...
ifacegen
  ifacegen.new_struct
    fn (name: string) -> struct_descriptor
    + creates a named empty struct descriptor
    # construction
  ifacegen.add_field
    fn (s: struct_descriptor, name: string, type_name: string, optional: bool) -> struct_descriptor
    + appends a field with its name, type, and optionality
    # construction
  ifacegen.map_type
    fn (src_type: string) -> string

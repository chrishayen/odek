...
proc_monitor
  proc_monitor.new
    fn (history_points: i32) -> proc_monitor_state
    + creates a monitor retaining the given number of history samples
    # construction
  proc_monitor.sample
    fn (s: proc_monitor_state) -> proc_monitor_state
  ...
  proc_monitor.top_processes
    fn (s: proc_monitor_state, n: i32, sort_by: string) -> list[proc_info]
  ...
  proc_monitor.history
    fn (s: proc_monitor_state, series: string) -> list[f64]
    + returns the history buffer for "cpu" or "memory"
    # query
  proc_monitor.render_sparkline
    fn (values: list[f64], width: i32) -> string
  ...
  proc_monitor.render_dashboard
    fn (s: proc_monitor_state, width: i32, height: i32) -> string

...
templater
  templater.render_string
    fn (text: string, vars: map[string,string]) -> result[string, string]
    + replaces every "{{ name }}" token with vars[name]
    - returns error when a token references an unknown variable
    # substitution
  templater.render_path
    fn (path: string, vars: map[string,string]) -> result[string, string]
    + applies substitution to a file path's segments
    # path_rewriting
    -> templater.render_string
  templater.generate
    fn (template_root: string, output_root: string, vars: map[string,string]) -> result[void, string]

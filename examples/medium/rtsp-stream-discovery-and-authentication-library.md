...
rtsp_scan
  rtsp_scan.probe_port
    fn (host: string, port: u16) -> result[bool, string]
  ...
  rtsp_scan.describe_stream
    fn (host: string, port: u16, path: string, credentials: optional[credentials]) -> result[stream_status, string]
  ...
  rtsp_scan.build_digest_response
    fn (user: string, password: string, realm: string, nonce: string, method: string, uri: string) -> string
    + returns the MD5-based HTTP Digest response string for the given challenge
    # auth
    -> std.crypto.md5
  rtsp_scan.try_credentials
    fn (host: string, port: u16, path: string, candidates: list[credentials]) -> result[optional[credentials], string]
    ...
    # auth
  rtsp_scan.scan_host
    fn (host: string, ports: list[u16], paths: list[string], candidates: list[credentials]) -> list[stream_finding]

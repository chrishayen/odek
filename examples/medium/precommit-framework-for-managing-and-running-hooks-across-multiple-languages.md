...
std
  ...
  std.process
    std.process.run
      @ (command: string, args: list[string], stdin: bytes) -> result[process_result, string]
...
precommit
  precommit.parse_config
    @ (raw: string) -> result[config, string]
  ...
  precommit.load_config
    @ (path: string) -> result[config, string]
    + reads and parses a config file
    # config
    -> std.fs.read_all
  precommit.select_hooks
    @ (cfg: config, changed_files: list[string]) -> list[hook]
    ...
    -> std.strings.ends_with
  precommit.run_hook
    @ (h: hook, files: list[string]) -> hook_result
    ...
    -> std.process.run
  precommit.run_all
    @ (cfg: config, changed_files: list[string]) -> list[hook_result]
    + selects and runs every applicable hook in config order
    # execution
  precommit.report
    @ (results: list[hook_result]) -> string

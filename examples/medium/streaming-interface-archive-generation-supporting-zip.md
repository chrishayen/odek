...
archiver
  archiver.new
    @ (format: string, sink: byte_sink) -> result[archive_state, string]
    + constructs a "zip" or "tar" writer bound to the sink
    - returns error for unknown formats
    # construction
  archiver.append_entry
    @ (state: archive_state, name: string, content: bytes, mode: u32, mtime: i64) -> result[archive_state, string]
  ...
  archiver.append_stream
    @ (state: archive_state, name: string, chunk_source: byte_source, mode: u32, mtime: i64) -> result[archive_state, string]
    ...
    -> std.hash.crc32
  archiver.finish
    @ (state: archive_state) -> result[void, string]

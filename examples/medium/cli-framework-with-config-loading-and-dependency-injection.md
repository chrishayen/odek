...
app
  app.new
    fn (name: string, version: string) -> app_state
    + constructs an empty application with the given name and version
    # construction
  app.register
    fn (state: app_state, name: string, handler: command_fn) -> app_state
    + registers a subcommand with its handler
    # registration
  app.parse_args
    fn (state: app_state, args: list[string]) -> result[parsed_invocation, string]
    ...
    # parsing
  app.load_config
    fn (paths: list[string]) -> result[map[string, string], string]
  ...
  app.container_register
    fn (state: container_state, key: string, value: dependency) -> container_state
    + stores a dependency under the given key
    # injection
  app.container_resolve
    fn (state: container_state, key: string) -> optional[dependency]
    + returns the stored dependency for key, if any
    # injection
  app.run
    fn (state: app_state, container: container_state, args: list[string]) -> result[i32, string]

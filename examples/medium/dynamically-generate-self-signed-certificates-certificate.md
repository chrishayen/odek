...
std
  std.crypto
    std.crypto.rsa_generate_key
      @ (bits: i32) -> result[rsa_keypair, string]
      + returns a fresh RSA keypair of the given size
      - returns error when bits is not a supported size
      # cryptography
    std.crypto.rsa_sign
      @ (key: rsa_keypair, data: bytes) -> bytes
      + returns an RSA signature over the data
      # cryptography
  std.pki
    std.pki.encode_certificate
      @ (subject: string, issuer: string, public_key: bytes, signature: bytes, not_before: i64, not_after: i64) -> bytes
...
testcerts
  testcerts.generate_ca
    @ (common_name: string, validity_days: i32) -> result[tuple[string, string], string]
  ...
  testcerts.issue_leaf
    @ (ca_cert_pem: string, ca_key_pem: string, common_name: string, sans: list[string], validity_days: i32) -> result[tuple[string, string], string]

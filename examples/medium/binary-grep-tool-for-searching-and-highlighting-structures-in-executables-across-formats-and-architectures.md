...
binary_grep
  ...
  binary_grep.parse_header
    @ (data: bytes, format: string) -> result[binary_header, string]
  ...
  binary_grep.list_sections
    @ (header: binary_header) -> list[section_info]
    + returns one entry per section with name, offset, size, and flags
    # inspection
  binary_grep.list_symbols
    @ (data: bytes, header: binary_header) -> result[list[symbol_info], string]
  ...
  binary_grep.find_matches
    @ (symbols: list[symbol_info], query: string) -> list[symbol_info]
    + returns symbols whose name contains query (case-insensitive)
    # query
  binary_grep.highlight
    @ (sections: list[section_info], matches: list[symbol_info]) -> list[highlight_span]
    + returns byte-offset spans suitable for colorized rendering
    # presentation
  binary_grep.open_path
    @ (path: string) -> result[binary_header, string]

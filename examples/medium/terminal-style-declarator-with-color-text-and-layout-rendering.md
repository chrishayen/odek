...
termstyle
  ...
  termstyle.foreground
    @ (style: style_state, rgb: u32) -> style_state
    + returns a style with the given 24-bit foreground color
    # color
  termstyle.bold
    @ (style: style_state, on: bool) -> style_state
  ...
  termstyle.italic
    ...
    # decoration
  termstyle.padding
    @ (style: style_state, left: i32, right: i32) -> style_state
    + returns a style with horizontal padding in spaces
    ? negative values are clamped to zero
    # layout
  termstyle.render
    @ (style: style_state, text: string) -> string

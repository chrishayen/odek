...
document_db
  document_db.new
    ...
    # construction
  document_db.table
    fn (state: db_state, name: string) -> db_state
    + ensures a table with the given name exists
    # tables
  document_db.insert
    fn (state: db_state, table: string, doc: json_value) -> result[tuple[db_state, i64], string]
    + inserts the document into the table and returns a newly assigned id
    - returns error when the table does not exist
    # insert
  document_db.get
    fn (state: db_state, table: string, id: i64) -> result[optional[json_value], string]
    + returns the document with the given id, or none if removed
    - returns error when the table does not exist
    # get
  document_db.update
    fn (state: db_state, table: string, id: i64, doc: json_value) -> result[db_state, string]
    + replaces the document at the given id
    - returns error when the id does not exist in the table
    # update
  document_db.remove
    fn (state: db_state, table: string, id: i64) -> result[db_state, string]
    + deletes the document with the given id
    - returns error when the id does not exist in the table
    # remove
  document_db.search
    fn (state: db_state, table: string, predicate: doc_predicate) -> result[list[json_value], string]
    + returns all documents in the table for which the predicate returns true
    - returns error when the table does not exist
    # search
  document_db.save
    fn (state: db_state, path: string) -> result[void, string]

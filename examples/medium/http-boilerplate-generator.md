...
httpgen
  httpgen.describe_field
    fn (name: string, type_name: string, source: string, required: bool) -> field_spec
  ...
  httpgen.describe_handler
    fn (name: string, method: string, path: string, inputs: list[field_spec], output_type: string) -> handler_spec
    + builds a handler spec with method, path, inputs, and the response type name
    # specification
  httpgen.render_decoder
    fn (h: handler_spec) -> string

...
cli
  cli.new_app
    ...
    # construction
  cli.command
    @ (app: app_state, name: string, params: list[param_spec], handler: fn(map[string, value]) -> i32) -> app_state
    ...
    # registration
  cli.parse
    @ (app: app_state, argv: list[string]) -> result[tuple[string, map[string, value]], string]
    ...
    # parsing
  cli.dispatch
    @ (app: app_state, argv: list[string]) -> i32
    ...
    # dispatch
  cli.help
    @ (app: app_state, command: optional[string]) -> string

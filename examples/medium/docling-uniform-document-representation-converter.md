...
docling
  docling.detect_format
    fn (filename: string, header: bytes) -> document_format
  ...
  docling.extract_text_blocks
    fn (raw: bytes, fmt: document_format) -> result[list[text_block], string]
  ...
  docling.build_document
    fn (blocks: list[text_block]) -> structured_document
  ...
  docling.convert_file
    fn (path: string) -> result[structured_document, string]

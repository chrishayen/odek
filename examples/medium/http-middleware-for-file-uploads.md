...
std
  std.http
    std.http.read_multipart_part
      @ (req: http_request) -> result[optional[multipart_part], string]
      + returns the next part from the request body, or none at end
      - returns error on malformed multipart framing
      # http
  std.io
    std.io.copy
      @ (src: byte_reader, dst: byte_writer, limit_bytes: i64) -> result[i64, string]
...
uploads
  uploads.new_config
    @ (max_file_bytes: i64, allowed_types: list[string]) -> upload_config
    + builds an upload policy
    # construction
  uploads.open_storage
    @ (root_dir: string) -> result[upload_storage, string]
  ...
  uploads.handle_request
    @ (cfg: upload_config, storage: upload_storage, req: http_request) -> result[list[upload_record], string]
    ...
    -> std.io.copy
  uploads.delete
    @ (storage: upload_storage, record: upload_record) -> result[void, string]

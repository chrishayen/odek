...
std
  std.strings
    std.strings.split_lines
      ...
      # strings
    std.strings.join
      fn (xs: list[string], sep: string) -> string
...
hls
  hls.parse
    fn (source: string) -> result[playlist, string]
  ...
  hls.is_media
    ...
    # classification
  hls.segments
    fn (p: playlist) -> list[media_segment]
    + returns every media segment with duration and URI
    # query
  hls.variants
    fn (p: playlist) -> list[variant_stream]

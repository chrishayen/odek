...
struct_gen
  struct_gen.infer_schema
    fn (root: json_value, root_name: string) -> schema
  ...
  struct_gen.unify_types
    fn (a: field_type, b: field_type) -> field_type
  ...
  struct_gen.from_sample
    fn (raw: string, root_name: string) -> result[schema, string]
    + convenience entry point that parses the sample and infers its schema
    - propagates parse errors
    # entry
    -> std.json.parse_value
  struct_gen.render
    fn (s: schema, emit: fn(record) -> string) -> string

...
std
  std.json
    ...
    std.json.kind
      ...
      # introspection
    std.json.as_object
      @ (value: json_value) -> result[map[string, json_value], string]
      + returns the object entries when value is an object
      - returns error when value is not an object
      # introspection
    std.json.as_array
      @ (value: json_value) -> result[list[json_value], string]
...
jsonassert
  jsonassert.compare
    @ (expected: string, actual: string) -> result[list[json_diff], string]
  ...
  jsonassert.compare_values
    @ (expected: json_value, actual: json_value, path: string) -> list[json_diff]

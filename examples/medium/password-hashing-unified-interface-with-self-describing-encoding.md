...
std
  std.crypto
    std.crypto.pbkdf2_sha256
      fn (password: bytes, salt: bytes, iterations: i32, dk_len: i32) -> bytes
      + computes PBKDF2-HMAC-SHA256
      # cryptography
    std.crypto.scrypt
      fn (password: bytes, salt: bytes, n: i32, r: i32, p: i32, dk_len: i32) -> bytes
...
password
  password.register_pbkdf2
    fn (state: registry_state, iterations: i32) -> registry_state
    + registers "pbkdf2-sha256" with the given iteration count
    # registration
  password.register_scrypt
    fn (state: registry_state, n: i32, r: i32, p: i32) -> registry_state
    + registers "scrypt" with the given parameters
    # registration
  password.hash
    fn (state: registry_state, algorithm: string, password: string) -> result[string, string]
    ...
    -> std.crypto.scrypt
  password.verify
    fn (state: registry_state, password: string, encoded: string) -> result[bool, string]
  ...
  password.needs_rehash
    fn (state: registry_state, encoded: string) -> result[bool, string]

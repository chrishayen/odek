...
workflow
  ...
  workflow.register_step
    fn (engine: workflow_engine, event: string, step_id: string, handler: step_fn) -> workflow_engine
  ...
  workflow.start_instance
    fn (engine: workflow_engine, initial_step: string, initial_state: map[string,string]) -> tuple[string, workflow_engine]
    + creates an instance in initial_step and returns its id
    # instance_lifecycle
  workflow.dispatch
    fn (engine: workflow_engine, instance_id: string, event: string, payload: map[string,string]) -> result[workflow_engine, string]
  ...
  workflow.instance_state
    fn (engine: workflow_engine, instance_id: string) -> result[map[string,string], string]

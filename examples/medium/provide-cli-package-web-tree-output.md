...
std
  ...
  std.io
    std.io.write_string
      @ (sink: writer, text: string) -> result[void, string]
...
tree_outline
  tree_outline.parse_markdown
    @ (text: string) -> result[tree_node, string]
  ...
  tree_outline.write_to
    @ (root: tree_node, sink: writer) -> result[void, string]
    + renders the tree and writes it to the given sink
    - returns error on write failure
    # output
    -> std.io.write_string
  tree_outline.materialize
    @ (root: tree_node, base_path: string) -> result[void, string]

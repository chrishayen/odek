...
tunnel
  tunnel.new_session
    fn (peer: string, mtu: i32) -> session_state
    + creates a new session targeting peer with the given segment size
    # construction
  tunnel.write
    fn (state: session_state, data: bytes) -> session_state
    + enqueues bytes for transmission, splitting into numbered segments
    # send
  tunnel.on_datagram
    fn (state: session_state, payload: bytes) -> tuple[session_state, bytes]
    ...
    # receive
  tunnel.tick
    fn (state: session_state) -> tuple[session_state, list[bytes]]
    + returns segments whose retransmit timer has expired
    ? retransmit timeout doubles per attempt up to a cap
    # retransmission
    -> std.time.now_millis
  tunnel.run
    fn (sock: udp_socket, state: session_state) -> result[void, string]

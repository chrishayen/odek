...
std
  std.fs
    ...
    std.fs.write_all
      ...
      # filesystem
    std.fs.read_at
      @ (path: string, offset: i64, n: i32) -> result[bytes, string]
...
readykv
  readykv.open
    @ (dir: string) -> result[kv_state, string]
  ...
  readykv.close
    ...
    # lifecycle
  readykv.put
    @ (state: kv_state, key: string, value: bytes) -> result[kv_state, string]
  ...
  readykv.get
    ...
    -> std.hash.fnv1a_64
  readykv.delete
    @ (state: kv_state, key: string) -> result[kv_state, string]

...
envmgr
  envmgr.create
    @ (root: string, name: string) -> result[env_handle, string]
  ...
  envmgr.load
    ...
    -> std.json.parse_object
  envmgr.add_package
    @ (env: env_handle, name: string, version: string) -> result[env_handle, string]
    + records a dependency in the manifest
    - returns error when the version string is empty
    # dependencies
  envmgr.remove_package
    @ (env: env_handle, name: string) -> env_handle
    + removes a package from the manifest if present
    # dependencies
  envmgr.resolve
    @ (env: env_handle, index: map[string, list[string]]) -> result[map[string, string], string]
    ...
    # resolution
  envmgr.install
    @ (env: env_handle, resolved: map[string, string]) -> result[env_handle, string]

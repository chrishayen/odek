...
subcmd
  subcmd.new
    @ (program_name: string) -> subcmd_registry
    + creates an empty registry with no commands
    # construction
  subcmd.register
    @ (r: subcmd_registry, name: string, summary: string, flag_spec: list[tuple[string, bool]]) -> subcmd_registry
    + registers a subcommand with a summary and its flag spec (name, takes_value)
    # registration
  subcmd.parse
    @ (r: subcmd_registry, argv: list[string]) -> result[subcmd_invocation, string]

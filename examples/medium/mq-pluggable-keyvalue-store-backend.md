...
std
  std.kv
    std.kv.lpush
      fn (store: kv_handle, key: string, value: bytes) -> result[void, string]
      + prepends a value to the list at key
      - returns error when the key exists and holds a non-list type
      # storage
    std.kv.rpop
      fn (store: kv_handle, key: string) -> result[optional[bytes], string]
      + removes and returns the tail element, or none when the list is empty
      # storage
    std.kv.zadd
      fn (store: kv_handle, key: string, score: f64, member: bytes) -> result[void, string]
      + adds a scored member to a sorted set
      # storage
    std.kv.zrange_by_score
      fn (store: kv_handle, key: string, max_score: f64, limit: i32) -> result[list[bytes], string]
...
mq
  mq.new_queue
    fn (store: kv_handle, name: string) -> queue_handle
    + binds a queue handle to a backing store and logical name
    # construction
  mq.publish
    fn (q: queue_handle, payload: bytes, delay_ms: i64) -> result[string, string]
    ...
    -> std.kv.zadd
  mq.consume
    fn (q: queue_handle) -> result[optional[queue_message], string]
    ...
    -> std.kv.zrange_by_score
  mq.ack
    fn (q: queue_handle, message_id: string) -> result[void, string]
    + removes the in-flight message
    - returns error when the message id is unknown
    # consumer
  mq.nack
    fn (q: queue_handle, message_id: string, retry_delay_ms: i64) -> result[void, string]
    + requeues an in-flight message after retry_delay_ms
    - returns error when the message id is unknown
    # consumer
    -> std.kv.zadd
  mq.reclaim_expired
    fn (q: queue_handle, visibility_ms: i64) -> result[i32, string]

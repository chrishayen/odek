...
std
  std.net
    std.net.tcp_listen
      @ (port: u16) -> result[listener_state, string]
      + binds a TCP listener on the given port
      - returns error when the port is already in use
      # network
    std.net.tcp_accept
      @ (l: listener_state) -> result[conn_state, string]
      + blocks until a client connects
      # network
    std.net.tcp_read
      @ (c: conn_state, max: i32) -> result[bytes, string]
      + reads up to max bytes from the connection
      # network
    std.net.tcp_write
      @ (c: conn_state, data: bytes) -> result[void, string]
...
remote
  remote.start_host
    @ (port: u16) -> result[host_state, string]
  ...
  remote.accept_pairing
    @ (host: host_state, code: string) -> result[session_state, string]

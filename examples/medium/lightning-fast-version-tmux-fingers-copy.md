...
std
  std.regex
    std.regex.compile
      ...
      # parsing
    std.regex.find_all
      @ (re: regex_handle, text: string) -> list[regex_match]
      + returns all non-overlapping matches with byte offsets and captured text
      # parsing

hinter
  hinter.scan
    @ (screen: string, patterns: list[string]) -> result[list[hint], string]
  ...
  hinter.assign_labels
    @ (hints: list[hint], alphabet: string) -> list[labeled_hint]
    ...
    # labeling
  hinter.resolve
    @ (labeled: list[labeled_hint], typed: string) -> hint_resolution

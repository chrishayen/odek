...
sqlite_tools
  ...
  sqlite_tools.create_table
    @ (conn: sqlite_conn, table: string, columns: map[string, string]) -> result[void, string]
  ...
  sqlite_tools.insert_row
    @ (conn: sqlite_conn, table: string, row: map[string, string]) -> result[void, string]
  ...
  sqlite_tools.upsert_row
    @ (conn: sqlite_conn, table: string, pk_column: string, row: map[string, string]) -> result[void, string]
    + inserts or replaces by primary key column
    - returns error when pk_column is not present in row
    # rows
    -> std.sqlite.exec
  sqlite_tools.find_rows
    @ (conn: sqlite_conn, table: string, filter: map[string, string]) -> result[list[map[string, string]], string]
    + returns all rows where every filter column equals its value
    # rows
    -> std.sqlite.query
  sqlite_tools.delete_rows
    @ (conn: sqlite_conn, table: string, filter: map[string, string]) -> result[i32, string]
    + deletes matching rows and returns the number removed
    # rows
    -> std.sqlite.exec
  sqlite_tools.list_tables
    @ (conn: sqlite_conn) -> result[list[string], string]

...
std
  std.io
    ...
    std.io.bit_reader_skip
      fn (state: bit_reader_state, count: i32) -> result[bit_reader_state, string]
...
aac_parser
  aac_parser.new
    fn (data: bytes) -> aac_parser_state
    + wraps data in a new parser starting at byte 0
    # construction
    -> std.io.bit_reader_new
  aac_parser.find_sync
    fn (state: aac_parser_state) -> result[aac_parser_state, string]
  ...
  aac_parser.parse_header
    fn (state: aac_parser_state) -> result[tuple[adts_header, aac_parser_state], string]
  ...
  aac_parser.read_frame
    fn (state: aac_parser_state) -> result[tuple[aac_frame, aac_parser_state], string]
    ...
    -> std.io.bit_reader_skip
  aac_parser.iterate
    fn (data: bytes) -> result[list[aac_frame], string]

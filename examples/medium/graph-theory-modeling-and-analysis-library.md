...
graph_lib
  graph_lib.new
    ...
    # construction
  graph_lib.add_node
    fn (state: graph_state, id: string) -> graph_state
    + adds a node with the given id
    - returns unchanged state when the node already exists
    # mutation
  graph_lib.add_edge
    fn (state: graph_state, from: string, to: string, weight: f64) -> result[graph_state, string]
    + adds a weighted edge between two nodes
    - returns error when either endpoint is unknown
    # mutation
  graph_lib.neighbors
    fn (state: graph_state, id: string) -> list[string]
    + returns the ids of nodes adjacent to id
    # query
  graph_lib.shortest_path
    fn (state: graph_state, from: string, to: string) -> optional[list[string]]
  ...
  graph_lib.degree_centrality
    fn (state: graph_state) -> map[string, f64]

...
std
  std.http
    std.http.forward
      fn (url: string, body: bytes) -> result[http_response, string]
...
ts_cache
  ts_cache.new
    ...
    # construction
  ts_cache.lookup
    fn (state: cache_state, key: string, start_ms: i64, end_ms: i64) -> cache_hit
    ...
    -> std.time.now_millis
  ts_cache.store
    fn (state: cache_state, key: string, start_ms: i64, end_ms: i64, series: list[sample]) -> cache_state
  ...
  ts_cache.handle_request
    fn (state: cache_state, upstream_url: string, key: string, start_ms: i64, end_ms: i64) -> result[tuple[list[sample], cache_state], string]
  ...
  ts_cache.merge_ranges
    fn (existing: list[sample], delta: list[sample]) -> list[sample]

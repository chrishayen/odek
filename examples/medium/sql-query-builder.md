...
sql_builder
  ...
  sql_builder.where_eq
    fn (state: query_state, column: string, value: string) -> query_state
    + appends an equality predicate and a positional parameter
    ? multiple calls are AND-ed together
    # where
  sql_builder.order_by
    fn (state: query_state, column: string, ascending: bool) -> query_state
    + appends an ORDER BY clause
    # ordering
  sql_builder.limit
    fn (state: query_state, n: i64) -> query_state
    + sets the LIMIT
    # limit
  sql_builder.insert
    fn (table: string, values: map[string, string]) -> query_state
    + builds an INSERT with parameter placeholders for each value
    - returns an empty state when values is empty
    # insert
  sql_builder.update
    fn (table: string, assignments: map[string, string]) -> query_state
    + builds an UPDATE with placeholders for each assignment
    # update
  sql_builder.render
    fn (state: query_state) -> tuple[string, list[string]]

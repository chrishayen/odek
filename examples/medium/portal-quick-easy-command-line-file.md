...
std
  std.net
    ...
    std.net.send_frame
      ...
      # networking
    std.net.recv_frame
      @ (conn: conn_handle) -> result[bytes, string]
  ...
  std.crypto
    std.crypto.derive_key
      ...
      # cryptography
    std.crypto.seal
      @ (key: bytes, plaintext: bytes) -> bytes
...
portal
  portal.send_file
    @ (host: string, port: u16, passphrase: string, path: string, contents: bytes) -> result[void, string]
    ...
    -> std.crypto.seal
  portal.receive_file
    @ (port: u16, passphrase: string) -> result[received_file, string]

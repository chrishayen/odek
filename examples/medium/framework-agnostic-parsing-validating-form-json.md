...
std
  ...
  std.multipart
    std.multipart.parse
      @ (body: bytes, boundary: string) -> result[list[multipart_part], string]
...
forms
  forms.new_schema
    ...
    # construction
  forms.field
    @ (schema: form_schema, name: string, type_name: string, required: bool) -> form_schema
    + declares a field with a type keyword like "string", "int", "float", "bool"
    # schema
  forms.rule
    @ (schema: form_schema, field: string, rule: validation_rule) -> form_schema
    + attaches a validation rule to a field (min, max, pattern, etc.)
    # schema
  forms.file_field
    @ (schema: form_schema, name: string, max_bytes: i64, required: bool) -> form_schema
    + declares a file upload field with a maximum size
    # schema
  forms.parse_body
    @ (schema: form_schema, content_type: string, body: bytes) -> result[parsed_form, string]
    ...
    -> std.multipart.parse
  forms.validate
    @ (schema: form_schema, form: parsed_form) -> result[parsed_form, list[validation_error]]
    ...
    # validation
  forms.get_string
    @ (form: parsed_form, name: string) -> result[string, string]
    + returns a string field value
    - returns error when the field is missing or of another type
    # access
  forms.get_file
    @ (form: parsed_form, name: string) -> result[uploaded_file, string]

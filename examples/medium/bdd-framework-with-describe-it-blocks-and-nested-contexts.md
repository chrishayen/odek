...
bdd
  bdd.describe
    @ (name: string, body: fn(context_builder) -> context_builder) -> context
    + creates a context node with the given name and child nodes from the body
    # structure
  bdd.it
    @ (builder: context_builder, name: string, test: fn() -> test_result) -> context_builder
    + adds a test case to the current context
    # structure
  bdd.before_each
    @ (builder: context_builder, hook: fn() -> void) -> context_builder
  ...
  bdd.after_each
    ...
    # hooks
  bdd.nested
    @ (builder: context_builder, name: string, body: fn(context_builder) -> context_builder) -> context_builder
    + adds a nested describe context
    # structure
  bdd.run
    @ (root: context) -> run_report

...
api_monitor
  ...
  api_monitor.start_span
    @ (state: monitor_state, route: string) -> tuple[span_id, monitor_state]
    + returns a fresh span id and a state with the pending span registered
    # tracing
    -> std.time.now_millis
  api_monitor.finish_span
    @ (state: monitor_state, id: span_id, status_code: i32) -> monitor_state
  ...
  api_monitor.route_stats
    @ (state: monitor_state, route: string) -> route_stats

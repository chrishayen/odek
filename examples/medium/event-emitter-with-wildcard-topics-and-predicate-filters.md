...
emitter
  emitter.new
    ...
    # construction
  emitter.on
    fn (state: emitter_state, pattern: string) -> tuple[emitter_state, subscription_id]
    ...
    # subscription
  emitter.on_where
    fn (state: emitter_state, pattern: string, predicate: fn(string, bytes) -> bool) -> tuple[emitter_state, subscription_id]
    + registers a subscription that only fires when the predicate returns true
    # subscription
  emitter.off
    fn (state: emitter_state, id: subscription_id) -> emitter_state
    + cancels a subscription; subsequent emits skip it
    + no-op when the id is not present
    # cancellation
  emitter.emit
    fn (state: emitter_state, topic: string, payload: bytes) -> list[delivery]
  ...
  emitter.match_pattern
    fn (pattern: string, topic: string) -> bool

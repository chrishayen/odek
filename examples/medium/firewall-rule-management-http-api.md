...
firewall_api
  firewall_api.parse_rule
    fn (raw: string) -> result[firewall_rule, string]
  ...
  firewall_api.handle_list
    fn (backend: firewall_backend) -> http_response
    + returns a 200 response with every current rule as json
    # handler
    -> std.json.encode_value
  firewall_api.handle_add
    fn (backend: firewall_backend, body: string) -> http_response
  ...
  firewall_api.handle_delete
    fn (backend: firewall_backend, rule_id: string) -> http_response
    + removes the rule and returns 204
    - returns 404 when the rule id does not exist
    # handler
  firewall_api.require_auth
    fn (req: http_request, checker: auth_checker, inner: request_handler) -> http_response

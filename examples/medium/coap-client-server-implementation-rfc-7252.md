...
coap
  ...
  coap.new_client
    ...
    # construction
  coap.build_request
    @ (state: client_state, method: string, uri_path: string, payload: bytes) -> tuple[bytes, client_state]
  ...
  coap.on_client_datagram
    @ (state: client_state, data: bytes) -> result[tuple[coap_message, client_state], string]
  ...
  coap.register_resource
    @ (state: server_state, uri_path: string, handler_name: string) -> server_state
  ...
  coap.dispatch_request
    @ (state: server_state, data: bytes) -> result[tuple[string, coap_message], string]
    ...
    # routing
  coap.build_response
    @ (request: coap_message, code: string, payload: bytes) -> bytes
    + builds an acknowledgment response matching the request's token and id
    # response
  coap.retransmit_due
    @ (state: client_state) -> tuple[list[bytes], client_state]

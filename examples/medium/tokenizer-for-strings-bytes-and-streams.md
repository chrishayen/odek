...
tokenizer
  ...
  tokenizer.add_literal
    @ (def: token_definition, kind: string, literal: string) -> token_definition
    + returns a definition that matches the exact literal as the given kind
    # definition
  tokenizer.add_pattern
    @ (def: token_definition, kind: string, match_fn: fn(string, i64) -> i64) -> token_definition
  ...
  tokenizer.scan_string
    @ (def: token_definition, source: string) -> result[list[token], string]
    + returns every token in the source in order
    - returns error when no definition matches at some offset
    # scanning
  tokenizer.scan_bytes
    @ (def: token_definition, source: bytes) -> result[list[token], string]
  ...
  tokenizer.new_stream_scanner
    @ (def: token_definition, read_fn: fn() -> optional[string]) -> stream_scanner
  ...
  tokenizer.next_token
    @ (scanner: stream_scanner) -> result[optional[token], string]

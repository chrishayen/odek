...
std
  std.net
    std.net.tcp_listen
      @ (addr: string) -> result[listener_handle, string]
    ...
    std.net.tcp_accept
      ...
      # network
    std.net.tcp_dial
      @ (addr: string) -> result[conn_handle, string]
...
tcp_msg
  tcp_msg.serve
    @ (addr: string, handler: fn(conn_handle, bytes) -> bytes) -> result[void, string]
  ...
  tcp_msg.connect
    ...
    -> std.net.tcp_dial
  tcp_msg.send
    @ (conn: conn_handle, msg: bytes) -> result[void, string]

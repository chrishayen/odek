...
std
  std.amqp
    ...
    std.amqp.consume_one
      fn (conn: amqp_conn, queue: string) -> result[optional[amqp_message], string]
      + returns the next message or none if the queue is empty
      # consumption

broker_tools
  broker_tools.list_queues
    fn (mgmt_url: string, credentials: string) -> result[list[queue_info], string]
    + returns queue name, message count, and consumer count
    - returns error when the management endpoint is unreachable
    # introspection
  broker_tools.tap
    fn (conn: amqp_conn, exchange: string, routing_key: string, handler: fn(amqp_message) -> void) -> result[void, string]
    + binds a temporary queue and invokes handler for every matching message until cancelled
    # observation
    -> std.amqp.declare_queue
    -> std.amqp.consume_one
  broker_tools.move_messages
    fn (conn: amqp_conn, from_queue: string, to_exchange: string, to_key: string, limit: i32) -> result[i32, string]
  ...
  broker_tools.publish_bulk
    fn (conn: amqp_conn, exchange: string, routing_key: string, bodies: list[bytes]) -> result[i32, string]

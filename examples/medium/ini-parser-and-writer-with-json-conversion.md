...
ini
  ini.parse
    @ (raw: string) -> result[ini_doc, string]
    ...
    # parsing
  ini.load_file
    @ (path: string) -> result[ini_doc, string]
    + reads and parses an ini file
    # loading
    -> std.fs.read_all
  ini.get
    @ (doc: ini_doc, section: string, key: string) -> optional[string]
    + returns the value for the section and key
    - returns none when absent
    # query
  ini.set
    @ (doc: ini_doc, section: string, key: string, value: string) -> ini_doc
    + sets or adds a key under the section
    # write
  ini.delete
    @ (doc: ini_doc, section: string, key: string) -> ini_doc
  ...
  ini.render
    ...
    # rendering
  ini.save_file
    @ (doc: ini_doc, path: string) -> result[void, string]

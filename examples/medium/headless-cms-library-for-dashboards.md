...
cms
  cms.new_store
    ...
    # construction
  cms.define_type
    fn (state: cms_state, type_name: string, fields: list[string]) -> result[cms_state, string]
    + registers a new content type with the given field names
    - returns error when the type already exists
    # schema
  cms.create_entry
    fn (state: cms_state, type_name: string, data: map[string, string]) -> result[tuple[string, cms_state], string]
    ...
    -> std.time.now_millis
  cms.get_entry
    fn (state: cms_state, id: string) -> optional[cms_entry]
    + returns the entry when present
    - returns none when the id is unknown
    # read
  cms.update_entry
    fn (state: cms_state, id: string, data: map[string, string]) -> result[cms_state, string]
    + merges the new data into the existing entry
    - returns error when the id is unknown
    # write
    -> std.time.now_millis
  cms.delete_entry
    fn (state: cms_state, id: string) -> result[cms_state, string]
    + removes the entry
    - returns error when the id is unknown
    # write
  cms.list_by_type
    fn (state: cms_state, type_name: string) -> list[cms_entry]

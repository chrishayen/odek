...
httpx
  httpx.new_client
    ...
    # construction
  httpx.with_timeout
    fn (c: client_state, ms: i64) -> client_state
    + sets the per-request timeout
    # configuration
  httpx.with_retries
    fn (c: client_state, max: i32, base_delay_ms: i64) -> client_state
    + sets the retry count and exponential backoff base delay
    # configuration
  httpx.build_url
    fn (base: string, query: map[string, string]) -> string
    + appends encoded query parameters to a base URL
    + returns base unchanged when query is empty
    # url_building
    -> std.encoding.url_encode
  httpx.get
    fn (c: client_state, url: string) -> result[http_response, string]
    ...
    -> std.time.sleep_millis
  httpx.post_json
    fn (c: client_state, url: string, body: string) -> result[http_response, string]

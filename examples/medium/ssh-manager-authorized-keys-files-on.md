...
authkeys
  authkeys.parse_file
    @ (content: string) -> list[auth_entry]
  ...
  authkeys.plan_changes
    ...
    # diffing
  authkeys.apply_plan
    @ (host: string, plan: change_plan, fetch: fn(string) -> result[string, string], push: fn(string, string) -> result[void, string]) -> result[apply_result, string]

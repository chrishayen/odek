...
std
  ...
  std.io
    std.io.write_string
      @ (sink: string, data: string) -> result[void, string]
...
http_interceptors
  ...
  http_interceptors.add
    @ (chain: chain_state, name: string, before: string, after: string) -> chain_state
  ...
  http_interceptors.execute
    @ (chain: chain_state, request: http_request) -> result[http_response, string]

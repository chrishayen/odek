...
std
  ...
  std.kv
    std.kv.open
      @ (path: string) -> result[kv_store, string]
    ...
    std.kv.put
      ...
      # storage
    std.kv.get
      @ (store: kv_store, key: bytes) -> result[optional[bytes], string]
    ...
    std.kv.delete
      ...
      # storage
    std.kv.scan_prefix
      @ (store: kv_store, prefix: bytes) -> result[list[tuple[bytes, bytes]], string]
...
job_queue
  job_queue.open
    ...
    -> std.kv.open
  job_queue.enqueue
    @ (state: queue_state, topic: string, payload: bytes, run_at_ms: i64) -> result[string, string]
    ...
    -> std.kv.put
  job_queue.claim
    @ (state: queue_state, topic: string, worker_id: string, lease_ms: i32) -> result[optional[job], string]
  ...
  job_queue.complete
    ...
    -> std.kv.delete
  job_queue.fail
    @ (state: queue_state, job_id: string, retry_delay_ms: i32) -> result[void, string]
  ...
  job_queue.pending_count
    @ (state: queue_state, topic: string) -> result[i64, string]

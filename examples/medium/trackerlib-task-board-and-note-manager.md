...
trackerlib
  ...
  trackerlib.add_board
    fn (state: tracker_state, title: string) -> tuple[i32, tracker_state]
    + creates a board and returns its id
    # mutation
  trackerlib.add_task
    fn (state: tracker_state, board_id: i32, title: string) -> result[tuple[i32, tracker_state], string]
    + creates a task on the board
    - returns error when the board does not exist
    # mutation
  trackerlib.add_note
    fn (state: tracker_state, board_id: i32, body: string) -> result[tuple[i32, tracker_state], string]
  ...
  trackerlib.move_task
    fn (state: tracker_state, task_id: i32, to_board_id: i32) -> result[tracker_state, string]
    + moves the task to a different board
    - returns error when either id does not exist
    # mutation
  trackerlib.delete_item
    fn (state: tracker_state, item_id: i32) -> result[tracker_state, string]
    + removes the task or note
    - returns error when the id does not exist
    # mutation
  trackerlib.list_board
    fn (state: tracker_state, board_id: i32) -> result[board_view, string]

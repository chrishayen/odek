...
std
  std.http
    std.http.get_json
      @ (url: string) -> result[json_value, string]
...
depcheck
  ...
  depcheck.fetch_advisories
    @ (advisory_url: string, name: string) -> result[list[advisory], string]
    + queries a pluggable advisory source for known vulnerabilities affecting a package
    - returns error on network failure
    # advisories
    -> std.http.get_json
  depcheck.classify
    @ (dep: dependency, latest: semver, advisories: list[advisory]) -> dependency_status
    ...
    -> std.semver.compare
  depcheck.scan
    @ (lockfile: string, registry_url: string, advisory_url: string) -> result[list[dependency_status], string]

...
std
  std.term
    ...
    std.term.write
      ...
      # terminal
    std.term.set_raw
      @ (enable: bool) -> result[bool, string]
...
liner
  liner.new
    ...
    # construction
  liner.insert
    @ (state: liner_state, ch: string) -> liner_state
    + inserts a character at the cursor and advances it
    # editing
  liner.backspace
    @ (state: liner_state) -> liner_state
    + deletes the character before the cursor
    - leaves state unchanged when the cursor is at 0
    # editing
  liner.move_cursor
    @ (state: liner_state, delta: i32) -> liner_state
  ...
  liner.history_next
    ...
    # history
  liner.set_completer
    @ (state: liner_state, completer: fn(string) -> list[string]) -> liner_state
    + registers a completion function invoked on tab
    # completion
  liner.read_line
    @ (state: liner_state, prompt: string) -> result[tuple[string, liner_state], string]

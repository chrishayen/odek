...
std
  ...
  std.ssh_key
    std.ssh_key.parse_public
      @ (raw: bytes) -> result[ssh_public_key, string]
      + parses an openssh-format public key
      - returns error on malformed input
      # keys
    std.ssh_key.parse_private
      @ (raw: bytes, passphrase: optional[string]) -> result[ssh_private_key, string]
...
secrets
  secrets.encrypt
    @ (public_key_path: string, plaintext: bytes) -> result[bytes, string]
    ...
    -> std.crypto.rsa_encrypt
  secrets.decrypt
    @ (private_key_path: string, passphrase: optional[string], vault_bytes: bytes) -> result[bytes, string]

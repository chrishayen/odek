...
std
  ...
  std.random
    std.random.new_seeded
      ...
      # randomness
    std.random.next_i64
      fn (rng: rng_state, lo: i64, hi: i64) -> tuple[i64, rng_state]
...
data_gen
  data_gen.new_schema
    ...
    # construction
  data_gen.add_table
    fn (schema: schema_state, name: string, columns: list[column_spec]) -> schema_state
  ...
  data_gen.add_foreign_key
    fn (schema: schema_state, from_table: string, from_column: string, to_table: string, to_column: string) -> schema_state
    + declares that from_table.from_column references to_table.to_column
    # relationships
  data_gen.generate
    fn (schema: schema_state, counts: map[string, i64], seed: i64) -> map[string, list[list[string]]]
    ...
    -> std.random.next_i64
  data_gen.encode_csv
    fn (header: list[string], rows: list[list[string]]) -> string
    + returns a CSV string with the given header and rows
    # csv
  data_gen.write
    fn (schema: schema_state, data: map[string, list[list[string]]], out_dir: string) -> result[i32, string]

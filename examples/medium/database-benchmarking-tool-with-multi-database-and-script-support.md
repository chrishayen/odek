...
bench
  bench.load_script
    fn (source: string) -> result[list[string], string]
    + parses a script into a list of statements separated by semicolons
    - returns error on unterminated quoted strings
    # scripting
  bench.new_run
    fn (driver_name: string, script: list[string], concurrency: i32, iterations: i32) -> bench_run
  ...
  bench.register_driver
    fn (name: string, executor: driver_fn) -> void
    + associates a name with a statement-executing function
    # driver_registry
  bench.execute
    fn (run: bench_run) -> result[bench_report, string]

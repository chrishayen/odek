...
flow_machine
  ...
  flow_machine.declare_state
    fn (state: machine_state, name: string, requires: list[string], removes: list[string]) -> result[machine_state, string]
  ...
  flow_machine.declare_order
    fn (state: machine_state, before: string, after: string) -> result[machine_state, string]
  ...
  flow_machine.add_hook
    fn (state: machine_state, target: string, phase: string, hook: closure[void]) -> result[machine_state, string]
    + registers a hook to run on "before" or "after" entering the target state
    - returns error on unknown phase
    # aspects
  flow_machine.add
    fn (state: machine_state, states: list[string]) -> result[machine_state, string]

...
redis_client
  ...
  redis_client.do_command
    @ (client: client_state, args: list[string]) -> result[reply_value, string]
    ...
    -> std.socket.read
  redis_client.get
    @ (client: client_state, key: string) -> result[optional[string], string]
    + returns the string value for key, or none when the key is absent
    # commands
  redis_client.set
    @ (client: client_state, key: string, value: string) -> result[void, string]
    + stores value under key
    # commands
  redis_client.del
    @ (client: client_state, key: string) -> result[i64, string]
    + deletes the key and returns the number of keys removed
    # commands
  redis_client.pipeline
    @ (client: client_state, commands: list[list[string]]) -> result[list[reply_value], string]
    + sends all commands before reading any reply, then reads replies in order
    # pipelining
  redis_client.close
    @ (client: client_state) -> result[void, string]

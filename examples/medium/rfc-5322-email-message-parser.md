...
std
  ...
  std.encoding
    std.encoding.decode_quoted_printable
      fn (data: string) -> result[string, string]
...
email_parser
  email_parser.parse
    fn (raw: bytes) -> result[email_message, string]
  ...
  email_parser.split_headers
    fn (raw: bytes) -> result[tuple[list[header_line], bytes], string]
  ...
  email_parser.parse_header_line
    fn (line: string) -> result[tuple[string, string], string]

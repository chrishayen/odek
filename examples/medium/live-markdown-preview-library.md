...
std
  std.fs
    std.fs.read_file
      ...
      # filesystem
    std.fs.watch
      @ (path: string) -> result[watcher_state, string]
...
md_preview
  md_preview.render
    ...
    # rendering
  md_preview.start
    @ (file_path: string, port: i32) -> result[preview_state, string]
    ...
    -> std.net.ws_listen
  md_preview.tick
    @ (state: preview_state) -> result[bool, string]
    ...
    -> std.net.ws_broadcast
  md_preview.stop
    @ (state: preview_state) -> result[void, string]

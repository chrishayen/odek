...
gql
  gql.new_schema
    ...
    # construction
  gql.object
    @ (s: schema_state, name: string, fields: list[field_def]) -> schema_state
    ...
    # type_registration
  gql.enum
    @ (s: schema_state, name: string, values: list[string]) -> schema_state
    + registers an enum type
    # type_registration
  gql.query_root
    @ (s: schema_state, type_name: string) -> schema_state
  ...
  gql.mutation_root
    ...
    # root
  gql.parse_query
    @ (source: string) -> result[query_doc, string]
    + parses a GraphQL query string into a document
    - returns error on syntax issues
    # parsing
  gql.validate_query
    @ (s: schema_state, doc: query_doc) -> result[void, string]
    ...
    # validation
  gql.execute
    @ (s: schema_state, doc: query_doc, variables: map[string, value]) -> result[value, string]
    + runs resolvers over the selection set and returns the resulting tree
    - returns error on validation or resolver failure
    # execution
  gql.run
    @ (s: schema_state, source: string, variables: map[string, value]) -> result[value, string]

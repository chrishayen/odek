...
std
  std.process
    std.process.run
      @ (workdir: string, program: string, args: list[string]) -> result[process_result, string]
...
depcharge
  depcharge.load_plan
    @ (text: string) -> result[dep_plan, string]
  ...
  depcharge.load_plan_from_file
    @ (path: string) -> result[dep_plan, string]
    + reads a plan from disk and parses it
    # plan
    -> std.fs.read_all
    -> depcharge.load_plan
  depcharge.run_command
    @ (plan: dep_plan, program: string, args: list[string]) -> list[command_result]
  ...
  depcharge.run_command_parallel
    @ (plan: dep_plan, program: string, args: list[string], workers: i32) -> list[command_result]

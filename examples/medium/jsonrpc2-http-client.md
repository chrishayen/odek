...
jsonrpc_client
  jsonrpc_client.new
    fn (endpoint: string) -> jsonrpc_client_state
    + creates a client bound to the given HTTP endpoint
    # construction
  jsonrpc_client.next_id
    fn (state: jsonrpc_client_state) -> tuple[string, jsonrpc_client_state]
  ...
  jsonrpc_client.parse_response
    ...
    -> std.json.parse_object
  jsonrpc_client.call
    fn (state: jsonrpc_client_state, method: string, params: map[string, string]) -> result[map[string, string], string]

...
std
  std.net
    std.net.listen_tcp
      fn (port: i32) -> result[listener, string]
...
protocol_server
  ...
  protocol_server.read_ws_frame
    fn (session: ws_session) -> result[ws_frame, string]
  ...
  protocol_server.dispatch_to_app
    fn (app: app_handler, scope: request_scope, body: bytes) -> result[bytes, string]

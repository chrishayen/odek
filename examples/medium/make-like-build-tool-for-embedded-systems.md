...
std
  ...
  std.process
    std.process.run_in
      fn (cwd: string, command: string, args: list[string], env: map[string, string]) -> result[i32, string]
...
build_tool
  build_tool.new
    fn (work_root: string) -> build_state
    + creates a build state rooted at a working directory
    # construction
  build_tool.load_recipe
    fn (state: build_state, path: string) -> result[build_state, string]
  ...
  build_tool.resolve_order
    fn (state: build_state, target: string) -> result[list[string], string]
  ...
  build_tool.build_one
    fn (state: build_state, recipe_name: string) -> result[void, string]
    ...
    -> std.process.run_in
  build_tool.build
    fn (state: build_state, target: string) -> result[void, string]

...
sessions
  sessions.new
    @ (session_tool: string) -> manager_state
    + creates a manager bound to a session-multiplexer command name
    # construction
  sessions.load_workspaces
    @ (state: manager_state, roots: list[string]) -> result[manager_state, string]
  ...
  sessions.load_recency
    @ (state: manager_state, frecency_cmd: string) -> result[manager_state, string]
    ...
    -> std.proc.run_capture
  sessions.list
    @ (state: manager_state) -> list[workspace_entry]
  ...
  sessions.session_name
    ...
    # naming
  sessions.open
    @ (state: manager_state, workspace: string) -> result[void, string]
    ...
    -> std.proc.run
  sessions.kill
    @ (state: manager_state, name: string) -> result[void, string]

...
std
  std.fs
    std.fs.list_dir
      ...
      # filesystem
    std.fs.read_head
      @ (path: string, max_bytes: i64) -> result[bytes, string]
...
file_manager
  file_manager.new
    @ (root: string) -> result[fm_state, string]
  ...
  file_manager.move_cursor
    @ (state: fm_state, delta: i32) -> fm_state
    + moves the cursor by delta, clamped to the entry list
    # navigation
  file_manager.enter
    @ (state: fm_state) -> result[fm_state, string]

...
extractor
  extractor.define_schema
    fn (fields: list[tuple[string, string]]) -> schema
  ...
  extractor.build_prompt
    fn (schema: schema, instruction: string) -> string
    + returns a prompt that embeds the schema and instructs the model to reply in JSON
    # prompting
    -> std.json.encode_object
  extractor.parse_response
    fn (schema: schema, response: string) -> result[map[string, string], string]
  ...
  extractor.build_repair_prompt
    fn (original: string, error: string) -> string
    + returns a follow-up prompt asking the model to fix its previous response, given the error
    # repair
  extractor.extract
    fn (schema: schema, instruction: string, call_model: fn(string) -> string, max_attempts: i32) -> result[map[string, string], string]

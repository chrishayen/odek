...
jsoniter
  jsoniter.new_tokenizer
    @ (input: bytes) -> tokenizer_state
    + creates a tokenizer positioned at the start of input
    # lexer
  jsoniter.next_token
    @ (state: tokenizer_state) -> result[tuple[json_token, tokenizer_state], string]
    ...
    # lexer
  jsoniter.parse
    @ (input: bytes) -> result[json_value, string]
  ...
  jsoniter.parse_stream
    @ (input: bytes, visit: fn(path: string, value: json_value) -> void) -> result[void, string]
  ...
  jsoniter.encode_indented
    @ (value: json_value, indent: string) -> bytes

...
std
  std.crypto
    ...
    std.crypto.pbkdf2_sha256
      @ (password: bytes, salt: bytes, iterations: i32, length: i32) -> bytes
  ...
  std.url
    std.url.parse_otpauth
      @ (uri: string) -> result[map[string, string], string]
...
authenticator
  ...
  authenticator.seal_vault
    @ (accounts: list[account], passphrase: string, salt: bytes, nonce: bytes) -> bytes
    + returns a passphrase-encrypted blob of the account list
    # vault_seal
    -> std.crypto.pbkdf2_sha256
    -> std.crypto.aead_encrypt
  authenticator.open_vault
    @ (blob: bytes, passphrase: string, salt: bytes, nonce: bytes) -> result[list[account], string]

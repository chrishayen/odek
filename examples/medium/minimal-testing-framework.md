...
testing
  testing.new_suite
    ...
    # construction
  testing.describe
    @ (state: suite_state, group: string, body: fn(suite_state) -> suite_state) -> suite_state
    + registers a nested group; nested describes concatenate names with " > "
    # grouping
  testing.it
    @ (state: suite_state, name: string, body: fn() -> optional[string]) -> suite_state
    + registers a test case; body returns none on success or an error message on failure
    # registration
  testing.assert_equal
    @ (expected: string, actual: string) -> optional[string]

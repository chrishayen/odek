...
mock
  mock.new_mock
    fn (name: string) -> mock_state
    + creates a mock with the given identifier
    # construction
  mock.expect
    fn (m: mock_state, method: string, args: list[value], returns: list[value]) -> mock_state
    + appends an expected call with argument matchers and the values to return
    # expectation
  mock.expect_times
    fn (m: mock_state, method: string, args: list[value], returns: list[value], times: i32) -> mock_state
    + records that the call must happen exactly times times
    # expectation
  mock.call
    fn (m: mock_state, method: string, args: list[value]) -> result[tuple[list[value], mock_state], string]
    ...
    # invocation
  mock.verify
    fn (m: mock_state) -> result[void, string]

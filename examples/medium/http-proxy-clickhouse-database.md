...
std
  std.http
    std.http.forward
      @ (url: string, body: bytes, headers: map[string,string]) -> result[http_response, string]
...
db_proxy
  db_proxy.new_config
    ...
    # construction
  db_proxy.add_user
    @ (cfg: proxy_config, username: string, password_hash: string, max_qps: i32) -> proxy_config
    + registers a user with credentials and a per-second quota
    # configuration
  db_proxy.add_upstream
    @ (cfg: proxy_config, name: string, url: string) -> proxy_config
    + registers an upstream by logical name
    # configuration
  db_proxy.authenticate
    @ (cfg: proxy_config, username: string, password: string) -> result[user_id, string]
  ...
  db_proxy.check_quota
    @ (state: quota_state, uid: user_id, limit: i32) -> tuple[bool, quota_state]
  ...
  db_proxy.handle_query
    @ (cfg: proxy_config, state: quota_state, uid: user_id, upstream: string, query: string) -> result[bytes, string]

...
std
  ...
  std.semver
    std.semver.parse
      fn (input: string) -> result[version, string]
      + parses a MAJOR.MINOR.PATCH string
      - returns error on malformed input
      # versioning
    std.semver.matches
      fn (requirement: string, v: version) -> bool
...
dep_manager
  dep_manager.load
    ...
    -> std.fs.read_all
  dep_manager.save
    fn (state: manifest_state, path: string) -> result[void, string]
    + serializes the manifest back to disk
    # manifest
    -> std.fs.write_atomic
  dep_manager.add
    fn (state: manifest_state, name: string, requirement: string) -> result[manifest_state, string]
    + adds or updates a dependency requirement
    - returns error when the requirement is not a valid semver range
    # edits
    -> std.semver.parse
  dep_manager.remove
    fn (state: manifest_state, name: string) -> result[manifest_state, string]
    + removes a dependency
    - returns error when the dependency is not in the manifest
    # edits
  dep_manager.resolve
    fn (state: manifest_state, registry_fn: string) -> result[map[string, string], string]
  ...
  dep_manager.write_lock
    fn (resolved: map[string, string], path: string) -> result[void, string]
    + writes the resolved name-to-version map to a lockfile
    # lockfile
    -> std.fs.write_atomic
  dep_manager.check_outdated
    fn (state: manifest_state, registry_fn: string) -> list[tuple[string, string, string]]

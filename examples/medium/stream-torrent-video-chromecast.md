...
std
  std.http
    ...
    std.http.parse_range_header
      @ (raw: string) -> result[tuple[i64, i64], string]
...
torrent_caster
  torrent_caster.open_source
    @ (torrent_path: string, file_index: i32) -> result[torrent_source, string]
  ...
  torrent_caster.read_range
    @ (source: torrent_source, start: i64, end: i64) -> result[bytes, string]
  ...
  torrent_caster.handle_stream_request
    @ (source: torrent_source, range_header: optional[string]) -> result[tuple[i32, map[string,string], bytes], string]
  ...
  torrent_caster.serve
    @ (source: torrent_source, addr: string) -> result[void, string]
    + serves the torrent file under a single endpoint until interrupted
    - returns error when the address cannot be bound
    # server
    -> std.http.serve
  torrent_caster.cast
    @ (controller: cast_controller, stream_url: string, mime: string) -> result[void, string]

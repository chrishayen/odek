...
bencher
  bencher.new_store
    ...
    # construction
  bencher.record
    fn (store: store_state, benchmark_id: string, value: f64) -> store_state
    + appends a new measurement with the current timestamp
    # ingest
    -> std.time.now_millis
  bencher.baseline_stats
    fn (store: store_state, benchmark_id: string, window: i32) -> result[baseline_stats, string]
  ...
  bencher.detect_regression
    fn (store: store_state, benchmark_id: string, value: f64, sigma_threshold: f64) -> result[bool, string]
    ...
    # regression
  bencher.history
    fn (store: store_state, benchmark_id: string) -> list[tuple[i64, f64]]
    + returns all (timestamp, value) pairs for a benchmark in order
    # query
  bencher.clear
    fn (store: store_state, benchmark_id: string) -> store_state

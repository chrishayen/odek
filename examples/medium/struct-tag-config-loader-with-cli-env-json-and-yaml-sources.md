...
config_loader
  config_loader.describe
    fn (schema: list[field_spec]) -> schema_state
    + returns a schema_state capturing field names, types, tag keys, and defaults
    # schema
  config_loader.parse_args
    fn (schema: schema_state, argv: list[string]) -> result[map[string, string], string]
  ...
  config_loader.from_env
    fn (schema: schema_state, prefix: string) -> map[string, string]
    + returns values read from environment variables matching the schema's env tags
    # source_env
    -> std.env.lookup
  config_loader.from_json
    fn (schema: schema_state, path: string) -> result[map[string, string], string]
  ...
  config_loader.coerce
    fn (schema: schema_state, raw: map[string, string]) -> result[map[string, config_value], string]

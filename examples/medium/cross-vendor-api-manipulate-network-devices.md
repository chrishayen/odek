...
std
  std.net
    std.net.ssh_connect
      @ (host: string, port: u16, user: string, password: string) -> result[ssh_session, string]
      + opens an ssh session to the device
      - returns error on auth failure or unreachable host
      # transport
    std.net.ssh_exec
      @ (session: ssh_session, command: string) -> result[string, string]
...
net_device
  net_device.register_driver
    @ (vendor: string, driver: device_driver) -> void
    ...
    # driver_registry
  net_device.open
    @ (vendor: string, host: string, user: string, password: string) -> result[device_handle, string]
  ...
  net_device.get_config
    @ (handle: device_handle) -> result[string, string]
  ...
  net_device.load_merge_config
    @ (handle: device_handle, config: string) -> result[void, string]

...
std
  ...
  std.collections
    std.collections.map_merge
      fn (base: map[string, config_value], overlay: map[string, config_value]) -> map[string, config_value]
      + returns a new map with overlay entries taking precedence
      # collections

config_loader
  config_loader.parse
    fn (raw: string) -> result[map[string, config_value], string]
  ...
  config_loader.load_file
    fn (path: string) -> result[map[string, config_value], string]
    + reads and parses a configuration file
    - propagates fs errors
    # loading
    -> std.fs.read_all
  config_loader.resolve_inheritance
    fn (child: map[string, config_value], parent: map[string, config_value]) -> map[string, config_value]
  ...
  config_loader.flatten_keys
    fn (config: map[string, config_value]) -> map[string, config_value]
    + rewrites nested keys as dotted paths ("db.host", "db.port")
    + leaves scalar roots unchanged
    # key_generation
  config_loader.get
    fn (config: map[string, config_value], key: string) -> optional[config_value]

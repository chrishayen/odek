...
http_pool
  http_pool.new_pool
    fn (max_per_host: i32) -> pool_state
    + returns an empty pool with the per-host cap
    # construction
  http_pool.acquire
    fn (state: pool_state, host: string, port: i32) -> result[tuple[connection, pool_state], string]
    ...
    -> std.net.dial
  http_pool.release
    fn (state: pool_state, host: string, port: i32, conn: connection) -> pool_state
  ...
  http_pool.post_multipart
    fn (state: pool_state, url: string, fields: map[string,string], files: list[upload_file]) -> result[http_response, string]
  ...
  http_pool.encode_multipart
    fn (boundary: string, fields: map[string,string], files: list[upload_file]) -> bytes

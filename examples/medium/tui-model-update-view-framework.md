...
tui
  tui.new_program
    fn (initial_model: bytes) -> program_state
    + creates a program wrapping the user-supplied initial model
    # construction
  tui.send_message
    fn (state: program_state, message: bytes) -> program_state
    + enqueues a message for the next update cycle
    # messaging
  tui.step
    fn (state: program_state, update_fn_id: string) -> tuple[program_state, list[bytes]]

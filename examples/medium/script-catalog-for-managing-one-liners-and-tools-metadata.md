...
script_catalog
  script_catalog.load
    fn (path: string) -> result[catalog_state, string]
    ...
    -> std.json.parse
  script_catalog.save
    fn (state: catalog_state, path: string) -> result[void, string]
    + writes the catalog to disk
    # persistence
    -> std.fs.write_all
    -> std.json.encode
  script_catalog.add
    fn (state: catalog_state, alias: string, body: string, tags: list[string]) -> result[catalog_state, string]
  ...
  script_catalog.remove
    fn (state: catalog_state, alias: string) -> result[catalog_state, string]
    + removes the entry for the alias
    - returns error when alias is not present
    # mutation
  script_catalog.get
    fn (state: catalog_state, alias: string) -> optional[script_entry]
    + returns the entry when present
    - returns none when alias is unknown
    # lookup
  script_catalog.search
    fn (state: catalog_state, query: string) -> list[script_entry]
  ...
  script_catalog.list_by_tag
    fn (state: catalog_state, tag: string) -> list[script_entry]

...
summarize
  summarize.from_html
    @ (html: string, max_sentences: i32) -> list[string]
    + strips HTML then delegates to from_text
    # pipeline
    -> std.html.strip_tags
  summarize.from_text
    @ (text: string, max_sentences: i32) -> list[string]

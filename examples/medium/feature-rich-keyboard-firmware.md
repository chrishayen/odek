...
keyboard
  keyboard.scan_matrix
    @ (rows: list[i32], cols: list[i32]) -> list[key_event]
    ...
    -> std.time.now_micros
  keyboard.new_keymap
    @ (layers: list[list[u16]]) -> keymap
    + builds a keymap with the given layer definitions
    # configuration
  keyboard.resolve_key
    @ (map: keymap, active_layer: i32, row: i32, col: i32) -> u16

...
std
  std.http
    std.http.post_json
      fn (url: string, body: string, headers: map[string, string]) -> result[tuple[i32, string], string]
      + sends a POST with JSON body and returns (status, response body)
      - returns error on network failure
      # http
    std.http.get
      fn (url: string, headers: map[string, string]) -> result[tuple[i32, string], string]
  ...
  std.jwt
    std.jwt.decode_unverified
      fn (token: string) -> result[map[string, string], string]
...
workload_secrets
  workload_secrets.extract_audience
    fn (svid_jwt: string) -> result[string, string]
    + reads the "aud" claim from an identity JWT
    - returns error when the claim is missing
    # claims
    -> std.jwt.decode_unverified
  workload_secrets.login
    fn (secret_store_url: string, role: string, svid_jwt: string) -> result[string, string]
  ...
  workload_secrets.read_secret
    fn (secret_store_url: string, session_token: string, path: string) -> result[map[string, string], string]

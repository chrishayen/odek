...
std
  std.net
    std.net.listen_tcp
      ...
      # network
    std.net.accept
      @ (l: listener_handle) -> result[conn_handle, string]
...
toxiproxy
  toxiproxy.new
    @ (listen_host: string, listen_port: i32, upstream_host: string, upstream_port: i32) -> proxy_state
    + returns a proxy configured with the given endpoints and no toxics
    # construction
  toxiproxy.add_toxic_latency
    @ (p: proxy_state, name: string, direction: string, latency_ms: i64, jitter_ms: i64) -> result[proxy_state, string]
  ...
  toxiproxy.add_toxic_bandwidth
    @ (p: proxy_state, name: string, direction: string, rate_kib_per_sec: i64) -> result[proxy_state, string]

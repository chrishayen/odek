...
lm_programs
  lm_programs.new_signature
    fn (inputs: list[string], outputs: list[string]) -> signature
    + returns a signature describing named inputs and outputs
    # construction
  lm_programs.new_module
    fn (sig: signature, instruction: string) -> module_state
    + returns a module that invokes the model under the given instruction
    # construction
  lm_programs.call_module
    fn (module: module_state, inputs: map[string, string]) -> result[map[string, string], string]
  ...
  lm_programs.compile_chain
    fn (modules: list[module_state]) -> module_state
    + returns one composite module that threads outputs of each step into the next
    # composition
  lm_programs.optimize_instruction
    fn (module: module_state, examples: list[tuple[map[string, string], map[string, string]]]) -> result[module_state, string]

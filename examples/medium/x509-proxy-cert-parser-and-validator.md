...
std
  ...
  std.crypto
    std.crypto.verify_signature
      @ (issuer_public_key: bytes, tbs_bytes: bytes, signature: bytes, algorithm: string) -> bool
      + returns true when the signature verifies under the issuer's public key
      # cryptography

proxy_cert
  proxy_cert.load_chain
    @ (pem_text: string) -> result[list[certificate], string]
  ...
  proxy_cert.validate_chain
    @ (chain: list[certificate], trust_roots: list[certificate]) -> result[void, string]
  ...
  proxy_cert.effective_identity
    @ (chain: list[certificate]) -> result[string, string]

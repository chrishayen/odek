...
docdb
  docdb.open
    ...
    -> std.json.parse_value
  docdb.insert
    fn (state: doc_state, doc: json_value) -> result[tuple[string, doc_state], string]
    ...
    -> std.json.encode_value
  docdb.update
    fn (state: doc_state, id: string, doc: json_value) -> result[doc_state, string]
    ...
    -> std.json.encode_value
  docdb.remove
    fn (state: doc_state, id: string) -> result[doc_state, string]
    + appends a tombstone and drops the document from memory
    - returns error when the id is unknown
    # mutation
    -> std.fs.write_all
  docdb.find_by_id
    fn (state: doc_state, id: string) -> optional[json_value]
    + returns the document with the given id
    # query
  docdb.find_where
    fn (state: doc_state, field: string, equals: json_value) -> list[json_value]

...
smtp_sm
  smtp_sm.new
    @ (hostname: string) -> smtp_state
    + creates a fresh session in the "expect HELO/EHLO" phase
    # construction
  smtp_sm.feed_line
    @ (state: smtp_state, line: string) -> tuple[smtp_response, smtp_state]
  ...
  smtp_sm.append_data_line
    @ (state: smtp_state, line: string) -> tuple[optional[smtp_response], smtp_state]

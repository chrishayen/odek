...
fixed_array
  fixed_array.new
    fn (length: i32, fill: i32) -> result[fixed_array_state, string]
    + creates an array of exactly length elements, all set to fill
    - returns error when length is negative
    # construction
  fixed_array.get
    fn (state: fixed_array_state, index: i32) -> result[i32, string]
    + returns the element at index
    - returns error when index is outside [0, length)
    # access
  fixed_array.set
    fn (state: fixed_array_state, index: i32, value: i32) -> result[fixed_array_state, string]

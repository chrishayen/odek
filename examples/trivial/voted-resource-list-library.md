...
voted_resources
  voted_resources.new
    ...
    # construction
  voted_resources.add
    fn (state: resources_state, title: string, url: string) -> resources_state
    + appends a resource with zero votes
    # mutation
  voted_resources.upvote
    fn (state: resources_state, title: string) -> resources_state
    + increments the vote count for a matching title
    ? unknown titles are ignored
    # mutation
  voted_resources.ranked
    fn (state: resources_state) -> list[tuple[string, string, i32]]

...
- Given a persisted user has no memberships, when membership resolution is requested, then a personal organization and owner membership are provisioned for that user.
...
- Every persisted user must resolve to at least one organization membership.
